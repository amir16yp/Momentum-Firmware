...
void furi_hal_subghz_init(void)
{
    ...
    do {
        ...
        furi_hal_gpio_init(&FURI_HAL_SUBGHZ_TX_GPIO, GpioModeOutputPushPull, GpioPullNo,
                           GpioSpeedLow);
#endif
        ...
        cc1101_write_reg(&furi_hal_spi_bus_handle_subghz, CC1101_IOCFG0,
...
bool furi_hal_subghz_rx_pipe_not_empty(void)
{
    ...
    furi_hal_spi_acquire(&furi_hal_spi_bus_handle_subghz);
    cc1101_read_reg(&furi_hal_spi_bus_handle_subghz, (CC1101_STATUS_RXBYTES) | CC1101_BURST,
                    (uint8_t *)status);
    furi_hal_spi_release(&furi_hal_spi_bus_handle_subghz);
...
/*
 Modified by @tkerby & MX to the full YARD Stick One extended range of 281-361 MHz, 378-481 MHz, and
 749-962 MHz. These changes are at your own risk. The PLL may not lock and FZ devs have warned of
 possible damage!
 */
...
{
    // Set these values to the extended frequency range only. They dont define if you can transmit
    // but do select the correct RF path
...
void furi_hal_subghz_set_path(FuriHalSubGhzPath path)
{
    ...
    if (path == FuriHalSubGhzPath433) {
        ...
        cc1101_write_reg(&furi_hal_spi_bus_handle_subghz, CC1101_IOCFG2,
                         CC1101IocfgHW | CC1101_IOCFG_INV);
...
static bool furi_hal_subghz_start_debug(void)
{
    ...
    if (furi_hal_subghz.async_mirror_pin != NULL) {
        ...
        furi_hal_gpio_init(furi_hal_subghz.async_mirror_pin, GpioModeOutputPushPull, GpioPullNo,
                           GpioSpeedVeryHigh);
        ret = true;
    }
    return ret;
...
static bool furi_hal_subghz_stop_debug(void)
{
    bool ret = false;
    if (furi_hal_subghz.async_mirror_pin != NULL) {
        furi_hal_gpio_init(furi_hal_subghz.async_mirror_pin, GpioModeAnalog, GpioPullNo,
                           GpioSpeedLow);
        ret = true;
    }
    return ret;
    ...
    if (LL_TIM_IsActiveFlag_CC2(TIM2)) {
        ...
        if (furi_hal_subghz_capture_callback) {
            ...
            furi_hal_subghz_capture_callback(
                false, LL_TIM_IC_GetCaptureCH2(TIM2) - furi_hal_subghz_capture_delta_duration,
                (void *)furi_hal_subghz_capture_callback_context);
        }
    }
}
...
void furi_hal_subghz_start_async_rx(FuriHalSubGhzCaptureCallback callback, void *context)
{
    ...
    furi_hal_gpio_init_ex(&gpio_cc1101_g0, GpioModeAltFunctionPushPull, GpioPullNo, GpioSpeedLow,
                          GpioAltFn1TIM2);
...
furi_hal_subghz_async_tx_middleware_get_duration(FuriHalSubGhzAsyncTxMiddleware *middleware,
                                                 FuriHalSubGhzAsyncTxCallback callback)
{
    uint32_t ret = 0;
    bool is_level = false;
...
// here we fill DMA buffer by signal durations until we recieve duration=0 (that mean protocol give
// as full data = signal_size*repeats) or until we reach the end of required samples count
static void furi_hal_subghz_async_tx_refill(uint32_t *buffer, size_t samples)
{
    ...
    while (samples > 0) {
        ...
        // if duration == 0 then we stop DMA interrupt(that used to refill buffer) and write to
        // buffer 0 as last element. later DMA write this 0 to ARR and timer TIM2 will be stopped.
        if (duration == 0) {
            *buffer = 0;
            buffer++;
            samples--;
...
static void furi_hal_subghz_async_tx_dma_isr(void *context)
{
    ...
    if (LL_DMA_IsActiveFlag_HT1(SUBGHZ_DMA)) {
        LL_DMA_ClearFlag_HT1(SUBGHZ_DMA);
        furi_hal_subghz_async_tx_refill(furi_hal_subghz_async_tx.buffer,
...
bool furi_hal_subghz_start_async_tx(FuriHalSubGhzAsyncTxCallback callback, void *context)
{
    ...
    // Here we use TIM2_CH2 (Timer 2 Channel 2) to generate HI/LOW signals for C1101 with current
    // durations. DMA update/rewrite TIM2 settings (ARR) with new duration each time TIM2 completes.
    // Every time when timer counter exeed current TIM2-ARR (AutoReload Register) value timer
    // generate event that call DMA DMA load next new value from buffer to TIM2-ARR and timer start
    // count up from 0 to new value again Totally we have timer that generate events and update they
    // settings with new durations by DMA action. When duration = 0 then DMA wirte 0 to ARR. So when
    // we set ARR=0 - thats mean TIM2 stop counting.
    ...
    dma_config.PeriphOrM2MSrcIncMode =
        LL_DMA_PERIPH_NOINCREMENT; // DMA destination not changed - allways stay on ARR (AutoReload
                                   // Register)
    ...
    // setup interrupt for DMA. When DMA generate interrupt event we call
    // furi_hal_subghz_async_tx_dma_isr
    ...
    // AutoReload Register (ARR) 1000 ticks = 1/1000 Mghz = 1 millisecond, will be changed by DMA by
    // new durations
    ...
    furi_hal_subghz_async_tx_refill(furi_hal_subghz_async_tx.buffer,
                                    FURI_HAL_SUBGHZ_ASYNC_TX_BUFFER_FULL);
...
void furi_hal_subghz_stop_async_tx(void)
{
    ...
    FURI_LOG_D(TAG, "Async TX Radio stats: on %0.0fus, off %0.0fus, DutyCycle: %0.0f%%",
