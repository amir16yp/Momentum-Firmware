...
static const struct CcidConfigDescriptor ccid_cfg_desc = {
    ...
    .intf_0 =
        {
            .ccid = {.bLength = sizeof(struct usb_interface_descriptor),
            ...
            .ccid_desc = {.bLength = sizeof(struct usb_ccid_descriptor),
                          ...
                          .bPINSupport = 0,
                          .bMaxCCIDBusySlots = 1},
            .ccid_bulk_in = {.bLength = sizeof(struct usb_endpoint_descriptor),
            ...
            .ccid_bulk_out = {.bLength = sizeof(struct usb_endpoint_descriptor),
                              .bDescriptorType = USB_DTYPE_ENDPOINT,
...
void CALLBACK_CCID_GetSlotStatus(uint8_t slot, uint8_t seq,
                                 struct rdr_to_pc_slot_status *responseSlotStatus)
{
    ...
    if (responseSlotStatus->bSlot == CCID_SLOT_INDEX) {
        ...
        if (furi_hal_usb_ccid->smartcard_inserted) {
            responseSlotStatus->bStatus =
                CCID_COMMANDSTATUS_PROCESSEDWITHOUTERROR | CCID_ICCSTATUS_PRESENTANDACTIVE;
        } else {
            responseSlotStatus->bStatus =
                CCID_COMMANDSTATUS_PROCESSEDWITHOUTERROR | CCID_ICCSTATUS_NOICCPRESENT;
        }
    } else {
    ...
    struct rdr_to_pc_parameters_t0 *responseSetParametersT0)
{
    ...
    if (responseSetParametersT0->bSlot == CCID_SLOT_INDEX) {
        ...
        if (furi_hal_usb_ccid->smartcard_inserted) {
            ...
            responseSetParametersT0->bStatus =
                CCID_COMMANDSTATUS_PROCESSEDWITHOUTERROR | CCID_ICCSTATUS_PRESENTANDACTIVE;
        } else {
            responseSetParametersT0->bStatus =
                CCID_COMMANDSTATUS_PROCESSEDWITHOUTERROR | CCID_ICCSTATUS_NOICCPRESENT;
...
void CALLBACK_CCID_IccPowerOn(uint8_t slot, uint8_t seq,
        ...
        if (furi_hal_usb_ccid->smartcard_inserted) {
            if (furi_hal_usb_ccid->callbacks[CCID_SLOT_INDEX] != NULL) {
                furi_hal_usb_ccid->callbacks[CCID_SLOT_INDEX]->icc_power_on_callback(
                    responseDataBlock->abData, &responseDataBlock->dwLength,
                    furi_hal_usb_ccid->cb_ctx[CCID_SLOT_INDEX]);
                responseDataBlock->bStatus =
                    CCID_COMMANDSTATUS_PROCESSEDWITHOUTERROR | CCID_ICCSTATUS_PRESENTANDACTIVE;
            } else {
                responseDataBlock->bStatus =
                    CCID_COMMANDSTATUS_PROCESSEDWITHOUTERROR | CCID_ICCSTATUS_PRESENTANDINACTIVE;
            }
        } else {
            responseDataBlock->bStatus =
                CCID_COMMANDSTATUS_PROCESSEDWITHOUTERROR | CCID_ICCSTATUS_NOICCPRESENT;
...
void CALLBACK_CCID_XfrBlock(struct pc_to_rdr_xfr_block *receivedXfrBlock,
                            struct rdr_to_pc_data_block *responseDataBlock)
{
        ...
        if (furi_hal_usb_ccid->smartcard_inserted) {
            if (furi_hal_usb_ccid->callbacks[CCID_SLOT_INDEX] != NULL) {
                furi_hal_usb_ccid->callbacks[CCID_SLOT_INDEX]->xfr_datablock_callback(
                    (const uint8_t *)receivedXfrBlock->abData, receivedXfrBlock->dwLength,
                    responseDataBlock->abData, &responseDataBlock->dwLength,
                    furi_hal_usb_ccid->cb_ctx[CCID_SLOT_INDEX]);
                responseDataBlock->bStatus =
                    CCID_COMMANDSTATUS_PROCESSEDWITHOUTERROR | CCID_ICCSTATUS_PRESENTANDACTIVE;
...
void ccid_send_packet(uint8_t *data, uint8_t len)
{
    ...
    if (furi_hal_usb_ccid->ccid_semaphore == NULL || furi_hal_usb_ccid->connected == false)
        return;
    furi_check(furi_semaphore_acquire(furi_hal_usb_ccid->ccid_semaphore, FuriWaitForever) ==
               FuriStatusOk);
...
static void ccid_tx_ep_callback(usbd_device *dev, uint8_t event, uint8_t ep)
{
    ...
    if (event == usbd_evt_eprx) {
        ...
        int32_t bytes_read = usbd_ep_read(
            furi_hal_usb_ccid->usb_dev, CCID_OUT_EPADDR,
        ...
        if (bytes_read > 0) {
            ...
            furi_thread_flags_set(furi_thread_get_id(furi_hal_usb_ccid->ccid_thread),
                                  WorkerEvtRequest);
        }
    }
}
...
static int32_t ccid_worker(void *context)
{
    ...
    while (1) {
        furi_check(furi_hal_usb_ccid);
        uint32_t flags = furi_thread_flags_wait(WorkerEvtStop | WorkerEvtRequest, FuriFlagWaitAny,
                                                FuriWaitForever);
        ...
        if (flags & WorkerEvtRequest) {
            ...
            if (message->bMessageType == PC_TO_RDR_ICCPOWERON) {
                ...
                CALLBACK_CCID_IccPowerOn(requestDataBlock->bSlot, requestDataBlock->bSeq,
                                         responseDataBlock);

                ccid_send_response(furi_hal_usb_ccid->send_buffer,
                ...
                CALLBACK_CCID_GetSlotStatus(requestIccPowerOff->bSlot, requestIccPowerOff->bSeq,
                                            responseSlotStatus);

                ccid_send_response(furi_hal_usb_ccid->send_buffer,
                                   sizeof(struct rdr_to_pc_slot_status));
                ...
                CALLBACK_CCID_GetSlotStatus(requestSlotStatus->bSlot, requestSlotStatus->bSeq,
                                            responseSlotStatus);
