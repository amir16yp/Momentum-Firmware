...
static bool furi_hal_i2c_wait_for_idle(I2C_TypeDef *i2c, FuriHalI2cBegin begin,
                                       FuriHalCortexTimer timer)
{
    ...
    // Only check if the bus is busy if starting a new transaction, if not we already control the
    // bus

    return true;
}

static bool furi_hal_i2c_wait_for_end(I2C_TypeDef *i2c, FuriHalI2cEnd end, FuriHalCortexTimer timer)
{
    // If ending the transaction with a stop condition, wait for it to be detected, otherwise wait
    // for a transfer complete flag
...
static uint32_t furi_hal_i2c_get_start_signal(FuriHalI2cBegin begin, bool ten_bit_address,
                                              bool read)
{
    switch (begin) {
...
static bool furi_hal_i2c_transfer(I2C_TypeDef *i2c, uint8_t *data, uint32_t size, FuriHalI2cEnd end,
                                  bool read, FuriHalCortexTimer timer)
{
    bool ret = true;

    while (size > 0) {
        bool should_stop =
            furi_hal_cortex_timer_is_expired(timer) || furi_hal_i2c_transfer_is_aborted(i2c);
...
static bool furi_hal_i2c_transaction(I2C_TypeDef *i2c, uint16_t address, bool ten_bit,
                                     uint8_t *data, size_t size, FuriHalI2cBegin begin,
                                     FuriHalI2cEnd end, bool read, FuriHalCortexTimer timer)
{
...
bool furi_hal_i2c_rx_ext(const FuriHalI2cBusHandle *handle, uint16_t address, bool ten_bit,
                         uint8_t *data, size_t size, FuriHalI2cBegin begin, FuriHalI2cEnd end,
                         uint32_t timeout)
{
    furi_check(handle->bus->current_handle == handle);
    ...
    return furi_hal_i2c_transaction(handle->bus->i2c, address, ten_bit, data, size, begin, end,
                                    true, timer);
}

bool furi_hal_i2c_tx_ext(const FuriHalI2cBusHandle *handle, uint16_t address, bool ten_bit,
                         const uint8_t *data, size_t size, FuriHalI2cBegin begin, FuriHalI2cEnd end,
                         uint32_t timeout)
{
    furi_check(handle->bus->current_handle == handle);
    ...
    return furi_hal_i2c_transaction(handle->bus->i2c, address, ten_bit, (uint8_t *)data, size,
                                    begin, end, false, timer);
}

bool furi_hal_i2c_tx(const FuriHalI2cBusHandle *handle, uint8_t address, const uint8_t *data,
                     size_t size, uint32_t timeout)
{
    furi_check(timeout > 0);

    return furi_hal_i2c_tx_ext(handle, address, false, data, size, FuriHalI2cBeginStart,
                               FuriHalI2cEndStop, timeout);
}

bool furi_hal_i2c_rx(const FuriHalI2cBusHandle *handle, uint8_t address, uint8_t *data, size_t size,
                     uint32_t timeout)
{
    furi_check(timeout > 0);

    return furi_hal_i2c_rx_ext(handle, address, false, data, size, FuriHalI2cBeginStart,
                               FuriHalI2cEndStop, timeout);
}

bool furi_hal_i2c_trx(const FuriHalI2cBusHandle *handle, uint8_t address, const uint8_t *tx_data,
                      size_t tx_size, uint8_t *rx_data, size_t rx_size, uint32_t timeout)
{
    return furi_hal_i2c_tx_ext(handle, address, false, tx_data, tx_size, FuriHalI2cBeginStart,
                               FuriHalI2cEndStop, timeout) &&
           furi_hal_i2c_rx_ext(handle, address, false, rx_data, rx_size, FuriHalI2cBeginStart,
                               FuriHalI2cEndStop, timeout);
}

bool furi_hal_i2c_is_device_ready(const FuriHalI2cBusHandle *handle, uint8_t i2c_addr,
                                  uint32_t timeout)
{
    furi_check(handle);
    furi_check(handle->bus->current_handle == handle);
    ...
    LL_I2C_HandleTransfer(handle->bus->i2c, i2c_addr, LL_I2C_ADDRSLAVE_7BIT, 0, LL_I2C_MODE_AUTOEND,
...
bool furi_hal_i2c_read_reg_8(const FuriHalI2cBusHandle *handle, uint8_t i2c_addr, uint8_t reg_addr,
...
bool furi_hal_i2c_read_reg_16(const FuriHalI2cBusHandle *handle, uint8_t i2c_addr, uint8_t reg_addr,
...
bool furi_hal_i2c_read_mem(const FuriHalI2cBusHandle *handle, uint8_t i2c_addr, uint8_t mem_addr,
...
bool furi_hal_i2c_write_reg_8(const FuriHalI2cBusHandle *handle, uint8_t i2c_addr, uint8_t reg_addr,
                              uint8_t data, uint32_t timeout)
{
    furi_check(handle);
...
bool furi_hal_i2c_write_reg_16(const FuriHalI2cBusHandle *handle, uint8_t i2c_addr,
                               uint8_t reg_addr, uint16_t data, uint32_t timeout)
{
    furi_check(handle);
...
bool furi_hal_i2c_write_mem(const FuriHalI2cBusHandle *handle, uint8_t i2c_addr, uint8_t mem_addr,
                            const uint8_t *data, size_t len, uint32_t timeout)
{
    ...
    return furi_hal_i2c_tx_ext(handle, i2c_addr, false, &mem_addr, 1, FuriHalI2cBeginStart,
                               FuriHalI2cEndPause, timeout) &&
           furi_hal_i2c_tx_ext(handle, i2c_addr, false, data, len, FuriHalI2cBeginResume,
                               FuriHalI2cEndStop, timeout);
}
