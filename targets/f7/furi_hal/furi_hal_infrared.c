...
typedef enum {
    ...
    InfraredStateAsyncTxStopInProgress, /** Async TX started, stop request is processed and we wait
                                           for last data to be sent */
...
static void furi_hal_infrared_tim_rx_isr(void *context)
{
    ...
    if (LL_TIM_IsActiveFlag_CC1(INFRARED_RX_TIMER)) {
        ...
        if (READ_BIT(INFRARED_RX_TIMER->CCMR1, TIM_CCMR1_CC1S)) {
            /* Low pin level is a Mark state of INFRARED signal. Invert level for further
             * processing. */
    ...
    if (LL_TIM_IsActiveFlag_CC2(INFRARED_RX_TIMER)) {
        ...
        if (READ_BIT(INFRARED_RX_TIMER->CCMR1, TIM_CCMR1_CC2S)) {
            /* High pin level is a Space state of INFRARED signal. Invert level for further
             * processing. */
...
void furi_hal_infrared_async_rx_start(void)
{
    ...
    furi_hal_gpio_init_ex(&gpio_infrared_rx, GpioModeAltFunctionPushPull, GpioPullNo, GpioSpeedLow,
...
static void furi_hal_infrared_tx_dma_isr(void *context)
{
    ...
    if (LL_DMA_IsActiveFlag_HT2(INFRARED_DMA) && LL_DMA_IsEnabledIT_HT(INFRARED_DMA_CH2_DEF)) {
        ...
        if (infrared_tim_tx.buffer[buf_num].last_packet_end) {
            LL_DMA_DisableIT_HT(INFRARED_DMA_CH2_DEF);
        } else if (!infrared_tim_tx.buffer[buf_num].packet_end ||
...
#error Update this code. Would you kindly?
    ...
    furi_hal_interrupt_set_isr_ex(INFRARED_DMA_CH1_IRQ, FuriHalInterruptPriorityKamiSama,
                                  furi_hal_infrared_tx_dma_polarity_isr, NULL);
}
...
static void furi_hal_infrared_async_tx_free_resources(void)
{
    ...
    furi_hal_gpio_init(infrared_tx_pins[infrared_tx_output], GpioModeAnalog, GpioPullDown,
                       GpioSpeedLow);
...
void furi_hal_infrared_async_tx_start(uint32_t freq, float duty_cycle)
{
    ...
    furi_hal_gpio_init_ex(tx_gpio, GpioModeAltFunctionPushPull, GpioPullNo, GpioSpeedHigh,
                          GpioAltFn1TIM1);
...
FuriHalInfraredTxPin furi_hal_infrared_detect_tx_output(void)
{
    for (FuriHalInfraredTxPin pin = FuriHalInfraredTxPinInternal + 1; //-V1008
         pin < FuriHalInfraredTxPinMax; ++pin) {
