...
static void furi_hal_serial_control_enable_expansion_irq(FuriHalSerialHandle *handle, bool enable)
{
...
static void furi_hal_serial_control_log_set_handle(FuriHalSerialHandle *handle)
{
    ...
    if (furi_hal_serial_control->expansion_serial) {
        furi_hal_serial_control_enable_expansion_irq(furi_hal_serial_control->expansion_serial,
                                                     false);
    }
    ...
    if (handle) {
        ...
        furi_hal_serial_init(furi_hal_serial_control->log_serial,
...
static bool furi_hal_serial_control_handler_logging(void *input, void *output)
{
    ...
    if (furi_hal_serial_control->log_config_serial_id < FuriHalSerialIdMax) {
        if (!furi_hal_serial_control->handles[furi_hal_serial_control->log_config_serial_id]
                 .in_use) {
            handle =
...
void furi_hal_serial_control_init(void)
{
    ...
    furi_hal_serial_control->thread =
        furi_thread_alloc_service("SerialControlDriver", 512, furi_hal_serial_control_thread, NULL);
...
void furi_hal_serial_control_set_logging_config(FuriHalSerialId serial_id, uint32_t baud_rate)
{
    ...
    if (!furi_hal_serial_control)
        return;

    furi_check(furi_hal_serial_is_baud_rate_supported(&furi_hal_serial_control->handles[serial_id],
                                                      baud_rate));
