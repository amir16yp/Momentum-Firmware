 ...
 * ProcessSingleFlashOperation, quote:
  > In most BLE application, the flash should not be blocked by the CPU2 longer than
 FLASH_TIMEOUT_VALUE (1000ms) > However, it could be that for some marginal application, this time
 is longer. > ... there is no other way than waiting the operation to be completed. > If for any
 reason this test is never passed, this means there is a failure in the system and there is no other
...
void furi_hal_flash_erase(uint8_t page)
{
    ...
    FURI_LOG_T(TAG, "erase took %lu clocks or %luus", op_stat,
...
void furi_hal_flash_write_dword(size_t address, uint64_t data)
{
    ...
    furi_check(furi_hal_flash_wait_last_operation(FURI_HAL_FLASH_TIMEOUT));
    op_stat = DWT->CYCCNT - op_stat;
    FURI_LOG_T(TAG, "write_dword took %lu clocks or %fus", op_stat,
...
void furi_hal_flash_program_page(const uint8_t page, const uint8_t *data, uint16_t _length)
{
    ...
    if (length >= FAST_PROG_BLOCK_SIZE) {
        ...
        while (length_written < (length / FAST_PROG_BLOCK_SIZE * FAST_PROG_BLOCK_SIZE)) {
            ...
            furi_hal_flash_write_dword_internal_nowait(page_start_address + length_written,
                                                       (uint64_t *)(data + length_written));
    ...
    if ((length % FAST_PROG_BLOCK_SIZE) != 0) {
        ...
        while (length_written < (length / DWORD_PROG_BLOCK_SIZE * DWORD_PROG_BLOCK_SIZE)) {
            furi_hal_flash_write_dword_internal(page_start_address + length_written,
                                                (uint64_t *)&data[length_written]);
            length_written += DWORD_PROG_BLOCK_SIZE;
        }
    }
    ...
    furi_hal_flash_end(false);
    op_stat = DWT->CYCCNT - op_stat;
    FURI_LOG_T(TAG, "program_page took %lu clocks or %luus", op_stat,
               op_stat / furi_hal_cortex_instructions_per_microsecond());
}
...
bool furi_hal_flash_ob_set_word(size_t word_idx, const uint32_t value)
{
    ...
    FURI_LOG_W(TAG, "Setting OB reg %d for word %d (addr 0x%08lX) to 0x%08lX", reg_def->ob_reg,
               word_idx, (uint32_t)reg_def->ob_register_address, value);
