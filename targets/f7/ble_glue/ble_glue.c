...
static void ble_glue_update_c2_fw_info(void)
{
    ...
    snprintf(local_info->StackTypeString, BLE_MAX_VERSION_STRING_LEN, "%d.%d.%d:%s",
             local_info->VersionMajor, local_info->VersionMinor, local_info->VersionSub,
...
static void ble_glue_dump_stack_info(void)
{
    ...
    FURI_LOG_I(TAG, "Core2: FUS: %d.%d.%d, mem %d/%d, flash %d pages", c2_info->FusVersionMajor,
               c2_info->FusVersionMinor, c2_info->FusVersionSub, c2_info->FusMemorySizeSram2B,
               c2_info->FusMemorySizeSram2A, c2_info->FusMemorySizeFlash);
    FURI_LOG_I(TAG, "Core2: Stack: %d.%d.%d, branch %d, reltype %d, stacktype %d, flash %d pages",
               c2_info->VersionMajor, c2_info->VersionMinor, c2_info->VersionSub,
               c2_info->VersionBranch, c2_info->VersionReleaseType, c2_info->StackType,
...
bool ble_glue_wait_for_c2_start(int32_t timeout_ms)
{
    ...
    FURI_LOG_I(TAG, "C2 boot completed, mode: %s",
...
static void ble_sys_status_not_callback(SHCI_TL_CmdStatus_t status)
{
    ...
    case SHCI_TL_CmdBusy:
        furi_check(furi_mutex_acquire(ble_glue->shci_mtx,
                                      furi_kernel_is_running() ? FuriWaitForever : 0) ==
                   FuriStatusOk);
        break;
...
static void ble_sys_user_event_callback(void *pPayload)
{
    ...
    if (p_sys_event->subevtcode == SHCI_SUB_EVT_CODE_READY) {
        ...
        if (ble_glue->callback) {
            ble_glue->callback((uint8_t *)p_sys_ble_nvm_ram_update_event->StartAddress,
                               p_sys_ble_nvm_ram_update_event->Size, ble_glue->context);
        }
    }
}
...
BleGlueCommandResult ble_glue_fus_get_status(void)
{
    ...
    if ((error_code != 0) || (fus_state == FUS_STATE_VALUE_ERROR)) {
        return BleGlueCommandResultError;
    } else if ((fus_state >= FUS_STATE_VALUE_FW_UPGRD_ONGOING) &&
