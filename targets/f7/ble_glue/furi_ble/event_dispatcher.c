...
BleEventFlowStatus ble_event_dispatcher_process_event(void *payload)
{
    ...
    for (GapSvcEventHandlerList_it(it, handlers); !GapSvcEventHandlerList_end_p(it);
        ...
        if (ack_status == BleEventNotAck) {
            /* Keep going */
            continue;
        } else if ((ack_status == BleEventAckFlowEnable) ||
                   (ack_status == BleEventAckFlowDisable)) {
            break;
        }
    }
...
GapSvcEventHandler *ble_event_dispatcher_register_svc_handler(BleSvcEventHandlerCb handler,
                                                              void *context)
{
    furi_check(handler);
    furi_check(context);
