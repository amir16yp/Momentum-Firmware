...
static void gap_verify_connection_parameters(Gap *gap)
{
    ...
    FURI_LOG_I(TAG,
               "Connection parameters: Connection Interval: %d (%d ms), Slave Latency: %d, "
               "Supervision Timeout: %d",
               gap->connection_params.conn_interval,
               GAP_INTERVAL_TO_MS(gap->connection_params.conn_interval),
               gap->connection_params.slave_latency, gap->connection_params.supervisor_timeout);
    ...
    uint16_t connection_interval_max =
        gap->negotiation_round ? params->conn_int_max : params->conn_int_min;

    // We do care about lower connection interval bound a lot: if it's lower than 30ms 2nd core will
    // not allow us to use flash controller
    ...
    if (negotiation_failed) {
        FURI_LOG_W(TAG, "Connection interval doesn't suite us. Trying to negotiate, round %u",
                   gap->negotiation_round + 1);
        if (aci_l2cap_connection_parameter_update_req(
                gap->service.connection_handle, params->conn_int_min, connection_interval_max,
                gap->connection_params.slave_latency, gap->connection_params.supervisor_timeout)) {
        ...
        FURI_LOG_I(TAG, "Connection interval suits us. Spent %u rounds to negotiate",
                   gap->negotiation_round);
...
BleEventFlowStatus ble_event_app_notification(void *pckt)
{
    ...
    case HCI_DISCONNECTION_COMPLETE_EVT_CODE: {
        ...
        if (disconnection_complete_event->Connection_Handle == gap->service.connection_handle) {
            ...
            FURI_LOG_I(TAG, "Disconnect from client. Reason: %02X",
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
        ...
        case ACI_ATT_EXCHANGE_MTU_RESP_VSEVT_CODE: {
            ...
            GapEvent event = {.type = GapEventTypeUpdateMTU,
        ...
        case ACI_GAP_PAIRING_COMPLETE_VSEVT_CODE:
            ...
            if (pairing_complete->Status) {
                FURI_LOG_E(TAG, "Pairing failed with status: %d. Terminating connection",
...
static void gap_init_svc(Gap *gap, const GapRootSecurityKeys *root_keys)
{
    ...
    aci_gap_init(GAP_PERIPHERAL_ROLE, 0, strlen(name), &gap->service.gap_svc_handle,
                 &gap->service.dev_name_char_handle, &gap->service.appearance_char_handle);

    // Set GAP characteristics
    status =
        aci_gatt_update_char_value(gap->service.gap_svc_handle, gap->service.dev_name_char_handle,
                                   0, strlen(name), (uint8_t *)name);
    if (status) {
        FURI_LOG_E(TAG, "Failed updating name characteristic: %d", status);
    }

    uint8_t gap_appearence_char_uuid[2] = {gap->config->appearance_char & 0xff,
                                           gap->config->appearance_char >> 8};
    status =
        aci_gatt_update_char_value(gap->service.gap_svc_handle, gap->service.appearance_char_handle,
                                   0, 2, gap_appearence_char_uuid);
    if (status) {
    ...
    aci_gap_set_authentication_requirement(gap->config->bonding_mode, auth_req_mitm_mode,
                                           CFG_SC_SUPPORT, keypress_supported,
...
static void gap_advertise_start(GapState new_state)
{
    ...
    status =
        aci_gap_set_discoverable(ADV_IND, min_interval, max_interval, CFG_IDENTITY_ADDRESS, 0,
...
bool gap_init(GapConfig *config, const GapRootSecurityKeys *root_keys, GapEventCallback on_event_cb,
              void *context)
{
    ...
    if (gap->config->adv_service.UUID_Type == UUID_TYPE_16) {
        ...
        set_advertisment_service_uid(gap->config->adv_service.Service_UUID_128,
