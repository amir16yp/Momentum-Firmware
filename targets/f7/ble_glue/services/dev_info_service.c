...
static bool dev_info_char_data_callback(const void *context, const uint8_t **data,
                                        uint16_t *data_len)
{
...
static const BleGattCharacteristicParams ble_svc_dev_info_chars[DevInfoSvcGattCharacteristicCount] =
    {[DevInfoSvcGattCharacteristicMfgName] = {.name = "Manufacturer Name",
     ...
     [DevInfoSvcGattCharacteristicSerial] = {.name = "Serial Number",
                                             .data_prop_type = FlipperGattCharacteristicDataFixed,
                                             .data.fixed.length = sizeof(dev_info_serial_num) - 1,
                                             .data.fixed.ptr =
                                                 (const uint8_t *)&dev_info_serial_num,
                                             ...
                                             .gatt_evt_mask = GATT_DONT_NOTIFY_EVENTS,
                                             .is_variable = CHAR_VALUE_LEN_CONSTANT},
     [DevInfoSvcGattCharacteristicFirmwareRev] = {.name = "Firmware Revision",
                                                  .data_prop_type =
                                                  ...
                                                  .gatt_evt_mask = GATT_DONT_NOTIFY_EVENTS,
                                                  .is_variable = CHAR_VALUE_LEN_CONSTANT},
     [DevInfoSvcGattCharacteristicSoftwareRev] = {.name = "Software Revision",
                                                  .data_prop_type =
                                                      FlipperGattCharacteristicDataCallback,
...
BleServiceDevInfo *ble_svc_dev_info_start(void)
{
    ...
    snprintf(software_revision, sizeof(software_revision), "%s %s %s %s", version_get_githash(NULL),
             version_get_gitbranch(NULL), version_get_gitbranchnum(NULL),
    ...
    if (!ble_gatt_service_add(UUID_TYPE_16, (Service_UUID_t *)&uuid, PRIMARY_SERVICE,
    ...
    for (size_t i = 0; i < DevInfoSvcGattCharacteristicCount; i++) {
        ble_gatt_characteristic_init(dev_info_svc->service_handle, &ble_svc_dev_info_chars[i],
                                     &dev_info_svc->characteristics[i]);
        ble_gatt_characteristic_update(dev_info_svc->service_handle,
                                       &dev_info_svc->characteristics[i], NULL);
    }
...
void ble_svc_dev_info_stop(BleServiceDevInfo *dev_info_svc)
{
    ...
    for (size_t i = 0; i < DevInfoSvcGattCharacteristicCount; i++) {
        ble_gatt_characteristic_delete(dev_info_svc->service_handle,
                                       &dev_info_svc->characteristics[i]);
    }
