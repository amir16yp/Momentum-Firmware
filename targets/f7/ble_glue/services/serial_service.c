...
static const BleGattCharacteristicParams ble_svc_serial_chars[SerialSvcGattCharacteristicCount] = {
    [SerialSvcGattCharacteristicRx] = {.name = "RX",
    ...
    [SerialSvcGattCharacteristicTx] = {.name = "TX",
                                       .data_prop_type = FlipperGattCharacteristicDataFixed,
                                       .data.fixed.length = BLE_SVC_SERIAL_DATA_LEN_MAX,
                                       ...
                                       .is_variable = CHAR_VALUE_LEN_VARIABLE},
    [SerialSvcGattCharacteristicFlowCtrl] = {.name = "Flow control",
                                             .data_prop_type = FlipperGattCharacteristicDataFixed,
        ...
        if (blecore_evt->ecode == ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE) {
            ...
            if (attribute_modified->Attr_Handle ==
                ...
                if (serial_svc->callback) {
                    furi_check(furi_mutex_acquire(serial_svc->buff_size_mtx, FuriWaitForever) ==
                               FuriStatusOk);
                    if (attribute_modified->Attr_Data_Length > serial_svc->bytes_ready_to_receive) {
                        FURI_LOG_W(TAG,
                                   "Received %d, while was ready to receive %d bytes. Can lead to "
                                   "buffer overflow!",
                                   attribute_modified->Attr_Data_Length,
                                   serial_svc->bytes_ready_to_receive);
                    }
                    serial_svc->bytes_ready_to_receive -= MIN(serial_svc->bytes_ready_to_receive,
                                                              attribute_modified->Attr_Data_Length);
                    SerialServiceEvent event = {.event = SerialServiceEventTypeDataReceived,
                                                .data = {
...
static void ble_svc_serial_update_rpc_char(BleServiceSerial *serial_svc,
                                           SerialServiceRpcStatus status)
{
    ble_gatt_characteristic_update(serial_svc->svc_handle,
                                   &serial_svc->chars[SerialSvcGattCharacteristicStatus], &status);
}
...
BleServiceSerial *ble_svc_serial_start(void)
{
    ...
    if (!ble_gatt_service_add(UUID_TYPE_128, &service_uuid, PRIMARY_SERVICE, 12,
                              &serial_svc->svc_handle)) {
        free(serial_svc);
        return NULL;
    }
    for (uint8_t i = 0; i < SerialSvcGattCharacteristicCount; i++) {
        ble_gatt_characteristic_init(serial_svc->svc_handle, &ble_svc_serial_chars[i],
                                     &serial_svc->chars[i]);
    }
...
void ble_svc_serial_set_callbacks(BleServiceSerial *serial_svc, uint16_t buff_size,
                                  SerialServiceEventCallback callback, void *context)
{
...
bool ble_svc_serial_update_tx(BleServiceSerial *serial_svc, uint8_t *data, uint16_t data_len)
{
    ...
    for (uint16_t remained = data_len; remained > 0;) {
        ...
        tBleStatus result = aci_gatt_update_char_value_ext(
            0, serial_svc->svc_handle, serial_svc->chars[SerialSvcGattCharacteristicTx].handle,
            remained ? 0x00 : 0x02, data_len, value_offset, value_len, data + value_offset);
...
{
    furi_check(serial_svc);
    ble_svc_serial_update_rpc_char(serial_svc, active ? SerialServiceRpcStatusActive
                                                      : SerialServiceRpcStatusNotActive);
}
