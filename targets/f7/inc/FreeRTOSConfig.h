...
//  ^^^^^   acquire errno directly from TCB because FreeRTOS assigns its `FreeRTOS_errno' _after_
//  our hook is called

// referencing `FreeRTOS_errno' here   vvvvv    because FreeRTOS calls our hook _before_ copying the
// value into the TCB, hence a manual write to the TCB would get overwritten
