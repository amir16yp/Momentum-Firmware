...
enum { CompressTypeHeatshrink, CompressTypeGzip };
typedef struct {
    char bytes[7];
...
{
    if (d->fail_at && d->position + count > d->fail_at)
        return false;
    d->uz.dest = out;
    d->uz.dest_limit = out + count;
    if (uzlib_uncompress_chksum(&d->uz) < 0 || d->uz.dest != out + count)
        return false;
    d->position += count;
    return true;
}
...
int main(int argc, char **argv)
{
    ...
    for (unsigned scenario = 0; scenario < 8; ++scenario) {
        ...
        bool result;
        if (scenario >= 6) {
            result = tar_archive_unpack_file(&archive, scenario == 6 ? "large" : "absent",
                                             "/ext/single");
