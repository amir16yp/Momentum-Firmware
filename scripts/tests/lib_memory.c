...
static bool flipper_format_read_string(FlipperFormat* ff, const char* key, FuriString* str) {
    if(!accepts(ff, key)) return false;
...
static bool
    flipper_format_read_uint32(FlipperFormat* ff, const char* key, uint32_t* out, size_t count) {
    assert(count == 1);
...
static bool
    flipper_format_read_hex(FlipperFormat* ff, const char* key, uint8_t* out, size_t count) {
    ff->hex_reads++;
    if(!accepts(ff, key)) return false;
...
static bool flipper_format_write_hex(
    FlipperFormat* ff,
    const char* key,
    const uint8_t* data,
    size_t count) {
...
static bool flipper_format_write_uint32(
    ...
    size_t count) {
    assert(count == 1);
    if(strcmp(key, "PIN try counter") == 0) ff->saved_pin = *data;
    return accepts(ff, key);
}
/* PRODUCTION_CODE */
...
int main(void) {
    ...
    memset(
        data.emv_application.application_label,
        'X',
