...
static size_t furi_stream_buffer_send(
    ...
    if(stream->count == stream->capacity) return 0;
    memcpy(&stream->queue[(stream->head + stream->count++) % stream->capacity], ptr, size);
    return size;
}
static size_t
    furi_stream_buffer_receive(FuriStreamBuffer* stream, void* ptr, size_t size, uint32_t timeout) {
    (void)timeout;
    assert(size == sizeof(void*));
    if(!stream->count) return 0;
...
int main(void) {
    ...
    *(uint32_t*)simple_array_get(b, 3) = 999;
    ...
    const SimpleArrayConfig callbacks = {
        element_init, element_reset, element_copy, sizeof(uint32_t)};
    ...
    for(size_t s = 0; s < sizeof(sizes) / sizeof(sizes[0]); ++s) {
        ...
        for(size_t i = 0; i < 130; ++i) {
            ...
            if(i == 0) assert(buffer_stream_send_from_isr(pool, bytes, sizes[s]));
        }
    ...
    puts(
        "Arrays: self-copy, full equality, callbacks; pools: aligned storage, 130 buffers, balanced ownership passed");
    return 0;
}
