...
class MemoryManagerTest(unittest.TestCase):
    def test_allocation_boundaries(self):
        ...
        calloc = glue[glue.index("void* calloc("):glue.index("char* strdup(")]
        calloc = calloc.replace("void* calloc(", "void* firmware_calloc(")
        newlib = glue[glue.index("void* __wrap__calloc_r("):glue.index("void* __wrap__realloc_r(")]
        newlib = newlib.replace("return calloc(", "return firmware_calloc(")
        ...
        harness = harness.replace("/* CALLOC */", calloc + newlib)
