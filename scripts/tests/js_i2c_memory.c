...
static bool furi_hal_i2c_rx(const int *handle, uint32_t address, uint8_t *data, size_t length,
                            uint32_t timeout)
{
...
static bool furi_hal_i2c_trx(const int *handle, uint32_t address, const uint8_t *tx,
                             size_t tx_length, uint8_t *rx, size_t rx_length, uint32_t timeout)
{
...
static void run(int32_t length, mjs_val_t tx_type, bool invalid_timeout)
{
    ...
    if (invalid_timeout)
        mjs.args[mjs.nargs++] = MJS_UNDEFINED;
    ...
    if (length <= 0 || invalid_timeout) {
        ...
        if (length <= 0)
            assert(allocation_calls == (tx_type == ARRAY ? 1U : 0U));
    } else {
