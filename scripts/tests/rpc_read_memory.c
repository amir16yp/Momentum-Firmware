...
void *tracked_realloc(void *pointer, size_t size)
{
    ...
    if (size > old_size)
        memset((uint8_t *)(block + 1) + old_size, 0, size - old_size);
    return block + 1;
}
...
static void rpc_system_storage_reset_state(RpcStorageSystem *storage, RpcSession *session,
                                           bool error)
{
...
static void rpc_send(RpcSession *session, PB_Main *message)
{
    ...
    if (size)
        assert(memcmp(data->bytes, input + output_size, size) == 0);
    output_size += size;
