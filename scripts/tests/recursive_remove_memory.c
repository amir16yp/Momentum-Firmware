...
enum { FSE_OK, FSE_ERROR, FSE_NOT_EXIST };
...
{
    if (storage->fail_remove && !strcmp(path, storage->fail_remove))
        return FSE_ERROR;
    for (size_t i = 0; i < storage->count; ++i) {
        Node *node = &storage->nodes[i];
        if (node->removed || strcmp(node->path, path))
            continue;
...
{
    if (file->storage->fail_open && !strcmp(path, file->storage->fail_open))
        return false;
...
int main(void)
{
    ...
    for (unsigned failure = 0; failure < 4; ++failure) {
        ...
        if (failure == 0)
            storage.fail_remove = "/ext/tree/file";
        if (failure == 1)
            storage.fail_remove = "/ext/tree";
