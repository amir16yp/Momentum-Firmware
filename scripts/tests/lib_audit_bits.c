...
static uint16_t reference_crc(const uint8_t *data, size_t size, unsigned bits, uint16_t poly,
                              uint16_t initial, bool reflect_in, bool reflect_out, uint16_t xor_out)
{
...
int main(void)
{
        ...
        *last = value;
        for (unsigned offset = 0; offset < 8; ++offset)
            for (unsigned length = 0; length <= 8 - offset; ++length)
                assert(bit_lib_get_bits(last, offset, length) ==
                       reference_get(last, offset, length));
        free(last);
    }
    ...
    for (size_t offset = 0; offset < 16; ++offset) {
        for (unsigned length = 0; length <= 64; ++length) {
            ...
            if (length <= 32)
                assert(bit_lib_get_bits_32(data, offset, length) == expected);
            if (length <= 16)
                assert(bit_lib_get_bits_16(data, offset, length) == expected);
        }
    }
    for (size_t size = 0; size <= sizeof(data); ++size) {
        for (unsigned flags = 0; flags < 4; ++flags) {
            bool in = flags & 1, out = flags & 2;
            assert(bit_lib_crc8(data, size, 0x31, 0xa5, in, out, 0x5a) ==
                   reference_crc(data, size, 8, 0x31, 0xa5, in, out, 0x5a));
            assert(bit_lib_crc16(data, size, 0x1021, 0xa5a5, in, out, 0x5a5a) ==
