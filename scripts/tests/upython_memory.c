...
static void *mp_flipper_print_data_alloc(void)
{
    return furi_string_alloc();
}
static void mp_flipper_print_data_free(void *data)
{
    furi_string_free(data);
}
static const char *mp_flipper_print_get_data(void *data)
{
...
static size_t mp_flipper_repl_autocomplete(const char *str, size_t len, const mp_print_t *print,
                                           const char **completion)
{
    (void)str;
    (void)len;
    if (completion_mode == (size_t)-1)
        print->print_strn(print->data, "matches", 7);
...
static size_t memmgr_get_free_heap(void)
{
    return 10000;
}
static void mp_flipper_set_root_module_path(const char *path)
{
    assert(!strcmp(path, "/ext"));
}
...
static void mp_flipper_deinit(void)
{
    deinit_calls++;
}
/* FILE */
