...
static void test_value(uint32_t value)
{
    ...
    assert(varint_uint32_unpack(&decoded, data, size) == size && decoded == value);
    assert(varint_int32_unpack(&signed_decoded, data, size) == size &&
...
int main(void)
{
    const uint32_t edges[] = {0,         1,          127,       128,       16383,
                              16384,     0x1fffff,   0x200000,  0xfffffff, 0x10000000,
                              INT32_MAX, 0x80000000, UINT32_MAX};
