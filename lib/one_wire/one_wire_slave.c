...
static bool onewire_slave_wait_while_gpio_is(OneWireSlave *bus, uint32_t time_us,
                                             const bool pin_value)
{
...
static inline bool onewire_slave_receive_and_process_command(OneWireSlave *bus)
{
    ...
    if (bus->error == OneWireSlaveErrorResetInProgress) {
        if (!bus->is_first_reset) {
            ...
            bus->is_short_reset =
                onewire_slave_wait_while_gpio_is(bus,
...
{
    furi_check(bus);
    const OneWireSlaveTimings *new_timings =
        set ? &onewire_slave_timings_overdrive : &onewire_slave_timings_normal;
