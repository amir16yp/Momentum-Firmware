...
bool subghz_tx_rx_worker_write(SubGhzTxRxWorker *instance, uint8_t *data, size_t size)
{
    ...
    if (size && (stream_tx_free_byte >= size)) {
        if (furi_stream_buffer_send(instance->stream_tx, data, size,
                                    SUBGHZ_TXRX_WORKER_TIMEOUT_READ_WRITE_BUF) == size) {
            ret = true;
        }
    }
    return ret;
...
void subghz_tx_rx_worker_set_callback_have_read(SubGhzTxRxWorker *instance,
...
bool subghz_tx_rx_worker_rx(SubGhzTxRxWorker *instance, uint8_t *data, uint8_t *size)
{
    ...
    if (subghz_devices_rx_pipe_not_empty(instance->device)) {
        FURI_LOG_I(TAG, "RSSI: %03.1fdbm LQI: %d",
    ...
    while (instance->worker_running) {
        ...
        if (size_tx > 0 && !timeout_tx) {
            ...
            if (size_tx > SUBGHZ_TXRX_WORKER_MAX_TXRX_SIZE) {
                furi_stream_buffer_receive(instance->stream_tx, &data,
                ...
                furi_stream_buffer_receive(instance->stream_tx, &data, size_tx,
                                           SUBGHZ_TXRX_WORKER_TIMEOUT_READ_WRITE_BUF);
            ...
            if (subghz_tx_rx_worker_rx(instance, data, size_rx)) {
                if (furi_stream_buffer_spaces_available(instance->stream_rx) >= size_rx[0]) {
                    ...
                    furi_stream_buffer_send(instance->stream_rx, &data, size_rx[0],
                                            SUBGHZ_TXRX_WORKER_TIMEOUT_READ_WRITE_BUF);
                    if (callback_rx) {
...
bool subghz_tx_rx_worker_start(SubGhzTxRxWorker *instance, const SubGhzDevice *device,
                               uint32_t frequency)
{
    furi_check(instance);
    furi_check(!instance->worker_running);
