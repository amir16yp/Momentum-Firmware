...
// So if we need change something inside of protocol data - we need use this API from protocols to
// get and set data
...
{
    // if override flag was enabled then return succes TRUE and return overrided counter, else
    // return success = FALSE we cut counter bits length to available protocol bits length by the
...
{
    // if override flag was enabled then return succes TRUE and return overrided button, else return
    // success = FALSE we cut button bits length to available protocol bits length by the logical
    // AND function
                                                           ...
                                                           SubGhzRadioPreset *preset)
{
    ...
    do {
        ...
        if (!flipper_format_write_header_cstr(flipper_format, SUBGHZ_KEY_FILE_TYPE,
                                              SUBGHZ_KEY_FILE_VERSION)) {
        ...
        if (!strcmp(furi_string_get_cstr(temp_str), "FuriHalSubGhzPresetCustom")) {
            if (!flipper_format_write_string_cstr(flipper_format, "Custom_preset_module",
                                                  "CC1101")) {
                FURI_LOG_E(TAG, "Unable to add Custom_preset_module");
                res = SubGhzProtocolStatusErrorParserCustomPreset;
                break;
            }
            if (!flipper_format_write_hex(flipper_format, "Custom_preset_data", preset->data,
                                          preset->data_size)) {
