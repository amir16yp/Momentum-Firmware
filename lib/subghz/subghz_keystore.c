...
static void subghz_keystore_add_key(SubGhzKeystore *instance, const char *name, uint64_t key,
                                    uint16_t type)
{
                ...
                if (buffer[i] == '\n' && encrypted_line_cursor > 0) {
                    ...
                    if (iv) {
                        ...
                        if (len % 32 == 0) {
                            ...
                            if (furi_hal_crypto_decrypt((uint8_t *)encrypted_line,
                                                        (uint8_t *)decrypted_line, len)) {
...
bool subghz_keystore_save(SubGhzKeystore *instance, const char *file_name, uint8_t *iv)
{
    ...
    do {
        ...
        if (!flipper_format_write_header_cstr(flipper_format, SUBGHZ_KEYSTORE_FILE_TYPE,
        ...
        for
            M_EACH(key, instance->data, SubGhzKeyArray_t)
            {
                ...
                int len = snprintf(decrypted_line, SUBGHZ_KEYSTORE_FILE_DECRYPTED_LINE_SIZE,
                                   "%08lX%08lX:%hu:%s", (uint32_t)(key->key >> 32),
                                   (uint32_t)key->key, key->type, furi_string_get_cstr(key->name));
                ...
                // Form encrypted line
                if (!furi_hal_crypto_encrypt((uint8_t *)decrypted_line, (uint8_t *)encrypted_line,
                                             len)) {
...
bool subghz_keystore_raw_encrypted_save(const char *input_file_name, const char *output_file_name,
                                        uint8_t *iv)
{
    ...
    do {
        ...
        if (!flipper_format_read_uint32(input_flipper_format, "Encryption", (uint32_t *)&encryption,
                                        1)) {
            FURI_LOG_E(TAG, "Missing encryption type");
            break;
        }
        ...
        if (!flipper_format_write_header_cstr(output_flipper_format, furi_string_get_cstr(filetype),
                                              SUBGHZ_KEYSTORE_FILE_VERSION)) {
            FURI_LOG_E(TAG, "Unable to add header");
            break;
        }
        ...
        do {
            ...
            memset(encrypted_line, 0, SUBGHZ_KEYSTORE_FILE_ENCRYPTED_LINE_SIZE);
            // Form encrypted line
            if (!furi_hal_crypto_encrypt((uint8_t *)buffer, (uint8_t *)encrypted_line,
                                         FILE_BUFFER_SIZE / 2)) {
                FURI_LOG_E(TAG, "Encryption failed");
                result = false;
                break;
...
bool subghz_keystore_raw_get_data(const char *file_name, size_t offset, uint8_t *data, size_t len)
{
    ...
    do {
        ...
        do {
            ...
            if (!furi_hal_crypto_decrypt((uint8_t *)buffer, (uint8_t *)decrypted_line,
                                         bufer_size / 2)) {
                decrypted = false;
                FURI_LOG_E(TAG, "Decryption failed");
                break;
