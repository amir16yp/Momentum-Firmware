...
static void subghz_protocol_encoder_hollarm_get_upload(SubGhzProtocolEncoderHollarm *instance)
{
    ...
    for (uint8_t i = instance->generic.data_count_bit; i > 0; i--) {
        // Read and prepare levels with 2 bit (was saved for better parsing) to the left offset to
        // fit with the original remote transmission
...
SubGhzProtocolStatus subghz_protocol_encoder_hollarm_deserialize(void *context,
        ...
        if (!level) {
            ...
            if ((DURATION_DIFF(instance->decoder.te_last, subghz_protocol_hollarm_const.te_short) <
                ...
                // Bit 1 is short 200us HIGH + short x8 = 1600us LOW timing
                                      ...
                                      subghz_protocol_hollarm_const.te_short) <
                        subghz_protocol_hollarm_const.te_delta) &&
                ...
                // (we have 42 high level pulses, last or first one may be a stop/start bit but we
                // will parse it as zero)
                subghz_protocol_blocks_add_bit(&instance->decoder, 0);
...
{
    const char *name_btn[16] = {"Unknown",
                                "Disarm", // B (2)
                                "Arm",    // A (1)
                                "0x3",
                                "Ringtone/Alarm", // C (3)
...
SubGhzProtocolStatus subghz_protocol_decoder_hollarm_serialize(void *context,
                                                               FlipperFormat *flipper_format,
...
SubGhzProtocolStatus subghz_protocol_decoder_hollarm_deserialize(void *context,
                                                                 FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderHollarm *instance = context;
...
void subghz_protocol_decoder_hollarm_get_string(void *context, FuriString *output)
{
    ...
    furi_string_cat_printf(output,
                           ...
                           (uint32_t)instance->generic.data, instance->generic.serial, bytesum,
