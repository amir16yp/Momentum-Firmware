...
SubGhzProtocolStatus subghz_protocol_encoder_came_deserialize(void *context,
...
void subghz_protocol_decoder_came_feed(void *context, bool level, uint32_t duration)
{
    ...
    case CameDecoderStepFoundStartBit:
        if (!level) {
            break;
        } else if (DURATION_DIFF(duration, subghz_protocol_came_const.te_short) <
        ...
        if (level) {
            if ((DURATION_DIFF(instance->decoder.te_last, subghz_protocol_came_const.te_short) <
                ...
                instance->decoder.parser_step = CameDecoderStepSaveDuration;
                                      ...
                                      subghz_protocol_came_const.te_long) <
                        subghz_protocol_came_const.te_delta) &&
...
SubGhzProtocolStatus subghz_protocol_decoder_came_serialize(void *context,
                                                            FlipperFormat *flipper_format,
...
SubGhzProtocolStatus subghz_protocol_decoder_came_deserialize(void *context,
                                                              FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderCame *instance = context;
...
void subghz_protocol_decoder_came_get_string(void *context, FuriString *output)
{
    ...
    furi_string_cat_printf(output,
                           ...
                           "Key:0x%08lX\r\n"
                           "Yek:0x%08lX\r\n",
                           name, instance->generic.data_count_bit, code_found_lo,
