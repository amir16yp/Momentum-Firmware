...
static bool subghz_protocol_somfy_telis_gen_data(SubGhzProtocolEncoderSomfyTelis *instance,
                                                 uint8_t btn, bool new_remote)
{
 ...
 */
static bool
subghz_protocol_encoder_somfy_telis_get_upload(SubGhzProtocolEncoderSomfyTelis *instance,
                                               uint8_t btn)
{
...
SubGhzProtocolStatus subghz_protocol_encoder_somfy_telis_deserialize(void *context,
...
void subghz_protocol_decoder_somfy_telis_feed(void *context, bool level, uint32_t duration)
{
    ...
    case SomfyTelisDecoderStepDecoderData:
        if (!level) {
            ...
            } else if (DURATION_DIFF(duration, subghz_protocol_somfy_telis_const.te_long) <
                       subghz_protocol_somfy_telis_const.te_delta) {
                event = ManchesterEventLongLow;
            } else if (duration >= (subghz_protocol_somfy_telis_const.te_long +
                                    subghz_protocol_somfy_telis_const.te_delta)) {
                if (instance->decoder.decode_count_bit ==
                    subghz_protocol_somfy_telis_const.min_count_bit_for_found) {
                    // check crc
                    uint64_t data_tmp =
                        instance->decoder.decode_data ^ (instance->decoder.decode_data >> 8);
...
static void subghz_protocol_somfy_telis_check_remote_controller(SubGhzBlockGeneric *instance)
{
     ...
     *      0x9     Sun + Flag  Enable sun and wind detector (SUN and FLAG symbol on the Telis
     * Soliris RC) 0xA     Flag        Disable sun detector (FLAG symbol on the Telis Soliris RC)
     *
     *      CRC
     *
     *      uint8_t frame[7];
...
{
    const char *name_btn[16] = {"Unknown", "My",   "Up",   "My+Up",    "Down", "My+Down",
                                "Up+Down", "0x07", "Prog", "Sun+Flag", "Flag", "0x0B",
                                "0x0C",    "0x0D", "0x0E", "0x0F"};
...
SubGhzProtocolStatus subghz_protocol_decoder_somfy_telis_serialize(void *context,
                                                                   FlipperFormat *flipper_format,
                                                                   SubGhzRadioPreset *preset)
{
    furi_assert(context);
...
SubGhzProtocolStatus subghz_protocol_decoder_somfy_telis_deserialize(void *context,
                                                                     FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderSomfyTelis *instance = context;
