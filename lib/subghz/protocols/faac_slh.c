...
static bool subghz_protocol_faac_slh_gen_data(SubGhzProtocolEncoderFaacSLH *instance)
{
    ...
    if (subghz_block_generic_global_button_override_get(&instance->generic.btn)) {
        ...
        // If we are using UP button - generate programming mode key and send it, otherwise - send
        // regular key if possible
        if ((custom_btn_id == SUBGHZ_CUSTOM_BTN_UP) &&
            ...
            if (allow_zero_seed || (instance->generic.seed != 0x0)) {
                ...
                if (temp_counter_backup != 0x0) {
                    // check OFEX mode
                    if (furi_hal_subghz_get_rolling_counter_mult() != -0x7FFFFFFF) {
                        // standart counter mode. PULL data from subghz_block_generic_global
                        // variables
                        if (!subghz_block_generic_global_counter_override_get(
                                &temp_counter_backup)) {
                            // if counter_override_get return FALSE then counter was not changed and
                            // we increase counter by standart mult value
            ...
            uint32_t enc_prg_1 =
                data_prg[7] << 24 | data_prg[6] << 16 | data_prg[5] << 8 | data_prg[4];
            uint32_t enc_prg_2 =
                data_prg[3] << 24 | data_prg[2] << 16 | data_prg[1] << 8 | data_prg[0];
            ...
            // If we are in prog mode and regular Send button is used - Do not generate new data,
    ...
    if (allow_zero_seed || (instance->generic.seed != 0x0)) {
        ...
        if (furi_hal_subghz_get_rolling_counter_mult() != -0x7FFFFFFF) {
            ...
            } else if ((instance->generic.cnt >= 0xFFFFF) &&
...
SubGhzProtocolStatus subghz_protocol_encoder_faac_slh_deserialize(void *context,
                                                                  FlipperFormat *flipper_format)
{
    ...
    do {
        ...
        subghz_protocol_faac_slh_check_remote_controller(&instance->generic, instance->keystore,
        ...
        if (!level) {
            if ((DURATION_DIFF(instance->decoder.te_last, subghz_protocol_faac_slh_const.te_short) <
                ...
                instance->decoder.parser_step = FaacSLHDecoderStepSaveDuration;
                                      ...
                                      subghz_protocol_faac_slh_const.te_long) <
                        subghz_protocol_faac_slh_const.te_delta) &&
                                                             ...
                                                             const char **manufacture_name)
{
    ...
    for
        M_EACH(manufacture_code, *subghz_keystore_get_data(keystore), SubGhzKeyArray_t)
        {
            if (strcmp(furi_string_get_cstr(manufacture_code->name), "FAAC_SLH") == 0) {
                // FAAC Learning
                man = subghz_protocol_keeloq_common_faac_learning(instance->seed,
                                                                  manufacture_code->key);
...
SubGhzProtocolStatus subghz_protocol_decoder_faac_slh_serialize(void *context,
                                                                FlipperFormat *flipper_format,
...
SubGhzProtocolStatus subghz_protocol_decoder_faac_slh_deserialize(void *context,
                                                                  FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderFaacSLH *instance = context;
...
void subghz_protocol_decoder_faac_slh_get_string(void *context, FuriString *output)
{
    ...
    if (faac_prog_mode == true) {
        furi_string_cat_printf(
            ...
            (uint32_t)(instance->generic.data_2 >> 32), (uint32_t)instance->generic.data_2,
            instance->generic.seed, (uint8_t)(instance->generic.cnt & 0xFF));
        ...
        furi_string_cat_printf(output,
                               ...
                               instance->generic.protocol_name, instance->generic.data_count_bit,
                               (uint32_t)(instance->generic.data >> 32),
                               (uint32_t)instance->generic.data, code_fix, instance->generic.cnt,
                               code_hop, instance->generic.btn, instance->generic.serial,
                               instance->generic.seed);
    }
}
