                                            ...
                                            SubGhzRadioPreset *preset)
{
    ...
    instance->generic.data_2 = ((uint64_t)0x7e << 56 | (uint64_t)cnt << 40 | (uint64_t)serial << 8);
...
SubGhzProtocolStatus subghz_protocol_encoder_came_atomo_deserialize(void *context,
...
void subghz_protocol_decoder_came_atomo_feed(void *context, bool level, uint32_t duration)
{
    ...
    case CameAtomoDecoderStepReset:
        // There are two known options for the header: 72K us (TOP42R, TOP44R) or 12k us (found on
        // TOP44RBN) / 19k us (TOPD4REN)
    ...
    case CameAtomoDecoderStepDecoderData:
        if (!level) {
            ...
            } else if (DURATION_DIFF(duration, subghz_protocol_came_atomo_const.te_long) <
                       subghz_protocol_came_atomo_const.te_delta) {
                event = ManchesterEventLongLow;
            } else if (duration >= ((uint32_t)subghz_protocol_came_atomo_const.te_long * 2 +
                                    subghz_protocol_came_atomo_const.te_delta)) {
...
static void subghz_protocol_came_atomo_remote_controller(SubGhzBlockGeneric *instance)
{
     ...
     *
     *  ***Actual***
     * Button hold-cycle counter (8-bit, from 0 to 0x7F) should DO full cycle or half cycle keeping
     * values like zero 0x1FF08D9924984115 - received data 0x00F7266DB67BEEA0 - inverted data
     ...
     *        0x0 - Button code (4-bit, 0x0 - #1 left-up; 0x2 - #2 right-up; 0x4 - #3 left-down; 0x6
     * - #4 right-down) 0x0 - Last zero nibble
     * */
...
SubGhzProtocolStatus subghz_protocol_decoder_came_atomo_serialize(void *context,
                                                                  FlipperFormat *flipper_format,
                                                                  SubGhzRadioPreset *preset)
{
    furi_assert(context);
...
SubGhzProtocolStatus subghz_protocol_decoder_came_atomo_deserialize(void *context,
                                                                    FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderCameAtomo *instance = context;
...
void subghz_protocol_decoder_came_atomo_get_string(void *context, FuriString *output)
{
    ...
    furi_string_cat_printf(output,
                           ...
                           code_found_hi, code_found_lo, instance->generic.serial,
                           instance->generic.btn, instance->generic.cnt, instance->generic.cnt_2);
}
