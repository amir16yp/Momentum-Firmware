...
SubGhzProtocolStatus subghz_protocol_encoder_hormann_deserialize(void *context,
        ...
        if (!level) {
            if ((DURATION_DIFF(instance->decoder.te_last, subghz_protocol_hormann_const.te_short) <
                ...
                instance->decoder.parser_step = HormannDecoderStepSaveDuration;
                                      ...
                                      subghz_protocol_hormann_const.te_long) <
                        subghz_protocol_hormann_const.te_delta) &&
...
SubGhzProtocolStatus subghz_protocol_decoder_hormann_serialize(void *context,
                                                               FlipperFormat *flipper_format,
...
SubGhzProtocolStatus subghz_protocol_decoder_hormann_deserialize(void *context,
                                                                 FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderHormann *instance = context;
...
void subghz_protocol_decoder_hormann_get_string(void *context, FuriString *output)
{
    ...
    furi_string_cat_printf(output,
                           ...
                           (uint32_t)instance->generic.data, instance->generic.btn);
}
