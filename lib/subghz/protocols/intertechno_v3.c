 ...
 */
static bool
subghz_protocol_encoder_intertechno_v3_get_upload(SubGhzProtocolEncoderIntertechno_V3 *instance)
{
    ...
    for (uint8_t i = instance->generic.data_count_bit; i > 0; i--) {
        if ((instance->generic.data_count_bit == INTERTECHNO_V3_DIMMING_COUNT_BIT) && (i == 9)) {
            ...
            instance->encoder.upload[index++] =
                level_duration_make(true, (uint32_t)subghz_protocol_intertechno_v3_const.te_short);
            instance->encoder.upload[index++] =
                level_duration_make(false, (uint32_t)subghz_protocol_intertechno_v3_const.te_short);
            instance->encoder.upload[index++] =
                level_duration_make(true, (uint32_t)subghz_protocol_intertechno_v3_const.te_short);
            instance->encoder.upload[index++] =
                level_duration_make(false, (uint32_t)subghz_protocol_intertechno_v3_const.te_short);
    ...
    case IntertechnoV3DecoderStepCheckDuration:
        if (level) {
            ...
            if ((DURATION_DIFF(instance->decoder.te_last,
                               subghz_protocol_intertechno_v3_const.te_short) <
                 subghz_protocol_intertechno_v3_const.te_delta) &&
                ...
                (DURATION_DIFF(instance->decoder.te_last,
                               subghz_protocol_intertechno_v3_const.te_long) <
...
void subghz_protocol_decoder_intertechno_v3_get_string(void *context, FuriString *output)
{
    ...
    if (instance->generic.data_count_bit ==
        ...
        if (instance->generic.cnt >> 5) {
            furi_string_cat_printf(output, "Ch: All Btn:%s\r\n",
                                   (instance->generic.btn ? "On" : "Off"));
            subghz_block_generic_global.btn_is_available = false;
            subghz_block_generic_global.btn_length_bit = 1;
        } else {
            furi_string_cat_printf(output, "Ch:" CH_PATTERN " Btn:%s\r\n",
