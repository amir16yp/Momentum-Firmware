...
static bool subghz_protocol_somfy_keytis_gen_data(SubGhzProtocolEncoderSomfyKeytis *instance,
                                                  uint8_t btn)
{
                                              ...
                                              SubGhzRadioPreset *preset)
{
    ...
    if ((res == SubGhzProtocolStatusOk) &&
        !flipper_format_insert_or_update_uint32(flipper_format, "Duration_Counter",
 ...
 */
static bool
subghz_protocol_encoder_somfy_keytis_get_upload(SubGhzProtocolEncoderSomfyKeytis *instance,
                                                uint8_t btn)
{
...
SubGhzProtocolStatus subghz_protocol_encoder_somfy_keytis_deserialize(void *context,
...
void subghz_protocol_decoder_somfy_keytis_feed(void *context, bool level, uint32_t duration)
{
    ...
    case SomfyKeytisDecoderStepCheckPreambula:
        if (level) {
            if (DURATION_DIFF(duration, subghz_protocol_somfy_keytis_const.te_short * 4) <
                ...
                instance->decoder.parser_step = SomfyKeytisDecoderStepFoundPreambula;
                instance->header_count++;
            } else if ((instance->header_count > 1) &&
    ...
    case SomfyKeytisDecoderStepDecoderData:
        if (!level) {
            ...
            } else if (DURATION_DIFF(duration, subghz_protocol_somfy_keytis_const.te_long) <
                       subghz_protocol_somfy_keytis_const.te_delta) {
                event = ManchesterEventLongLow;
            } else if (duration >= (subghz_protocol_somfy_keytis_const.te_long +
                                    subghz_protocol_somfy_keytis_const.te_delta)) {
...
{
    const char *name_btn[0x10] = {"Unknown", "0x01", "0x02", "Prog", "Key_1", "0x05",
...
SubGhzProtocolStatus subghz_protocol_decoder_somfy_keytis_serialize(void *context,
                                                                    FlipperFormat *flipper_format,
                                                                    SubGhzRadioPreset *preset)
{
    furi_assert(context);
    ...
    if ((ret == SubGhzProtocolStatusOk) &&
        !flipper_format_write_uint32(flipper_format, "Duration_Counter", &instance->generic.seed,
                                     1)) {
        FURI_LOG_E(TAG, "Unable to add Duration_Counter");
...
SubGhzProtocolStatus subghz_protocol_decoder_somfy_keytis_deserialize(void *context,
                                                                      FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderSomfyKeytis *instance = context;
    ...
    do {
        ...
        if (!flipper_format_read_uint32(flipper_format, "Duration_Counter",
                                        (uint32_t *)&instance->generic.seed, 1)) {
...
void subghz_protocol_decoder_somfy_keytis_get_string(void *context, FuriString *output)
{
    ...
    furi_string_cat_printf(output,
                           ...
                           (uint32_t)instance->generic.data, instance->generic.seed,
