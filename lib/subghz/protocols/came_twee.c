...
static const uint32_t came_twee_magic_numbers_xor[15] = {
    0x0E0E0E00, 0x1D1D1D11, 0x2C2C2C22, 0x3B3B3B33, 0x4A4A4A44, 0x59595955, 0x68686866, 0x77777777,
    0x86868688, 0x95959599, 0xA4A4A4AA, 0xB3B3B3BB, 0xC2C2C2CC, 0xD1D1D1DD, 0xE0E0E0EE,
};
...
SubGhzProtocolStatus subghz_protocol_encoder_came_twee_deserialize(void *context,
...
void subghz_protocol_decoder_came_twee_feed(void *context, bool level, uint32_t duration)
{
    ...
    case CameTweeDecoderStepReset:
        if ((!level) && ((DURATION_DIFF(duration, subghz_protocol_came_twee_const.te_long * 51) <
            ...
            manchester_advance(instance->manchester_saved_state, ManchesterEventLongLow,
                               &instance->manchester_saved_state, NULL);
    ...
    case CameTweeDecoderStepDecoderData:
        if (!level) {
            ...
            } else if (DURATION_DIFF(duration, subghz_protocol_came_twee_const.te_long) <
                       subghz_protocol_came_twee_const.te_delta) {
                event = ManchesterEventLongLow;
            } else if (duration >= ((uint32_t)subghz_protocol_came_twee_const.te_long * 2 +
                                    subghz_protocol_came_twee_const.te_delta)) {
...
SubGhzProtocolStatus subghz_protocol_decoder_came_twee_serialize(void *context,
                                                                 FlipperFormat *flipper_format,
...
SubGhzProtocolStatus subghz_protocol_decoder_came_twee_deserialize(void *context,
                                                                   FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderCameTwee *instance = context;
...
void subghz_protocol_decoder_came_twee_get_string(void *context, FuriString *output)
{
    ...
    furi_string_cat_printf(output,
                           ...
                           code_found_hi, code_found_lo, instance->generic.btn,
