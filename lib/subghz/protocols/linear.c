...
SubGhzProtocolStatus subghz_protocol_encoder_linear_deserialize(void *context,
        ...
        if (!level) { // save interval
            if (duration >= (subghz_protocol_linear_const.te_short * 5)) {
                ...
                if (DURATION_DIFF(instance->decoder.te_last,
                                  subghz_protocol_linear_const.te_short) <
                    subghz_protocol_linear_const.te_delta) {
                    subghz_protocol_blocks_add_bit(&instance->decoder, 0);
                } else if (DURATION_DIFF(instance->decoder.te_last,
                                         subghz_protocol_linear_const.te_long) <
                           subghz_protocol_linear_const.te_delta) {
...
SubGhzProtocolStatus subghz_protocol_decoder_linear_serialize(void *context,
                                                              FlipperFormat *flipper_format,
...
SubGhzProtocolStatus subghz_protocol_decoder_linear_deserialize(void *context,
                                                                FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderLinear *instance = context;
...
void subghz_protocol_decoder_linear_get_string(void *context, FuriString *output)
{
    ...
    furi_string_cat_printf(output,
                           ...
                           code_found_lo, code_found_reverse_lo, DATA_TO_DIP(code_found_lo));
}
