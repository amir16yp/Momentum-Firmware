 ...
 */
static void subghz_protocol_encoder_ditec_gol4_get_upload(SubGhzProtocolEncoderDitecGOL4 *instance)
{
...
static void subghz_protocol_ditec_gol4_encode_key(SubGhzBlockGeneric *instance)
{
    // Encoder crypto part:
    //
    // TODO: Current issue - last bit at original remote sometimes 0 but we encode as 1, or vice
    // versa. This does not affect decoding but may have issue on real receiver
    //
    uint8_t decrypted[GOL4_RAW_BYTES];
...
SubGhzProtocolStatus subghz_protocol_encoder_ditec_gol4_deserialize(void *context,
    ...
    case DitecGOL4DecoderStepCheckDuration:
        if (level) {
            if ((DURATION_DIFF(instance->decoder.te_last,
                               subghz_protocol_ditec_gol4_const.te_short) <
                 subghz_protocol_ditec_gol4_const.te_delta) &&
                ...
                instance->decoder.parser_step = DitecGOL4DecoderStepSaveDuration;
                                      ...
                                      subghz_protocol_ditec_gol4_const.te_long) <
                        subghz_protocol_ditec_gol4_const.te_delta) &&
            ...
            if (DURATION_DIFF(instance->decoder.te_last,
                              subghz_protocol_ditec_gol4_const.te_long * 20) <
...
SubGhzProtocolStatus subghz_protocol_decoder_ditec_gol4_serialize(void *context,
                                                                  FlipperFormat *flipper_format,
...
SubGhzProtocolStatus subghz_protocol_decoder_ditec_gol4_deserialize(void *context,
                                                                    FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderDitecGOL4 *instance = context;
...
void subghz_protocol_decoder_ditec_gol4_get_string(void *context, FuriString *output)
{
    ...
    furi_string_cat_printf(output,
                           ...
                           (instance->generic.btn == 0x0) ? "- Prog" : "", instance->generic.cnt);
}
