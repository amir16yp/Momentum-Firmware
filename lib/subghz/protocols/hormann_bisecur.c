...
bool manchester_advance_alt(ManchesterState state, ManchesterEvent event,
 ...
 */
static bool
subghz_protocol_decoder_hormann_bisecur_check_crc(SubGhzProtocolDecoderHormannBiSecur *instance);
 ...
 */
static void
subghz_protocol_decoder_hormann_bisecur_add_bit(SubGhzProtocolDecoderHormannBiSecur *instance,
                                                bool level);
 ...
 */
static bool
subghz_protocol_encoder_hormann_bisecur_get_upload(SubGhzProtocolEncoderHormannBiSecur *instance)
{
...
SubGhzProtocolStatus
subghz_protocol_decoder_hormann_bisecur_serialize(void *context, FlipperFormat *flipper_format,
...
void subghz_protocol_decoder_hormann_bisecur_get_string(void *context, FuriString *output)
{
    ...
    furi_string_cat_printf(output,
                           ...
                           instance->crc, valid_crc ? "OK" : "WRONG", instance->type,
                           instance->generic.serial, instance->generic.data,
...
void subghz_protocol_decoder_hormann_bisecur_get_string_brief(void *context, FuriString *output)
{
    ...
    uint8_t data_hash = subghz_protocol_blocks_xor_bytes((const uint8_t *)&instance->generic.data,
                                                         sizeof(uint64_t));
    uint8_t data_2_hash = subghz_protocol_blocks_xor_bytes(
        (const uint8_t *)&instance->generic.data_2, sizeof(uint64_t));

    furi_string_cat_printf(output, "HBS %08lX:%02X%02X", instance->generic.serial, data_hash,
                           data_2_hash);
}
