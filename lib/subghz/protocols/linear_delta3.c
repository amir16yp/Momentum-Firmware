...
subghz_protocol_encoder_linear_delta3_get_upload(SubGhzProtocolEncoderLinearDelta3 *instance)
{
    ...
    if (bit_read(instance->generic.data, 0)) {
        ...
        // Send PT_GUARD
        instance->encoder.upload[index] =
            level_duration_make(false, (uint32_t)subghz_protocol_linear_delta3_const.te_short * 73);
    } else {
        // send bit 0
        instance->encoder.upload[index++] =
            level_duration_make(true, (uint32_t)subghz_protocol_linear_delta3_const.te_long);
        // Send PT_GUARD
        instance->encoder.upload[index] =
            level_duration_make(false, (uint32_t)subghz_protocol_linear_delta3_const.te_short * 70);
    }
        ...
        if (!level) {
            if (duration >= (subghz_protocol_linear_delta3_const.te_short * 10)) {
                ...
                if (DURATION_DIFF(instance->decoder.te_last,
                                  subghz_protocol_linear_delta3_const.te_short) <
...
SubGhzProtocolStatus subghz_protocol_decoder_linear_delta3_serialize(void *context,
