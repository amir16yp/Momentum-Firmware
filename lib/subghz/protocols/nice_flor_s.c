...
static void subghz_protocol_nice_flor_s_remote_controller(SubGhzBlockGeneric *instance,
...
void *subghz_protocol_encoder_nice_flor_s_alloc(SubGhzEnvironment *environment)
{
    ...
    if (instance->nice_flor_s_rainbow_table_file_name) {
        FURI_LOG_D(TAG, "Loading rainbow table from %s",
                   instance->nice_flor_s_rainbow_table_file_name);
    }
 ...
 */
static void subghz_protocol_encoder_nice_flor_s_get_upload(SubGhzProtocolEncoderNiceFlorS *instance,
                                                           uint8_t btn, const char *file_name)
{
    ...
    for (int i = 0; i < 16; i++) {
        static const uint64_t loops[16] = {0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7,
...
SubGhzProtocolStatus subghz_protocol_encoder_nice_flor_s_deserialize(void *context,
...
uint64_t subghz_protocol_nice_flor_s_encrypt(uint64_t data, const char *file_name)
{
    ...
    if (subghz_keystore_raw_get_data(file_name, 0, buffer,
                                     SUBGHZ_NICE_FLOR_S_RAINBOW_TABLE_SIZE_BYTES)) {
    } else {
        return SUBGHZ_NO_NICE_FLOR_S_RAINBOW_TABLE;
    }
...
static uint64_t subghz_protocol_nice_flor_s_decrypt(SubGhzBlockGeneric *instance,
                                                    const char *file_name)
{
    furi_assert(instance);
                                             ...
                                             SubGhzRadioPreset *preset, bool nice_one)
{
    ...
    uint64_t decrypt = ((uint64_t)instance->generic.serial << 16) | instance->generic.cnt;
    uint64_t enc_part =
        subghz_protocol_nice_flor_s_encrypt(decrypt, instance->nice_flor_s_rainbow_table_file_name);
    ...
    case NiceFlorSDecoderStepCheckDuration:
        if (!level) {
            if ((DURATION_DIFF(instance->decoder.te_last,
                               subghz_protocol_nice_flor_s_const.te_short) <
                 subghz_protocol_nice_flor_s_const.te_delta) &&
...
SubGhzProtocolStatus subghz_protocol_decoder_nice_flor_s_serialize(void *context,
                                                                   FlipperFormat *flipper_format,
                                                                   SubGhzRadioPreset *preset)
{
    ...
    if (instance->generic.data_count_bit == NICE_ONE_COUNT_BIT) {
        ...
        if ((ret == SubGhzProtocolStatusOk) &&
            !flipper_format_insert_or_update_uint32(flipper_format, "Data",
                                                    (uint32_t *)&instance->generic.data_2, 1)) {
...
SubGhzProtocolStatus subghz_protocol_decoder_nice_flor_s_deserialize(void *context,
                                                                     FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderNiceFlorS *instance = context;
...
void subghz_protocol_decoder_nice_flor_s_get_string(void *context, FuriString *output)
{
    ...
    subghz_protocol_nice_flor_s_remote_controller(&instance->generic,
                                                  instance->nice_flor_s_rainbow_table_file_name);
    ...
    if (instance->generic.data_count_bit == NICE_ONE_COUNT_BIT) {
        furi_string_cat_printf(output,
                               ...
                               "Sn:%05lX\r\n"
                               "Cnt:%04lX Btn:%02X\r\n",
                               NICE_ONE_NAME, instance->generic.data_count_bit,
                               instance->generic.data, instance->generic.data_2,
