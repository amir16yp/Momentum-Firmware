...
static uint64_t subghz_protocol_beninca_arc_decrypt(SubGhzBlockGeneric *generic,
                                                    SubGhzKeystore *keystore)
{
    // Beninca ARC Decoder
    ...
    // In case you have the remote with ARC rolling code please share RAW recording where you hold
    // button for 15+ sec with us to improve this part!
...
static void subghz_protocol_beninca_arc_encrypt(SubGhzBlockGeneric *generic,
                                                SubGhzKeystore *keystore, uint64_t middle_bytes)
{
    // Beninca ARC Encoder
    // 01.2026 - @xMasterX (MMX) & @zero-mega
    // Encrypt data
...
static void
subghz_protocol_beninca_arc_encoder_get_upload(SubGhzProtocolEncoderBenincaARC *instance,
                                               size_t *index)
{
...
static void
subghz_protocol_beninca_arc_encoder_prepare_packets(SubGhzProtocolEncoderBenincaARC *instance)
{
    furi_assert(instance);
    ...
    for (uint8_t i = 0; i < 3; i++) {
        subghz_protocol_beninca_arc_encrypt(&instance->generic, instance->keystore,
                                            (uint64_t)((i + 1) * 2));
...
SubGhzProtocolStatus subghz_protocol_encoder_beninca_arc_deserialize(void *context,
    ...
    case BenincaARCDecoderHighLevel:
        if (level) {
            ...
            if ((instance->decoder.decode_count_bit ==
                ...
                instance->decoder.decode_data = 0;
            } else if (instance->decoder.decode_count_bit ==
    ...
    case BenincaARCDecoderLowLevel:
        if (!level) {
            ...
            if ((DURATION_DIFF(instance->decoder.te_last,
                               subghz_protocol_beninca_arc_const.te_short) <
                ...
                // Bit 0 is long and short timing = 600us HIGH (te_last) and 300us LOW
                                      ...
                                      subghz_protocol_beninca_arc_const.te_long) <
                        subghz_protocol_beninca_arc_const.te_delta) &&
...
SubGhzProtocolStatus subghz_protocol_decoder_beninca_arc_serialize(void *context,
                                                                   FlipperFormat *flipper_format,
                                                                   SubGhzRadioPreset *preset)
{
    furi_assert(context);
...
SubGhzProtocolStatus subghz_protocol_decoder_beninca_arc_deserialize(void *context,
                                                                     FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderBenincaARC *instance = context;
...
void subghz_protocol_decoder_beninca_arc_get_string(void *context, FuriString *output)
{
    ...
    furi_string_printf(output,
                       ...
                       "Mc:%0lX Cnt:%0lX\r\n"
                       "Fx:%04lX",
                       instance->base.protocol->name, instance->generic.data_count_bit,
                       instance->generic.data, instance->generic.data_2, instance->generic.serial,
                       instance->generic.btn, (uint32_t)(middle_bytes_dec & 0xFFFFFFFF),
                       instance->generic.cnt, instance->generic.seed & 0xFFFF);
}
