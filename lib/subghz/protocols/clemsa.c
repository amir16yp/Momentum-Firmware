...
SubGhzProtocolStatus subghz_protocol_encoder_clemsa_deserialize(void *context,
    ...
    case ClemsaDecoderStepCheckDuration:
        if (!level) {
            if ((DURATION_DIFF(instance->decoder.te_last, subghz_protocol_clemsa_const.te_short) <
                ...
                instance->decoder.parser_step = ClemsaDecoderStepSaveDuration;
            } else if (DURATION_DIFF(duration, subghz_protocol_clemsa_const.te_short * 51) <
                       subghz_protocol_clemsa_const.te_delta * 25) {
                if (DURATION_DIFF(instance->decoder.te_last,
                                  subghz_protocol_clemsa_const.te_short) <
                    subghz_protocol_clemsa_const.te_delta) {
                    subghz_protocol_blocks_add_bit(&instance->decoder, 0);
                } else if (DURATION_DIFF(instance->decoder.te_last,
                                         subghz_protocol_clemsa_const.te_long) <
...
SubGhzProtocolStatus subghz_protocol_decoder_clemsa_serialize(void *context,
                                                              FlipperFormat *flipper_format,
...
SubGhzProtocolStatus subghz_protocol_decoder_clemsa_deserialize(void *context,
                                                                FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderClemsa *instance = context;
...
void subghz_protocol_decoder_clemsa_get_string(void *context, FuriString *output)
{
    ...
    furi_string_cat_printf(output,
                           ...
                           (uint32_t)(instance->generic.data & 0x3FFFF), instance->generic.btn,
