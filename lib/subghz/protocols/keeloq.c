 ...
 */
static uint32_t subghz_protocol_keeloq_check_remote_controller(SubGhzBlockGeneric *instance,
                                                               SubGhzKeystore *keystore,
                                                               const char **manufacture_name);
...
static bool subghz_protocol_keeloq_gen_data(SubGhzProtocolEncoderKeeloq *instance, uint8_t btn,
                                            bool counter_up, bool skip_btn_check)
{
    ...
    if (prog_mode == PROG_MODE_KEELOQ_BFT) {
        ...
        // If we using Aprimatic programming mode we will trasmit some strange looking hop value,
        // why? cuz manufacturer did it this way :)
        hop = 0x1A2B3C4D;
    } else if (prog_mode == PROG_MODE_KEELOQ_DEA_MIO) {
        // If we using DEA Mio programming mode we will trasmit only FIX value with button code 0xF,
        // hop is zero
        hop = 0x00000000;
    }
        ...
        if (keeloq_counter_mode == 0 || bypass) {
            ...
            if (furi_hal_subghz_get_rolling_counter_mult() != -0x7FFFFFFF || bypass) {
                ...
                if (!subghz_block_generic_global_counter_override_get(&instance->generic.cnt)) {
                    // if counter_override_get return FALSE then counter was not changed and we
                    // increase counter by standart mult value
    ...
    if (prog_mode == PROG_MODE_OFF) {
        ...
        if (strcmp(instance->manufacture_name, "Unknown") == 0) {
            ...
            if (strcmp(instance->manufacture_name, "Aprimatic") == 0) {
                // Aprimatic uses 12bit serial number + 2bit APR1 "parity" bit in front of it
                // replacing first 2 bits of serial Thats in theory! We need to check if this is
                // true for all Aprimatic remotes but we got only 3 recordings to test For now lets
                // assume that this is true for all Aprimatic remotes, if not we will need to add
                // some more code here
            ...
            } else if ((strcmp(instance->manufacture_name, "DTM_Neo") == 0) ||
                ...
                decrypt =
                    btn << 28 | (instance->generic.serial & 0xFFF) << 16 | instance->generic.cnt;
                ...
                decrypt =
                    btn << 28 | (instance->generic.serial & 0xFF) << 16 | instance->generic.cnt;
                ...
                uint32_t dea_serial =
                    (instance->generic.serial & 0xFF) | (((uint32_t)result_disc) << 8);
                decrypt = btn << 28 | (dea_serial & 0xFFF) << 16 | instance->generic.cnt;
                // Dea_Mio -> modified serial in hop, uses last 3 digits modifying first one
                // (example - 419 -> C19) (see formula in result_disc var) - simple learning
            }
            // Old type selector fixage for compatibilitiy with old signal files
            uint8_t kl_type_en = instance->keystore->kl_type;
            for
                M_EACH(manufacture_code, *subghz_keystore_get_data(instance->keystore),
                       SubGhzKeyArray_t)
                {
                    res = strcmp(furi_string_get_cstr(manufacture_code->name),
                                 instance->manufacture_name);
                    if (res == 0) {
                        switch (manufacture_code->type) {
                        case KEELOQ_LEARNING_SIMPLE:
                            // Simple Learning
                            hop = subghz_protocol_keeloq_common_encrypt(decrypt,
                                                                        manufacture_code->key);
                            break;
    ...
    if (hop || (prog_mode == PROG_MODE_KEELOQ_DEA_MIO) || (prog_mode == PROG_MODE_KEELOQ_BFT)) {
        // If we have hop - we will save it to generic data var that will be used later in
        // transmission
...
static size_t
subghz_protocol_encoder_keeloq_encode_to_timings(SubGhzProtocolEncoderKeeloq *instance, uint8_t btn,
                                                 bool counter_up, size_t index)
{
 ...
 */
static bool subghz_protocol_encoder_keeloq_get_upload(SubGhzProtocolEncoderKeeloq *instance,
                                                      uint8_t btn)
{
    furi_assert(instance);
    ...
    // if we change counter/button in SignalSettings menu then we must bypass counter_modes, just
    // gen and save signal file.
    ...
    if (keeloq_counter_mode == 7 && !bypass) {
        ...
        for (uint8_t i = 7; i > 0; i--) {
            if (i == 3) {
                ...
                upindex = subghz_protocol_encoder_keeloq_encode_to_timings(instance, (uint8_t)0x00,
                                                                           false, upindex);
                continue;
            } else if (i == 2) {
                instance->generic.cnt = temp_cnt;
                upindex =
                    subghz_protocol_encoder_keeloq_encode_to_timings(instance, btn, false, upindex);
                continue;
            } else if (i == 1) {
...
SubGhzProtocolStatus subghz_protocol_encoder_keeloq_deserialize(void *context,
                                                                FlipperFormat *flipper_format)
{
    ...
    do {
        ...
        if (flipper_format_read_string(flipper_format, "Manufacture",
                                       instance->manufacture_from_file)) {
            ...
            if (strcmp(instance->manufacture_name, "Sommer(fsk476)") == 0) {
                ...
                if (!flipper_format_update_string_cstr(flipper_format, "Manufacture",
                                                       instance->manufacture_name)) {
        ...
        subghz_protocol_keeloq_check_remote_controller(&instance->generic, instance->keystore,
                                                       &instance->manufacture_name);
        ...
        if (!level) {
            if (duration >= ((uint32_t)subghz_protocol_keeloq_const.te_short * 2 +
                ...
                break;
                                      ...
                                      subghz_protocol_keeloq_const.te_short) <
                        subghz_protocol_keeloq_const.te_delta) &&
                ...
                instance->decoder.parser_step = KeeloqDecoderStepSaveDuration;
                                      ...
                                      subghz_protocol_keeloq_const.te_long) <
                        subghz_protocol_keeloq_const.te_delta * 2) &&
 ...
 * @return true On success
 */
static inline bool subghz_protocol_keeloq_check_decrypt(SubGhzBlockGeneric *instance,
                                                        uint32_t decrypt, uint8_t btn,
                                                        uint32_t end_serial)
{
    furi_assert(instance);
...
static inline bool subghz_protocol_keeloq_check_decrypt_centurion(SubGhzBlockGeneric *instance,
                                                                  uint32_t decrypt, uint8_t btn)
{
    furi_assert(instance);
...
static uint32_t
subghz_protocol_keeloq_check_remote_controller_selector(SubGhzBlockGeneric *instance, uint32_t fix,
                                                        uint32_t hop, SubGhzKeystore *keystore,
                                                        const char **manufacture_name)
{
    // protocol HCS300 uses 10 bits in discriminator, HCS200 uses 8 bits, for backward
    // compatibility, we are looking for the 8-bit pattern HCS300 -> uint16_t end_serial =
    // (uint16_t)(fix & 0x3FF); HCS200 -> uint16_t end_serial = (uint16_t)(fix & 0xFF);
            ...
            if (mf_not_set || (strcmp(furi_string_get_cstr(manufacture_code->name), mfname) == 0)) {
                ...
                case KEELOQ_LEARNING_NORMAL:
                    ...
                    if ((strcmp(furi_string_get_cstr(manufacture_code->name), "Centurion") == 0)) {
                        if (subghz_protocol_keeloq_check_decrypt_centurion(instance, decrypt,
                                                                           btn)) {
                            *manufacture_name = furi_string_get_cstr(manufacture_code->name);
                            keystore->mfname = *manufacture_name;
                            return decrypt;
                        ...
                        if (subghz_protocol_keeloq_check_decrypt(instance, decrypt, btn,
                                                                 end_serial)) {
                            *manufacture_name = furi_string_get_cstr(manufacture_code->name);
                            keystore->mfname = *manufacture_name;
                            return decrypt;
                ...
                case KEELOQ_LEARNING_SECURE:
                    ...
                    if ((strcmp(furi_string_get_cstr(manufacture_code->name), "BFT") == 0)) {
                        ...
                        man = subghz_protocol_keeloq_common_secure_learning(fix, instance->seed,
                                                                            manufacture_code->key);
                        decrypt = subghz_protocol_keeloq_common_decrypt(hop, man);
                        if (subghz_protocol_keeloq_check_decrypt(instance, decrypt, btn,
                                                                 end_serial)) {
                    ...
                    if (subghz_protocol_keeloq_check_decrypt(instance, decrypt, btn, end_serial)) {
                        *manufacture_name = furi_string_get_cstr(manufacture_code->name);
                        ...
                        if (reset_seed_back)
                            instance->seed = 0;
                        // Try with zero seed (some strange remotes have been reported to use 0
                        // seed)
                        man = subghz_protocol_keeloq_common_secure_learning(fix, instance->seed,
                                                                            manufacture_code->key);
                        decrypt = subghz_protocol_keeloq_common_decrypt(hop, man);
                ...
                case KEELOQ_LEARNING_AERF:
                    ...
                    break;
                case KEELOQ_LEARNING_ERREKA:
                    man = subghz_protocol_keeloq_common_learning_erreka(fix, instance->seed,
                                                                        manufacture_code->key);
                    decrypt = subghz_protocol_keeloq_common_decrypt(hop, man);
                    if (subghz_protocol_keeloq_check_decrypt(instance, decrypt, btn, end_serial)) {
                        *manufacture_name = furi_string_get_cstr(manufacture_code->name);
                ...
                case KEELOQ_LEARNING_UNKNOWN:
                    ...
                    // Check for mirrored man
                    man =
                        subghz_protocol_keeloq_common_secure_learning(fix, instance->seed, man_rev);
                    decrypt = subghz_protocol_keeloq_common_decrypt(hop, man);
                    if (subghz_protocol_keeloq_check_decrypt(instance, decrypt, btn, end_serial)) {
                        *manufacture_name = furi_string_get_cstr(manufacture_code->name);
                        keystore->mfname = *manufacture_name;
                                                               ...
                                                               const char **manufacture_name)
{
    ...
    // If we are in BFT / Aprimatic / Dea_Mio programming mode we will set previous remembered
    // counter and skip mf keys check
    ProgMode prog_mode = subghz_custom_btn_get_prog_mode();
    if (prog_mode == PROG_MODE_OFF) {
        ...
        // Case when we have no mf name means that we are checking for the first time and we have to
        ...
        if (strlen(keystore->mfname) < 1) {
            ...
            } else if ((key_hop & 0xFFF) == (0x000) &&
                       (key_fix >> 28) == ((key_hop >> 12) & 0x0f)) {
            ...
            // If we have mfname and its one of AN-Motors or HCS101 we should preform only check for
            // this system
            if (strcmp(keystore->mfname, "AN-Motors") == 0) {
                ...
                *manufacture_name = "HCS101";
                ...
                // Else we have mfname that is not AN-Motors or HCS101 we should check it via
                // default selector
                resdecrypt = subghz_protocol_keeloq_check_remote_controller_selector(
                    instance, key_fix, key_hop, keystore, manufacture_name);
            }
...
SubGhzProtocolStatus subghz_protocol_decoder_keeloq_serialize(void *context,
                                                              FlipperFormat *flipper_format,
                                                              SubGhzRadioPreset *preset)
{
    furi_assert(context);
...
SubGhzProtocolStatus subghz_protocol_decoder_keeloq_deserialize(void *context,
                                                                FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderKeeloq *instance = context;
...
void subghz_protocol_decoder_keeloq_get_string(void *context, FuriString *output)
{
    ...
    // Try to get decrypt for display if mf is known, if not it will be 0 and display will be
    // without decrypt part since it might come already decrypted like in HCS101 or AN-Motors Or we
    // might have Unknown MF
    ...
    if (strcmp(instance->manufacture_name, "AN-Motors") == 0) {
        ...
        furi_string_cat_printf(output,
                               ...
                               instance->generic.cnt, code_found_reverse_lo, instance->generic.btn,
                               instance->manufacture_name);
    } else if (strcmp(instance->manufacture_name, "HCS101") == 0) {
        ...
        if (prog_mode == PROG_MODE_KEELOQ_BFT) {
            furi_string_cat_printf(
                ...
                code_found_lo, code_found_reverse_hi, instance->generic.cnt, code_found_reverse_lo,
            ...
            furi_string_cat_printf(
                ...
                "MF:%s Sd:%08lX",
                instance->generic.protocol_name, instance->generic.data_count_bit, code_found_hi,
                code_found_lo, code_found_reverse_hi, instance->generic.cnt, hopdecrypt,
                instance->generic.btn, instance->manufacture_name, instance->generic.seed);
        }
        ...
        furi_string_cat_printf(output,
                               ...
                               instance->generic.protocol_name, instance->generic.data_count_bit,
                               code_found_hi, code_found_lo, code_found_reverse_hi,
                               code_found_reverse_lo, instance->generic.btn,
                               instance->manufacture_name);
    } else {
        ...
        furi_string_cat_printf(output,
                               ...
                               instance->generic.protocol_name, instance->generic.data_count_bit,
                               code_found_hi, code_found_lo, code_found_reverse_hi,
                               instance->generic.cnt, hopdecrypt, instance->generic.btn,
                               instance->manufacture_name);
    }
}
