...
static void subghz_protocol_encoder_roger_get_upload(SubGhzProtocolEncoderRoger *instance)
{
    ...
    if ((instance->generic.data & 0xFF) == instance->generic.btn) {
        instance->generic.data =
            (uint64_t)instance->generic.serial << 12 | ((uint64_t)btn << 8) | btn;
    } else if (((instance->generic.data & 0xFF) == 0x23) && btn == 0x1) {
        instance->generic.data =
            (uint64_t)instance->generic.serial << 12 | ((uint64_t)btn << 8) | 0x20;
    } else if (((instance->generic.data & 0xFF) == 0x20) && btn == 0x2) {
        instance->generic.data =
            (uint64_t)instance->generic.serial << 12 | ((uint64_t)btn << 8) | 0x23;
    }
    ...
    for (uint8_t i = instance->generic.data_count_bit; i > 0; i--) {
        if (bit_read(instance->generic.data, i - 1)) {
            ...
            if (i == 1) {
                ...
                instance->encoder.upload[index++] =
                    level_duration_make(false, (uint32_t)subghz_protocol_roger_const.te_short * 19);
            } else {
                instance->encoder.upload[index++] =
...
SubGhzProtocolStatus subghz_protocol_encoder_roger_deserialize(void *context,
        ...
        if (!level) {
            ...
            if ((DURATION_DIFF(instance->decoder.te_last, subghz_protocol_roger_const.te_long) <
                ...
                // Bit 0 is short and long timing = 500us HIGH (te_last) and 1000us LOW
                                      ...
                                      subghz_protocol_roger_const.te_short) <
                        subghz_protocol_roger_const.te_delta) &&
...
SubGhzProtocolStatus subghz_protocol_decoder_roger_serialize(void *context,
                                                             FlipperFormat *flipper_format,
...
SubGhzProtocolStatus subghz_protocol_decoder_roger_deserialize(void *context,
                                                               FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderRoger *instance = context;
...
void subghz_protocol_decoder_roger_get_string(void *context, FuriString *output)
{
    ...
    furi_string_cat_printf(output,
                           ...
                           (uint32_t)(instance->generic.data & 0xFFFFFFF), instance->generic.serial,
                           (uint32_t)(instance->generic.data & 0xFF), instance->generic.btn);
}
