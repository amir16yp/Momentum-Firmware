...
static bool subghz_protocol_encoder_bin_raw_get_upload(SubGhzProtocolEncoderBinRAW *instance)
{
    ...
    while ((i < BIN_RAW_MAX_MARKUP_COUNT) && (instance->data_markup[i].bit_count != 0)) {
        ...
        bin_raw_debug("\t%d\t%d\t%d :\t\t%d\r\n", i, instance->data_markup[i].byte_bias,
                      instance->data_markup[i].bit_count, bit_bias);
        for (uint16_t y = instance->data_markup[i].byte_bias * 8;
            ...
            subghz_protocol_blocks_set_bit_array(
                subghz_protocol_blocks_get_bit_array(instance->data, y + bit_bias), instance->data,
                ind++, BIN_RAW_BUF_DATA_SIZE);
        }
        i++;
    }
    bin_raw_debug("\r\n");
    ...
    bin_raw_debug_tag(TAG, "Maximum levels encoded in upload %zu\r\n",
                      instance->encoder.size_upload);
#endif
    instance->encoder.size_upload = subghz_protocol_blocks_get_upload_from_bit_array(
        instance->data, ind, instance->encoder.upload, instance->encoder.size_upload, instance->te,
...
SubGhzProtocolStatus subghz_protocol_encoder_bin_raw_deserialize(void *context,
                                                                 FlipperFormat *flipper_format)
{
    ...
    do {
        ...
        while (flipper_format_read_uint32(flipper_format, "Bit_RAW", (uint32_t *)&temp_data, 1)) {
            ...
            if (!flipper_format_read_hex(flipper_format, "Data_RAW",
        ...
        while ((i < BIN_RAW_MAX_MARKUP_COUNT) && (instance->data_markup[i].bit_count != 0)) {
            bin_raw_debug("\r\n\t%d\t%d\t%d :\t", i, instance->data_markup[i].byte_bias,
                          instance->data_markup[i].bit_count);
 ...
 */
static bool subghz_protocol_bin_raw_check_remote_controller(SubGhzProtocolDecoderBinRAW *instance)
{
    struct {
        float data;
        uint16_t count;
    ...
    if (instance->data_raw_ind < 512) {
        ind = instance->data_raw_ind - 100; // there is usually garbage at the end of the record, we
    ...
    for (size_t i = 0; i < ind; i++) {
        for (size_t k = 0; k < BIN_RAW_SEARCH_CLASSES; k++) {
            if (classes[k].count == 0) {
                ...
                classes[k].count++;
                break;
            } else if (DURATION_DIFF((float)(abs(instance->data_raw[i])), (classes[k].data)) <
                       (classes[k].data /
                        4)) { // if the test value does not differ by more than 25%
    ...
    if (bin_raw_type == BinRAWTypeGap) {
        ...
        do {
            ...
            if (data_temp == 0)
                bit_count++; // there is noise in the package
            for (size_t i = 0; i < (size_t)abs(data_temp); i++) {
                ...
                if (data_temp > 0) {
                    subghz_protocol_blocks_set_bit_array(true, instance->data, ind,
                                                         BIN_RAW_BUF_DATA_SIZE);
                } else {
                    subghz_protocol_blocks_set_bit_array(false, instance->data, ind,
                                                         BIN_RAW_BUF_DATA_SIZE);
                }
            }
        ...
        if (data_temp != 0) {
            ...
            for (uint16_t i = 0; i < data_markup_ind - 1; i++) {
                if ((instance->data_markup[i].bit_count == data_temp) &&
                    ...
                    bin_raw_debug_tag(TAG,
                                      "Comparison of neighboring sequences ind_1=%d ind_2=%d "
                                      "%02X=%02X .... %02X=%02X\r\n",
                                      i, i + 1, instance->data[instance->data_markup[i].byte_bias],
                    ...
                    uint16_t byte_count =
                        subghz_protocol_bin_raw_get_full_byte(instance->data_markup[i].bit_count);
                    if (memcmp(instance->data + instance->data_markup[i].byte_bias,
                               instance->data + instance->data_markup[i + 1].byte_bias,
                               byte_count - 1) == 0) {
                        bin_raw_debug_tag(
        ...
        if (bin_raw_type == BinRAWTypeGap) {
            ...
            for (uint16_t i = 0; i < data_markup_ind - 2; i++) {
                ...
                for (uint16_t y = i + 1; y < data_markup_ind - 1; y++) {
                    bin_raw_debug_tag(TAG,
                                      "Comparison every N sequences ind_1=%d ind_2=%d %02X=%02X "
                                      ".... %02X=%02X\r\n",
                                      i, y, instance->data[instance->data_markup[i].byte_bias],
                    ...
                    if (byte_count ==
                        ...
                        if ((memcmp(instance->data + instance->data_markup[i].byte_bias,
                                    instance->data + instance->data_markup[y].byte_bias,
                                    byte_count - 1) == 0) &&
                            (memcmp(instance->data + instance->data_markup[i + 1].byte_bias,
                            ...
                            for (size_t z = instance->data_markup[y].byte_bias + byte_count;
                                 z < instance->data_markup[i].byte_bias + byte_count; z++) {
                            ...
                            memcpy(markup_temp, instance->data_markup,
                                   BIN_RAW_MAX_MARKUP_COUNT * sizeof(BinRAW_Markup));
                            memset(instance->data_markup, 0x00,
                                   BIN_RAW_MAX_MARKUP_COUNT * sizeof(BinRAW_Markup));
        ...
        for (size_t i = 0; i < instance->data_raw_ind; i++) {
            ...
            if (data_temp == 0)
                break; // found an interval 2 times shorter than TE, this is noise
            bin_raw_debug("%d  ", data_temp);
        ...
        if (ind != 0) {
            ...
            bin_raw_debug("\r\n\t count bit= %zu\tcount full byte= %d\tbias bit= %d\r\n\r\n", ind,
                          subghz_protocol_bin_raw_get_full_byte(ind), bit_bias);
            ...
            if (data_check) {
                for (size_t i = subghz_protocol_bin_raw_get_full_byte(ind) - 1; i > 0; i--) {
                    instance->data[i] =
                        (instance->data[i - 1] << (8 - bit_bias)) | (instance->data[i] >> bit_bias);
                }
                                                     ...
                                                     float rssi)
{
    ...
    default:
        // if instance->decoder.parser_step == BinRAWDecoderStepNoParse or others, restore the
        // initial state
        if (rssi < instance->adaptive_threshold_rssi + BIN_RAW_DELTA_RSSI) {
            instance->decoder.parser_step = BinRAWDecoderStepReset;
        }
...
SubGhzProtocolStatus subghz_protocol_decoder_bin_raw_serialize(void *context,
                                                               FlipperFormat *flipper_format,
    ...
    do {
        ...
        while ((i < BIN_RAW_MAX_MARKUP_COUNT) && (instance->data_markup[i].bit_count != 0)) {
            ...
            if (!flipper_format_write_hex(
                    flipper_format, "Data_RAW", instance->data + instance->data_markup[i].byte_bias,
                    subghz_protocol_bin_raw_get_full_byte(instance->data_markup[i].bit_count))) {
...
SubGhzProtocolStatus subghz_protocol_decoder_bin_raw_deserialize(void *context,
                                                                 FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderBinRAW *instance = context;
