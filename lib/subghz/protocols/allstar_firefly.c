...
subghz_protocol_encoder_allstar_firefly_get_upload(SubGhzProtocolEncoderAllstarFirefly *instance)
{
    ...
    for (uint8_t i = instance->generic.data_count_bit; i > 0; i--) {
        if (bit_read(instance->generic.data, i - 1)) {
            ...
            // Send bit 0
            instance->encoder.upload[index++] =
                level_duration_make(true, (uint32_t)subghz_protocol_allstar_firefly_const.te_short);
            if (i == 1) {
                // Send gap if bit was last
                instance->encoder.upload[index++] = level_duration_make(
                    false, (uint32_t)(subghz_protocol_allstar_firefly_const.te_short * 50) + 400);
...
void subghz_protocol_decoder_allstar_firefly_feed(void *context, bool level,
    ...
    case AllstarFireflyDecoderStepCheckDuration:
        if (!level) {
            ...
            if ((DURATION_DIFF(instance->decoder.te_last,
                               subghz_protocol_allstar_firefly_const.te_long) <
...
void subghz_protocol_decoder_allstar_firefly_get_string(void *context, FuriString *output)
{
    ...
    furi_string_cat_printf(
        ...
        (uint32_t)(instance->generic.data & 0xFFFFF), (uint32_t)(code_found_reverse & 0xFFFFF),
        SHOW_DIP_P(instance->generic.data, DIP_P), SHOW_DIP_P(instance->generic.data, DIP_O),
