    ...
    for (uint8_t i = instance->generic.data_count_bit; i > 0; i--) {
        if (bit_read(instance->generic.data, i - 1)) {
            ...
            if (i == 1) {
                ...
                instance->encoder.upload[index++] =
                    level_duration_make(false, (uint32_t)subghz_protocol_elplast_const.te_long * 8);
            } else {
                instance->encoder.upload[index++] =
...
SubGhzProtocolStatus subghz_protocol_encoder_elplast_deserialize(void *context,
        ...
        if (!level) {
            ...
            if ((DURATION_DIFF(instance->decoder.te_last, subghz_protocol_elplast_const.te_long) <
                ...
                // Bit 0 is short and long timing = 230us HIGH (te_last) and 1550us LOW
                                      ...
                                      subghz_protocol_elplast_const.te_short) <
                        subghz_protocol_elplast_const.te_delta) &&
                ...
                if ((DURATION_DIFF(instance->decoder.te_last,
                                   subghz_protocol_elplast_const.te_long) <
                     subghz_protocol_elplast_const.te_delta)) {
                    subghz_protocol_blocks_add_bit(&instance->decoder, 1);
                }
                if ((DURATION_DIFF(instance->decoder.te_last,
...
SubGhzProtocolStatus subghz_protocol_decoder_elplast_serialize(void *context,
                                                               FlipperFormat *flipper_format,
...
SubGhzProtocolStatus subghz_protocol_decoder_elplast_deserialize(void *context,
                                                                 FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderElplast *instance = context;
...
void subghz_protocol_decoder_elplast_get_string(void *context, FuriString *output)
{
    ...
    furi_string_cat_printf(output,
                           ...
                           (uint32_t)(instance->generic.data & 0xFFFFFF), code_found_reverse_lo);
}
