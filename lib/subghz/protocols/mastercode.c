 ...
 */
static bool subghz_protocol_encoder_mastercode_get_upload(SubGhzProtocolEncoderMastercode *instance)
{
...
SubGhzProtocolStatus subghz_protocol_encoder_mastercode_deserialize(void *context,
    ...
    case MastercodeDecoderStepCheckDuration:
        if (!level) {
            if ((DURATION_DIFF(instance->decoder.te_last,
                               subghz_protocol_mastercode_const.te_short) <
                ...
                instance->decoder.parser_step = MastercodeDecoderStepSaveDuration;
            } else if (DURATION_DIFF(duration, subghz_protocol_mastercode_const.te_short * 15) <
                       subghz_protocol_mastercode_const.te_delta * 15) {
                if (DURATION_DIFF(instance->decoder.te_last,
                                  subghz_protocol_mastercode_const.te_short) <
                    subghz_protocol_mastercode_const.te_delta) {
...
SubGhzProtocolStatus subghz_protocol_decoder_mastercode_serialize(void *context,
                                                                  FlipperFormat *flipper_format,
...
SubGhzProtocolStatus subghz_protocol_decoder_mastercode_deserialize(void *context,
                                                                    FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderMastercode *instance = context;
...
void subghz_protocol_decoder_mastercode_get_string(void *context, FuriString *output)
{
    ...
    furi_string_cat_printf(output,
                           ...
                           (uint64_t)(instance->generic.data), instance->generic.btn,
