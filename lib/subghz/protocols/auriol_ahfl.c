        ...
        if (!level) {
            if (DURATION_DIFF(instance->decoder.te_last, ws_protocol_auriol_ahfl_const.te_short) <
                ...
                if (DURATION_DIFF(duration, ws_protocol_auriol_ahfl_const.te_short * 18) <
                    ...
                    instance->decoder.decode_data = 0;
                    instance->decoder.decode_count_bit = 0;
                } else if (DURATION_DIFF(duration, ws_protocol_auriol_ahfl_const.te_long) <
                           ws_protocol_auriol_ahfl_const.te_delta * 2) {
                    subghz_protocol_blocks_add_bit(&instance->decoder, 0);
                    instance->decoder.parser_step = auriol_AHFLDecoderStepSaveDuration;
                } else if (DURATION_DIFF(duration, ws_protocol_auriol_ahfl_const.te_long * 2) <
...
SubGhzProtocolStatus ws_protocol_decoder_auriol_ahfl_serialize(void *context,
...
SubGhzProtocolStatus ws_protocol_decoder_auriol_ahfl_deserialize(void *context,
                                                                 FlipperFormat *flipper_format)
{
    furi_assert(context);
    WSProtocolDecoderAuriol_AHFL *instance = context;
