...
static const uint32_t subghz_hopper_frequency_list[] = {
    315000000, 390000000, 430500000, 433920000, 434420000, 868350000, 0,
};
...
static void subghz_setting_load_default_preset(SubGhzSetting *instance, const char *preset_name,
...
static void subghz_setting_load_default_region(SubGhzSetting *instance)
{
    ...
    subghz_setting_load_default_preset(instance, "AM270",
                                       subghz_device_cc1101_preset_ook_270khz_async_regs);
    subghz_setting_load_default_preset(instance, "AM650",
    ...
    subghz_setting_load_default_preset(instance, "FM476",
                                       subghz_device_cc1101_preset_2fsk_dev47_6khz_async_regs);
    subghz_setting_load_default_preset(instance, "FM12K",
...
void subghz_setting_load(SubGhzSetting *instance, const char *file_path)
{
    ...
    if (file_path) {
        do {
            ...
            if (!temp_bool) {
                ...
                subghz_setting_load_frequencies(instance->hopper_frequencies,
            ...
            while (flipper_format_read_uint32(fff_data_file, "Frequency", (uint32_t *)&temp_data32,
                                              1)) {
                // Todo FL-3535: add a frequency support check depending on the selected radio
                // device
            ...
            while (flipper_format_read_uint32(fff_data_file, "Hopper_frequency",
                                              (uint32_t *)&temp_data32, 1)) {
                if (furi_hal_subghz_is_frequency_valid(temp_data32)) {
            ...
            while (flipper_format_read_string(fff_data_file, "Custom_preset_name", temp_str)) {
                ...
                subghz_setting_load_custom_preset(instance, furi_string_get_cstr(temp_str),
                                                  fff_data_file);
            }
    ...
    if (!FrequencyList_size(instance->hopper_frequencies)) {
        ...
        subghz_setting_load_frequencies(instance->hopper_frequencies, subghz_hopper_frequency_list);
    }
}
...
bool subghz_setting_load_custom_preset(SubGhzSetting *instance, const char *preset_name,
                                       FlipperFormat *fff_data_file)
{
    ...
    do {
        ...
        item->custom_preset_data = malloc(item->custom_preset_data_size);
        if (!flipper_format_read_hex(fff_data_file, "Custom_preset_data", item->custom_preset_data,
...
{
    furi_check(instance);
    return subghz_setting_get_frequency(instance,
                                        subghz_setting_get_frequency_default_index(instance));
}
...
uint8_t subghz_setting_customs_presets_to_log(SubGhzSetting *instance)
{
    ...
    for
        M_EACH(item, instance->preset->data, SubGhzSettingCustomPresetItemArray_t)
        {
            ...
            FURI_LOG_I(TAG, "%u  -  %s", count + 1, furi_string_get_cstr(item->custom_preset_name));
