...
static bool lfrfid_search_data(Storage *storage, uint16_t country_code, FuriString *out_line)
{
    ...
    do {
        // Open file
        if (!flipper_format_file_open_existing(file, RESOURCE_FILE_PATH))
            break;
        ...
        if (furi_string_cmp_str(temp_str, lfrfid_resources_header) ||
            ...
            break;
        if (!flipper_format_read_string(file, furi_string_get_cstr(key), out_line))
            break;
        found = true;
    } while (false);
...
bool iso_3166_get_three_letter(Storage *storage, uint16_t country_code,
