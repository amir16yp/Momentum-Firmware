...
bool lfrfid_raw_file_write_header(LFRFIDRawFile *file, float frequency, float duty_cycle,
                                  uint32_t max_buffer_size)
{
    furi_check(file);

    LFRFIDRawFileHeader header = {.magic = LFRFID_RAW_FILE_MAGIC,
...
bool lfrfid_raw_file_read_pair(LFRFIDRawFile *file, uint32_t *duration, uint32_t *pulse,
                               bool *pass_end)
{
    furi_check(file);
    furi_check(duration);
    ...
    size_t size = 0;
    bool result = varint_pair_unpack(&file->buffer[file->buffer_counter],
                                     (size_t)(file->buffer_size - file->buffer_counter), pulse,
                                     duration, &size);
