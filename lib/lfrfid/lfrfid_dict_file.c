...
bool lfrfid_dict_file_save(ProtocolDict *dict, ProtocolId protocol, const char *filename)
{
    ...
    do {
        if (!flipper_format_file_open_always(file, filename))
            break;
        if (!flipper_format_write_header_cstr(file, LFRFID_DICT_FILETYPE, 1))
            break;

        // TODO FL-3517: write comment about protocol types into file

        if (!flipper_format_write_string_cstr(file, "Key type",
        ...
        if (!flipper_format_write_hex(file, "Data", data, data_size))
            break;
        result = true;
    } while (false);
...
static void lfrfid_dict_protocol_indala_data(uint8_t *data, size_t data_size,
                                             uint8_t *protocol_data, size_t protocol_data_size)
{
    UNUSED(data_size);
...
static ProtocolId lfrfid_dict_protocol_fallback(ProtocolDict *dict, const char *protocol_name,
                                                FlipperFormat *file)
{
...
ProtocolId lfrfid_dict_file_load(ProtocolDict *dict, const char *filename)
{
    ...
    do {
        ...
        if (!flipper_format_read_header(file, str_result, &version))
            break;
        if (furi_string_cmp_str(str_result, LFRFID_DICT_FILETYPE) != 0)
            break;
        if (version != 1)
            break;

        // type
        if (!flipper_format_read_string(file, "Key type", str_result))
            break;
        ProtocolId protocol;
