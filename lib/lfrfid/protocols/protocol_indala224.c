...
static bool protocol_indala224_check_preamble(uint8_t *data, size_t bit_index)
{
    ...
    if (bit_lib_get_bits(data, bit_index, 8) != 0b10000000)
        return false;
    if (bit_lib_get_bits(data, bit_index + 8, 8) != 0)
        return false;
    if (bit_lib_get_bits(data, bit_index + 16, 8) != 0)
        return false;
    if (bit_lib_get_bits(data, bit_index + 24, 6) != 0)
        return false;
    return true;
}
...
static bool protocol_indala224_check_inverted_preamble(uint8_t *data, size_t bit_index)
{
    ...
    if (bit_lib_get_bits(data, bit_index, 8) != 0b01111111)
        return false;
    if (bit_lib_get_bits(data, bit_index + 8, 8) != 0xFF)
        return false;
    if (bit_lib_get_bits(data, bit_index + 16, 8) != 0xFF)
        return false;
    if (bit_lib_get_bits(data, bit_index + 24, 6) != 0b111111)
        return false;
    return true;
}
...
{
    if (!protocol_indala224_check_preamble(data, 0))
        return false;
...
bool protocol_indala224_decoder_feed(ProtocolIndala224 *protocol, bool level, uint32_t duration)
{
    ...
    if (duration > (INDALA224_US_PER_BIT / 2)) {
        ...
        if (protocol_indala224_decoder_feed_internal(!level, duration,
...
static void protocol_indala224_render_data_internal(ProtocolIndala224 *protocol, FuriString *result,
                                                    bool brief)
{
    if (brief) {
        furi_string_printf(result,
                           "Raw: %02X%02X%02X%02X\n"
                           "     %02X%02X%02X%02X...",
                           protocol->data[0], protocol->data[1], protocol->data[2],
                           protocol->data[3], protocol->data[4], protocol->data[5],
                           protocol->data[6], protocol->data[7]);
    } else {
        furi_string_printf(
            result,
            ...
            "%02X%02X%02X%02X %02X%02X%02X%02X\n"
            "%02X%02X%02X%02X",
            protocol->data[0], protocol->data[1], protocol->data[2], protocol->data[3],
            protocol->data[4], protocol->data[5], protocol->data[6], protocol->data[7],
            protocol->data[8], protocol->data[9], protocol->data[10], protocol->data[11],
            protocol->data[12], protocol->data[13], protocol->data[14], protocol->data[15],
            protocol->data[16], protocol->data[17], protocol->data[18], protocol->data[19],
            protocol->data[20], protocol->data[21], protocol->data[22], protocol->data[23],
            protocol->data[24], protocol->data[25], protocol->data[26], protocol->data[27]);
    }
}
