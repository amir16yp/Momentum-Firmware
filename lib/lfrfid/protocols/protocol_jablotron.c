...
static bool protocol_jablotron_can_be_decoded(ProtocolJablotron *protocol)
{
    ...
    if (bit_lib_get_bits_16(protocol->encoded_data, 0, 16) != 0b1111111111111111)
        return false;
    // check next 11 bits preamble
    if (bit_lib_get_bits_16(protocol->encoded_data, 64, 16) != 0b1111111111111111)
        return false;

    uint8_t checksum = bit_lib_get_bits(protocol->encoded_data, 56, 8);
    if (checksum != protocol_jablotron_checksum(protocol->encoded_data))
        return false;
...
bool protocol_jablotron_encoder_start(ProtocolJablotron *protocol)
{
    ...
    // Checksum
    bit_lib_set_bits(protocol->encoded_data, 56,
                     protocol_jablotron_checksum(protocol->encoded_data), 8);
