...
static const char *protocol_insta_fob_get_encoded_data(ProtocolInstaFob *protocol)
{
    ...
    for (size_t i = 0; i < INSTAFOB_ENCODED_DATA_SIZE_BITS; i++) {
        furi_string_cat(protocol->debug_string,
                        bit_lib_get_bit(protocol->encoded_data, i) ? "1" : "0");
    }
...
static ManchesterEvent protocol_insta_fob_manchester_event(bool level, uint32_t duration_us)
{
    ...
    if (protocol_insta_fob_in_range(duration_us, INSTAFOB_READ_SHORT_TIME_BASE,
                                    INSTAFOB_READ_JITTER_TIME_BASE)) {
        if (!level) {
            event = ManchesterEventShortHigh;
        } else {
            event = ManchesterEventShortLow;
        }
    } else if (protocol_insta_fob_in_range(duration_us, INSTAFOB_READ_LONG_TIME_BASE,
                                           INSTAFOB_READ_JITTER_TIME_BASE)) {
        if (!level) {
...
{
    return protocol_insta_fob_in_range(duration_us, INSTAFOB_READ_SEQTERM_TIME_BASE,
                                       INSTAFOB_READ_JITTER_TIME_BASE);
}

static bool protocol_insta_fob_decoder_seq_terminator(ProtocolInstaFob *protocol, bool level,
...
bool protocol_insta_fob_encoder_start(ProtocolInstaFob *protocol)
{
    ...
    if (block1 != INSTAFOB_BLOCK1) {
        FURI_LOG_E(TAG, "Block 1 has wrong data (%08lx). Updating to %08lx.", block1,
    ...
    bit_lib_copy_bits(protocol->encoded_data, 0, 8 * INSTAFOB_DECODED_DATA_SIZE_BYTES,
                      protocol->data, 0);
...
{
    furi_string_printf(result, "InstaFob\nBlk[1]: %08lX\nBlk[2]: %08lX",
...
{
    furi_string_printf(result, "Fob %08lX %08lX", bit_lib_get_bits_32(protocol->data, 0, 32),
                       bit_lib_get_bits_32(protocol->data, 32, 32));
}
...
bool protocol_insta_fob_write_data(ProtocolInstaFob *protocol, void *data)
{
    ...
    if (request->write_type == LFRFIDWriteTypeT5577) {
        ...
        FURI_LOG_D(TAG, "block[0]: %08lx block[1]: %08lx block[2]: %08lx", request->t5577.block[0],
                   request->t5577.block[1], request->t5577.block[2]);
    }
    return result;
}
