...
static bool protocol_gproxii_can_be_decoded(ProtocolGProxII *protocol)
{
    ...
    // |           |             |             |             |             |             | | | |
    // 012345 6789 0 1234 5 6789 0 1234 5 6789 0 1234 5 6789 0 1234 5 6789 0 1234 5 6789 0 1234 5
    // 6789 0 1234 5 6789 0 1234 5 6789 0 1234 5
    // ------------------------------------------------------------------------------------------------------------------------------------
    // 111110 0000 0 1001 0 1101 0 1111 0 1000 0 1001 0 0000 0 1001 0 0000 0 1001 0 0000 0 1001 0
    // 0000 0 1001 0 0000 0 1000 0 0000 0 1001 0
    ...
    // --------------------------------------------------------------------------------
    // 00001001 11011111 10001001 00001001 00001001 00001001 00001001 00001000 00001001 - Without
    // parity 10010000 11111011 10010001 10010000 10010000 10010000 10010000 00010000 10010000 -
    // Reversed 10010000 01101011 00000001 00000000 00000000 00000000 00000000 10000000 00000000 -
    // XOR all bytes from 1 using byte 0
    ...
    // 10010000 011010 11 0000000100000000 0 00000000 0000000000000001 0 00000000000000 - Profile:
    // 256 FC: 0 Card: 1
    ...
    // 10111000 100100 10 00000001 00000000 0 00000000010100 00001000100010111000 1 0000 - Profile:
    // 256 FC: 20 Card: 35000

    // X = XOR Key, L = Message length, D = 2 bit check digits, P = Profile, E = Wiegand leading
    // even parity F = Faclity code, C = Card number, O = Wiegand trailing odd parity, U = Unused
    // bits

    // Check 6 bits preamble 111110
    if (bit_lib_get_bits(protocol->data, 0, 6) != 0b111110)
        return false;
    ...
    if (card_len == 26) {
        ...
        if (!wiegand_check(fc_and_card, even_parity, odd_parity, card_len))
            return false;
    } else if (card_len == 36) {
...
void protocol_gproxii_render_data(ProtocolGProxII *protocol, FuriString *result)
{
    ...
    if (card_len == 26) { // 26 Bit card
        ...
        furi_string_cat_printf(result, "FC: %u Card: %u LEN: %hhu\n",
                               bit_lib_get_bits(protocol->decoded_data, 33, 8),
                               bit_lib_get_bits_16(protocol->decoded_data, 41, 16), card_len);
        // XOR Key, CRC and Profile
        furi_string_cat_printf(result, "XOR: %hhu CRC: %hhu P: %04hX", xor_code, crc_code,
        ...
        furi_string_cat_printf(result, "FC: %u Card: %u LEN: %hhu\n",
                               ...
                               bit_lib_get_bits_16(protocol->decoded_data, 51, 16), card_len);
        // XOR Key, CRC and Profile
        furi_string_cat_printf(result, "XOR: %hhu CRC: %hhu P: %04hX", xor_code, crc_code,
                               bit_lib_get_bits_16(protocol->decoded_data, 16, 16));
    } else {
