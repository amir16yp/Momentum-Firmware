...
static void protocol_hid_generic_string_cat_protocol_bits(ProtocolHID *protocol,
                                                          uint8_t protocol_size, FuriString *result)
{
    ...
    for (size_t i = 0; i < hex_character_count; i++) {
        uint8_t nibble = i == 0 ? bit_lib_get_bits(protocol->data, protocol_bit_index,
...
void protocol_hid_generic_render_data(ProtocolHID *protocol, FuriString *result)
{
    ...
    if (protocol_size == HID_PROTOCOL_SIZE_UNKNOWN) {
        furi_string_printf(result,
                           ...
                           protocol->data[3], protocol->data[4], protocol->data[5] >> 4);
    } else {
        furi_string_printf(result,
