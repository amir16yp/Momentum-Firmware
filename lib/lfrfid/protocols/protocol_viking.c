...
static bool protocol_viking_can_be_decoded(ProtocolViking *protocol)
{
    ...
    if (bit_lib_get_bits_16(protocol->encoded_data, 0, 16) != 0b1111001000000000)
        return false;
    if (bit_lib_get_bits(protocol->encoded_data, 16, 8) != 0b00000000)
        return false;

    // check next 24 bits preamble
    if (bit_lib_get_bits_16(protocol->encoded_data, 64, 16) != 0b1111001000000000)
        return false;
    if (bit_lib_get_bits(protocol->encoded_data, 80, 8) != 0b00000000)
        return false;
...
bool protocol_viking_encoder_start(ProtocolViking *protocol)
{
    ...
    uint8_t checksum =
        ((id >> 24) & 0xFF) ^ ((id >> 16) & 0xFF) ^ ((id >> 8) & 0xFF) ^ (id & 0xFF) ^ 0xF2 ^ 0xA8;
