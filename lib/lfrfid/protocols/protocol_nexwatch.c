...
static ProtocolNexwatchMagic magic_items[] = {
    {0xBE, "Quadrakey", 0}, {0x88, "Nexkey", 0}, {0x86, "Honeywell", 0}};
...
{
    // 01010110
    if (bit_lib_get_bits(data, bit_index, 8) != 0b01010110)
        return false;
    return true;
}
...
{
    if (!protocol_nexwatch_check_preamble(data, 0))
        return false;
...
bool protocol_nexwatch_decoder_feed(ProtocolNexwatch *protocol, bool level, uint32_t duration)
{
    ...
    if (duration > (NEXWATCH_US_PER_BIT / 2)) {
        ...
        if (protocol_nexwatch_decoder_feed_internal(!level, duration,
    ...
    if (duration > (NEXWATCH_US_PER_BIT / 4)) {
        ...
        if (protocol_nexwatch_decoder_feed_internal(!level, duration,
            ...
            protocol_nexwatch_decoder_save(protocol->data,
                                           protocol->corrupted_negative_encoded_data);
...
static void protocol_nexwatch_render_data_internal(ProtocolNexwatch *protocol, FuriString *result,
                                                   bool brief)
{
    uint32_t id = 0;
