...
static void em4100_decode(const uint8_t *encoded_data, const uint8_t encoded_data_size,
                          uint8_t *decoded_data, const uint8_t decoded_data_size)
{
    ...
    for (uint8_t r = 0; r < EM_ROW_COUNT; r++) {
        ...
        for (uint8_t i = 0; i < 5; i++) {
            if (i < 4)
                nibble = (nibble << 1) | (card_data & (1LLU << 63) ? 1 : 0);
            card_data = card_data << 1;
        }
                                  ...
                                  const uint8_t *encoded_epilogue)
{
    ...
    if ((*card_data & EM_HEADER_AND_STOP_MASK) != EM_HEADER_AND_STOP_DATA)
        return false;
...
bool protocol_em4100_decoder_feed(ProtocolEM4100 *proto, bool level, uint32_t duration)
{
    ...
    } else if (duration > protocol_em4100_get_long_time_low(proto) &&
    ...
    if (event != ManchesterEventReset) {
        ...
        if (data_ok) {
            ...
            if (em4100_can_be_decoded((uint8_t *)&proto->encoded_data, sizeof(EM4100DecodedData),
                ...
                em4100_decode((uint8_t *)&proto->encoded_data, sizeof(EM4100DecodedData),
                              proto->data, EM4100_DECODED_DATA_SIZE);
                result = true;
            }
        }
    }
...
bool protocol_em4100_write_data(ProtocolEM4100 *protocol, void *data)
{
    ...
    em4100_decode((uint8_t *)&protocol->encoded_data, sizeof(EM4100DecodedData), protocol->data,
    ...
    if (request->write_type == LFRFIDWriteTypeT5577) {
        ...
        for (uint8_t i = 0; i < 64; i++) {
            encoded_data_reversed =
                (encoded_data_reversed << 1) | ((protocol->encoded_data >> i) & 1);
        }
...
void protocol_em4100_render_data(ProtocolEM4100 *protocol, FuriString *result)
{
    ...
    furi_string_printf(result,
                       ...
                       "DEZ 8: %08lu\n"
                       "DEZ 10: %010lu",
                       data[2], (uint16_t)((data[3] << 8) | (data[4])), protocol->clock_per_bit,
