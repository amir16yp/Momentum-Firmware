...
#define INDALA26_ENCODED_DATA_SIZE (((INDALA26_ENCODED_BIT_SIZE) / 8) + INDALA26_PREAMBLE_DATA_SIZE)
...
static bool protocol_indala26_check_preamble(uint8_t *data, size_t bit_index)
{
    ...
    if (*(uint32_t *)&data[bit_index / 8] != 0b00000000000000000000000010100000)
        return false;
    if (bit_lib_get_bit(data, bit_index + 32) != 1)
        return false;
    return true;
}
...
{
    if (!protocol_indala26_check_preamble(data, 0))
        return false;
    if (!protocol_indala26_check_preamble(data, 64))
        return false;
    if (bit_lib_get_bit(data, 61) != 0)
        return false;
    if (bit_lib_get_bit(data, 60) != 0)
        return false;
    return true;
}
...
bool protocol_indala26_decoder_feed(ProtocolIndala *protocol, bool level, uint32_t duration)
{
    ...
    if (duration > (INDALA26_US_PER_BIT / 2)) {
        ...
        if (protocol_indala26_decoder_feed_internal(!level, duration,
    ...
    if (duration > (INDALA26_US_PER_BIT / 4)) {
        ...
        if (protocol_indala26_decoder_feed_internal(!level, duration,
            ...
            protocol_indala26_decoder_save(protocol->data,
                                           protocol->corrupted_negative_encoded_data);
...
void protocol_indala26_render_data_internal(ProtocolIndala *protocol, FuriString *result,
                                            bool brief)
{
    ...
    const uint8_t checksum =
        bit_lib_get_bit(protocol->data, 27) << 1 | bit_lib_get_bit(protocol->data, 28);
    ...
    if (brief) {
        ...
        furi_string_printf(result,
                           ...
                           "Parity: %c\n"
                           "Checksum: %c",
                           fc, card, (wiegand_correct ? '+' : '-'), (checksum_correct ? '+' : '-'));
    }
}
