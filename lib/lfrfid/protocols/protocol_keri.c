...
static bool protocol_keri_check_preamble(uint8_t *data, size_t bit_index)
{
    ...
    if (*(uint32_t *)&data[bit_index / 8] != 0b00000000000000000000000011100000)
        return false;
...
{
    if (!protocol_keri_check_preamble(data, 0))
        return false;
    if (!protocol_keri_check_preamble(data, 64))
        return false;
...
bool protocol_keri_decoder_feed(ProtocolKeri *protocol, bool level, uint32_t duration)
{
    ...
    if (duration > (KERI_US_PER_BIT / 2)) {
        ...
        if (protocol_keri_decoder_feed_internal(!level, duration,
...
bool protocol_keri_encoder_start(ProtocolKeri *protocol)
{
    ...
    *(uint32_t *)&protocol->encoded_data[0] = 0b00000000000000000000000011100000;
    ...
    protocol->encoder.last_bit = bit_lib_get_bit(protocol->encoded_data, KERI_ENCODED_BIT_SIZE - 1);
...
static void protocol_keri_render_data_internal(ProtocolKeri *protocol, FuriString *result,
                                               bool brief)
{
