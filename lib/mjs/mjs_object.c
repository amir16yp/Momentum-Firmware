...
MJS_PRIVATE void mjs_obj_destructor(struct mjs *mjs, void *cell)
{
    ...
    struct mjs_property *destructor = mjs_get_own_property(mjs, obj_val, MJS_DESTRUCTOR_PROP_NAME,
                                                           strlen(MJS_DESTRUCTOR_PROP_NAME));
    if (!destructor)
        return;
    if (!mjs_is_foreign(destructor->value))
        return;

    mjs_custom_obj_destructor_t destructor_fn = mjs_get_ptr(mjs, destructor->value);
    if (destructor_fn)
        destructor_fn(mjs, obj_val);
}
...
MJS_PRIVATE struct mjs_property *mjs_get_own_property(struct mjs *mjs, mjs_val_t obj,
...
MJS_PRIVATE struct mjs_property *mjs_get_own_property_v(struct mjs *mjs, mjs_val_t obj,
                                                        mjs_val_t key)
{
    size_t n;
    char *s = NULL;
...
mjs_val_t mjs_get_v_proto(struct mjs *mjs, mjs_val_t obj, mjs_val_t key)
{
    ...
    if ((p = mjs_get_own_property_v(mjs, obj, key)) != NULL)
        return p->value;
    if ((p = mjs_get_own_property_v(mjs, obj, pn)) == NULL)
        return MJS_UNDEFINED;
    return mjs_get_v_proto(mjs, p->value, key);
}

mjs_err_t mjs_set(struct mjs *mjs, mjs_val_t obj, const char *name, size_t name_len, mjs_val_t val)
{
...
MJS_PRIVATE mjs_err_t mjs_set_internal(struct mjs *mjs, mjs_val_t obj, mjs_val_t name_v, char *name,
