...
#define FAIL_ERR(p, code)                                                                          \
    do {                                                                                           \
        mjs_set_errorf(p->mjs, code, "parse error at line %d: [%.*s]", p->line_no, 10,             \
                       p->tok.ptr);                                                                \
        return code;                                                                               \
    } while (0)
...
static const int s_assign_ops[] = {TOK_ASSIGN,
                                   TOK_PLUS_ASSIGN,
                                   TOK_MINUS_ASSIGN,
                                   TOK_MUL_ASSIGN,
                                   TOK_DIV_ASSIGN,
    ...
    do {                                                                                           \
        ...
        if (findtok(ops, p->tok.tok) != TOK_EOF) {                                                 \
            ...
            if (ops[0] == TOK_LOGICAL_AND || ops[0] == TOK_LOGICAL_OR) {                           \
                emit_byte(p, (uint8_t)(ops[0] == TOK_LOGICAL_AND ? OP_JMP_NEUTRAL_FALSE            \
                                                                 : OP_JMP_NEUTRAL_TRUE));          \
                off_if = p->cur_idx;                                                               \
                emit_init_offset(p);                                                               \
...
static mjs_err_t parse_ternary(struct pstate *p, int prev_op)
{
    ...
    if (p->tok.tok == TOK_QUESTION) {
        ...
        off_endif += mjs_bcode_insert_offset(p, p->mjs, off_else,
                                             p->cur_idx - off_else - MJS_INIT_OFFSET_SIZE);
...
static mjs_err_t parse_if(struct pstate *p)
{
    ...
    if (p->tok.tok == TOK_KEYWORD_ELSE) {
         ...
         */
        off_endif += mjs_bcode_insert_offset(p, p->mjs, off_else,
                                             off_endelse - off_else - MJS_INIT_OFFSET_SIZE);
    } else {
...
static mjs_err_t parse_statement(struct pstate *p)
{
    ...
    case TOK_KEYWORD_WITH:
        mjs_set_errorf(p->mjs, MJS_SYNTAX_ERROR, "[%.*s] is not implemented", p->tok.len,
                       p->tok.ptr);
...
MJS_PRIVATE mjs_err_t mjs_parse(const char *path, const char *buf, struct mjs *mjs)
{
    ...
    memcpy(p.mjs->bcode_gen.buf + start_idx + sizeof(mjs_header_item_t) * MJS_HDR_ITEM_BCODE_OFFSET,
    ...
    memcpy(p.mjs->bcode_gen.buf + start_idx + sizeof(mjs_header_item_t) * MJS_HDR_ITEM_MAP_OFFSET,
    ...
    memcpy(p.mjs->bcode_gen.buf + start_idx + sizeof(mjs_header_item_t) * MJS_HDR_ITEM_TOTAL_SIZE,
