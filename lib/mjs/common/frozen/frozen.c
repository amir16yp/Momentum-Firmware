...
#define CALL_BACK(fr, tok, value, len)                                                             \
    do {                                                                                           \
        if ((fr)->callback && ((fr)->path_len == 0 || (fr)->path[(fr)->path_len - 1] != '.')) {    \
            ...
            (fr)->callback((fr)->callback_data, (fr)->cur_name, (fr)->cur_name_len, (fr)->path,    \
                           &t);                                                                    \
                                                                                                   \
            /* Reset the name */                                                                   \
            (fr)->cur_name = NULL;                                                                 \
...
static void json_scanf_array_elem_cb(void *callback_data, const char *name, size_t name_len,
...
static void json_scanf_cb(void *callback_data, const char *name, size_t name_len, const char *path,
...
static void json_vsetf_cb(void *userdata, const char *name, size_t name_len, const char *path,
...
int json_vsetf(const char *s, int len, struct json_out *out, const char *json_path,
               const char *json_fmt, va_list ap) WEAK;
int json_vsetf(const char *s, int len, struct json_out *out, const char *json_path,
...
static void prettify_cb(void *userdata, const char *name, size_t name_len, const char *path,
                        const struct json_token *t)
{
...
static void json_next_cb(void *userdata, const char *name, size_t name_len, const char *path,
                         const struct json_token *t)
{
