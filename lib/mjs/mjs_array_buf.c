...
static mjs_val_t mjs_mk_dataview_from_buf(struct mjs *mjs, mjs_val_t buf, mjs_dataview_type_t type)
{
    size_t len = 0;
    mjs_array_buf_get_ptr(mjs, buf, &len);
    if (len % mjs_dataview_get_element_len(type) != 0) {
        mjs_prepend_errorf(mjs, MJS_BAD_ARGS_ERROR, "Buffer len is not a multiple of element size");
        return MJS_UNDEFINED;
    }
...
static mjs_val_t mjs_mk_dataview(struct mjs *mjs, size_t len, mjs_val_t arr,
                                 mjs_dataview_type_t type)
{
