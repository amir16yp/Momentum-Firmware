...
{
    mjs_return(mjs, mjs_struct_to_obj(
                        mjs, mjs_get_ptr(mjs, mjs_arg(mjs, 0)),
...
void mjs_init_builtin(struct mjs *mjs, mjs_val_t obj)
{
    ...
    mjs_set(mjs, obj, "ffi_cb_free", ~0, mjs_mk_foreign_func(mjs, (mjs_func_ptr_t)mjs_ffi_cb_free));
    ...
    // v = mjs_mk_object(mjs);
    // mjs_set(
    //     mjs, v, "stringify", ~0, mjs_mk_foreign_func(mjs,
    //     (mjs_func_ptr_t)mjs_op_json_stringify));
    ...
    mjs_set(mjs, v, "defineProperty", ~0,
            mjs_mk_foreign_func(mjs,
                                (mjs_func_ptr_t)mjs_op_object_define_property)); // stub, do not use
