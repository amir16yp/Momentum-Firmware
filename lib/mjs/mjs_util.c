...
void mjs_jprintf(mjs_val_t v, struct mjs *mjs, struct json_out *out)
{
    if (mjs_is_number(v)) {
        ...
        json_printf(out, "%s%lx%s", "<foreign_ptr@", (unsigned long)(uintptr_t)mjs_get_ptr(mjs, v),
                    ">");
...
MJS_PRIVATE size_t mjs_disasm_single(const uint8_t *code, size_t i, MjsPrintCallback print_cb,
                                     void *print_ctx)
{
    char buf[40];
    size_t start_i = i;
    ...
    case OP_SET_ARG: {
        ...
        print_cb(print_ctx, "%s\t[%.*s] %u", buf, (int)n, code + i + 1 + llen + llen2,
                 (unsigned)arg_no);
        i += llen + llen2 + n;
        break;
    }
    ...
    case OP_LOOP: {
        ...
        print_cb(print_ctx, "%s\tB:%lu C:%lu (%d)", buf,
    ...
    case OP_BCODE_HEADER: {
        ...
        memcpy(&map_offset, &code[i + 1 + MJS_HDR_ITEM_MAP_OFFSET * sizeof(total_size)],
               sizeof(map_offset));
        i += sizeof(mjs_header_item_t) * MJS_HDR_ITEMS_CNT;
        print_cb(print_ctx, "%s\t[%s] end:%lu map_offset: %lu", buf, &code[i + 1],
                 (unsigned long)start + total_size, (unsigned long)start + map_offset);
...
static void mjs_dump_obj_stack(struct mjs *mjs, const char *name, const struct mbuf *m,
                               MjsPrintCallback print_cb, void *print_ctx)
{
    char buf[50];
    size_t i, n;
    n = mjs_stack_size(m);
...
MJS_PRIVATE int mjs_check_arg(struct mjs *mjs, int arg_num, const char *arg_name,
                              enum mjs_type expected_type, mjs_val_t *parg)
{
    ...
    if (actual_type != expected_type) {
        mjs_prepend_errorf(mjs, MJS_TYPE_ERROR, "%s should be a %s, %s given", arg_name,
                           mjs_stringify_type(expected_type), mjs_stringify_type(actual_type));
        return 0;
    }
