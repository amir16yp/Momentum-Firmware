...
bool update_operation_get_current_package_manifest_path(Storage *storage, FuriString *out_path)
{
    ...
    if (storage_common_stat(storage, UPDATE_FILE_POINTER_FN, NULL) == FSE_OK) {
        ...
        do {
            ...
            if (!storage_file_open(upd_file, UPDATE_FILE_POINTER_FN, FSAM_READ,
                                   FSOM_OPEN_EXISTING)) {
                break;
            }
            size_t bytes_read =
...
UpdatePrepareResult update_operation_prepare(const char *manifest_file_path)
{
    ...
    do {
        ...
        if (!storage_file_open(file, furi_string_get_cstr(stage_path), FSAM_READ,
                               FSOM_OPEN_EXISTING)) {
        ...
        if (!update_operation_get_current_package_manifest_path(storage, manifest_path_check) ||
            ...
            FURI_LOG_E("update", "Manifest pointer check failed: '%s' != '%s'",
                       furi_string_get_cstr(manifest_path_check), manifest_file_path);
...
bool update_operation_is_armed(void)
{
    ...
    const uint32_t rtc_upd_index = furi_hal_rtc_get_register(FuriHalRtcRegisterUpdateFolderFSIndex);
    Storage *storage = furi_record_open(RECORD_STORAGE);
