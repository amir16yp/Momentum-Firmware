...
static bool flipper_format_stream_write_key(Stream *stream, const char *key)
{
    ...
    do {
        if (!flipper_format_stream_write(stream, key, strlen(key)))
            break;
        if (!flipper_format_stream_write(stream, &flipper_format_delimiter, 1))
            break;
        if (!flipper_format_stream_write(stream, " ", 1))
            break;
        result = true;
    } while (false);
...
bool flipper_format_stream_write_value_line(Stream *stream, FlipperStreamWriteData *write_data)
{
    ...
    if (write_data->type == FlipperStreamValueIgnore) {
        ...
        do {
            if (!flipper_format_stream_write_key(stream, write_data->key))
                break;

            if (write_data->type == FlipperStreamValueStr)
                write_data->data_size = 1;
            ...
            for (uint16_t i = 0; i < write_data->data_size; i++) {
                ...
                case FlipperStreamValueHexUint64: {
                    ...
                    furi_string_printf(value, "%08lX%08lX", (uint32_t)(data[i] >> 32),
                                       (uint32_t)data[i]);
                }; break;
                ...
                if (!flipper_format_stream_write(stream, furi_string_get_cstr(value),
                                                 furi_string_size(value))) {
                    cycle_error = true;
                    break;
                }
            }
            ...
            if (!flipper_format_stream_write_eol(stream))
                break;
            result = true;
        } while (false);
...
bool flipper_format_stream_read_value_line(Stream *stream, const char *key, FlipperStreamValue type,
                                           void *_data, size_t data_size, bool strict_mode)
{
    bool result = false;

    do {
        if (!flipper_format_stream_seek_to_key(stream, key, strict_mode))
            break;
                ...
                if (result) {
                    ...
                    case FlipperStreamValueHex: {
                        ...
                        if (furi_string_size(value) >= 2) {
                            ...
                            if (hex_char_to_uint8(furi_string_get_char(value, 0),
                                                  furi_string_get_char(value, 1), &data[i])) {
                                scan_values = 1;
                            }
                        }
                    }; break;
...
bool flipper_format_stream_get_value_count(Stream *stream, const char *key, uint32_t *count,
                                           bool strict_mode)
{
    bool result = false;
    bool last = false;
...
bool flipper_format_stream_delete_key_and_write(Stream *stream, FlipperStreamWriteData *write_data,
                                                bool strict_mode)
{
    bool result = false;
    ...
    do {
        ...
        // find key
        if (!flipper_format_stream_seek_to_key(stream, write_data->key, strict_mode))
            break;
        ...
        if (!flipper_format_stream_seek_to_next_line(stream))
            break;
        ...
        if (!stream_seek(stream, start_position, StreamOffsetFromStart))
            break;
        if (!stream_delete_and_insert(stream, end_position - start_position,
