...
Iso15693Parser *iso15693_parser_alloc(const GpioPin *pin, size_t max_frame_size)
{
    ...
    signal_reader_set_sample_rate(instance->signal_reader, SignalReaderTimeUnit64Mhz,
                                  ISO15693_PARSER_BITRATE_F64MHZ);
...
void iso15693_parser_start(Iso15693Parser *instance, Iso15693ParserCallback callback, void *context)
{
    furi_assert(instance);
            ...
            if (instance->bitstream_buff[i] != 0x00) {
                for (size_t j = 0; j < 8; j++) {
                    if (FURI_BIT(instance->bitstream_buff[i], j) == 1) {
                        bit_buffer_append_byte(instance->parsed_frame,
                                               instance->next_byte_part * 4 + j / 2);
                    }
                }
            } else {
...
void iso15693_parser_get_data(Iso15693Parser *instance, uint8_t *buff, size_t buff_size,
                              size_t *data_bits)
{
    furi_assert(instance);
    furi_assert(buff);
