...
static void infrared_worker_rx_timeout_callback(void *context)
{
    ...
    uint32_t flags_set = furi_thread_flags_set(furi_thread_get_id(instance->thread),
...
static void infrared_worker_rx_callback(void *context, bool level, uint32_t duration)
{
    ...
    uint32_t events =
        (ret == sizeof(LevelDuration)) ? INFRARED_WORKER_RX_RECEIVED : INFRARED_WORKER_OVERRUN;
...
static void infrared_worker_process_timeout(InfraredWorker *instance)
{
    ...
    if (instance->rx.received_signal_callback)
        instance->rx.received_signal_callback(instance->rx.received_signal_context,
                                              &instance->signal);
}
...
static int32_t infrared_worker_rx_thread(void *thread_context)
{
    ...
    while (1) {
        ...
        if (events & INFRARED_WORKER_RX_RECEIVED) {
            ...
            while (sizeof(LevelDuration) == furi_stream_buffer_receive(instance->stream,
                                                                       &level_duration,
                                                                       sizeof(LevelDuration), 0)) {
...
void infrared_worker_rx_set_received_signal_callback(InfraredWorker *instance,
...
InfraredWorker *infrared_worker_alloc(void)
{
    ...
    size_t buffer_size = MAX(sizeof(InfraredWorkerTiming) * (MAX_TIMINGS_AMOUNT + 1),
...
void infrared_worker_rx_start(InfraredWorker *instance)
{
    ...
    furi_hal_infrared_async_rx_set_timeout_isr_callback(infrared_worker_rx_timeout_callback,
                                                        instance);
...
void infrared_worker_get_raw_signal(const InfraredWorkerSignal *signal, const uint32_t **timings,
                                    size_t *timings_cnt)
{
    furi_check(signal);
    furi_check(timings);
...
void infrared_worker_tx_start(InfraredWorker *instance)
{
    ...
    furi_hal_infrared_async_tx_set_data_isr_callback(infrared_worker_furi_hal_data_isr_callback,
                                                     instance);
...
static void infrared_worker_furi_hal_message_sent_isr_callback(void *context)
{
    InfraredWorker *instance = context;
    uint32_t flags_set = furi_thread_flags_set(furi_thread_get_id(instance->thread),
                                               INFRARED_WORKER_TX_MESSAGE_SENT);
...
infrared_worker_furi_hal_data_isr_callback(void *context, uint32_t *duration, bool *level)
{
    ...
    uint32_t flags_set =
        furi_thread_flags_set(furi_thread_get_id(instance->thread), INFRARED_WORKER_TX_FILL_BUFFER);
...
static int32_t infrared_worker_tx_thread(void *thread_context)
{
    ...
    while (running) {
        ...
        case InfraredWorkerStateStartTx:
            --repeats_left; /* The first message does not result in TX_MESSAGE_SENT event for some
                               reason */
            instance->tx.need_reinitialization = false;
        ...
        case InfraredWorkerStateRunTx:
            events = furi_thread_flags_wait(INFRARED_WORKER_ALL_TX_EVENTS, FuriFlagWaitAny,
                                            FuriWaitForever);
...
void infrared_worker_tx_set_get_signal_callback(InfraredWorker *instance,
...
void infrared_worker_tx_set_signal_sent_callback(InfraredWorker *instance,
...
void infrared_worker_set_raw_signal(InfraredWorker *instance, const uint32_t *timings,
                                    size_t timings_cnt, uint32_t frequency, float duty_cycle)
{
    furi_check(instance);
    furi_check(timings);
