 ...
 *
 * \param[in]   handler     - handler to INFRARED decoders. Should be acquired with \c
 * infrared_alloc_decoder(). \param[in]   level       - high(true) or low(false) level of input
 * signal to analyze. it should alternate every call, otherwise it is an error case, and decoder
 * resets its state and start decoding from the start. \param[in]   duration    - duration of steady
 * high/low input signal. \return      if message is ready, returns pointer to decoded message,
 * returns NULL. Note: ownership of returned ptr belongs to handler. So pointer is valid up to next
 * infrared_free_decoder(), infrared_reset_decoder(), infrared_decode(),
 * infrared_check_decoder_ready() calls.
 */
const InfraredMessage *infrared_decode(InfraredDecoderHandler *handler, bool level,
                                       uint32_t duration);
 ...
 *
 * \param[in]   handler     - handler to INFRARED decoders. Should be acquired with \c
 * infrared_alloc_decoder(). \return      if message is ready, returns pointer to decoded message,
 * returns NULL. Note: ownership of returned ptr belongs to handler. So pointer is valid up to next
 * infrared_free_decoder(), infrared_reset_decoder(), infrared_decode(),
 * infrared_check_decoder_ready() calls.
 */
 ...
 *
 * \param[in]   handler     - handler to INFRARED encoder. Should be acquired with \c
 ...
 *
 * \param[in]   handler     - handler to INFRARED encoder. Should be acquired with \c
 * infrared_alloc_encoder(). \param[out]  duration    - encoded timing. \param[out]  level       -
 * encoded level.
 *
 ...
 *
 * \param[in]   handler     - handler to INFRARED encoder. Should be acquired with \c
 * infrared_alloc_encoder(). \param[in]   message     - message to encode.
 */
