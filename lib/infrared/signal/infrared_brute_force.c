...
static inline void ir_bf_record_init_set(InfraredBruteForceRecord *dest,
                                         const InfraredBruteForceRecord *src)
{
...
static inline void ir_bf_record_set(InfraredBruteForceRecord *dest,
                                    const InfraredBruteForceRecord *src)
{
    dest->index = src->index;
...
DICT_DEF2(InfraredBruteForceRecordDict, FuriString *, FURI_STRING_OPLIST, InfraredBruteForceRecord,
          IR_BF_RECORD_OPLIST);
        ...
        while (infrared_signal_read_name(ff, signal_name) == InfraredErrorCodeNone) {
            ...
            if (!record) {
                if (auto_detect_buttons) {
                    infrared_brute_force_add_record(brute_force, auto_detect_button_index++,
                                ...
                                uint32_t *record_count)
{
    ...
    *record_count = 0;
    ...
    for (InfraredBruteForceRecordDict_it(it, brute_force->records);
         !InfraredBruteForceRecordDict_end_p(it); InfraredBruteForceRecordDict_next(it)) {
...
bool infrared_brute_force_send(InfraredBruteForce *brute_force, uint32_t signal_index)
{
    ...
    if (signal_index >= SignalPositionArray_size(brute_force->current_record.signals))
        return false;
