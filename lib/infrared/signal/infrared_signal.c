...
static bool infrared_signal_is_message_valid(const InfraredMessage *message)
{
    ...
    if (message->address != (message->address & address_mask)) {
        FURI_LOG_E(TAG, "Address is out of range (mask 0x%08lX): 0x%lX\r\n", address_mask,
                   message->address);
        return false;
    }
    ...
    if (message->command != (message->command & command_mask)) {
        FURI_LOG_E(TAG, "Command is out of range (mask 0x%08lX): 0x%lX\r\n", command_mask,
                   message->command);
        return false;
    }
...
static bool infrared_signal_is_raw_valid(const InfraredRawSignal *raw)
{
    if ((raw->frequency > INFRARED_MAX_FREQUENCY) || (raw->frequency < INFRARED_MIN_FREQUENCY)) {
        FURI_LOG_E(TAG, "Frequency is out of range (%X - %X): %lX", INFRARED_MIN_FREQUENCY,
                   INFRARED_MAX_FREQUENCY, raw->frequency);
        return false;
        ...
        FURI_LOG_E(TAG, "Timings amount is out of range (0 - %X): %zX", MAX_TIMINGS_AMOUNT,
                   raw->timings_size);
        return false;
    }
...
static inline InfraredErrorCode infrared_signal_save_message(const InfraredMessage *message,
                                                             FlipperFormat *ff)
{
    const char *protocol_name = infrared_get_protocol_name(message->protocol);
    InfraredErrorCode error = InfraredErrorCodeNone;
    do {
        if (!flipper_format_write_string_cstr(ff, INFRARED_SIGNAL_TYPE_KEY,
                                              INFRARED_SIGNAL_TYPE_PARSED)) {
        ...
        if (!flipper_format_write_hex(ff, INFRARED_SIGNAL_ADDRESS_KEY, (uint8_t *)&message->address,
                                      4)) {
            error = InfraredErrorCodeSignalMessageUnableToWriteAddress;
            break;
        }

        if (!flipper_format_write_hex(ff, INFRARED_SIGNAL_COMMAND_KEY, (uint8_t *)&message->command,
                                      4)) {
...
static inline InfraredErrorCode infrared_signal_save_raw(const InfraredRawSignal *raw,
                                                         FlipperFormat *ff)
{
    ...
    do {
        ...
        if (!flipper_format_write_uint32(ff, INFRARED_SIGNAL_DATA_KEY, raw->timings,
                                         raw->timings_size)) {
...
static inline InfraredErrorCode infrared_signal_read_message(InfraredSignal *signal,
                                                             FlipperFormat *ff)
{
    FuriString *buf;
    ...
    do {
        ...
        if (!flipper_format_read_hex(ff, INFRARED_SIGNAL_ADDRESS_KEY, (uint8_t *)&message.address,
                                     4)) {
            error = InfraredErrorCodeSignalMessageUnableToReadAddress;
            break;
        }
        if (!flipper_format_read_hex(ff, INFRARED_SIGNAL_COMMAND_KEY, (uint8_t *)&message.command,
                                     4)) {
...
static inline InfraredErrorCode infrared_signal_read_raw(InfraredSignal *signal, FlipperFormat *ff)
{
    InfraredErrorCode error = InfraredErrorCodeNone;
...
void infrared_signal_set_signal(InfraredSignal *signal, const InfraredSignal *other)
{
    if (other->is_raw) {
        ...
        infrared_signal_set_raw_signal(signal, raw->timings, raw->timings_size, raw->frequency,
                                       raw->duty_cycle);
    } else {
                                    ...
                                    size_t timings_size, uint32_t frequency, float duty_cycle)
{
    ...
    // TODO: Should we return error instead? Also infrared_signal_is_valid is used only in CLI for
    // some reason?!
...
void infrared_signal_transmit(const InfraredSignal *signal)
{
    if (signal->is_raw) {
        ...
        infrared_send_raw_ext(raw_signal->timings, raw_signal->timings_size, true,
                              raw_signal->frequency, raw_signal->duty_cycle);
    } else {
