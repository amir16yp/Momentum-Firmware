...
#define CC1101_STROBE_SFSTXON                                                                      \
    0x31 /** Enable and calibrate frequency synthesizer (if MCSM0.FS_AUTOCAL=1). If in RX (with    \
            CCA): Go to a wait state where only the synthesizer is running (for quick RX / TX      \
            turnaround). */
#define CC1101_STROBE_SXOFF 0x32 /** Turn off crystal oscillator. */
#define CC1101_STROBE_SCAL                                                                         \
    0x33 /** Calibrate frequency synthesizer and turn it off. SCAL can be strobed from IDLE mode   \
...
#define CC1101_STROBE_STX                                                                          \
    0x35 /** In IDLE state: Enable TX. Perform calibration first if MCSM0.FS_AUTOCAL=1. If in RX   \
            state and CCA is enabled: Only go to TX if channel is clear. */
#define CC1101_STROBE_SIDLE                                                                        \
    0x36 /** Exit RX / TX, turn off frequency synthesizer and exit Wake-On-Radio mode if           \
            applicable. */
...
typedef enum {
    ...
    CC1101StateRXFIFO_OVERFLOW = 0b110,  /** RX FIFO has overflowed. Read out any useful data, then
