...
void lp5562_enable(const FuriHalI2cBusHandle *handle)
{
    ...
    //>488μs delay is required after writing to 0x00 register, otherwise program engine will not
    //work
    furi_delay_us(500);
}

void lp5562_set_channel_current(const FuriHalI2cBusHandle *handle, LP5562Channel channel,
                                uint8_t value)
{
    uint8_t reg_no;
...
void lp5562_set_channel_value(const FuriHalI2cBusHandle *handle, LP5562Channel channel,
                              uint8_t value)
{
    uint8_t reg_no;
    if (channel == LP5562ChannelRed) {
...
void lp5562_set_channel_src(const FuriHalI2cBusHandle *handle, LP5562Channel channel,
                            LP5562Engine src)
{
    uint8_t reg_val = 0;
...
void lp5562_execute_program(const FuriHalI2cBusHandle *handle, LP5562Engine eng, LP5562Channel ch,
                            uint16_t *program)
{
    if ((eng < LP5562Engine1) || (eng > LP5562Engine3))
        return;
    uint8_t reg_val = 0;
    uint8_t bit_offset = 0;
    ...
    furi_hal_i2c_write_mem(handle, LP5562_ADDRESS, 0x10 + (0x20 * (eng - 1)), (uint8_t *)program,
                           16 * 2, LP5562_I2C_TIMEOUT);
...
void lp5562_execute_ramp(const FuriHalI2cBusHandle *handle, LP5562Engine eng, LP5562Channel ch,
                         uint8_t val_start, uint8_t val_end, uint16_t time)
{
    ...
    if (val_end > val_start) {
        ...
        program[1] =
            (prescaller << 14) | (time_step << 8) | 0x80 | ((diff / 2) & 0x7F); // Ramp Down
    }
...
void lp5562_execute_blink(const FuriHalI2cBusHandle *handle, LP5562Engine eng, LP5562Channel ch,
                          uint16_t on_time, uint16_t period, uint8_t brightness)
{
    // Temporary switch to constant value from register
    lp5562_set_channel_src(handle, ch, LP5562Direct);
