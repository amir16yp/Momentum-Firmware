...
FelicaListener *felica_listener_alloc(Nfc *nfc, FelicaData *data)
{
    ...
    nfc_felica_listener_set_sensf_res_data(nfc, data->idm.data, sizeof(data->idm), data->pmm.data,
                                           sizeof(data->pmm), system_code);
...
void felica_listener_set_callback(FelicaListener *listener, NfcGenericCallback callback,
                                  void *context)
{
    UNUSED(listener);
    UNUSED(callback);
...
static FelicaError
felica_listener_command_handler_read(FelicaListener *instance,
                                     const FelicaListenerGenericRequest *const generic_request)
{
    ...
    FelicaListenerReadCommandResponse *resp =
        malloc(sizeof(FelicaCommandResponseHeader) + 1 +
...
static FelicaError
felica_listener_command_handler_write(FelicaListener *instance,
                                      const FelicaListenerGenericRequest *const generic_request)
{
...
static FelicaError
felica_listener_process_request(FelicaListener *instance,
...
static void
felica_listener_populate_polling_response_header(FelicaListener *instance,
...
static bool
felica_listener_check_system_code(const FelicaListenerGenericRequest *const generic_request,
                                  uint16_t code)
{
    return (generic_request->polling.system_code == code ||
            generic_request->polling.system_code == (code | 0x00FFU) ||
            generic_request->polling.system_code == (code | 0xFF00U));
}

static uint16_t
felica_listener_get_response_system_code(FelicaListener *instance,
                                         const FelicaListenerGenericRequest *const generic_request)
{
    ...
    } else if (felica_listener_check_system_code(generic_request,
                                                 FELICA_LISTENER_SYSTEM_CODE_LITES)) {
        // Lite-S
...
static FelicaError
felica_listener_process_system_code(FelicaListener *instance,
                                    const FelicaListenerGenericRequest *const generic_request)
{
            ...
            if (request->header.code == FELICA_LISTENER_CMD_POLLING) {
                ...
                if (request->polling.system_code != FELICA_SYSTEM_CODE_CODE) {
                    ...
                    if (error == FelicaErrorFeatureUnsupported) {
                        ...
                        FURI_LOG_E(TAG, "Error when handling Polling with System Code: %2X", error);
                    }
                    break;
                } else {
