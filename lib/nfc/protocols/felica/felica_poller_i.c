                                  ...
                                  FelicaPollerPollingResponse *resp)
{
    ...
    do {
        ...
        bit_buffer_set_byte(instance->tx_buffer, 0,
                            sizeof(FelicaPollerPollingCommand) + FELICA_CRC_SIZE);
        // Set command code
        bit_buffer_set_byte(instance->tx_buffer, 1, FELICA_POLLER_CMD_POLLING_REQ_CODE);
        // Set other data
        bit_buffer_append_bytes(instance->tx_buffer, (uint8_t *)cmd,
                                sizeof(FelicaPollerPollingCommand));

        error = felica_poller_frame_exchange(instance, instance->tx_buffer, instance->rx_buffer,
                                             FELICA_POLLER_POLLING_FWT);
        ...
        bit_buffer_write_bytes_mid(instance->rx_buffer, resp->pmm.data, sizeof(FelicaIDm) + 2,
                                   sizeof(FelicaPMm));
...
// This is in fact a buffer preparer for a specified service. It should be have the _ex suffix. The
// prepare_tx_buffer_raw should have this name.
static void felica_poller_prepare_tx_buffer(const FelicaPoller *instance, const uint8_t command,
                                            const uint16_t service_code, const uint8_t block_count,
                                            const uint8_t *const blocks,
                                            const uint8_t data_block_count, const uint8_t *data)
{
    ...
    if (data_block_count != 0) {
        bit_buffer_append_bytes(instance->tx_buffer, data,
                                data_block_count * FELICA_DATA_BLOCK_SIZE);
    }
}

FelicaError felica_poller_read_blocks(FelicaPoller *instance, const uint8_t block_count,
                                      const uint8_t *const block_numbers, uint16_t service_code,
                                      FelicaPollerReadCommandResponse **const response_ptr)
{
    ...
    felica_poller_prepare_tx_buffer(instance, FELICA_CMD_READ_WITHOUT_ENCRYPTION, service_code,
                                    block_count, block_numbers, 0, NULL);
                                       ...
                                       FelicaPollerWriteCommandResponse **const response_ptr)
{
    ...
    felica_poller_prepare_tx_buffer(instance, FELICA_CMD_WRITE_WITHOUT_ENCRYPTION,
                                    FELICA_SERVICE_RW_ACCESS, block_count, block_numbers,
                                    block_count, data);
    bit_buffer_reset(instance->rx_buffer);
...
static void felica_poller_prepare_tx_buffer_raw(const FelicaPoller *instance, const uint8_t command,
...
FelicaError
felica_poller_list_service_by_cursor(FelicaPoller *instance, uint16_t cursor,
                                     FelicaListServiceCommandResponse **const response_ptr)
{
    ...
    felica_poller_prepare_tx_buffer_raw(instance, FELICA_CMD_LIST_SERVICE_CODE, data, sizeof(data));
    ...
    if (rx_len < sizeof(FelicaCommandHeaderRaw) + 2)
                                           ...
                                           FelicaListSystemCodeCommandResponse **const response_ptr)
{
    ...
    size_t rx_len = bit_buffer_get_size_bytes(instance->rx_buffer);
    if (rx_len < sizeof(FelicaCommandHeaderRaw) + 3)
        return FelicaErrorProtocol;
