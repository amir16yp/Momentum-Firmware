...
felica_listener_block_list_iterate(FelicaListener *instance,
                                   const FelicaBlockListElement *prev_item, const uint8_t item_size)
{
                                         ...
                                         const FelicaBlockListElement *prev_item)
{
    ...
    return felica_listener_block_list_iterate(instance, prev_item,
                                              FELICA_LISTENER_BLOCK_LIST_ITEM_SIZE(prev_item));
}

const FelicaListenerWriteBlockData *felica_listener_get_write_request_data_pointer(
    const FelicaListener *const instance, const FelicaListenerGenericRequest *const generic_request)
{
    furi_assert(instance);
...
static bool felica_listener_check_write_request_data_size(const FelicaListener *const instance,
                                                          const FelicaListenerRequest *request,
...
static uint8_t felica_listener_get_block_list_item_count_size(FelicaListener *instance,
...
{
    return !((number > FELICA_BLOCK_INDEX_REG && number < FELICA_BLOCK_INDEX_RC) ||
...
static bool felica_get_mc_bit(const FelicaListener *instance, uint8_t byte_index,
                              uint8_t bit_number)
{
...
static bool felica_block_requires_auth(const FelicaListener *instance, uint8_t command,
...
static void felica_handler_read_block(FelicaListener *instance, const uint8_t block_number,
...
static void felica_handler_read_all_zeros(FelicaListener *instance, const uint8_t block_number,
                                          const uint8_t resp_data_index,
                                          FelicaListenerReadCommandResponse *response)
{
    UNUSED(instance);
...
static void felica_handler_read_mac_a_block(FelicaListener *instance, const uint8_t block_number,
                                            const uint8_t resp_data_index,
                                            FelicaListenerReadCommandResponse *response)
{
    if ((resp_data_index != response->block_count - 1) || (resp_data_index == 0)) {
        ...
        felica_calculate_mac_read(
            ...
            instance->data->data.fs.rc.data, &instance->requested_blocks[instance->mac_calc_start],
...
static bool felica_validate_read_block_list(FelicaListener *instance,
...
static bool felica_validate_write_block_list(FelicaListener *instance,
                                             ...
                                             FelicaListenerWriteCommandResponse *response)
{
    ...
    if (request->base.header.block_count == FELICA_LISTENER_WRITE_BLOCK_COUNT_MAX &&
        ...
        write_with_mac = true;
    } else if (request->base.header.block_count < FELICA_LISTENER_WRITE_BLOCK_COUNT_MIN ||
    ...
    for (uint8_t i = 0; i < request->base.header.block_count; i++) {
        ...
        if (item->service_code != 0) {
            ...
            felica_calculate_mac_write(&instance->auth.des_context, instance->auth.session_key.data,
...
static void felica_handler_write_block(FelicaListener *instance, const uint8_t block_number,
                                       const FelicaBlockData *data_block)
{
    uint8_t num = felica_listener_get_block_index(block_number);

    memcpy(&instance->data->data.dump[num * (FELICA_DATA_BLOCK_SIZE + 2) + 2], data_block->data,
           FELICA_DATA_BLOCK_SIZE);
}

static void felica_handler_write_rc_block(FelicaListener *instance, const uint8_t block_number,
                                          const FelicaBlockData *data_block)
{
    felica_handler_write_block(instance, block_number, data_block);

    felica_calculate_session_key(&instance->auth.des_context, instance->data->data.fs.ck.data,
                                 instance->data->data.fs.rc.data, instance->auth.session_key.data);
    instance->rc_written = true;
}

static void felica_handler_write_reg_block(FelicaListener *instance, const uint8_t block_number,
                                           const FelicaBlockData *data_block)
{
    UNUSED(block_number);
...
static void felica_handler_write_mc_block(FelicaListener *instance, const uint8_t block_number,
                                          const FelicaBlockData *data_block)
{
    UNUSED(block_number);
    ...
    for (uint8_t i = 0; i < FELICA_DATA_BLOCK_SIZE; i++) {
        if ((i <= 1) && mc_bits_permission) {
            ...
            instance->mc_shadow.data[i] = data_block->data[i];
        } else if ((i >= FELICA_MC_SP_REG_R_RESTR_BYTES_6_7 && i <= FELICA_MC_STATE_W_MAC_A) &&
                   (mc_bits_permission)) {
            instance->mc_shadow.data[i] |= data_block->data[i];
...
static void felica_handler_write_state_block(FelicaListener *instance, const uint8_t block_number,
                                             const FelicaBlockData *data_block)
{
    felica_handler_write_block(instance, block_number, data_block);
...
static void felica_handler_write_id_block(FelicaListener *instance, const uint8_t block_number,
                                          const FelicaBlockData *data_block)
{
    UNUSED(block_number);
