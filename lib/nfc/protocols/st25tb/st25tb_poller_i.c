...
St25tbError st25tb_poller_send_frame(St25tbPoller *instance, const BitBuffer *tx_buffer,
                                     BitBuffer *rx_buffer, uint32_t fwt)
{
    ...
    furi_assert(tx_bytes <= bit_buffer_get_capacity_bytes(instance->tx_buffer) - ISO14443_CRC_SIZE);
...
St25tbError st25tb_poller_initiate(St25tbPoller *instance, uint8_t *chip_id_ptr)
{
    ...
    St25tbError ret;
    do {
        ret = st25tb_poller_send_frame(instance, instance->tx_buffer, instance->rx_buffer,
                                       ST25TB_FDT_FC);
...
St25tbError st25tb_poller_get_uid(St25tbPoller *instance, uint8_t *uid)
{
    ...
    do {
        ...
        FURI_LOG_I(TAG, "Got tag with uid: %02X %02X %02X %02X %02X %02X %02X %02X", uid[0], uid[1],
                   uid[2], uid[3], uid[4], uid[5], uid[6], uid[7]);
    } while (false);
    return ret;
}

St25tbError st25tb_poller_read_block(St25tbPoller *instance, uint32_t *block, uint8_t block_number)
{
    furi_check(instance);
    furi_check(instance->nfc);
    furi_check(block);
    furi_check((block_number <= st25tb_get_block_count(instance->data->type)) ||
...
St25tbError st25tb_poller_write_block(St25tbPoller *instance, uint32_t block, uint8_t block_number)
{
    furi_check(instance);
    furi_check(instance->nfc);
    furi_check((block_number <= st25tb_get_block_count(instance->data->type)) ||
    ...
    do {
        ...
        if (block_check != block) {
            FURI_LOG_E(TAG, "write verification failed: wrote %08lX but read back %08lX", block,
                       block_check);
