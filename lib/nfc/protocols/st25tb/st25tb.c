...
bool st25tb_load(St25tbData *data, FlipperFormat *ff, uint32_t version)
{
    ...
    do {
        if (version < NFC_UNIFIED_FORMAT_VERSION)
            break;
        if (!flipper_format_read_string(ff, ST25TB_TYPE_KEY, temp_str))
            break;
        ...
        if (!flipper_format_read_hex(ff, ST25TB_SYSTEM_BLOCK_KEY,
                                     (uint8_t *)&data->system_otp_block, 4))
            break;
...
bool st25tb_save(const St25tbData *data, FlipperFormat *ff)
{
    ...
    do {
        if (!flipper_format_write_comment_cstr(ff, ST25TB_PROTOCOL_NAME " specific data"))
            break;
        if (!flipper_format_write_string_cstr(ff, ST25TB_TYPE_KEY,
                                              st25tb_features[data->type].type_name))
            break;
        ...
        if (!flipper_format_write_hex(ff, ST25TB_SYSTEM_BLOCK_KEY,
                                      (uint8_t *)&data->system_otp_block, 4))
            break;
