...
static bool slix_load_passwords(SlixPassword *passwords, SlixType slix_type, FlipperFormat *ff)
{
    ...
    for (uint32_t i = 0; i < COUNT_OF(slix_password_configs); ++i) {
        ...
        if (!slix_type_has_features(slix_type, password_config->feature_flag))
            continue;
        if (!flipper_format_key_exist(ff, password_config->key)) {
            passwords[i] = password_config->default_value;
            continue;
        }
        if (!flipper_format_read_hex(ff, password_config->key, (uint8_t *)&passwords[i],
                                     sizeof(SlixPassword))) {
            ret = false;
            break;
        }
...
bool slix_load(SlixData *data, FlipperFormat *ff, uint32_t version)
{
    ...
    bool loaded = false;
    do {
        if (!iso15693_3_load(data->iso15693_3_data, ff, version))
            break;

        const SlixType slix_type = slix_get_type(data);
        if (slix_type >= SlixTypeCount)
            break;

        if (!slix_load_capabilities(data, ff))
            break;

        if (!slix_load_passwords(data->passwords, slix_type, ff))
            break;

        if (slix_type_has_features(slix_type, SLIX_TYPE_FEATURE_SIGNATURE)) {
            if (flipper_format_key_exist(ff, SLIX_SIGNATURE_KEY)) {
                if (!flipper_format_read_hex(ff, SLIX_SIGNATURE_KEY, data->signature,
                                             SLIX_SIGNATURE_SIZE))
                    break;
            }
        }
        ...
        if (slix_type_has_features(slix_type, SLIX_TYPE_FEATURE_PRIVACY)) {
            if (flipper_format_key_exist(ff, SLIX_PRIVACY_MODE_KEY)) {
                if (!flipper_format_read_bool(ff, SLIX_PRIVACY_MODE_KEY, &data->privacy, 1))
                    break;
            }
        }
        ...
        if (slix_type_has_features(slix_type, SLIX_TYPE_FEATURE_PROTECTION)) {
            ...
            if (flipper_format_key_exist(ff, SLIX_PROTECTION_POINTER_KEY) &&
                ...
                if (!flipper_format_read_hex(ff, SLIX_PROTECTION_POINTER_KEY, &protection->pointer,
                                             1))
                    break;
                if (!flipper_format_read_hex(ff, SLIX_PROTECTION_CONDITION_KEY,
                                             &protection->condition, 1))
                    break;
            }
        }
        ...
        if (slix_type_has_features(slix_type, SLIX_TYPE_FEATURE_EAS)) {
            if (flipper_format_key_exist(ff, SLIX_LOCK_EAS_KEY)) {
                ...
                if (!flipper_format_read_bool(ff, SLIX_LOCK_EAS_KEY, &lock_bits->eas, 1))
                    break;
            }
        }
        ...
        if (slix_type_has_features(slix_type, SLIX_TYPE_FEATURE_PROTECTION)) {
            if (flipper_format_key_exist(ff, SLIX_LOCK_PPL_KEY)) {
                SlixLockBits *lock_bits = &data->system_info.lock_bits;
                if (!flipper_format_read_bool(ff, SLIX_LOCK_PPL_KEY, &lock_bits->ppl, 1))
                    break;
            }
        }
...
static bool slix_save_capabilities(const SlixData *data, FlipperFormat *ff)
{
    ...
    do {
        furi_string_set_str(tmp_str,
                            "SLIX capabilities field affects emulation modes. Possible options: ");
        ...
        if (!flipper_format_write_comment_cstr(ff, furi_string_get_cstr(tmp_str)))
            break;

        if (!flipper_format_write_string_cstr(ff, SLIX_CAPABILITIES_KEY,
                                              slix_capabilities_names[data->capabilities]))
            break;
...
static bool slix_save_passwords(const SlixPassword *passwords, SlixType slix_type,
                                FlipperFormat *ff)
{
    bool ret = true;
    ...
    for (uint32_t i = 0; i < COUNT_OF(slix_password_configs); ++i) {
        ...
        if (!slix_type_has_features(slix_type, password_config->feature_flag))
            continue;
        if (!flipper_format_write_hex(ff, password_config->key, (uint8_t *)&passwords[i],
                                      sizeof(SlixPassword))) {
            ret = false;
            break;
        }
...
bool slix_save(const SlixData *data, FlipperFormat *ff)
{
    ...
    do {
        ...
        if (!iso15693_3_save(data->iso15693_3_data, ff))
            break;
        if (!flipper_format_write_comment_cstr(ff, SLIX_PROTOCOL_NAME " specific data"))
            break;

        if (!slix_save_capabilities(data, ff))
            break;
        ...
        if (!slix_save_passwords(data->passwords, slix_type, ff))
            break;

        if (slix_type_has_features(slix_type, SLIX_TYPE_FEATURE_SIGNATURE)) {
            if (!flipper_format_write_comment_cstr(
                    ff, "This is the card's secp128r1 elliptic curve signature. It can not be "
                        "calculated without knowing NXP's private key."))
                break;
            if (!flipper_format_write_hex(ff, SLIX_SIGNATURE_KEY, data->signature,
                                          SLIX_SIGNATURE_SIZE))
                break;
        }

        if (slix_type_has_features(slix_type, SLIX_TYPE_FEATURE_PRIVACY)) {
            if (!flipper_format_write_bool(ff, SLIX_PRIVACY_MODE_KEY, &data->privacy, 1))
                break;
        }
        ...
        if (slix_type_has_features(slix_type, SLIX_TYPE_FEATURE_PROTECTION)) {
            ...
            if (!flipper_format_write_comment_cstr(ff, "Protection pointer configuration"))
                break;
            if (!flipper_format_write_hex(ff, SLIX_PROTECTION_POINTER_KEY, &protection->pointer,
                                          sizeof(uint8_t)))
                break;
            if (!flipper_format_write_hex(ff, SLIX_PROTECTION_CONDITION_KEY, &protection->condition,
                                          sizeof(uint8_t)))
                break;
        }
        ...
        if (slix_type_has_features(slix_type, SLIX_TYPE_FEATURE_EAS) ||
            ...
            if (!flipper_format_write_comment_cstr(ff, "SLIX Lock Bits"))
                break;
        }

        if (slix_type_has_features(slix_type, SLIX_TYPE_FEATURE_EAS)) {
            const SlixLockBits *lock_bits = &data->system_info.lock_bits;
            if (!flipper_format_write_bool(ff, SLIX_LOCK_EAS_KEY, &lock_bits->eas, 1))
                break;
        }

        if (slix_type_has_features(slix_type, SLIX_TYPE_FEATURE_PROTECTION)) {
            const SlixLockBits *lock_bits = &data->system_info.lock_bits;
            if (!flipper_format_write_bool(ff, SLIX_LOCK_PPL_KEY, &lock_bits->ppl, 1))
                break;
        }
...
bool slix_is_equal(const SlixData *data, const SlixData *other)
{
    ...
    return iso15693_3_is_equal(data->iso15693_3_data, other->iso15693_3_data) &&
           ...
           memcmp(data->passwords, other->passwords,
                  sizeof(SlixPassword) * SlixPasswordTypeCount) == 0 &&
...
bool slix_is_block_protected(const SlixData *data, SlixPasswordType password_type,
                             uint8_t block_num)
{
    furi_check(data);
    furi_check(password_type < SlixPasswordTypeCount);
    ...
    do {
        if (password_type != SlixPasswordTypeRead && password_type != SlixPasswordTypeWrite)
            break;
        if (block_num >= iso15693_3_get_block_count(data->iso15693_3_data))
            break;
