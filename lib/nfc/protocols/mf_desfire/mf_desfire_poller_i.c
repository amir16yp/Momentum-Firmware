                                             ...
                                             BitBuffer *rx_buffer)
{
    ...
    Iso14443_4aError iso14443_4a_error = nxp_native_command_iso14443_4a_poller(
        instance->iso14443_4a_poller, &status_code, tx_buffer, rx_buffer, instance->command_mode,
        instance->tx_buffer, instance->rx_buffer);
...
MfDesfireError mf_desfire_poller_read_version(MfDesfirePoller *instance, MfDesfireVersion *data)
{
    ...
    do {
        error = mf_desfire_poller_send_chunks(instance, instance->input_buffer,
...
MfDesfireError mf_desfire_poller_read_application_ids(MfDesfirePoller *instance, SimpleArray *data)
{
    ...
    do {
        ...
        for (uint32_t i = 0; i < app_id_count; ++i) {
            if (!mf_desfire_application_id_parse(simple_array_get(data, i), i,
                                                 instance->result_buffer)) {
                error = MfDesfireErrorProtocol;
                break;
            }
                                                    ...
                                                    const MfDesfireApplicationId *id)
{
    ...
    bit_buffer_append_bytes(instance->input_buffer, (const uint8_t *)id,
                                             ...
                                             const MfDesfireFileSettings *data, uint16_t iso_ef_id)
{
    ...
    bit_buffer_append_bytes(instance->input_buffer, (const uint8_t *)data->access_rights,
...
static MfDesfireError mf_desfire_poller_read_file(MfDesfirePoller *instance, MfDesfireFileId id,
                                                  uint8_t read_cmd, uint32_t offset, size_t size,
...
MfDesfireError mf_desfire_poller_read_file_records(MfDesfirePoller *instance, MfDesfireFileId id,
                                                   ...
                                                   MfDesfireFileData *data)
{
    return mf_desfire_poller_read_file(instance, id, MF_DESFIRE_CMD_READ_RECORDS, offset, size,
                                       data);
}
                                                      ...
                                                      SimpleArray *data)
{
    ...
    for (size_t i = 0; i < file_id_count; ++i) {
        ...
        if (file_type == MfDesfireFileTypeStandard || file_type == MfDesfireFileTypeBackup) {
            error = mf_desfire_poller_read_file_data(instance, file_id, 0,
                                                     file_settings_cur->data.size, file_data);
        } else if (file_type == MfDesfireFileTypeValue) {
            error = mf_desfire_poller_read_file_value(instance, file_id, file_data);
        } else if (file_type == MfDesfireFileTypeLinearRecord ||
                   file_type == MfDesfireFileTypeCyclicRecord) {
            error = mf_desfire_poller_read_file_records(instance, file_id, 0,
                                                        file_settings_cur->record.size, file_data);
        }
    }
                                                  ...
                                                  MfDesfireApplication *data)
{
    ...
    do {
        ...
        error = mf_desfire_poller_read_key_versions(instance, data->key_versions,
                                                    data->key_settings.max_keys);
        if (error != MfDesfireErrorNone) {
        ...
        error = mf_desfire_poller_read_file_settings_multi(instance, data->file_ids,
                                                           data->file_settings);
        if (error != MfDesfireErrorNone) {
        ...
        error = mf_desfire_poller_read_file_data_multi(instance, data->file_ids,
                                                       data->file_settings, data->file_data);
