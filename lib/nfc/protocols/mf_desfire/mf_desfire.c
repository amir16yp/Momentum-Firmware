...
static const char *const mf_desfire_size_strings[] = {
    [MfDesfireSize2k] = "2K",   [MfDesfireSize4k] = "4K",   [MfDesfireSize8k] = "8K",
    [MfDesfireSize16k] = "16K", [MfDesfireSize32k] = "32K", [MfDesfireSizeUnknown] = "",
};
...
bool mf_desfire_load(MfDesfireData *data, FlipperFormat *ff, uint32_t version)
{
    ...
    do {
        ...
        if (!mf_desfire_version_load(&data->version, ff))
            break;
        if (!mf_desfire_free_memory_load(&data->free_memory, ff))
            break;
        if (!mf_desfire_key_settings_load(&data->master_key_settings, MF_DESFIRE_FFF_PICC_PREFIX,
                                          ff))
            break;
        ...
        for (i = 0; i < master_key_version_count; ++i) {
            if (!mf_desfire_key_version_load(simple_array_get(data->master_key_versions, i),
                                             MF_DESFIRE_FFF_PICC_PREFIX, i, ff))
                break;
        }

        if (i != master_key_version_count)
            break;

        uint32_t application_count;
        if (!mf_desfire_application_count_load(&application_count, ff))
            break;

        if (application_count > 0) {
            simple_array_init(data->application_ids, application_count);
            if (!mf_desfire_application_ids_load(simple_array_get_data(data->application_ids),
                                                 application_count, ff))
                break;
            ...
            for (i = 0; i < application_count; ++i) {
                ...
                furi_string_printf(prefix, "%s %02x%02x%02x", MF_DESFIRE_FFF_APP_PREFIX,
                                   app_id->data[0], app_id->data[1], app_id->data[2]);

                if (!mf_desfire_application_load(simple_array_get(data->applications, i),
                                                 furi_string_get_cstr(prefix), ff))
                    break;
            }

            if (i != application_count)
                break;
        }
...
bool mf_desfire_save(const MfDesfireData *data, FlipperFormat *ff)
{
    ...
    do {
        ...
        if (!flipper_format_write_comment_cstr(ff, MF_DESFIRE_PROTOCOL_NAME " specific data"))
            break;
        if (!mf_desfire_version_save(&data->version, ff))
            break;
        if (!mf_desfire_free_memory_save(&data->free_memory, ff))
            break;
        if (!mf_desfire_key_settings_save(&data->master_key_settings, MF_DESFIRE_FFF_PICC_PREFIX,
                                          ff))
            break;

        const uint32_t master_key_version_count = simple_array_get_count(data->master_key_versions);

        uint32_t i;
        for (i = 0; i < master_key_version_count; ++i) {
            if (!mf_desfire_key_version_save(simple_array_cget(data->master_key_versions, i),
                                             MF_DESFIRE_FFF_PICC_PREFIX, i, ff))
                break;
        }
        ...
        if (!mf_desfire_application_count_save(&application_count, ff))
            break;

        if (application_count > 0) {
            if (!mf_desfire_application_ids_save(simple_array_cget_data(data->application_ids),
                                                 application_count, ff))
                break;
            ...
            for (i = 0; i < application_count; ++i) {
                ...
                if (!mf_desfire_application_save(app, furi_string_get_cstr(prefix), ff))
                    break;
            }
...
bool mf_desfire_is_equal(const MfDesfireData *data, const MfDesfireData *other)
{
    ...
    return iso14443_4a_is_equal(data->iso14443_4a_data, other->iso14443_4a_data) &&
           ...
           memcmp( //-V1103
               &data->free_memory, &other->free_memory, sizeof(MfDesfireFreeMemory)) == 0 &&
           memcmp(&data->master_key_settings, &other->master_key_settings,
...
static MfDesfireType mf_desfire_get_type_from_version(const MfDesfireVersion *const version)
{
    ...
    case MF_DESFIRE_HW_MAJOR_TYPE_EV3:
        ...
        break;
    default:
        if (MF_DESFIRE_TEST_TYPE_MF3ICD40(version->hw_major, version->hw_minor,
                                          version->hw_storage))
...
const char *mf_desfire_get_device_name(const MfDesfireData *data, NfcDeviceNameType name_type)
{
    ...
    if (type == MfDesfireTypeUnknown) {
        ...
        furi_string_printf(data->device_name, "%s %s %s", MF_DESFIRE_PROTOCOL_NAME,
                           mf_desfire_type_strings[type], mf_desfire_size_strings[size]);
    } else {
        furi_string_printf(data->device_name, "%s %s", mf_desfire_type_strings[type],
