...
bool mf_desfire_application_id_parse(MfDesfireApplicationId *data, uint32_t index,
                                     const BitBuffer *buf)
{
    ...
    if (can_parse) {
        bit_buffer_write_bytes_mid(buf, data, index * sizeof(MfDesfireApplicationId),
...
bool mf_desfire_file_id_parse(MfDesfireFileId *data, uint32_t index, const BitBuffer *buf)
{
    ...
    if (can_parse) {
        bit_buffer_write_bytes_mid(buf, data, index * sizeof(MfDesfireFileId),
                                   sizeof(MfDesfireFileId));
    }
...
bool mf_desfire_file_settings_parse(MfDesfireFileSettings *data, const BitBuffer *buf)
{
    ...
    do {
        ...
        if (data_size < min_data_size) {
            FURI_LOG_E(TAG, "File settings size %zu less than minimum %zu", data_size,
                       min_data_size);
            break;
        }
        ...
        if (file_settings_temp.type == MfDesfireFileTypeStandard ||
        ...
        } else if (file_settings_temp.type == MfDesfireFileTypeLinearRecord ||
                   file_settings_temp.type == MfDesfireFileTypeCyclicRecord) {
            memcpy(&layout.record, &data_ptr[bytes_processed], sizeof(MfDesfireFileSettingsRecord));
            ...
            memcpy(&layout.transaction_mac, &data_ptr[bytes_processed],
            ...
            if (!has_counter_limit) {
                ...
                // AES (4b) or LRP (2b)
                const size_t counter_limit_size =
                    (layout.transaction_mac.key_option & 0x02) ? 4 : 2;
                memcpy(&layout.transaction_mac, &data_ptr[bytes_processed],
        ...
        if (has_additional_access_rights) {
            ...
            if (additional_access_rights_len > MF_DESFIRE_MAX_KEYS - 1)
                break;
...
{
    return flipper_format_read_hex(ff, MF_DESFIRE_FFF_VERSION_KEY, (uint8_t *)data,
...
bool mf_desfire_key_settings_load(MfDesfireKeySettings *data, const char *prefix, FlipperFormat *ff)
{
    bool success = false;
    ...
    do {
        ...
        if (!flipper_format_read_hex(ff, furi_string_get_cstr(key), &data->change_key_id, 1))
            break;

        furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_CONFIG_CHANGEABLE_KEY);
        if (!flipper_format_read_bool(ff, furi_string_get_cstr(key), &data->is_config_changeable,
                                      1))
            break;

        furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_FREE_CREATE_DELETE_KEY);
        if (!flipper_format_read_bool(ff, furi_string_get_cstr(key), &data->is_free_create_delete,
                                      1))
            break;

        furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_FREE_DIR_LIST_KEY);
        if (!flipper_format_read_bool(ff, furi_string_get_cstr(key), &data->is_free_directory_list,
                                      1))
            break;

        furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_KEY_CHANGEABLE_KEY);
        if (!flipper_format_read_bool(ff, furi_string_get_cstr(key),
                                      &data->is_master_key_changeable, 1))
            break;

        furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_FLAGS_KEY);
        if (flipper_format_key_exist(ff, furi_string_get_cstr(key))) {
            if (!flipper_format_read_hex(ff, furi_string_get_cstr(key), &data->flags, 1))
                break;
        }

        furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_MAX_KEYS_KEY);
        if (!flipper_format_read_hex(ff, furi_string_get_cstr(key), &data->max_keys, 1))
            break;
...
bool mf_desfire_key_version_load(MfDesfireKeyVersion *data, const char *prefix, uint32_t index,
                                 FlipperFormat *ff)
{
    FuriString *key =
        furi_string_alloc_printf("%s %s %lu %s", prefix, MF_DESFIRE_FFF_KEY_SUB_PREFIX, index,
...
bool mf_desfire_file_ids_load(MfDesfireFileId *data, uint32_t count, const char *prefix,
                              FlipperFormat *ff)
{
    FuriString *key = furi_string_alloc_printf("%s %s", prefix, MF_DESFIRE_FFF_FILE_IDS_KEY);
...
bool mf_desfire_file_settings_load(MfDesfireFileSettings *data, const char *prefix,
                                   FlipperFormat *ff)
{
    bool success = false;
    ...
    do {
        ...
        if (!flipper_format_get_value_count(ff, furi_string_get_cstr(key), &access_rights_len))
            break;
        if ((access_rights_len == 0) || ((access_rights_len % 2) != 0))
            break;
        if (!flipper_format_read_hex(ff, furi_string_get_cstr(key), (uint8_t *)&data->access_rights,
                                     access_rights_len))
            break;
        ...
        if (data->type == MfDesfireFileTypeStandard || data->type == MfDesfireFileTypeBackup) {
            ...
            if (!flipper_format_read_uint32(ff, furi_string_get_cstr(key), &data->value.hi_limit,
                                            1))
                break;

            furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_FILE_LO_LIMIT_KEY);
            if (!flipper_format_read_uint32(ff, furi_string_get_cstr(key), &data->value.lo_limit,
                                            1))
                break;

            furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_FILE_LIMIT_CREDIT_VALUE_KEY);
            if (!flipper_format_read_uint32(ff, furi_string_get_cstr(key),
                                            &data->value.limited_credit_value, 1))
                break;

            furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_FILE_LIMIT_CREDIT_ENABLED_KEY);
            if (!flipper_format_read_bool(ff, furi_string_get_cstr(key),
                                          &data->value.limited_credit_enabled, 1))
                break;
        } else if (data->type == MfDesfireFileTypeLinearRecord ||
            ...
            if (!flipper_format_read_hex(ff, furi_string_get_cstr(key),
                                         &data->transaction_mac.key_option, 1))
                break;

            furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_FILE_KEY_VERSION_KEY);
            if (!flipper_format_read_hex(ff, furi_string_get_cstr(key),
                                         &data->transaction_mac.key_version, 1))
                break;

            furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_FILE_COUNTER_LIMIT_KEY);
            if (!flipper_format_read_uint32(ff, furi_string_get_cstr(key),
                                            &data->transaction_mac.counter_limit, 1))
                break;
        }
...
bool mf_desfire_file_data_load(MfDesfireFileData *data, const char *prefix, FlipperFormat *ff)
{
    ...
    do {
        ...
        uint32_t data_size;
        if (!flipper_format_get_value_count(ff, prefix, &data_size))
            break;
...
bool mf_desfire_application_ids_load(MfDesfireApplicationId *data, uint32_t count,
                                     FlipperFormat *ff)
{
    return flipper_format_read_hex(ff, MF_DESFIRE_FFF_APPLICATION_IDS_KEY, data->data,
...
bool mf_desfire_application_load(MfDesfireApplication *data, const char *prefix, FlipperFormat *ff)
{
    ...
    do {
        if (!mf_desfire_key_settings_load(&data->key_settings, prefix, ff))
            break;
        ...
        if (key_version_count) {
            ...
            for (i = 0; i < key_version_count; ++i) {
                if (!mf_desfire_key_version_load(simple_array_get(data->key_versions, i), prefix, i,
                                                 ff))
                    break;
            }

            if (i != key_version_count)
                break;
        }
        ...
        if (!mf_desfire_file_ids_load(simple_array_get_data(data->file_ids), file_count, prefix,
                                      ff))
            break;
        ...
        for (i = 0; i < file_count; ++i) {
            ...
            furi_string_printf(sub_prefix, "%s %s %u", prefix, MF_DESFIRE_FFF_FILE_SUB_PREFIX,
                               *file_id);
            ...
            if (!mf_desfire_file_data_load(file_data, furi_string_get_cstr(sub_prefix), ff))
                break;
        }
...
{
    return flipper_format_write_hex(ff, MF_DESFIRE_FFF_VERSION_KEY, (const uint8_t *)data,
                                    sizeof(MfDesfireVersion));
}
...
bool mf_desfire_key_settings_save(const MfDesfireKeySettings *data, const char *prefix,
                                  FlipperFormat *ff)
{
    bool success = false;
    ...
    do {
        ...
        furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_CONFIG_CHANGEABLE_KEY);
        if (!flipper_format_write_bool(ff, furi_string_get_cstr(key), &data->is_config_changeable,
                                       1))
            break;

        furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_FREE_CREATE_DELETE_KEY);
        if (!flipper_format_write_bool(ff, furi_string_get_cstr(key), &data->is_free_create_delete,
                                       1))
            break;

        furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_FREE_DIR_LIST_KEY);
        if (!flipper_format_write_bool(ff, furi_string_get_cstr(key), &data->is_free_directory_list,
                                       1))
            break;

        furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_KEY_CHANGEABLE_KEY);
        if (!flipper_format_write_bool(ff, furi_string_get_cstr(key),
                                       &data->is_master_key_changeable, 1))
            break;
...
bool mf_desfire_key_version_save(const MfDesfireKeyVersion *data, const char *prefix,
                                 uint32_t index, FlipperFormat *ff)
{
    FuriString *key =
        furi_string_alloc_printf("%s %s %lu %s", prefix, MF_DESFIRE_FFF_KEY_SUB_PREFIX, index,
                                 MF_DESFIRE_FFF_KEY_VERSION_KEY);
...
bool mf_desfire_file_ids_save(const MfDesfireFileId *data, uint32_t count, const char *prefix,
                              FlipperFormat *ff)
{
    FuriString *key = furi_string_alloc_printf("%s %s", prefix, MF_DESFIRE_FFF_FILE_IDS_KEY);
...
bool mf_desfire_file_settings_save(const MfDesfireFileSettings *data, const char *prefix,
                                   FlipperFormat *ff)
{
    bool success = false;
    ...
    do {
        furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_FILE_TYPE_KEY);
        if (!flipper_format_write_hex(ff, furi_string_get_cstr(key), (const uint8_t *)&data->type,
                                      1))
            break;

        furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_FILE_COMM_SETTINGS_KEY);
        if (!flipper_format_write_hex(ff, furi_string_get_cstr(key), (const uint8_t *)&data->comm,
                                      1))
            break;

        furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_FILE_ACCESS_RIGHTS_KEY);
        if (!flipper_format_write_hex(ff, furi_string_get_cstr(key),
        ...
        if (data->type == MfDesfireFileTypeStandard || data->type == MfDesfireFileTypeBackup) {
            ...
            furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_FILE_HI_LIMIT_KEY);
            if (!flipper_format_write_uint32(ff, furi_string_get_cstr(key), &data->value.hi_limit,
                                             1))
                break;

            furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_FILE_LO_LIMIT_KEY);
            if (!flipper_format_write_uint32(ff, furi_string_get_cstr(key), &data->value.lo_limit,
                                             1))
                break;

            furi_string_printf(key, "%s %s", prefix, MF_DESFIRE_FFF_FILE_LIMIT_CREDIT_VALUE_KEY);
            if (!flipper_format_write_uint32(ff, furi_string_get_cstr(key),
                                             &data->value.limited_credit_value, 1))
                break;
...
bool mf_desfire_file_data_save(const MfDesfireFileData *data, const char *prefix, FlipperFormat *ff)
{
    const uint32_t data_size = simple_array_get_count(data->data);
    return data_size > 0
               ? flipper_format_write_hex(ff, prefix, simple_array_cget_data(data->data), data_size)
               : true;
}
...
bool mf_desfire_application_ids_save(const MfDesfireApplicationId *data, uint32_t count,
                                     FlipperFormat *ff)
{
    return flipper_format_write_hex(ff, MF_DESFIRE_FFF_APPLICATION_IDS_KEY, data->data,
                                    count * sizeof(MfDesfireApplicationId));
}

bool mf_desfire_application_save(const MfDesfireApplication *data, const char *prefix,
                                 FlipperFormat *ff)
{
    FuriString *sub_prefix = furi_string_alloc();
    bool success = false;
    ...
    do {
        if (!mf_desfire_key_settings_save(&data->key_settings, prefix, ff))
            break;
        ...
        uint32_t i;
        for (i = 0; i < key_version_count; ++i) {
            if (!mf_desfire_key_version_save(simple_array_cget(data->key_versions, i), prefix, i,
                                             ff))
                break;
        }
        ...
        if (file_count > 0) {
            if (!mf_desfire_file_ids_save(simple_array_get_data(data->file_ids), file_count, prefix,
                                          ff))
                break;
        }
