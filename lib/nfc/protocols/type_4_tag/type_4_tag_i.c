...
void type_4_tag_cc_dump(const Type4TagData *data, uint8_t *buf, size_t len)
{
    ...
    bit_lib_num_to_bytes_be(data->is_tag_specific ? MIN(data->chunk_max_read, TYPE_4_TAG_CHUNK_LEN)
                                                  : TYPE_4_TAG_CHUNK_LEN,
                            sizeof(cc->mle), (void *)&cc->mle);
    bit_lib_num_to_bytes_be(data->is_tag_specific ? MIN(data->chunk_max_write, TYPE_4_TAG_CHUNK_LEN)
                                                  : TYPE_4_TAG_CHUNK_LEN,
                            sizeof(cc->mlc), (void *)&cc->mlc);

    cc->tlv[0].type = Type4TagCcTlvTypeNdefFileCtrl;
    cc->tlv[0].len = sizeof(cc->tlv[0].value.ndef_file_ctrl);

    bit_lib_num_to_bytes_be(data->is_tag_specific ? data->ndef_file_id : TYPE_4_TAG_T4T_NDEF_EF_ID,
                            sizeof(cc->tlv[0].value.ndef_file_ctrl.file_id),
                            (void *)&cc->tlv[0].value.ndef_file_ctrl.file_id);
    bit_lib_num_to_bytes_be(sizeof(uint16_t) + (data->is_tag_specific
                                                    ? data->ndef_max_len
...
bool type_4_tag_ndef_data_load(Type4TagData *data, FlipperFormat *ff)
{
    ...
    while (ndef_data_size > 0) {
        ...
        if (!flipper_format_read_hex(ff, TYPE_4_TAG_FFF_NDEF_DATA_KEY, &ndef_data[ndef_data_pos],
                                     ndef_line_size)) {
...
bool type_4_tag_ndef_data_save(const Type4TagData *data, FlipperFormat *ff)
{
    ...
    while (ndef_data_size > 0) {
        ...
        if (!flipper_format_write_hex(ff, TYPE_4_TAG_FFF_NDEF_DATA_KEY, &ndef_data[ndef_data_pos],
                                      ndef_line_size)) {
            return false;
        }
