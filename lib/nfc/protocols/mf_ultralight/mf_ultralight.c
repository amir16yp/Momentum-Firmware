    ...
    [MfUltralightTypeMfulC] =
        {
            ...
            .config_page = 0,
            .feature_set =
                MfUltralightFeatureSupportCompatibleWrite | MfUltralightFeatureSupportAuthenticate,
        },
    ...
    [MfUltralightTypeUL21] =
        {
            ...
            .feature_set =
                ...
                MfUltralightFeatureSupportFastRead | MfUltralightFeatureSupportIncCounter |
                MfUltralightFeatureSupportCompatibleWrite | MfUltralightFeatureSupportPasswordAuth |
                MfUltralightFeatureSupportVcsl | MfUltralightFeatureSupportDynamicLock,
        },
...
static const char *mf_ultralight_get_device_name_by_type(MfUltralightType type,
...
bool mf_ultralight_load(MfUltralightData *data, FlipperFormat *ff, uint32_t version)
{
    ...
    do {
        ...
        if (!flipper_format_read_uint32(ff, MF_ULTRALIGHT_FORMAT_VERSION_KEY, &data_format_version,
                                        1)) {
        ...
        if (data_format_version > 1) {
            if (!flipper_format_read_string(ff, MF_ULTRALIGHT_TYPE_KEY, temp_str))
                break;
            if (!mf_ultralight_verify(data, temp_str))
                break;
        }

        // Read signature
        if (!flipper_format_read_hex(ff, MF_ULTRALIGHT_SIGNATURE_KEY, data->signature.data,
                                     sizeof(MfUltralightSignature)))
            break;
        // Read Mifare version
        if (!flipper_format_read_hex(ff, MF_ULTRALIGHT_MIFARE_VERSION_KEY,
                                     (uint8_t *)&data->version, sizeof(MfUltralightVersion)))
            break;
        // Read counters and tearing flags
        bool counters_parsed = true;
        for (size_t i = 0; i < 3; i++) {
            furi_string_printf(temp_str, "%s %d", MF_ULTRALIGHT_COUNTER_KEY, i);
            if (!flipper_format_read_uint32(ff, furi_string_get_cstr(temp_str),
                                            &data->counter[i].counter, 1)) {
        ...
        if (!flipper_format_read_uint32(ff, MF_ULTRALIGHT_PAGES_TOTAL_KEY, &pages_total, 1))
            break;
        ...
        for (size_t i = 0; i < pages_total; i++) {
            ...
            if (!flipper_format_read_hex(ff, furi_string_get_cstr(temp_str), data->page[i].data,
        ...
        if (!flipper_format_read_uint32(ff, MF_ULTRALIGHT_FAILED_ATTEMPTS_KEY, &data->auth_attempts,
                                        1)) {
...
bool mf_ultralight_save(const MfUltralightData *data, FlipperFormat *ff)
{
    ...
    do {
        ...
        if (!flipper_format_write_comment_cstr(ff, MF_ULTRALIGHT_PROTOCOL_NAME " specific data"))
            break;
        if (!flipper_format_write_uint32(ff, MF_ULTRALIGHT_FORMAT_VERSION_KEY,
                                         &mf_ultralight_data_format_version, 1))
            break;

        const char *device_type_name =
            mf_ultralight_get_device_name_by_type(data->type, NfcDeviceNameTypeFull);
        if (!flipper_format_write_string_cstr(ff, MF_ULTRALIGHT_TYPE_KEY, device_type_name))
            break;
        if (!flipper_format_write_hex(ff, MF_ULTRALIGHT_SIGNATURE_KEY, data->signature.data,
                                      sizeof(MfUltralightSignature)))
            break;
        if (!flipper_format_write_hex(ff, MF_ULTRALIGHT_MIFARE_VERSION_KEY,
                                      (uint8_t *)&data->version, sizeof(MfUltralightVersion)))
            break;
        ...
        for (size_t i = 0; i < 3; i++) {
            furi_string_printf(temp_str, "%s %d", MF_ULTRALIGHT_COUNTER_KEY, i);
            if (!flipper_format_write_uint32(ff, furi_string_get_cstr(temp_str),
                                             &data->counter[i].counter, 1)) {
        ...
        if (!flipper_format_write_uint32(ff, MF_ULTRALIGHT_PAGES_TOTAL_KEY, &pages_total, 1))
            break;
        if (!flipper_format_write_uint32(ff, MF_ULTRALIGHT_PAGES_READ_KEY, &pages_read, 1))
            break;
        bool pages_saved = true;
        for (size_t i = 0; i < data->pages_total; i++) {
            furi_string_printf(temp_str, "%s %d", MF_ULTRALIGHT_PAGE_KEY, i);
            if (!flipper_format_write_hex(ff, furi_string_get_cstr(temp_str), data->page[i].data,
                                          sizeof(MfUltralightPage))) {
                pages_saved = false;
                break;
            }
        ...
        if (!flipper_format_write_uint32(ff, MF_ULTRALIGHT_FAILED_ATTEMPTS_KEY,
...
bool mf_ultralight_is_equal(const MfUltralightData *data, const MfUltralightData *other)
{
    ...
    do {
        ...
        if (data->type != other->type)
            break;
        if (data->pages_read != other->pages_read)
            break;
        if (data->pages_total != other->pages_total)
            break;
        if (data->auth_attempts != other->auth_attempts)
            break;

        if (memcmp(&data->version, &other->version, sizeof(data->version)) != 0)
            break;
        if (memcmp(&data->signature, &other->signature, sizeof(data->signature)) != 0)
            break;
        ...
        if (!data_array_is_equal)
            break;

        for (size_t i = 0; i < COUNT_OF(data->tearing_flag); i++) {
            if (memcmp(&data->tearing_flag[i], &other->tearing_flag[i],
...
const char *mf_ultralight_get_device_name(const MfUltralightData *data, NfcDeviceNameType name_type)
{
    furi_check(data);
...
{
    furi_check(type < MfUltralightTypeNum);
    furi_assert(type == MfUltralightTypeUL11 || type == MfUltralightTypeUL21 ||
...
bool mf_ultralight_is_all_data_read(const MfUltralightData *data)
{
    ...
    if (data->pages_read == data->pages_total) {
        ...
        if ((data->type == MfUltralightTypeMfulC) &&
            ...
            all_read = true;
        } else if (!mf_ultralight_support_feature(feature_set,
            ...
            if (mf_ultralight_get_config_page(data, &config)) {
                uint32_t pass = bit_lib_bytes_to_num_be(config->password.data,
...
void mf_ultralight_3des_encrypt(mbedtls_des3_context *ctx, const uint8_t *ck, const uint8_t *iv,
                                const uint8_t *input, const uint8_t length, uint8_t *out)
{
    furi_check(ctx);
    furi_check(ck);
    furi_check(iv);
...
void mf_ultralight_3des_decrypt(mbedtls_des3_context *ctx, const uint8_t *ck, const uint8_t *iv,
                                const uint8_t *input, const uint8_t length, uint8_t *out)
{
    furi_check(ctx);
    furi_check(ck);
