                                                ...
                                                MfUltralightPollerAuthContext *data)
{
    ...
    do {
        error = iso14443_3a_poller_send_standard_frame(instance->iso14443_3a_poller,
...
static MfUltralightError mf_ultralight_poller_send_authenticate_cmd(MfUltralightPoller *instance,
                                                                    const uint8_t *cmd,
                                                                    ...
                                                                    uint8_t *response)
{
    ...
    do {
        ...
        memcpy(response, bit_buffer_get_data(instance->rx_buffer) + 1,
...
MfUltralightError mf_ultralight_poller_authentication_test(MfUltralightPoller *instance)
{
    ...
    return mf_ultralight_poller_send_authenticate_cmd(instance, auth_cmd, sizeof(auth_cmd), true,
                                                      dummy);
}
                                                          ...
                                                          const uint8_t *RndA, uint8_t *output)
{
    ...
    do {
        ...
        uint8_t auth_cmd[2] = {MF_ULTRALIGHT_CMD_AUTH, 0x00};
        ret = mf_ultralight_poller_send_authenticate_cmd(instance, auth_cmd, sizeof(auth_cmd), true,
                                                         encRndB /* instance->encRndB */);
        ...
        mf_ultralight_3des_decrypt(&instance->des_context, instance->auth_context.tdes_key.data, iv,
                                   encRndB, sizeof(encRndB), RndB);
        mf_ultralight_3des_shift_data(RndB);

        memcpy(output, RndA, MF_ULTRALIGHT_C_AUTH_RND_BLOCK_SIZE);

        mf_ultralight_3des_encrypt(&instance->des_context, instance->auth_context.tdes_key.data,
                                   encRndB, output, MF_ULTRALIGHT_C_AUTH_DATA_SIZE, output);
                                                        ...
                                                        uint8_t *response)
{
    ...
    MfUltralightError ret = MfUltralightErrorNone;
    do {
        ret = mf_ultralight_poller_send_authenticate_cmd(instance, auth_cmd, sizeof(auth_cmd),
                                                         false, response);

        if (ret != MfUltralightErrorNone)
            break;

        mf_ultralight_3des_decrypt(&instance->des_context, instance->auth_context.tdes_key.data,
                                   RndB, bit_buffer_get_data(instance->rx_buffer) + 1,
                                   MF_ULTRALIGHT_C_AUTH_RND_BLOCK_SIZE, response);
    } while (false);
                                                    ...
                                                    MfUltralightVersion *data)
{
    ...
    do {
        ...
        if (bit_buffer_get_size_bytes(instance->rx_buffer) != sizeof(MfUltralightVersion)) {
            FURI_LOG_I(TAG, "Read Version failed: %zu",
                       bit_buffer_get_size_bytes(instance->rx_buffer));
            ret = MfUltralightErrorProtocol;
            break;
        }
