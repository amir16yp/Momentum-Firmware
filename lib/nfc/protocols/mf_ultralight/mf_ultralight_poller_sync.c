...
typedef MfUltralightError (*MfUltralightPollerCmdHandler)(MfUltralightPoller *poller,
                                                          MfUltralightPollerContextData *data);

MfUltralightError mf_ultralight_poller_read_page_handler(MfUltralightPoller *poller,
                                                         MfUltralightPollerContextData *data)
{
    return mf_ultralight_poller_read_page(poller, data->read_cmd.start_page, &data->read_cmd.data);
}

MfUltralightError mf_ultralight_poller_write_page_handler(MfUltralightPoller *poller,
                                                          MfUltralightPollerContextData *data)
{
    return mf_ultralight_poller_write_page(poller, data->write_cmd.page_to_write,
                                           &data->write_cmd.page);
}

MfUltralightError mf_ultralight_poller_read_version_handler(MfUltralightPoller *poller,
                                                            MfUltralightPollerContextData *data)
{
    return mf_ultralight_poller_read_version(poller, &data->version);
}

MfUltralightError mf_ultralight_poller_read_signature_handler(MfUltralightPoller *poller,
                                                              MfUltralightPollerContextData *data)
{
    return mf_ultralight_poller_read_signature(poller, &data->signature);
}

MfUltralightError mf_ultralight_poller_read_counter_handler(MfUltralightPoller *poller,
                                                            MfUltralightPollerContextData *data)
{
    return mf_ultralight_poller_read_counter(poller, data->counter_cmd.counter_num,
                                             &data->counter_cmd.data);
}

MfUltralightError
mf_ultralight_poller_read_tearing_flag_handler(MfUltralightPoller *poller,
                                               MfUltralightPollerContextData *data)
{
    return mf_ultralight_poller_read_tearing_flag(poller, data->tearing_flag_cmd.tearing_flag_num,
                                                  &data->tearing_flag_cmd.data);
}
...
static const MfUltralightPollerCmdHandler
    mf_ultralight_poller_cmd_handlers[MfUltralightPollerCmdTypeNum] = {
        ...
        [MfUltralightPollerCmdTypeReadTearingFlag] = mf_ultralight_poller_read_tearing_flag_handler,
};
...
static MfUltralightError mf_ultralight_poller_cmd_execute(Nfc *nfc,
                                                          MfUltralightPollerContext *poller_ctx)
{
...
MfUltralightError mf_ultralight_poller_sync_read_page(Nfc *nfc, uint16_t page,
...
MfUltralightError mf_ultralight_poller_sync_write_page(Nfc *nfc, uint16_t page,
                                                       MfUltralightPage *data)
{
    furi_check(nfc);
    furi_check(data);
...
MfUltralightError mf_ultralight_poller_sync_read_counter(Nfc *nfc, uint8_t counter_num,
...
MfUltralightError mf_ultralight_poller_sync_read_tearing_flag(Nfc *nfc, uint8_t flag_num,
    ...
    if (mfu_event->type == MfUltralightPollerEventTypeReadSuccess) {
        ...
        if (poller_context->auth_context != NULL) {
            ...
            if (mfu_poller->data->type == MfUltralightTypeMfulC) {
                ...
                memset(mfu_poller->auth_context.tdes_key.data, 0x00,
...
MfUltralightError
mf_ultralight_poller_sync_read_card(Nfc *nfc, MfUltralightData *data,
                                    const MfUltralightPollerAuthContext *auth_context)
{
    furi_check(nfc);
    furi_check(data);
