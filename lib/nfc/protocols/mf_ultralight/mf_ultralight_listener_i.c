...
static MfUltralightMirrorConf
mf_ultralight_mirror_check_mode(const MfUltralightConfigPages *const config,
...
static uint8_t mf_ultralight_get_ascii_offset(uint8_t start_page, MfUltralightListener *instance)
{
    ...
    if (instance->config->mirror.mirror_conf == MfUltralightMirrorCounter)
        start_offset = 15;
...
static uint8_t mf_ultralight_get_byte_offset(uint8_t current_page,
                                             const MfUltralightConfigPages *const config)
{
    return (current_page > config->mirror_page) ? 0 : config->mirror.mirror_byte;
}

static void mf_ultralight_format_mirror_data(FuriString *str, const uint8_t *const data,
...
void mf_ultralight_mirror_read_handler(uint8_t mirror_page_num, uint8_t *dest,
...
{
    mf_ultralight_format_mirror_data(instance->mirror.ascii_mirror_data,
...
{
    furi_string_left(instance->mirror.ascii_mirror_data,
                     instance->data->iso14443_3a_data->uid_len * 2 + 1);
...
MfUltralightCommand mf_ultralight_composite_command_run(MfUltralightListener *instance,
                                                        BitBuffer *buffer)
{
...
void mf_ultralight_composite_command_set_next(MfUltralightListener *instance,
...
void mf_ultralight_single_counter_try_to_unlock(MfUltralightListener *instance,
...
static bool mf_ultralight_i2c_page_validator_for_sector0(uint16_t start_page, uint16_t end_page,
                                                         MfUltralightType type)
{
    ...
    if (type == MfUltralightTypeNTAGI2CPlus1K || type == MfUltralightTypeNTAGI2CPlus2K) {
        if (start_page <= 0xE9 && end_page <= 0xE9) {
            valid = true;
        } else if (MF_ULTRALIGHT_I2C_PAGE_ON_SESSION_REG(start_page) &&
...
static bool mf_ultralight_i2c_page_validator_for_sector1(uint16_t start_page, uint16_t end_page,
                                                         MfUltralightType type)
{
    bool valid = false;
...
static bool mf_ultralight_i2c_page_validator_for_sector2(uint16_t start_page, uint16_t end_page,
                                                         MfUltralightType type)
{
    UNUSED(start_page);
    UNUSED(end_page);
...
static bool mf_ultralight_i2c_page_validator_for_sector3(uint16_t start_page, uint16_t end_page,
                                                         MfUltralightType type)
{
    UNUSED(type);
    UNUSED(end_page);
...
typedef bool (*MfUltralightI2CValidator)(uint16_t start_page, uint16_t end_page,
                                         MfUltralightType type);
...
bool mf_ultralight_i2c_validate_pages(uint16_t start_page, uint16_t end_page,
                                      MfUltralightListener *instance)
{
    bool valid = false;
...
uint16_t mf_ultralight_i2c_provide_page_by_requested(uint16_t page, MfUltralightListener *instance)
{
...
void mf_ultralight_static_lock_bytes_write(MfUltralightStaticLockData *const lock_bits,
                                           uint16_t new_bits)
{
...
bool mf_ultralight_static_lock_check_page(const MfUltralightStaticLockData *const lock_bits,
                                          uint16_t page)
{
    bool locked = false;
...
void mf_ultralight_capability_container_write(MfUltralightPage *const current_page,
...
void mf_ultralight_dynamic_lock_bytes_write(MfUltralightDynamicLockData *const lock_bits,
                                            uint32_t new_bits)
{
...
bool mf_ultralight_auth_limit_check_and_update(MfUltralightListener *instance, bool auth_success)
{
    ...
    do {
        if (instance->config->access.authlim == 0)
            break;
...
bool mf_ultralight_auth_check_password(const MfUltralightAuthPassword *config_pass,
                                       const MfUltralightAuthPassword *auth_pass)
{
    return memcmp(config_pass->data, auth_pass->data, sizeof(MfUltralightAuthPassword)) == 0;
}

bool mf_ultralight_common_check_access(const MfUltralightListener *instance,
...
bool mf_ultralight_c_check_access(const MfUltralightData *data, const uint16_t start_page,
                                  const MfUltralightListenerAccessType access_type,
                                  const MfUltralightListenerAuthState auth_state)
{
    bool access_success = false;
