...
static bool mf_ultralight_poller_ntag_i2c_addr_lin_to_tag_ntag_i2c_1k(uint16_t lin_addr,
...
static bool mf_ultralight_poller_ntag_i2c_addr_lin_to_tag_ntag_i2c_2k(uint16_t lin_addr,
                                                                      uint8_t *sector, uint8_t *tag,
                                                                      uint8_t *pages_left)
{
    bool tag_calculated = false;
...
static bool mf_ultralight_poller_ntag_i2c_addr_lin_to_tag_ntag_i2c_plus_1k(uint16_t lin_addr,
                                                                           uint8_t *sector,
                                                                           uint8_t *tag,
                                                                           uint8_t *pages_left)
{
...
static bool mf_ultralight_poller_ntag_i2c_addr_lin_to_tag_ntag_i2c_plus_2k(uint16_t lin_addr,
                                                                           uint8_t *sector,
                                                                           uint8_t *tag,
                                                                           uint8_t *pages_left)
{
                                                   ...
                                                   uint8_t *pages_left)
{
    ...
    if (instance->data->type == MfUltralightTypeNTAGI2C1K) {
        tag_calculated = mf_ultralight_poller_ntag_i2c_addr_lin_to_tag_ntag_i2c_1k(lin_addr, sector,
                                                                                   tag, pages_left);
    } else if (instance->data->type == MfUltralightTypeNTAGI2C2K) {
        tag_calculated = mf_ultralight_poller_ntag_i2c_addr_lin_to_tag_ntag_i2c_2k(lin_addr, sector,
                                                                                   tag, pages_left);
...
static void mf_ultralight_poller_set_callback(MfUltralightPoller *instance,
...
static NfcCommand mf_ultralight_poller_handler_get_feature_set(MfUltralightPoller *instance)
{
    ...
    FURI_LOG_D(TAG, "%s detected. Total pages: %d",
...
static NfcCommand mf_ultralight_poller_handler_read_signature(MfUltralightPoller *instance)
{
    ...
    if (mf_ultralight_support_feature(instance->feature_set,
                                      MfUltralightFeatureSupportReadSignature)) {
        FURI_LOG_D(TAG, "Reading signature");
        instance->error = mf_ultralight_poller_read_signature(instance, &instance->data->signature);
        if (instance->error != MfUltralightErrorNone) {
...
static NfcCommand mf_ultralight_poller_handler_auth(MfUltralightPoller *instance)
{
    ...
    if (mf_ultralight_support_feature(instance->feature_set,
        ...
        if (!instance->mfu_event.data->auth_context.skip_auth) {
            ...
            uint32_t pass = bit_lib_bytes_to_num_be(instance->auth_context.password.data,
...
static NfcCommand mf_ultralight_poller_handler_auth_ultralight_c(MfUltralightPoller *instance)
{
    ...
    if (mf_ultralight_support_feature(instance->feature_set,
        ...
        if (!instance->mfu_event.data->auth_context.skip_auth) {
            ...
            // For debugging
            FURI_LOG_D(
                "TAG", "Key data: %02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X",
                instance->auth_context.tdes_key.data[0], instance->auth_context.tdes_key.data[1],
                instance->auth_context.tdes_key.data[2], instance->auth_context.tdes_key.data[3],
                instance->auth_context.tdes_key.data[4], instance->auth_context.tdes_key.data[5],
                instance->auth_context.tdes_key.data[6], instance->auth_context.tdes_key.data[7],
                instance->auth_context.tdes_key.data[8], instance->auth_context.tdes_key.data[9],
                instance->auth_context.tdes_key.data[10], instance->auth_context.tdes_key.data[11],
                instance->auth_context.tdes_key.data[12], instance->auth_context.tdes_key.data[13],
                instance->auth_context.tdes_key.data[14], instance->auth_context.tdes_key.data[15]);
            do {
                ...
                instance->error = mf_ultralight_poller_authenticate_end(instance, RndB, output,
                ...
                if (instance->auth_context.auth_success) {
                    ...
                    if (instance->mode == MfUltralightPollerModeDictAttack) {
                        memcpy(&instance->data->page[44], instance->auth_context.tdes_key.data,
...
static NfcCommand mf_ultralight_poller_handler_read_pages(MfUltralightPoller *instance)
{
    ...
    if (MF_ULTRALIGHT_IS_NTAG_I2C(instance->data->type)) {
        ...
        if (mf_ultralight_poller_ntag_i2c_addr_lin_to_tag(instance, start_page, &sector, &tag,
                                                          &pages_left)) {
            instance->error =
...
static NfcCommand mf_ultralight_poller_handler_try_default_pass(MfUltralightPoller *instance)
{
    do {
        ...
        if (instance->auth_context.auth_success) {
            ...
            bit_lib_num_to_bytes_be(MF_ULTRALIGHT_DEFAULT_PASSWORD,
                                    sizeof(MfUltralightAuthPassword),
            ...
            if (instance->error == MfUltralightErrorNone) {
                ...
                bit_lib_num_to_bytes_be(MF_ULTRALIGHT_DEFAULT_PASSWORD,
                                        sizeof(MfUltralightAuthPassword), config->password.data);
                config->pack = instance->auth_context.pack;
                instance->auth_context.auth_success = true;
            }
        }
...
static NfcCommand mf_ultralight_poller_handler_check_mfuc_auth_status(MfUltralightPoller *instance)
{
    instance->state = MfUltralightPollerStateReadSuccess;
...
static NfcCommand mf_ultralight_poller_handler_request_write_data(MfUltralightPoller *instance)
{
    ...
    if (check_passed &&
        ...
        if (auth_ok) {
            ...
            FURI_LOG_D(TAG, "ULC write: key %s",
                       instance->write_skip_key ? "kept (target unchanged)"
...
static NfcCommand mf_ultralight_poller_handler_write_pages(MfUltralightPoller *instance)
{
    ...
    do {
        ...
        if (instance->current_page >= 44 && instance->current_page <= 47 &&
            ...
            FURI_LOG_D(TAG, "Writing KEY page %d (byte-order corrected): %02X %02X %02X %02X",
                       instance->current_page, page_to_write.data[0], page_to_write.data[1],
                       page_to_write.data[2], page_to_write.data[3]);
        } else {
...
static const MfUltralightPollerReadHandler
    mf_ultralight_poller_read_handler[MfUltralightPollerStateNum] = {
        ...
        [MfUltralightPollerStateReadTearingFlags] = mf_ultralight_poller_handler_read_tearing_flags,
        ...
        [MfUltralightPollerStateRequestWriteData] = mf_ultralight_poller_handler_request_write_data,
