...
static bool mf_ultralight_listener_check_access(MfUltralightListener *instance, uint16_t start_page,
                                                MfUltralightListenerAccessType access_type)
{
    ...
    if (mf_ultralight_support_feature(instance->features, MfUltralightFeatureSupportAuthenticate)) {
        access_success = mf_ultralight_c_check_access(instance->data, start_page, access_type,
                                                      instance->auth_state);
    } else if (mf_ultralight_support_feature(instance->features,
...
static void mf_ultralight_listener_perform_read(MfUltralightPage *pages,
                                                MfUltralightListener *instance, uint16_t start_page,
                                                uint8_t page_cnt, bool do_i2c_page_check)
{
...
static MfUltralightCommand mf_ultralight_listener_perform_write(MfUltralightListener *instance,
                                                                ...
                                                                bool do_i2c_check)
{
    ...
    if (start_page < 2 && instance->sector == 0)
        ...
        if (do_i2c_check)
            page = mf_ultralight_i2c_provide_page_by_requested(start_page, instance);
...
static MfUltralightCommand mf_ultralight_listener_read_page_handler(MfUltralightListener *instance,
                                                                    BitBuffer *buffer)
{
    ...
    do {
        ...
        if (do_i2c_check) {
            if (!mf_ultralight_i2c_validate_pages(start_page, start_page, instance))
                break;
        } else if (pages_total < start_page) {
            break;
        }

        if (!mf_ultralight_listener_check_access(instance, start_page,
...
static MfUltralightCommand mf_ultralight_listener_fast_read_handler(MfUltralightListener *instance,
                                                                    BitBuffer *buffer)
{
    ...
    do {
        ...
        if (!mf_ultralight_listener_check_access(instance, start_page,
                                                 MfUltralightListenerAccessTypeRead) ||
            !mf_ultralight_listener_check_access(instance, end_page,
                                                 MfUltralightListenerAccessTypeRead)) {
            command = MfUltralightCommandNotProcessedNAK;
            break;
        }
...
static MfUltralightCommand mf_ultralight_listener_write_page_handler(MfUltralightListener *instance,
                                                                     BitBuffer *buffer)
{
    ...
    do {
        ...
        if (!is_ulc_key_page) {
            ...
            if (mf_ultralight_static_lock_check_page(instance->static_lock, start_page))
                break;
            if (mf_ultralight_dynamic_lock_check_page(instance, start_page))
                break;
        } else {
...
static MfUltralightCommand mf_ultralight_listener_fast_write_handler(MfUltralightListener *instance,
                                                                     BitBuffer *buffer)
{
    MfUltralightCommand command = MfUltralightCommandNotProcessedSilent;
...
mf_ultralight_listener_read_version_handler(MfUltralightListener *instance, BitBuffer *buffer)
{
    ...
    if (mf_ultralight_support_feature(instance->features, MfUltralightFeatureSupportReadVersion)) {
        bit_buffer_copy_bytes(instance->tx_buffer, (uint8_t *)&instance->data->version,
...
static MfUltralightCommand
mf_ultralight_listener_read_signature_handler(MfUltralightListener *instance, BitBuffer *buffer)
{
    UNUSED(buffer);
    MfUltralightCommand command = MfUltralightCommandNotProcessedSilent;
    ...
    if (mf_ultralight_support_feature(instance->features,
        ...
        bit_buffer_copy_bytes(instance->tx_buffer, instance->data->signature.data,
...
mf_ultralight_listener_read_counter_handler(MfUltralightListener *instance, BitBuffer *buffer)
{
    ...
    do {
        ...
        if (mf_ultralight_support_feature(instance->features,
            ...
            if (!instance->config->access.nfc_cnt_en || counter_num != 2)
                break;
...
static MfUltralightCommand
mf_ultralight_listener_increase_counter_handler(MfUltralightListener *instance, BitBuffer *buffer)
{
    MfUltralightCommand command = MfUltralightCommandNotProcessedNAK;
...
static MfUltralightCommand
mf_ultralight_listener_check_tearing_handler(MfUltralightListener *instance, BitBuffer *buffer)
{
    MfUltralightCommand command = MfUltralightCommandNotProcessedNAK;
    ...
    do {
        ...
        bit_buffer_set_byte(instance->tx_buffer, 0,
                            instance->data->tearing_flag[tearing_flag_num].data);
        iso14443_3a_listener_send_standard_frame(instance->iso14443_3a_listener,
                                                 instance->tx_buffer);
        command = MfUltralightCommandProcessed;
...
static MfUltralightCommand mf_ultralight_listener_vcsl_handler(MfUltralightListener *instance,
                                                               BitBuffer *buffer)
{
    MfUltralightCommand command = MfUltralightCommandNotProcessedSilent;
    UNUSED(instance);
...
static MfUltralightCommand mf_ultralight_listener_auth_handler(MfUltralightListener *instance,
                                                               BitBuffer *buffer)
{
    MfUltralightCommand command = MfUltralightCommandNotProcessedNAK;
    ...
    do {
        ...
        bit_buffer_copy_bytes(instance->tx_buffer, instance->config->pack.data,
                              sizeof(MfUltralightAuthPack));
...
static MfUltralightCommand mf_ultralight_comp_write_handler_p2(MfUltralightListener *instance,
                                                               BitBuffer *buffer)
{
    MfUltralightCommand command = MfUltralightCommandNotProcessedNAK;
    ...
    do {
        if (bit_buffer_get_size_bytes(buffer) != 16)
            break;
...
static MfUltralightCommand mf_ultralight_comp_write_handler_p1(MfUltralightListener *instance,
                                                               BitBuffer *buffer)
{
    MfUltralightCommand command = MfUltralightCommandNotProcessedSilent;
...
static MfUltralightCommand mf_ultralight_sector_select_handler_p2(MfUltralightListener *instance,
                                                                  BitBuffer *buffer)
{
    MfUltralightCommand command = MfUltralightCommandNotProcessedNAK;
    UNUSED(instance);
    ...
    do {
        if (bit_buffer_get_size_bytes(buffer) != 4)
            break;
...
static MfUltralightCommand mf_ultralight_sector_select_handler_p1(MfUltralightListener *instance,
                                                                  BitBuffer *buffer)
{
    MfUltralightCommand command = MfUltralightCommandNotProcessedNAK;
    UNUSED(buffer);
...
static MfUltralightCommand mf_ultralight_c_authenticate_handler_p2(MfUltralightListener *instance,
                                                                   BitBuffer *buffer)
{
    MfUltralightCommand command = MfUltralightCommandNotProcessedNAK;
    ...
    do {
        ...
        mf_ultralight_3des_decrypt(&instance->des_context, ck, instance->encB, data, sizeof(out),
                                   out);
        ...
        mf_ultralight_3des_encrypt(&instance->des_context, ck, iv, rndA,
                                   MF_ULTRALIGHT_C_AUTH_RND_BLOCK_SIZE, rndA);
...
static MfUltralightCommand mf_ultralight_c_authenticate_handler_p1(MfUltralightListener *instance,
                                                                   BitBuffer *buffer)
{
    MfUltralightCommand command = MfUltralightCommandNotProcessedNAK;
    ...
    do {
        ...
        mf_ultralight_3des_encrypt(&instance->des_context, ck, iv, instance->rndB,
                                   sizeof(instance->rndB), instance->encB);
...
static NfcCommand mf_ultralight_command_postprocess(MfUltralightCommand mfu_command,
...
static NfcCommand mf_ultralight_reset_listener_state(MfUltralightListener *instance,
...
void mf_ultralight_listener_set_callback(MfUltralightListener *instance,
    ...
    if (iso14443_3a_event->type == Iso14443_3aListenerEventTypeReceivedStandardFrame) {
        ...
        if (mf_ultralight_composite_command_in_progress(instance)) {
            ...
            for (size_t i = 0; i < COUNT_OF(mf_ultralight_command); i++) {
                if (size != mf_ultralight_command[i].cmd_len_bits)
                    continue;
                if (cmd != mf_ultralight_command[i].cmd)
                    continue;
                mfu_command = mf_ultralight_command[i].callback(instance, rx_buffer);

                if (mfu_command != MfUltralightCommandNotFound)
                    break;
            }
        }
