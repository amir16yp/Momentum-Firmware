...
bool ntag4xx_load(Ntag4xxData *data, FlipperFormat *ff, uint32_t version)
{
    ...
    do {
        ...
        if (!ntag4xx_version_load(&data->version, ff))
            break;
...
bool ntag4xx_save(const Ntag4xxData *data, FlipperFormat *ff)
{
    ...
    do {
        ...
        if (!flipper_format_write_comment_cstr(ff, NTAG4XX_PROTOCOL_NAME " specific data"))
            break;
        if (!ntag4xx_version_save(&data->version, ff))
            break;
...
Ntag4xxType ntag4xx_get_type_from_version(const Ntag4xxVersion *const version)
{
    ...
    // about what the HWMajorVersion is supposed to be for NTAG426Q DNA, and they don't seem to be
    // for sale case NTAG4XX_HW_MAJOR_TYPE_426Q_DNA:
    //     if(version->hw_subtype & NTAG4XX_HW_SUBTYPE_TAGTAMPER_FLAG) {
