...
bool mf_classic_load(MfClassicData *data, FlipperFormat *ff, uint32_t version)
{
    ...
    do {
        ...
        if (!flipper_format_read_string(ff, "Mifare Classic type", temp_str))
            break;
...
static void mf_classic_set_block_str(FuriString *block_str, const MfClassicData *data,
                                     uint8_t block_num)
{
...
bool mf_classic_save(const MfClassicData *data, FlipperFormat *ff)
{
    ...
    do {
        ...
        if (!flipper_format_write_comment_cstr(ff, "Mifare Classic specific data"))
            break;
        if (!flipper_format_write_string_cstr(ff, "Mifare Classic type",
                                              mf_classic_features[data->type].type_name))
            break;
        if (!flipper_format_write_uint32(ff, "Data format version", &mf_classic_data_format_version,
                                         1))
            break;
...
bool mf_classic_is_equal(const MfClassicData *data, const MfClassicData *other)
{
    ...
    do {
        ...
        if (data->type != other->type)
            break;
        if (data->key_a_mask != other->key_a_mask)
            break;
        if (data->key_b_mask != other->key_b_mask)
            break;
...
MfClassicSectorTrailer *mf_classic_get_sector_trailer_by_sector(const MfClassicData *data,
                                                                uint8_t sector_num)
{
    furi_check(data);
...
void mf_classic_set_sector_trailer_read(MfClassicData *data, uint8_t block_num,
                                        MfClassicSectorTrailer *sec_tr)
{
    ...
    uint8_t sector_num = mf_classic_get_sector_by_block(block_num);
    MfClassicSectorTrailer *sec_trailer = mf_classic_get_sector_trailer_by_sector(data, sector_num);
...
bool mf_classic_is_key_found(const MfClassicData *data, uint8_t sector_num,
...
void mf_classic_set_key_found(MfClassicData *data, uint8_t sector_num, MfClassicKeyType key_type,
                              uint64_t key)
{
    furi_check(data);
...
void mf_classic_set_key_not_found(MfClassicData *data, uint8_t sector_num,
                                  MfClassicKeyType key_type)
{
    furi_check(data);
...
MfClassicKey mf_classic_get_key(const MfClassicData *data, uint8_t sector_num,
                                MfClassicKeyType key_type)
{
    furi_check(data);
...
void mf_classic_get_read_sectors_and_keys(const MfClassicData *data, uint8_t *sectors_read,
                                          uint8_t *keys_found)
{
    furi_check(data);
...
static bool mf_classic_is_allowed_access_sector_trailer(MfClassicData *data, uint8_t block_num,
                                                        MfClassicKeyType key_type,
...
bool mf_classic_is_allowed_access_data_block(MfClassicSectorTrailer *sec_tr, uint8_t block_num,
                                             MfClassicKeyType key_type, MfClassicAction action)
{
    furi_check(sec_tr);
...
bool mf_classic_is_allowed_access(MfClassicData *data, uint8_t block_num, MfClassicKeyType key_type,
                                  MfClassicAction action)
{
    furi_check(data);
...
bool mf_classic_is_value_block(MfClassicSectorTrailer *sec_tr, uint8_t block_num)
{
    ...
    return !mf_classic_is_allowed_access_data_block(sec_tr, block_num, MfClassicKeyTypeA,
                                                    MfClassicActionDataWrite) &&
           (mf_classic_is_allowed_access_data_block(sec_tr, block_num, MfClassicKeyTypeB,
