...
// TODO FL-3926: Buffer writes for Hardnested, set state to Log when finished and sum property
// matches
...
const uint16_t valid_sums[] = {0,   32,  56,  64,  80,  96,  104, 112, 120, 128,
...
static NfcCommand mf_classic_poller_handle_data_update(MfClassicPoller *instance)
{
    ...
    mf_classic_get_read_sectors_and_keys(instance->data, &data_update->sectors_read,
                                         &data_update->keys_found);
...
static void mf_classic_poller_check_key_b_is_readable(MfClassicPoller *instance, uint8_t block_num,
                                                      MfClassicBlock *data)
{
    do {
        if (!mf_classic_is_sector_trailer(block_num))
            break;
        if (!mf_classic_is_allowed_access(instance->data, block_num, MfClassicKeyTypeA,
...
NfcCommand mf_classic_poller_handler_detect_type(MfClassicPoller *instance)
{
    ...
    if (instance->current_type_check == MfClassicType4k) {
        iso14443_3a_copy(instance->data->iso14443_3a_data,
...
NfcCommand mf_classic_poller_handler_request_sector_trailer(MfClassicPoller *instance)
{
    ...
    if (write_ctx->current_sector == instance->sectors_total) {
        ...
        if (instance->mfc_event_data.sec_tr_data.sector_trailer_provided) {
            ...
            memcpy(&write_ctx->sec_tr, &instance->mfc_event_data.sec_tr_data.sector_trailer,
...
NfcCommand mf_classic_handler_check_write_conditions(MfClassicPoller *instance)
{
    ...
    do {
        ...
        if (mf_classic_is_allowed_access_data_block(sec_tr, write_ctx->current_block,
                                                    MfClassicKeyTypeA, MfClassicActionDataWrite)) {
            write_ctx->key_type_write = MfClassicKeyTypeA;
        } else if (mf_classic_is_allowed_access_data_block(sec_tr, write_ctx->current_block,
                                                           MfClassicKeyTypeB,
        ...
        write_ctx->need_halt_before_write = (write_ctx->key_type_read != write_ctx->key_type_write);
...
NfcCommand mf_classic_poller_handler_read_block(MfClassicPoller *instance)
{
    ...
    do {
        ...
        error = mf_classic_poller_auth(instance, write_ctx->current_block, auth_key,
                                       write_ctx->key_type_read, NULL, false);
        if (error != MfClassicErrorNone) {
...
NfcCommand mf_classic_poller_handler_write_block(MfClassicPoller *instance)
{
    ...
    do {
        ...
        command = instance->callback(instance->general_event, instance->context);
        if (!instance->mfc_event_data.write_block_data.write_block_provided)
            break;

        // Compare tag and saved block
        if (memcmp(write_ctx->tag_block.data,
        ...
        if (write_ctx->need_halt_before_write) {
            error = mf_classic_poller_auth(instance, write_ctx->current_block, auth_key,
                                           write_ctx->key_type_write, NULL, false);
            if (error != MfClassicErrorNone) {
                FURI_LOG_D(TAG, "Failed to auth to block %d for writing", write_ctx->current_block);
                instance->state = MfClassicPollerStateFail;
                break;
            }
        }
        ...
        // Write block
        error =
            mf_classic_poller_write_block(instance, write_ctx->current_block,
...
NfcCommand mf_classic_poller_handler_write_value_block(MfClassicPoller *instance)
{
    ...
    do {
        ...
        bool key_a_inc_allowed =
            mf_classic_is_allowed_access_data_block(&write_ctx->sec_tr, write_ctx->current_block,
                                                    MfClassicKeyTypeA, MfClassicActionDataInc);
        bool key_b_inc_allowed =
            mf_classic_is_allowed_access_data_block(&write_ctx->sec_tr, write_ctx->current_block,
                                                    MfClassicKeyTypeB, MfClassicActionDataInc);
        bool key_a_dec_allowed =
            mf_classic_is_allowed_access_data_block(&write_ctx->sec_tr, write_ctx->current_block,
                                                    MfClassicKeyTypeA, MfClassicActionDataDec);
        bool key_b_dec_allowed =
            mf_classic_is_allowed_access_data_block(&write_ctx->sec_tr, write_ctx->current_block,
                                                    MfClassicKeyTypeB, MfClassicActionDataDec);

        int32_t source_value = 0;
        int32_t target_value = 0;
        if (!mf_classic_block_to_value(&instance->mfc_event_data.write_block_data.write_block,
                                       &source_value, NULL))
            break;
        if (!mf_classic_block_to_value(&write_ctx->tag_block, &target_value, NULL))
            break;
        ...
        MfClassicError error = mf_classic_poller_auth(instance, write_ctx->current_block, key,
                                                      auth_key_type, NULL, false);
        if (error != MfClassicErrorNone)
            break;
...
NfcCommand mf_classic_poller_handler_request_read_sector_blocks(MfClassicPoller *instance)
{
    ...
    do {
        ...
        if (!sec_read_ctx->auth_passed) {
            ...
            FURI_LOG_D(TAG, "Auth to block %d with key %c: %06llx", sec_read_ctx->current_block,
                       sec_read_ctx->key_type == MfClassicKeyTypeA ? 'A' : 'B', key);
            error = mf_classic_poller_auth(instance, sec_read_ctx->current_block,
                                           &sec_read_ctx->key, sec_read_ctx->key_type, NULL, false);
            if (error != MfClassicErrorNone)
                break;

            sec_read_ctx->auth_passed = true;
            if (!mf_classic_is_key_found(instance->data, sec_read_ctx->current_sector,
                                         sec_read_ctx->key_type)) {
                mf_classic_set_key_found(instance->data, sec_read_ctx->current_sector,
                                         sec_read_ctx->key_type, key);
            }
        }
        if (mf_classic_is_block_read(instance->data, sec_read_ctx->current_block))
            break;
        ...
        if (error == MfClassicErrorNone) {
            ...
            if (sec_read_ctx->key_type == MfClassicKeyTypeA) {
                mf_classic_poller_check_key_b_is_readable(instance, sec_read_ctx->current_block,
                                                          &read_block);
            }
        } else {
            mf_classic_poller_halt(instance);
...
NfcCommand mf_classic_poller_handler_analyze_backdoor(MfClassicPoller *instance)
{
    ...
    for (size_t i = 0; i < mf_classic_backdoor_keys_count; i++) {
        if (memcmp(dict_attack_ctx->current_key.data, mf_classic_backdoor_keys[i].key.data,
    ...
    MfClassicError error = mf_classic_poller_auth(instance, 0, &dict_attack_ctx->current_key,
                                                  MfClassicKeyTypeA, NULL, true);
    if ((next_key_index == 0) &&
    ...
    } else if ((error == MfClassicErrorAuth) &&
...
NfcCommand mf_classic_poller_handler_backdoor_read_sector(MfClassicPoller *instance)
{
    ...
    do {
        ...
        error = mf_classic_poller_auth(
            ...
            &(dict_attack_ctx->current_key), MfClassicKeyTypeA, NULL, true);

        if (error != MfClassicErrorNone) {
            FURI_LOG_E(TAG, "Failed to authenticate with backdoor key for sector %d",
                       current_sector);
            break;
        }
...
NfcCommand mf_classic_poller_handler_auth_a(MfClassicPoller *instance)
{
    ...
    if (mf_classic_is_key_found(instance->data, dict_attack_ctx->current_sector,
                                MfClassicKeyTypeA)) {
        ...
        if (error == MfClassicErrorNone) {
            ...
            mf_classic_set_key_found(instance->data, dict_attack_ctx->current_sector,
                                     MfClassicKeyTypeA, key);
...
NfcCommand mf_classic_poller_handler_auth_b(MfClassicPoller *instance)
{
    ...
    if (mf_classic_is_key_found(instance->data, dict_attack_ctx->current_sector,
        ...
        if (dict_attack_ctx->mode == MfClassicPollerModeDictAttackCUID) {
            instance->state = MfClassicPollerStateRequestKey;
        } else if (mf_classic_is_key_found(instance->data, dict_attack_ctx->current_sector,
                                           MfClassicKeyTypeA)) {
            instance->state = MfClassicPollerStateNextSector;
        } else {
            instance->state = MfClassicPollerStateRequestKey;
...
NfcCommand mf_classic_poller_handler_read_sector(MfClassicPoller *instance)
{
    ...
    do {
        if (mf_classic_is_block_read(instance->data, block_num))
            break;

        if (!dict_attack_ctx->auth_passed) {
            error = mf_classic_poller_auth(instance, block_num, &dict_attack_ctx->current_key,
                                           dict_attack_ctx->current_key_type, NULL, false);
            if (error != MfClassicErrorNone) {
                instance->state = MfClassicPollerStateNextSector;
...
NfcCommand mf_classic_poller_handler_key_reuse_auth_key_a(MfClassicPoller *instance)
{
    ...
    if (mf_classic_is_key_found(instance->data, dict_attack_ctx->reuse_key_sector,
                                MfClassicKeyTypeA)) {
        instance->state = MfClassicPollerStateKeyReuseStart;
    } else {
        ...
        if (error == MfClassicErrorNone) {
            FURI_LOG_I(TAG, "Key A found");
            mf_classic_set_key_found(instance->data, dict_attack_ctx->reuse_key_sector,
                                     MfClassicKeyTypeA, key);
...
static bool add_nested_nonce(MfClassicNestedNonceArray *array, uint32_t cuid, uint16_t key_idx,
                             uint32_t nt, uint32_t nt_enc, uint8_t par, uint16_t dist)
{
...
NfcCommand mf_classic_poller_handler_nested_analyze_prng(MfClassicPoller *instance)
{
    ...
    for (uint8_t i = 0; i < dict_attack_ctx->nested_nonce.count; i++) {
        ...
        if (!crypto1_is_weak_prng_nonce(nonce->nt))
            hard_nt_count++;
    }
...
NfcCommand mf_classic_poller_handler_nested_collect_nt(MfClassicPoller *instance)
{
    ...
    if (error != MfClassicErrorNone) {
        ...
        if (!add_nested_nonce(&dict_attack_ctx->nested_nonce,
                              iso14443_3a_get_cuid(instance->data->iso14443_3a_data), 0, nt_data, 0,
                              0, 0)) {
            dict_attack_ctx->prng_type = MfClassicPrngTypeNoTag;
        }
    }
...
NfcCommand mf_classic_poller_handler_nested_calibrate(MfClassicPoller *instance)
{
    ...
    error = mf_classic_poller_auth(instance, block, &dict_attack_ctx->nested_known_key,
                                   dict_attack_ctx->nested_known_key_type, &auth_ctx, use_backdoor);
    ...
    if ((dict_attack_ctx->static_encrypted) &&
        ...
        error = mf_classic_poller_auth_nested(instance, target_block,
                                              &dict_attack_ctx->nested_known_key, target_key_type,
                                              &auth_ctx, use_backdoor, false);
    ...
    for (uint8_t collection_cycle = 0; collection_cycle < MF_CLASSIC_NESTED_CALIBRATION_COUNT;
         collection_cycle++) {
        error = mf_classic_poller_auth_nested(instance, block, &dict_attack_ctx->nested_known_key,
                                              dict_attack_ctx->nested_known_key_type, &auth_ctx,
                                              use_backdoor, false);
    ...
    for (uint32_t collection_cycle = 1; collection_cycle < MF_CLASSIC_NESTED_CALIBRATION_COUNT;
         collection_cycle++) {
        bool found = false;
        uint32_t decrypted_nt_enc = crypto1_decrypt_nt_enc(cuid, nt_enc_temp_arr[collection_cycle],
        ...
        if (!found) {
            FURI_LOG_E(TAG, "Failed to find distance for nt_enc %08lx",
                       nt_enc_temp_arr[collection_cycle]);
            FURI_LOG_E(TAG, "using key %06llx and uid %08lx, nt_prev is %08lx", known_key, cuid,
                       nt_prev);
        }
    }
    ...
    if (valid_distances > 0) {
        ...
        for (uint8_t i = 0; i < valid_distances; i++) {
            if (fabsf((float)distances[i] - median) <= 3 * std_dev) {
                if (distances[i] < dict_attack_ctx->d_min)
                    dict_attack_ctx->d_min = distances[i];
                if (distances[i] > dict_attack_ctx->d_max)
                    dict_attack_ctx->d_max = distances[i];
            }
        }
    ...
    FURI_LOG_D(TAG, "Calibration completed: min=%u max=%u static=%s", dict_attack_ctx->d_min,
               dict_attack_ctx->d_max, ((d_dist >= 3) && (d_dist <= 6)) ? "true" : "false");
...
{
    // TODO FL-3926: Handle when nonce is not collected (retry counter? Do not increment
    // nested_target_key)
    ...
    do {
        ...
        // Step 1: Perform full authentication once
        error =
            mf_classic_poller_auth(instance, block, &dict_attack_ctx->nested_known_key,
                                   dict_attack_ctx->nested_known_key_type, &auth_ctx, use_backdoor);
        ...
        // Step 2: Perform nested authentication a variable number of times to get nt_enc at a
        // different PRNG offset eg. Collect most commonly observed nonce from 3 auths to known
        // sector and 4th to target, then separately the
        //     most commonly observed nonce from 4 auths to known sector and 5th to target. This
        //     gets us a nonce pair, at a known distance (confirmed by parity bits) telling us the
        //     nt_enc plain.
        for (uint8_t collection_cycle = 0; collection_cycle < (nt_enc_per_collection - 1);
            ...
            error = mf_classic_poller_auth_nested(
                instance, block, &dict_attack_ctx->nested_known_key,
                dict_attack_ctx->nested_known_key_type, &auth_ctx, use_backdoor, false);
        ...
        if (is_weak && !(dict_attack_ctx->static_encrypted)) {
            ...
            while (current_dist <= dict_attack_ctx->d_max) {
                ...
                if (crypto1_nonce_matches_encrypted_parity_bits(nth_successor,
                                                                nth_successor ^ nt_enc, parity)) {
                    found_nt_cnt++;
                    if (found_nt_cnt > 1) {
        ...
        if (add_nested_nonce(&dict_attack_ctx->nested_nonce, cuid,
                             dict_attack_ctx->nested_target_key, found_nt, nt_enc, parity, dist)) {
            dict_attack_ctx->auth_passed = true;
        } else {
            FURI_LOG_E(TAG, "Failed to add nested nonce to array. OOM?");
        }

        FURI_LOG_D(TAG, "Target: %u (nonce pair %u, key type %s, block %u)",
                   dict_attack_ctx->nested_target_key, nonce_pair_index,
                   (target_key_type == MfClassicKeyTypeA) ? "A" : "B", target_block);
        FURI_LOG_T(TAG, "cuid: %08lx", cuid);
        FURI_LOG_T(TAG, "nt_enc: %08lx", nt_enc);
        FURI_LOG_T(TAG, "parity: %u%u%u%u", ((parity >> 3) & 1), ((parity >> 2) & 1),
                   ((parity >> 1) & 1), (parity & 1));
...
static MfClassicKey *search_dicts_for_nonce_key(MfClassicPollerDictAttackContext *dict_attack_ctx,
                                                MfClassicNestedNonceArray *nonce_array,
                                                KeysDict *system_dict, KeysDict *user_dict,
                                                bool is_weak)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        while (keys_dict_get_next_key(dicts[i], stack_key.data, sizeof(MfClassicKey))) {
            if (is_resumed && !found_resume_point) {
                found_resume_point = (memcmp(dict_attack_ctx->current_key.data, stack_key.data,
                                             sizeof(MfClassicKey)) == 0);
                continue;
            }
            ...
            for (uint8_t j = 0; j < nonce_array->count; j++) {
                ...
                full_match &= crypto1_nonce_matches_encrypted_parity_bits(
                    nt_enc_plain, nt_enc_plain ^ nonce_array->nonces[j].nt_enc,
...
NfcCommand mf_classic_poller_handler_nested_dict_attack(MfClassicPoller *instance)
{
    ...
    do {
        ...
        if (((is_weak) && (dict_attack_ctx->nested_nonce.count == 0)) ||
            ...
            error = mf_classic_poller_auth_nested(instance, target_block,
                                                  &dict_attack_ctx->nested_known_key,
                                                  target_key_type, &auth_ctx, false, true);
            ...
            bool success =
                add_nested_nonce(&dict_attack_ctx->nested_nonce, cuid,
                                 dict_attack_ctx->nested_target_key, 0, nt_enc, parity, 0);
            if (!success) {
                FURI_LOG_E(TAG, "Failed to add nested nonce to array. OOM?");
                dict_attack_ctx->auth_passed = false;
                break;
        ...
        if ((is_weak && (dict_attack_ctx->nested_nonce.count == 1)) ||
            ...
            MfClassicKey *key_candidate =
                search_dicts_for_nonce_key(dict_attack_ctx, &dict_attack_ctx->nested_nonce,
                                           dict_attack_ctx->mf_classic_system_dict,
                                           dict_attack_ctx->mf_classic_user_dict, is_weak);
            if (key_candidate != NULL) {
                FURI_LOG_I(TAG, "Found key candidate %06llx",
        ...
        FURI_LOG_D(TAG, "Target: %u (key type %s, block %u) cuid: %08lx",
                   dict_attack_ctx->nested_target_key,
                   (target_key_type == MfClassicKeyTypeA) ? "A" : "B", target_block, cuid);
    } while (false);
...
NfcCommand mf_classic_poller_handler_nested_log(MfClassicPoller *instance)
{
    ...
    do {
        if (weak_prng && (!(static_encrypted)) && (dict_attack_ctx->nested_nonce.count != 2)) {
            FURI_LOG_E(TAG, "MfClassicPollerStateNestedLog expected 2 nonces, received %zu",
        ...
        if (!buffered_file_stream_open(stream, MF_CLASSIC_NESTED_LOGS_FILE_PATH, FSAM_WRITE,
                                       FSOM_OPEN_APPEND))
            break;
        ...
        for (size_t i = 0; i < nonce_pair_count; i++) {
            ...
            MfClassicKeyType nonce_key_type =
                (nonce->key_idx % (weak_prng ? 4 : 2) < (weak_prng ? 2 : 1)) ? MfClassicKeyTypeA
                                                                             : MfClassicKeyTypeB;
            furi_string_printf(temp_str, "Sec %d key %c cuid %08lx", nonce_sector,
                               (nonce_key_type == MfClassicKeyTypeA) ? 'A' : 'B', nonce->cuid);
            for (uint8_t nt_idx = 0; nt_idx < ((weak_prng && (!(static_encrypted))) ? 2 : 1);
                 nt_idx++) {
                if (nt_idx == 1) {
                    nonce = &dict_attack_ctx->nested_nonce.nonces[i + 1];
                }
                furi_string_cat_printf(temp_str, " nt%u %08lx ks%u %08lx par%u ", nt_idx, nonce->nt,
                                       nt_idx, nonce->nt_enc ^ nonce->nt, nt_idx);
...
NfcCommand mf_classic_poller_handler_nested_controller(MfClassicPoller *instance)
{
    ...
    if (dict_attack_ctx->nested_phase == MfClassicNestedPhaseAnalyzePRNG) {
        if (dict_attack_ctx->nested_nonce.count < MF_CLASSIC_NESTED_ANALYZE_NT_COUNT) {
            ...
            return command;
        } else if ((dict_attack_ctx->nested_nonce.count == MF_CLASSIC_NESTED_ANALYZE_NT_COUNT) &&
        ...
        if (initial_dict_attack_iter) {
            ...
            dict_attack_ctx->mf_classic_system_dict =
                keys_dict_check_presence(MF_CLASSIC_NESTED_SYSTEM_DICT_PATH)
                    ? keys_dict_alloc(MF_CLASSIC_NESTED_SYSTEM_DICT_PATH, KeysDictModeOpenExisting,
                                      sizeof(MfClassicKey))
                    : NULL;

            dict_attack_ctx->mf_classic_user_dict =
                keys_dict_check_presence(MF_CLASSIC_NESTED_USER_DICT_PATH)
                    ? keys_dict_alloc(MF_CLASSIC_NESTED_USER_DICT_PATH, KeysDictModeOpenExisting,
                                      sizeof(MfClassicKey))
                    : NULL;
        }
        if ((is_weak && (dict_attack_ctx->nested_nonce.count == 1)) ||
    ...
    if (dict_attack_ctx->nested_phase == MfClassicNestedPhaseCollectNtEnc) {
        ...
        if (dict_attack_ctx->nested_target_key < nonce_collect_key_max) {
            ...
            FURI_LOG_D(TAG, "Nested target key: %u (max: %u)", dict_attack_ctx->nested_target_key,
            ...
            if (!(dict_attack_ctx->current_key_checked)) {
                ...
                // If it is not a known key, we'll need to calibrate for static encrypted backdoored
                // tags
                if ((dict_attack_ctx->backdoor == MfClassicBackdoorAuth3) &&
...
void mf_classic_poller_set_callback(MfClassicPoller *instance, NfcGenericCallback callback,
                                    void *context)
{
    furi_assert(instance);
