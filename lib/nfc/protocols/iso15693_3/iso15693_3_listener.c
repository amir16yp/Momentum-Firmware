...
void iso15693_3_listener_set_callback(Iso15693_3Listener *instance, NfcGenericCallback callback,
                                      void *context)
{
    furi_assert(instance);
...
NfcCommand iso15693_3_listener_run(NfcGenericEvent event, void *context)
{
    ...
    if (nfc_event->type == NfcEventTypeRxEnd) {
        ...
        if (iso13239_crc_check(Iso13239CrcTypeDefault, rx_buffer)) {
            ...
            FURI_LOG_D(TAG, "Wrong CRC, buffer size: %zu",
                       bit_buffer_get_size(nfc_event->data.buffer));
        }
    }
