...
bool iso15693_3_error_response_parse(Iso15693_3Error *error, const BitBuffer *buf)
{
    ...
    if ((resp->flags & ISO15693_3_RESP_FLAG_ERROR) == 0) {
        ...
        *error = Iso15693_3ErrorUnexpectedResponse;
        return true;
    } else if (resp->error >= ISO15693_3_RESP_ERROR_CUSTOM_START &&
...
Iso15693_3Error iso15693_3_inventory_response_parse(uint8_t *data, const BitBuffer *buf)
{
    ...
    do {
        if (iso15693_3_error_response_parse(&ret, buf))
            break;
...
Iso15693_3Error iso15693_3_system_info_response_parse(Iso15693_3SystemInfo *data,
                                                      const BitBuffer *buf)
{
    furi_assert(data);
...
Iso15693_3Error iso15693_3_read_block_response_parse(uint8_t *data, uint8_t block_size,
                                                     const BitBuffer *buf)
{
    furi_assert(data);
...
Iso15693_3Error iso15693_3_get_block_security_response_parse(uint8_t *data, uint16_t block_count,
                                                             const BitBuffer *buf)
{
    furi_assert(data);
...
void iso15693_3_set_block_data(Iso15693_3Data *data, uint8_t block_num, const uint8_t *block_data,
...
void iso15693_3_append_block_security(const Iso15693_3Data *data, uint8_t block_num, BitBuffer *buf)
{
...
{
    for (size_t i = 0; i < ISO15693_3_UID_SIZE; ++i) {
        if (data->uid[i] != uid[ISO15693_3_UID_SIZE - i - 1])
            return false;
    }
    return true;
}
