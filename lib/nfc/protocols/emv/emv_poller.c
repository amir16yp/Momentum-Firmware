...
static NfcCommand emv_poller_handler_read_extra_data(EmvPoller *instance)
{
    ...
    // Search cardholder name. This operation may break communication with the card, so it should be
    // the last one
...
static void emv_poller_set_callback(EmvPoller *instance, NfcGenericCallback callback, void *context)
{
    furi_assert(instance);
