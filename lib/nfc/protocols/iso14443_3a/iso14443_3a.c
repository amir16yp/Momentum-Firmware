...
bool iso14443_3a_load(Iso14443_3aData *data, FlipperFormat *ff, uint32_t version)
{
    ...
    do {
        ...
        if (!flipper_format_read_hex(ff, ISO14443_3A_ATQA_KEY, data->atqa, 2))
            break;
        if (!flipper_format_read_hex(ff, ISO14443_3A_SAK_KEY, &data->sak, 1))
            break;
...
bool iso14443_3a_save(const Iso14443_3aData *data, FlipperFormat *ff)
{
    ...
    do {
        ...
        // Write ATQA and SAK
        if (!flipper_format_write_hex(ff, ISO14443_3A_ATQA_KEY, atqa, 2))
            break;
        if (!flipper_format_write_hex(ff, ISO14443_3A_SAK_KEY, &data->sak, 1))
            break;
        saved = true;
    } while (false);
