...
bool nfc_device_save(NfcDevice *instance, const char *path)
{
    ...
    do {
        ...
        if (!flipper_format_write_comment(ff, temp_str))
            break;

        // Write device type
        if (!flipper_format_write_string_cstr(ff, NFC_DEVICE_TYPE_KEY,
                                              nfc_devices[instance->protocol]->protocol_name))
            break;
        ...
        if (!flipper_format_write_hex(ff, NFC_DEVICE_UID_KEY, uid, uid_len))
            break;
...
static bool nfc_device_load_uid(FlipperFormat *ff, uint8_t *uid, uint32_t *uid_len,
                                const uint32_t uid_maxlen)
{
    ...
    do {
        ...
        if (!flipper_format_get_value_count(ff, NFC_DEVICE_UID_KEY, &uid_len_current))
            break;
        if (uid_len_current > uid_maxlen)
            break;
        if (!flipper_format_read_hex(ff, NFC_DEVICE_UID_KEY, uid, uid_len_current))
            break;
...
static bool nfc_device_load_unified(NfcDevice *instance, FlipperFormat *ff, uint32_t version)
{
    ...
    do {
        ...
        if (!flipper_format_read_string(ff, NFC_DEVICE_TYPE_KEY, temp_str))
            break;
        ...
        if (!nfc_device_load_uid(ff, uid, &uid_len, NFC_DEVICE_UID_MAX_LEN))
            break;
        if (!nfc_device_set_uid(instance, uid, uid_len))
            break;
...
bool nfc_device_load(NfcDevice *instance, const char *path)
{
    ...
    do {
        ...
        if (furi_string_cmp_str(temp_str, NFC_FILE_HEADER))
            break;
        if (version < NFC_MINIMUM_SUPPORTED_FORMAT_VERSION)
            break;
