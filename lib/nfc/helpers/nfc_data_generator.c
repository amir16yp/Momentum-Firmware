...
static const uint8_t default_data_ntag203[] = {0xE1, 0x10, 0x12, 0x00, 0x01, 0x03,
                                               0xA0, 0x10, 0x44, 0x03, 0x00, 0xFE};
...
static void nfc_generate_ntag_i2c_common(MfUltralightData *mfu_data, MfUltralightType type,
                                         uint16_t num_pages)
{
    nfc_generate_mf_ul_common(mfu_data);
    ...
    mfu_data->pages_total = num_pages;
    mfu_data->pages_read = num_pages;
    memcpy(mfu_data->page[0].data, mfu_data->iso14443_3a_data->uid,
    ...
    memcpy(&mfu_data->page[config_register_page], default_config_ntag_i2c,
           sizeof(default_config_ntag_i2c));
    memcpy(&mfu_data->page[session_register_page], default_config_ntag_i2c,
           sizeof(default_config_ntag_i2c));
}
...
static void nfc_generate_ntag_i2c_plus_common(MfUltralightData *mfu_data, MfUltralightType type,
                                              uint16_t num_pages)
{
...
static void nfc_generate_mf_classic_common(MfClassicData *data, uint8_t uid_len, MfClassicType type)
{
...
static void nfc_generate_mf_classic_sector_trailer(MfClassicData *data, uint8_t block)
{
    ...
    mf_classic_set_key_found(data, mf_classic_get_sector_by_block(block), MfClassicKeyTypeA,
                             0xFFFFFFFFFFFF);
    mf_classic_set_key_found(data, mf_classic_get_sector_by_block(block), MfClassicKeyTypeB,
                             0xFFFFFFFFFFFF);
}

static void nfc_generate_mf_classic_block_0(uint8_t *block, uint8_t uid_len, uint8_t sak,
                                            uint8_t atqa0, uint8_t atqa1)
{
...
static void nfc_generate_mf_classic(NfcDevice *nfc_device, uint8_t uid_len, MfClassicType type)
{
    ...
    nfc_generate_mf_classic_block_0(
        mfc_data->block[0].data, uid_len, mfc_data->iso14443_3a_data->sak,
        mfc_data->iso14443_3a_data->atqa[0], mfc_data->iso14443_3a_data->atqa[1]);
