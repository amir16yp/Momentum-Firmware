...
void pulse_reader_start(PulseReader *signal)
{
    ...
    LL_DMAMUX_SetRequestSignalID(NULL, LL_DMAMUX_REQ_GEN_0,
                                 GET_DMAMUX_EXTI_LINE(signal->gpio->pin));
    ...
    furi_hal_gpio_init_ex(signal->gpio, GpioModeInterruptRiseFall, signal->pull, GpioSpeedVeryHigh,
                          GpioAltFnUnused);
