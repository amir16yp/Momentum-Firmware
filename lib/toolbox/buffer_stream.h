 ...
 * Then the buffer pointer is written to the stream, and the new write buffer is taken from the
 * buffer pool. After the buffer has been read by the receiving thread, it is sent to the free
 * buffer pool.
 *
 * This will speed up sending large chunks of data between threads, compared to using a stream
 * directly.
 */
#pragma once
#include <furi.h>
 ...
 * Data will be written to the buffer until the buffer is full, and only then will the buffer be
 * sent.
 * @param buffer_stream
 * @param data
 * @param size
