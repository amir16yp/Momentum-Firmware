...
static bool file_stream_delete_and_insert(FileStream *stream, size_t delete_size,
                                          StreamWriteCB write_callback, const void *ctx);
...
bool file_stream_open(Stream *_stream, const char *path, FS_AccessMode access_mode,
...
static bool file_stream_delete_and_insert(FileStream *_stream, size_t delete_size,
                                          StreamWriteCB write_callback, const void *ctx)
{
    bool result = false;
    ...
    storage_get_next_filename(_stream->storage, STORAGE_EXT_PATH_PREFIX, ".scratch", ".pad",
                              tmp_name, 255);
    scratch_name = furi_string_alloc_printf(EXT_PATH("%s.pad"), furi_string_get_cstr(tmp_name));
    furi_string_free(tmp_name);

    do {
        if (!file_stream_open(scratch_stream, furi_string_get_cstr(scratch_name), FSAM_READ_WRITE,
                              FSOM_CREATE_NEW))
            break;
        ...
        if (!stream_rewind(stream))
            break;
        if (stream_copy(stream, scratch_stream, size_to_copy_before) != size_to_copy_before)
            break;

        if (write_callback) {
            if (!write_callback(scratch_stream, ctx))
                break;
        }
        size_t new_position = stream_tell(scratch_stream);

        // copy key file after insert position + size_to_delete to scratchpad
        if (!stream_seek(stream, size_to_delete, StreamOffsetFromCurrent))
            break;
        if (stream_copy(stream, scratch_stream, size_to_copy_after) != size_to_copy_after)
            break;
        ...
        if (!stream_rewind(stream))
            break;
        if (!stream_rewind(scratch_stream))
            break;
        if (stream_copy(scratch_stream, stream, new_file_size) != new_file_size)
            break;

        // and truncate original file
        if (!storage_file_truncate(_stream->file))
            break;

        // move seek pointer at insert end
        if (!stream_seek(stream, new_position, StreamOffsetFromStart))
            break;
