...
bool stream_delete_and_insert(Stream *stream, size_t delete_size, StreamWriteCB write_callback,
                              const void *ctx)
{
    furi_check(stream);
...
{
    furi_check(stream);
    return stream_write(stream, (const uint8_t *)furi_string_get_cstr(string),
                        furi_string_size(string));
}
...
{
    furi_check(stream);
    StreamWriteData write_data = {.data = (uint8_t *)furi_string_get_cstr(string),
                                  .size = furi_string_size(string)};
    return stream_delete_and_insert(stream, delete_size, stream_write_struct, &write_data);
}
...
bool stream_delete_and_insert_vaformat(Stream *stream, size_t delete_size, const char *format,
                                       va_list args)
{
    furi_check(stream);

    FuriString *data = furi_string_alloc_vprintf(format, args);
    StreamWriteData write_data = {.data = (uint8_t *)furi_string_get_cstr(data),
                                  .size = furi_string_size(data)};
...
size_t stream_copy_full(Stream *stream_from, Stream *stream_to)
{
    ...
    do {
        if (!stream_seek(stream_from, 0, StreamOffsetFromStart))
            break;
        if (!stream_seek(stream_to, 0, StreamOffsetFromStart))
            break;
...
bool stream_split(Stream *stream, Stream *stream_left, Stream *stream_right)
{
    ...
    do {
        // copy right
        if (stream_copy(stream, stream_right, size - tell) != (size - tell))
            break;

        // copy left
        if (!stream_rewind(stream))
            break;
        if (stream_copy(stream, stream_left, tell) != tell)
            break;

        // restore RW pointer
        if (!stream_seek(stream, tell, StreamOffsetFromStart))
            break;
        result = true;
    } while (false);
...
size_t stream_save_to_file(Stream *stream, Storage *storage, const char *path, FS_OpenMode mode)
{
    ...
    do {
        if (!file_stream_open(file, path, FSAM_WRITE, mode))
            break;
