...
static bool buffered_file_stream_seek(BufferedFileStream *stream, int32_t offset,
...
static size_t buffered_file_stream_write(BufferedFileStream *stream, const uint8_t *data,
                                         size_t size);
static size_t buffered_file_stream_read(BufferedFileStream *stream, uint8_t *data, size_t size);
static bool buffered_file_stream_delete_and_insert(BufferedFileStream *stream, size_t delete_size,
...
bool buffered_file_stream_open(Stream *_stream, const char *path, FS_AccessMode access_mode,
...
bool buffered_file_stream_close(Stream *_stream)
{
    ...
    do {
        if (!(stream->sync_pending ? buffered_file_stream_flush(stream)
            ...
            break;
        if (!file_stream_close(stream->file_stream))
            break;
        success = true;
    } while (false);
    return success;
...
static bool buffered_file_stream_eof(BufferedFileStream *stream)
{
    ...
    if (!stream->sync_pending) {
        ...
        ret = stream_cache_size(stream->cache) >= (remaining_size ? cache_at_end : file_stream_eof);
    }
    return ret;
}
                                         ...
                                         size_t size)
{
    ...
    do {
        if (!stream->sync_pending) {
            if (!buffered_file_stream_unread(stream))
                break;
        }
        while (need_to_write) {
...
static size_t buffered_file_stream_read(BufferedFileStream *stream, uint8_t *data, size_t size)
{
    ...
    while (need_to_read) {
        ...
        if (need_to_read) {
            if (stream->sync_pending) {
                if (!buffered_file_stream_flush(stream))
                    break;
            }
                                                   ...
                                                   StreamWriteCB write_callback, const void *ctx)
{
    ...
    do {
        if (!(stream->sync_pending ? buffered_file_stream_flush(stream)
                                   : buffered_file_stream_unread(stream)))
            break;
        if (!stream_delete_and_insert(stream->file_stream, delete_size, write_callback, ctx))
            break;
        success = true;
    } while (false);
    return success;
...
static bool buffered_file_stream_flush(BufferedFileStream *stream)
{
    ...
    do {
        ...
        if (!stream_cache_flush(stream->cache, stream->file_stream))
            break;
        if (offset > 0) {
            if (!stream_seek(stream->file_stream, -offset, StreamOffsetFromCurrent))
                break;
        }
        success = true;
    } while (false);
