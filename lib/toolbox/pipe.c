...
size_t pipe_receive(PipeSide *pipe, void *data, size_t length)
{
    ...
    while (length) {
        ...
        if (!received_this_time && pipe_state(pipe) == PipeStateBroken)
            break;
...
size_t pipe_send(PipeSide *pipe, const void *data, size_t length)
{
    ...
    while (length) {
        ...
        if (!sent_this_time && pipe_state(pipe) == PipeStateBroken)
            break;
...
static void pipe_receiving_buffer_callback(FuriEventLoopObject *buffer, void *context)
{
    ...
    if (pipe->on_data_arrived)
        pipe->on_data_arrived(pipe, pipe->callback_context);
}
...
static void pipe_semaphore_callback(FuriEventLoopObject *semaphore, void *context)
{
    ...
    PipeSide *pipe = context;
    furi_assert(pipe);
    if (pipe->on_pipe_broken)
        pipe->on_pipe_broken(pipe, pipe->callback_context);
}
...
void pipe_set_data_arrived_callback(PipeSide *pipe, PipeSideDataArrivedCallback callback,
                                    FuriEventLoopEvent event)
{
    ...
    if (callback)
        furi_event_loop_subscribe_stream_buffer(pipe->event_loop, pipe->receiving,
                                                FuriEventLoopEventIn | event,
                                                pipe_receiving_buffer_callback, pipe);
}

void pipe_set_space_freed_callback(PipeSide *pipe, PipeSideSpaceFreedCallback callback,
                                   FuriEventLoopEvent event)
{
    furi_check(pipe);
    furi_check(pipe->event_loop);
    ...
    if (callback)
        furi_event_loop_subscribe_stream_buffer(pipe->event_loop, pipe->sending,
                                                FuriEventLoopEventOut | event,
                                                pipe_sending_buffer_callback, pipe);
}

void pipe_set_broken_callback(PipeSide *pipe, PipeSideBrokenCallback callback,
                              FuriEventLoopEvent event)
{
    furi_check(pipe);
    furi_check(pipe->event_loop);
    ...
    if (callback)
        furi_event_loop_subscribe_semaphore(pipe->event_loop, pipe->shared->instance_count,
                                            FuriEventLoopEventOut | event, pipe_semaphore_callback,
                                            pipe);
}
