                                         ...
                                         bool prefix_after)
{
    ...
    if (prefix) {
        if (prefix_after) {
            snprintf(name, max_name_size, "%s-%s_%s", name_generator_left[name_generator_left_i],
                     name_generator_right[name_generator_right_i], prefix);
        } else {
            snprintf(name, max_name_size, "%s_%s-%s", prefix,
                     name_generator_left[name_generator_left_i],
                     name_generator_right[name_generator_right_i]);
        }
    } else {
        snprintf(name, max_name_size, "%s-%s", name_generator_left[name_generator_left_i],
                 name_generator_right[name_generator_right_i]);
    }
                                           ...
                                           DateTime *custom_time, bool prefix_after)
{
    ...
    if (prefix) {
        if (prefix_after) {
            snprintf(name, max_name_size, "%.4d-%.2d-%.2d_%.2d,%.2d,%.2d_%s", dateTime.year,
                     dateTime.month, dateTime.day, dateTime.hour, dateTime.minute, dateTime.second,
                     prefix);
        } else {
            snprintf(name, max_name_size, "%s_%.4d-%.2d-%.2d_%.2d,%.2d,%.2d", prefix, dateTime.year,
                     dateTime.month, dateTime.day, dateTime.hour, dateTime.minute, dateTime.second);
        }
    } else {
        snprintf(name, max_name_size, "%.4d-%.2d-%.2d_%.2d,%.2d,%.2d", dateTime.year,
                 dateTime.month, dateTime.day, dateTime.hour, dateTime.minute, dateTime.second);
    }
