...
/**
 * @brief Extract the first quoted argument from the argument string and trim the argument string.
 * If the argument is not quoted, calls args_read_string_and_trim.
 *
 * @param args arguments string
 ...
 *
 * @param [in] args the input string containing the duration value. The string may include units
 * (e.g., "10s", "0.5m").
 * @param [out] value pointer to store the parsed value in milliseconds
 * @param [in] default_unit  A default unit to be used if the input string does not contain a valid
 * suffix. Supported units: `"ms"`, `"s"`, `"m"`, `"h"` If NULL, the function will assume
