...
void cli_registry_add_command(CliRegistry *registry, const char *name, CliCommandFlag flags,
                              CliCommandExecuteCallback callback, void *context)
{
    cli_registry_add_command_ex(registry, name, flags, callback, context,
                                CLI_BUILTIN_COMMAND_STACK_SIZE);
}

void cli_registry_add_command_ex(CliRegistry *registry, const char *name, CliCommandFlag flags,
                                 CliCommandExecuteCallback callback, void *context,
                                 size_t stack_size)
{
    furi_check(registry);
    ...
    if (flags & CliCommandFlagUseShellThread)
        furi_check(!(flags & CliCommandFlagDontAttachStdio));
                                           ...
                                           const CliCommandExternalConfig *config)
{
    ...
    if (storage_dir_open(plugin_dir, config->search_directory)) {
        ...
        while (storage_dir_read(plugin_dir, NULL, plugin_filename, sizeof(plugin_filename))) {
            ...
            if (!furi_string_end_with_str(plugin_name, ".fal"))
                continue;
            furi_string_replace_all_str(plugin_name, ".fal", "");
            if (!furi_string_start_with_str(plugin_name, config->fal_prefix))
                continue;
