...
size_t cli_shell_line_skip_run(FuriString *string, size_t original_pos, CliSkipDirection direction)
{
    if (furi_string_size(string) == 0)
        return original_pos;
    if (direction == CliSkipDirectionLeft && original_pos == 0)
        return original_pos;
    ...
    while (true) {
        ...
        if (position < 0)
            break;
        if (position >= (int32_t)furi_string_size(string))
            break;
...
static bool cli_shell_line_input_cr(CliKeyCombo combo, void *context)
{
    ...
    if (line->history_position > 0) {
        ...
        memmove(&line->history[pos], &line->history[pos + 1],
                (len - pos - 1) * sizeof(FuriString *));
...
static bool cli_shell_line_input_up_down(CliKeyCombo combo, void *context)
{
    ...
    if (new_pos != line->history_position) {
        ...
        FuriString *command = cli_shell_line_get_selected(line);
        printf(ANSI_CURSOR_HOR_POS("1") "%s%s" ANSI_ERASE_LINE(ANSI_ERASE_FROM_CURSOR_TO_END),
               prompt, furi_string_get_cstr(command));
        fflush(stdout);
...
static bool cli_shell_line_input_left_right(CliKeyCombo combo, void *context)
{
    ...
    size_t new_pos = CLAMP((int)line->line_position + increment, (int)furi_string_size(command), 0);
...
static bool cli_shell_line_input_bksp(CliKeyCombo combo, void *context)
{
    ...
    printf(ANSI_CURSOR_LEFT_BY("1") "%s" ANSI_ERASE_LINE(ANSI_ERASE_FROM_CURSOR_TO_END),
...
static bool cli_shell_line_input_ctrl_l(CliKeyCombo combo, void *context)
{
    ...
    char prompt[64];
    cli_shell_line_format_prompt(line, prompt, sizeof(prompt));
    printf(ANSI_ERASE_DISPLAY(ANSI_ERASE_ENTIRE) ANSI_ERASE_SCROLLBACK_BUFFER ANSI_CURSOR_POS(
...
static bool cli_shell_line_input_ctrl_left_right(CliKeyCombo combo, void *context)
{
    ...
    CliSkipDirection direction =
        (combo.key == CliKeyLeft) ? CliSkipDirectionLeft : CliSkipDirectionRight;
...
{
    CliShellLine *line = context;
    if (combo.modifiers != CliModKeyNo)
        return false;
    if (combo.key < CliKeySpace || combo.key >= CliKeyDEL)
        return false;
    // insert character
    cli_shell_line_ensure_not_overwriting_history(line);
    FuriString *editing_line = cli_shell_line_get_editing(line);
