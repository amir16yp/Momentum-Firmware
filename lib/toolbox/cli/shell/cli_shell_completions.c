...
CliShellCompletionSegment cli_shell_completions_segment(CliShellCompletions *completions)
{
    ...
    while (1) {
        ...
        if (ret == FURI_STRING_FAILURE)
            break;
        if (ret - first_non_space > 1)
            break;
        first_non_space++;
    }
...
void cli_shell_completions_render(CliShellCompletions *completions,
                                  CliShellCompletionsAction action)
{
    furi_assert(completions);
    if (action == CliShellCompletionsActionOpen)
        furi_check(!completions->is_displaying);
    if (action == CliShellCompletionsActionClose)
        furi_check(completions->is_displaying);
    ...
    if (action == CliShellCompletionsActionOpen) {
        ...
        for
            ...
            {
                if (position == completions->selected)
                    printf(ANSI_INVERT);
        ...
        printf(ANSI_ERASE_DISPLAY(ANSI_ERASE_FROM_CURSOR_TO_END) ANSI_CURSOR_UP_BY("%zu")
        ...
        printf(ANSI_CURSOR_HOR_POS("%zu") ANSI_ERASE_DISPLAY(ANSI_ERASE_FROM_CURSOR_TO_END)
                   ANSI_CURSOR_HOR_POS("%zu"),
    ...
    } else if (action == CliShellCompletionsActionUp || action == CliShellCompletionsActionDown ||
               action == CliShellCompletionsActionLeft ||
               action == CliShellCompletionsActionRight) {
        if (CommandCompletions_empty_p(completions->variants))
            return;
        ...
        int n_columns =
            (completions_size >= COMPLETION_COLUMNS) ? COMPLETION_COLUMNS : completions_size;
        ...
        if (action == CliShellCompletionsActionLeft) {
            ...
            if (action == CliShellCompletionsActionUp)
                selection_y_unclamped--;
            if (action == CliShellCompletionsActionDown)
                selection_y_unclamped++;
            ...
            } else if ((size_t)selection_y_unclamped >
        ...
        if (new_selection != old_selection) {
            ...
            if (old_y)
                printf(ANSI_CURSOR_DOWN_BY("%zu"), old_y);
            printf(ANSI_CURSOR_HOR_POS("%zu"), old_x + 1);
            printf("%-" COMPLETION_COLUMN_WIDTH "s", furi_string_get_cstr(*CommandCompletions_cget(
                                                         completions->variants, old_selection)));
            if (old_y)
                printf(ANSI_CURSOR_UP_BY("%zu"), old_y);
            printf(ANSI_CURSOR_HOR_POS("1"));

            // print new selection in inverted colors
            if (new_y)
                printf(ANSI_CURSOR_DOWN_BY("%zu"), new_y);
            printf(ANSI_CURSOR_HOR_POS("%zu"), new_x + 1);
            printf(ANSI_INVERT "%-" COMPLETION_COLUMN_WIDTH "s" ANSI_RESET,
                   furi_string_get_cstr(
                       *CommandCompletions_cget(completions->variants, new_selection)));

            // return cursor
            printf(ANSI_CURSOR_UP_BY("%zu"), new_y + 1);
            printf(ANSI_CURSOR_HOR_POS("%zu"),
                   strlen(prompt) +
                       furi_string_size(cli_shell_line_get_selected(completions->line)) + 1);
        }

    } else if (action == CliShellCompletionsActionSelectNoClose) {
        if (!CommandCompletions_size(completions->variants))
            return;
        ...
        furi_string_replace_at(input, segment.start, segment.length,
                               furi_string_get_cstr(completion));
        printf(ANSI_CURSOR_HOR_POS("%zu") "%s" ANSI_ERASE_LINE(ANSI_ERASE_FROM_CURSOR_TO_END),
               strlen(prompt) + 1, furi_string_get_cstr(input));
...
static bool key_combo_up_down(CliKeyCombo combo, void *context)
{
    ...
    if (!completions->is_displaying)
        return false;
    cli_shell_completions_render(completions, (combo.key == CliKeyUp)
                                                  ? CliShellCompletionsActionUp
                                                  : CliShellCompletionsActionDown);
    return true;
}
...
static bool key_combo_left_right(CliKeyCombo combo, void *context)
{
    ...
    if (!completions->is_displaying)
        return false;
    cli_shell_completions_render(completions, (combo.key == CliKeyLeft)
                                                  ? CliShellCompletionsActionLeft
                                                  : CliShellCompletionsActionRight);
    return true;
}
...
static bool key_combo_tab(CliKeyCombo combo, void *context)
{
    UNUSED(combo);
    CliShellCompletions *completions = context;
    cli_shell_completions_render(completions, completions->is_displaying
                                                  ? CliShellCompletionsActionRight
                                                  : CliShellCompletionsActionOpen);
    return true;
}
