 ...
 * sign is positive
 */
StrintParseError strint_to_uint64_internal(const char *str, char **end, uint64_t *abs_out,
                                           bool *negative_out, uint8_t base,
                                           uint64_t max_abs_negative, uint64_t max_positive)
{
    // skip whitespace
    while (((*str >= '\t') && (*str <= '\r')) || *str == ' ') {
        str++;
    ...
    if (max_abs_negative == 0 && negative)
        return StrintParseSignError;
    ...
    while (*str != 0) {
        ...
        if (result > mul_limit)
            return StrintParseOverflowError;
        result *= base;
        if (result > limit - digit_value)
            return StrintParseOverflowError; //-V658
        result += digit_value;
    ...
    if (end)
        *end = (char *)str; // rabbit hole: https://c-faq.com/ansi/constmismatch.html
...
#define STRINT_MONO(name, ret_type, neg_abs_limit, pos_limit)                                      \
    ...
    {                                                                                              \
        uint64_t absolute;                                                                         \
        bool negative;                                                                             \
        StrintParseError err = strint_to_uint64_internal(str, end, &absolute, &negative, base,     \
                                                         (neg_abs_limit), (pos_limit));            \
        if (err)                                                                                   \
            return err;                                                                            \
        if (out)                                                                                   \
            *out = (negative ? (-(ret_type)absolute) : ((ret_type)absolute));                      \
        return StrintParseNoError;                                                                 \
    }
