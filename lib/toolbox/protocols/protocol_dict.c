...
void protocol_dict_set_data(ProtocolDict *dict, size_t protocol_index, const uint8_t *data,
                            size_t data_size)
{
...
void protocol_dict_get_data(ProtocolDict *dict, size_t protocol_index, uint8_t *data,
                            size_t data_size)
{
    furi_check(protocol_index < dict->count);
    furi_check(dict->base[protocol_index]->get_data != NULL);
...
ProtocolId protocol_dict_decoders_feed_by_feature(ProtocolDict *dict, uint32_t feature, bool level,
                                                  uint32_t duration)
{
    furi_check(dict);
...
ProtocolId protocol_dict_decoders_feed_by_id(ProtocolDict *dict, size_t protocol_index, bool level,
                                             uint32_t duration)
{
    furi_check(protocol_index < dict->count);
