...
/** Decode one varint. Return bytes consumed, or 0 for truncated/overflowing input.
 * On failure, value is unchanged.
 */
 ...
 * @param value value from INT32_MIN to INT32_MAX
 * @param output output array, need to be at least 5 bytes long
 * @return size_t
 */
size_t varint_int32_pack(int32_t value, uint8_t *output);

/** Decode one signed varint, with the same failure contract as varint_uint32_unpack. */
