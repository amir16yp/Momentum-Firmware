...
static int mtar_compressed_file_seek(void *stream, unsigned offset)
{
    ...
    if (offset == 0 && compress_stream_decoder_tell(compressed_stream->decoder) != 0) {
        ...
        while (position < offset) {
            ...
            if (!compress_stream_decoder_read(compressed_stream->decoder,
                                              (uint8_t *)compressed_stream->tar->buffer, count)) {
                success = false;
                break;
            }
            position += count;
...
static bool archive_extract_current_file(TarArchive *archive, const char *dst_path,
                                         uint8_t *readbuf, size_t archive_size)
{
    ...
    do {
        ...
        while (!mtar_eof_data(tar)) {
            ...
            if (archive->read_cb) {
                archive->read_cb(storage_file_tell(archive->stream), archive_size,
...
static int archive_extract_foreach_cb(mtar_t *tar, const mtar_header_t *header, void *param)
{
    ...
    if (archive->unpack_cb) {
        skip_entry = !archive->unpack_cb(header->name, header->type == MTAR_TDIR,
                                         archive->unpack_cb_context);
    }
    ...
    if (header->type == MTAR_TDIR) {
        // Skip "/" entry since concat would leave it dangling, also want caller to mkdir
        // destination
    ...
    if (!op_params->readbuf)
        op_params->readbuf = malloc(FILE_BLOCK_SIZE);
    bool success = archive_extract_current_file(archive, furi_string_get_cstr(full_extracted_fname),
                                                op_params->readbuf, op_params->archive_size);
                           ...
                           TarArchiveNameConverter converter)
{
    ...
    bool success = mtar_foreach(&archive->tar, archive_extract_foreach_cb, &param) == MTAR_ESUCCESS;
    free(param.readbuf);
    furi_string_free(param.path);
    if (param.converted_name)
        furi_string_free(param.converted_name);
    return success;
}
        ...
        while (true) {
            ...
            if (file_info_is_dir(&file_info)) {
                success =
                    ...
                    tar_archive_add_dir(archive, furi_string_get_cstr(element_fs_abs_path),
                                        furi_string_get_cstr(element_name));
            } else {
                success = tar_archive_add_file(archive, furi_string_get_cstr(element_fs_abs_path),
                                               furi_string_get_cstr(element_name), file_info.size);
            }
