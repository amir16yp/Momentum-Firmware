                       ...
                       uint8_t version)
{
    ...
    if (!saved) {
        FURI_LOG_E(TAG, "Open failed \"%s\". Error: \'%s\'", path,
    ...
    if (result) {
        ...
        if (bytes_count != (size + sizeof(header))) {
            FURI_LOG_E(TAG, "Write failed \"%s\". Error: \'%s\'", path,
                       storage_file_get_error_desc(file));
            result = false;
        }
    }
...
bool saved_struct_load(const char *path, void *data, size_t size, uint8_t magic, uint8_t version)
{
    ...
    if (!loaded) {
        FURI_LOG_E(TAG, "Failed to read \"%s\". Error: %s", path,
                   storage_file_get_error_desc(file));
        result = false;
    }
    ...
    if (result && (header.magic != magic || header.version != version)) {
        FURI_LOG_E(TAG, "Magic(%d != %d) or Version(%d != %d) mismatch of file \"%s\"",
                   header.magic, magic, header.version, version, path);
        result = false;
    }
    ...
    if (result) {
        ...
        if (header.checksum != checksum) {
            FURI_LOG_E(TAG, "Checksum(%d != %d) mismatch of file \"%s\"", header.checksum, checksum,
                       path);
            result = false;
        }
    }
