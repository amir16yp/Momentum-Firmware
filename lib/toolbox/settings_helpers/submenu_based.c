...
void submenu_settings_helpers_assign_objects(SubmenuSettingsHelper *helper,
                                             ViewDispatcher *view_dispatcher,
                                             SceneManager *scene_manager, Submenu *submenu,
                                             uint32_t submenu_view_id, uint32_t main_scene_id)
{
    furi_check(helper);
...
bool submenu_settings_helpers_app_start(SubmenuSettingsHelper *helper, void *arg)
{
    ...
    for (size_t i = 0; i < helper->descriptor->options_cnt; i++) {
        if (strcmp(helper->descriptor->options[i].name, option) == 0) {
            scene_manager_next_scene(helper->scene_manager,
                                     helper->descriptor->options[i].scene_id);
            return true;
        }
    }
...
static void submenu_settings_helpers_callback(void *context, InputType input_type, uint32_t index)
{
    ...
    if (input_type == InputTypeShort) {
        ...
        archive_favorites_handle_setting_pin_unpin(helper->descriptor->app_name,
                                                   helper->descriptor->options[index].name);
    }
}
...
void submenu_settings_helpers_scene_enter(SubmenuSettingsHelper *helper)
{
    furi_check(helper);
    for (size_t i = 0; i < helper->descriptor->options_cnt; i++) {
        submenu_add_item_ex(helper->submenu, helper->descriptor->options[i].name, i,
                            submenu_settings_helpers_callback, helper);
    }

    submenu_set_selected_item(helper->submenu, scene_manager_get_scene_state(
                                                   helper->scene_manager, helper->main_scene_id));
...
bool submenu_settings_helpers_scene_event(SubmenuSettingsHelper *helper, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        scene_manager_next_scene(helper->scene_manager,
                                 helper->descriptor->options[event.event].scene_id);
