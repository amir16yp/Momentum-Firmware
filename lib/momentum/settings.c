...
#define entry momentum_settings_entries[entry_i]
            ...
            case momentum_settings_type_str:
                ...
                if (ok)
                    strlcpy((char *)entry.val, furi_string_get_cstr(val_str), entry.str_len);
                break;
            case momentum_settings_type_int:
                ...
                if (ok)
                    memcpy(entry.val, &val_int, entry.i_sz);
                break;
