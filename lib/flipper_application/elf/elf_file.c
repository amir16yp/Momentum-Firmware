...
bx r12
*/
const uint8_t trampoline_code_little_endian[TRAMPOLINE_CODE_SIZE] = {0xdf, 0xf8, 0x02,
                                                                     0xc0, 0x60, 0x47};
...
static ELFSection *elf_file_get_or_put_section(ELFFile *elf, const char *name)
{
    ...
    if (!section_p) {
        ELFSectionDict_set_at(elf->sections, strdup(name),
                              (ELFSection){
                                  .data = NULL,
                                  .sec_idx = 0,
                                  .size = 0,
...
static bool elf_read_string_from_offset(ELFFile *elf, off_t offset, FuriString *name)
{
    ...
    do {
        if (!storage_file_seek(elf->fd, offset, true))
            break;
        ...
        while (true) {
            ...
            if (storage_file_get_error(elf->fd) != FSE_OK || read == 0)
                break;
        }
...
static bool elf_read_section(ELFFile *elf, size_t section_idx, Elf32_Shdr *section_header,
                             FuriString *name)
{
    ...
    * if target is to be entered in arm mode:
    ...
    if (!to_thumb || offset >= 0x1000000 || offset < -0x1000000) {
        if (to_thumb || (symAddr & 2) || (!is_call)) {
            FURI_LOG_D(TAG, "can't relocate value at %lx, %s, doing trampoline", relAddr,
...
static bool elf_relocate_symbol(ELFFile *elf, Elf32_Addr relAddr, int type, Elf32_Addr symAddr)
{
    ...
    case R_ARM_THM_JUMP24:
        ...
        FURI_LOG_D(TAG, "  R_ARM_THM_CALL/JMP relocated is 0x%08X",
                   (unsigned int)*((uint32_t *)relAddr));
        break;
    case R_ARM_THM_MOVW_ABS_NC:
    case R_ARM_THM_MOVT_ABS:
        elf_relocate_mov(relAddr, type, symAddr);
        FURI_LOG_D(TAG, "  R_ARM_THM_MOVW_ABS_NC/MOVT_ABS relocated is 0x%08X",
                   (unsigned int)*((uint32_t *)relAddr));
        break;
    default:
    ...
    if (s->data) {
        ...
        for (relCount = 0; relCount < relEntries; relCount++) {
            ...
            if (!address_cache_get(elf->relocation_cache, symEntry, &symAddr)) {
                ...
                FURI_LOG_D(TAG, " %08X %08X %-16s %s", (unsigned int)rel.r_offset,
                           (unsigned int)rel.r_info, elf_reloc_type_to_str(relType),
            ...
            if (symAddr != ELF_INVALID_ADDRESS) {
                FURI_LOG_D(TAG, "  symAddr=%08X relAddr=%08X", (unsigned int)symAddr,
...
static ELFLoadSectionResult elf_load_section_data(ELFFile *elf, ELFSection *section,
...
static SectionTypeInfo elf_preload_section(ELFFile *elf, size_t section_idx,
                                           Elf32_Shdr *section_header, FuriString *name_string)
{
...
static bool elf_relocate_fast(ELFFile *elf, ELFSection *s)
{
    ...
    for (uint32_t i = 0; i < records_count; i++) {
        ...
        FURI_LOG_D(TAG,
                   "Fast relocation record %ld: is_section=%d, type=%d, hash_or_section_index=%lX, "
                   "offsets_count=%ld",
                   i, is_section, type, hash_or_section_index, offsets_count);
        ...
        if (address == ELF_INVALID_ADDRESS) {
            ...
            if (elf_file_find_string_by_hash(elf, hash_or_section_index, symbol_name)) {
                FURI_LOG_E(TAG, "Failed to resolve address for symbol %s (hash %lX)",
                           furi_string_get_cstr(symbol_name), hash_or_section_index);
            } else {
                FURI_LOG_E(TAG, "Failed to resolve address for hash %lX (string not found)",
...
ElfLoadSectionTableResult elf_file_load_section_table(ELFFile *elf)
{
    ...
    for (size_t section_idx = 1; section_idx < elf->sections_count; section_idx++) {
        ...
        FURI_LOG_D(TAG, "Preloading data for section #%d %s", section_idx,
...
ElfProcessSectionResult elf_process_section(ELFFile *elf, const char *name,
                                            ElfProcessSection *process_section, void *context)
{
