...
static bool flipper_application_assets_process_files(Storage *storage, File *file,
                                                     FuriString *app_name, uint32_t files_count)
{
    furi_assert(storage);
    furi_assert(file);
    furi_assert(app_name);
    ...
    for (uint32_t i = 0; i < files_count; i++) {
        ...
        if (!storage_file_open(destination, furi_string_get_cstr(file_path), FSAM_WRITE,
                               FSOM_CREATE_ALWAYS)) {
...
static bool flipper_application_assets_process_dirs(Storage *storage, File *file,
                                                    FuriString *app_name, uint32_t dirs_count)
{
    furi_assert(storage);
    furi_assert(file);
    furi_assert(app_name);
...
static AssetsSignatureResult
flipper_application_assets_process_signature(Storage *storage, File *file, FuriString *app_name,
                                             uint8_t **signature_data, size_t *signature_data_size)
{
    furi_assert(storage);
    furi_assert(file);
    furi_assert(app_name);
    ...
    do {
        // read signature
        *signature_data = flipper_application_assets_alloc_and_load_data(file, signature_data_size);
        ...
        if (!storage_file_open(signature_file, furi_string_get_cstr(signature_file_path),
                               FSAM_READ_WRITE, FSOM_OPEN_EXISTING)) {
            FURI_LOG_E(TAG, "Can't open signature file");
            break;
        }

        if (storage_file_size(signature_file) != *signature_data_size)
            break;
        ...
        while (compared < *signature_data_size) {
            ...
            if (memcmp(*signature_data + compared, buffer, count) != 0)
                break;
            compared += count;
        }
...
bool flipper_application_assets_load(File *file, const char *elf_path, size_t offset, size_t size)
{
    ...
    do {
        ...
        // process files
        if (header.files_count && !flipper_application_assets_process_files(storage, file, app_name,
                                                                            header.files_count)) {
            break;
        }
        ...
        if (storage_file_open(signature_file, furi_string_get_cstr(signature_file_path), FSAM_WRITE,
                              FSOM_CREATE_ALWAYS)) {
