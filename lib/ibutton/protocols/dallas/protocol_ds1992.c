...
{
    DS1992ProtocolData *data = protocol_data;
    return dallas_common_write_mem(host, DS1992_COPY_SCRATCH_TIMEOUT_US, DS1992_SRAM_PAGE_SIZE,
                                   data->sram_data, DS1992_SRAM_DATA_SIZE);
}
...
bool dallas_ds1992_load(FlipperFormat *ff, uint32_t format_version,
                        iButtonProtocolData *protocol_data)
{
    ...
    do {
        ...
        if (!dallas_common_load_rom_data(ff, format_version, &data->rom_data))
            break;
        if (!flipper_format_read_hex(ff, DS1992_SRAM_DATA_KEY, data->sram_data,
...
bool dallas_ds1992_save(FlipperFormat *ff, const iButtonProtocolData *protocol_data)
{
    ...
    do {
        if (!dallas_common_save_rom_data(ff, &data->rom_data))
            break;
        if (!flipper_format_write_hex(ff, DS1992_SRAM_DATA_KEY, data->sram_data,
                                      DS1992_SRAM_DATA_SIZE))
            break;
        success = true;
    } while (false);
...
void dallas_ds1992_render_data(FuriString *result, const iButtonProtocolData *protocol_data)
{
    ...
    pretty_format_bytes_hex_canonical(result, DS1992_DATA_BYTE_COUNT, PRETTY_FORMAT_FONT_MONOSPACE,
                                      data->sram_data, DS1992_SRAM_DATA_SIZE);
}
...
void dallas_ds1992_get_editable_data(iButtonEditableData *editable_data,
                                     iButtonProtocolData *protocol_data)
{
    DS1992ProtocolData *data = protocol_data;
