...
bool dallas_common_write_scratchpad(OneWireHost *host, uint16_t address, const uint8_t *data,
                                    size_t data_size)
{
...
bool dallas_common_read_scratchpad(OneWireHost *host, DallasCommonAddressRegs *regs, uint8_t *data,
                                   size_t data_size)
{
                                   ...
                                   uint32_t timeout_us)
{
    ...
    const uint32_t poll_delay = MAX(timeout_us / DALLAS_COMMON_COPY_SCRATCH_POLL_COUNT,
...
bool dallas_common_write_mem(OneWireHost *host, uint32_t timeout_us, size_t page_size,
                             const uint8_t *data, size_t data_size)
{
    ...
    for (i = 0; i < data_size; i += page_size) {
        ...
        if (!onewire_host_reset(host))
            break;
        if (!dallas_common_skip_rom(host))
            break;
        if (!dallas_common_write_scratchpad(host, i, data_ptr, page_size))
            break;
        ...
        if (!dallas_common_skip_rom(host))
            break;
        if (!dallas_common_read_scratchpad(host, &regs, scratch, page_size))
            break;

        // Verify scratchpad contents
        if (memcmp(data_ptr, scratch, page_size) != 0)
            break;
        ...
        if (!dallas_common_skip_rom(host))
            break;
        if (!dallas_common_copy_scratchpad(host, &regs, timeout_us))
            break;
        ...
        if (!dallas_common_skip_rom(host))
            break;
        if (!dallas_common_read_scratchpad(host, &regs, scratch, 0))
            break;

        // Check if AA flag is set
        if (!(regs.fields.status & DALLAS_COMMON_STATUS_FLAG_AA))
            break;
    }
...
bool dallas_common_emulate_search_rom(OneWireSlave *bus, const DallasCommonRomData *rom_data)
{
    for (size_t i = 0; i < sizeof(DallasCommonRomData); i++) {
        for (size_t j = 0; j < BITS_IN_BYTE; j++) {
            ...
            if (!onewire_slave_send_bit(bus, bit))
                return false;
            if (!onewire_slave_send_bit(bus, !bit))
                return false;
...
bool dallas_common_emulate_read_mem(OneWireSlave *bus, const uint8_t *data, size_t data_size)
{
    ...
    do {
        if (!onewire_slave_receive(bus, address.bytes, sizeof(address)))
            break;
        if (address.word >= data_size)
            break;
        if (!onewire_slave_send(bus, data + address.word, data_size - address.word))
            break;
...
{
    return flipper_format_write_hex(ff, DALLAS_COMMON_ROM_DATA_KEY_V2, rom_data->bytes,
                                    sizeof(DallasCommonRomData));
}

bool dallas_common_load_rom_data(FlipperFormat *ff, uint32_t format_version,
                                 DallasCommonRomData *rom_data)
{
    switch (format_version) {
    case 1:
        return flipper_format_read_hex(ff, DALLAS_COMMON_ROM_DATA_KEY_V1, rom_data->bytes,
                                       sizeof(DallasCommonRomData));
    case 2:
        return flipper_format_read_hex(ff, DALLAS_COMMON_ROM_DATA_KEY_V2, rom_data->bytes,
                                       sizeof(DallasCommonRomData));
    default:
        return false;
    }
...
void dallas_common_render_brief_data(FuriString *result, const DallasCommonRomData *rom_data,
                                     const uint8_t *mem_data, size_t mem_size, const char *mem_name)
{
    UNUSED(mem_data);
...
void dallas_common_render_crc_error(FuriString *result, const DallasCommonRomData *rom_data)
{
    ...
    for (size_t i = 0; i < data_size; ++i) {
        furi_string_cat_printf(result, (i < data_size - 1) ? "%02X " : "\e!%02X\e!",
                               rom_data->bytes[i]);
    }
