...
static bool dallas_ds1971_read_mem(OneWireHost *host, uint8_t address, uint8_t *data,
                                   size_t data_size);
...
bool dallas_ds1971_load(FlipperFormat *ff, uint32_t format_version,
                        iButtonProtocolData *protocol_data)
{
    ...
    do {
        if (format_version < 2)
            break;
        if (!dallas_common_load_rom_data(ff, format_version, &data->rom_data))
            break;
        if (!flipper_format_read_hex(ff, DS1971_EEPROM_DATA_KEY, data->eeprom_data,
...
bool dallas_ds1971_save(FlipperFormat *ff, const iButtonProtocolData *protocol_data)
{
    ...
    do {
        if (!dallas_common_save_rom_data(ff, &data->rom_data))
            break;
        if (!flipper_format_write_hex(ff, DS1971_EEPROM_DATA_KEY, data->eeprom_data,
                                      DS1971_EEPROM_DATA_SIZE))
            break;
        success = true;
    } while (false);
...
void dallas_ds1971_render_data(FuriString *result, const iButtonProtocolData *protocol_data)
{
    ...
    pretty_format_bytes_hex_canonical(result, DS1971_DATA_BYTE_COUNT, PRETTY_FORMAT_FONT_MONOSPACE,
                                      data->eeprom_data, DS1971_EEPROM_DATA_SIZE);
}

void dallas_ds1971_render_brief_data(FuriString *result, const iButtonProtocolData *protocol_data)
{
    const DS1971ProtocolData *data = protocol_data;
    dallas_common_render_brief_data(result, &data->rom_data, data->eeprom_data,
                                    DS1971_EEPROM_DATA_SIZE, DS1971_MEMORY_TYPE);
}
...
void dallas_ds1971_get_editable_data(iButtonEditableData *editable_data,
                                     iButtonProtocolData *protocol_data)
{
    DS1971ProtocolData *data = protocol_data;
...
bool ds1971_emulate_read_mem(OneWireSlave *bus, const uint8_t *data, size_t data_size)
{
    ...
    do {
        uint8_t address;
        if (!onewire_slave_receive(bus, &address, sizeof(address)))
            break;
        if (address >= data_size)
            break;
        if (!onewire_slave_send(bus, data + address, data_size - address))
            break;
