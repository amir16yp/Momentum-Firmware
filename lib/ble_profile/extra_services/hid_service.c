...
static bool ble_svc_hid_char_desc_data_callback(const void *context, const uint8_t **data,
                                                uint16_t *data_len)
{
...
static bool ble_svc_hid_report_data_callback(const void *context, const uint8_t **data,
                                             uint16_t *data_len)
{
...
static const BleGattCharacteristicParams ble_svc_hid_chars[HidSvcGattCharacteristicCount] = {
    [HidSvcGattCharacteristicProtocolMode] = {.name = "Protocol Mode",
    ...
    [HidSvcGattCharacteristicReportMap] = {.name = "Report Map",
    ...
    [HidSvcGattCharacteristicInfo] = {.name = "HID Information",
                                      .data_prop_type = FlipperGattCharacteristicDataFixed,
                                      ...
                                      .gatt_evt_mask = GATT_DONT_NOTIFY_EVENTS,
                                      .is_variable = CHAR_VALUE_LEN_CONSTANT},
    [HidSvcGattCharacteristicCtrlPoint] = {.name = "HID Control Point",
                                           .data_prop_type = FlipperGattCharacteristicDataFixed,
...
BleServiceHid *ble_svc_hid_start(void)
{
    ...
    if (!ble_gatt_service_add(
            UUID_TYPE_16, &ble_svc_hid_uuid, PRIMARY_SERVICE,
            2 + /* protocol mode */
    ...
    ble_gatt_characteristic_init(hid_svc->svc_handle,
    ...
    ble_gatt_characteristic_update(
        hid_svc->svc_handle, &hid_svc->chars[HidSvcGattCharacteristicProtocolMode], &protocol_mode);
    ...
    memcpy(&ble_svc_hid_char_descr, &ble_svc_hid_char_descr_template,
           sizeof(ble_svc_hid_char_descr));
    ...
    for (size_t report_type_idx = 0; report_type_idx < COUNT_OF(hid_report_chars);
        ...
        for (size_t report_idx = 0; report_idx < hid_report_chars[report_type_idx].report_count;
            ...
            ble_gatt_characteristic_init(hid_svc->svc_handle, &report_char,
    ...
    for (size_t i = HidSvcGattCharacteristicReportMap; i < HidSvcGattCharacteristicCount; i++) {
        ble_gatt_characteristic_init(hid_svc->svc_handle, &ble_svc_hid_chars[i],
                                     &hid_svc->chars[i]);
    }
...
bool ble_svc_hid_update_input_report(BleServiceHid *hid_svc, uint8_t input_report_num,
...
bool ble_svc_hid_update_info(BleServiceHid *hid_svc, uint8_t *data)
{
    ...
    return ble_gatt_characteristic_update(hid_svc->svc_handle,
                                          &hid_svc->chars[HidSvcGattCharacteristicInfo], &data);
}
...
void ble_svc_hid_stop(BleServiceHid *hid_svc)
{
    ...
    for (size_t report_type_idx = 0; report_type_idx < COUNT_OF(hid_report_chars);
        ...
        for (size_t report_idx = 0; report_idx < hid_report_chars[report_type_idx].report_count;
             report_idx++) {
            ble_gatt_characteristic_delete(hid_svc->svc_handle,
                                           &hid_report_chars[report_type_idx].chars[report_idx]);
        }
    }
