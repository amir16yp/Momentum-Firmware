...
static FuriEventLoopItem *furi_event_loop_item_alloc(FuriEventLoop *owner,
                                                     const FuriEventLoopContract *contract,
                                                     void *object, FuriEventLoopEvent event);
...
void furi_event_loop_free(FuriEventLoop *instance)
{
    ...
    uint32_t flags = 0;
    BaseType_t ret = xTaskNotifyWaitIndexed(FURI_EVENT_LOOP_FLAG_NOTIFY_INDEX, 0,
                                            FuriEventLoopFlagAll, &flags, 0);
    if (ret == pdTRUE) {
...
static inline FuriEventLoopProcessStatus furi_event_loop_process_edge_event(FuriEventLoopItem *item)
{
...
static inline FuriEventLoopProcessStatus furi_event_loop_process_event(FuriEventLoop *instance,
...
static inline void furi_event_loop_sync_flags(FuriEventLoop *instance)
{
    ...
    if (!WaitingList_empty_p(instance->waiting_list)) {
        xTaskNotifyIndexed((TaskHandle_t)instance->thread_id, FURI_EVENT_LOOP_FLAG_NOTIFY_INDEX,
                           FuriEventLoopFlagEvent, eSetBits);
    }
...
void furi_event_loop_run(FuriEventLoop *instance)
{
    ...
    while (true) {
        const TickType_t ticks_to_sleep = MIN(furi_event_loop_get_timer_wait_time(instance),
                                              furi_event_loop_get_tick_wait_time(instance));

        uint32_t flags = 0;
        BaseType_t ret = xTaskNotifyWaitIndexed(FURI_EVENT_LOOP_FLAG_NOTIFY_INDEX, 0,
                                                FuriEventLoopFlagAll, &flags, ticks_to_sleep);
...
static void furi_event_loop_notify(FuriEventLoop *instance, FuriEventLoopFlag flag)
{
    if (FURI_IS_IRQ_MODE()) {
        ...
        (void)xTaskNotifyIndexedFromISR((TaskHandle_t)instance->thread_id,
                                        FURI_EVENT_LOOP_FLAG_NOTIFY_INDEX, flag, eSetBits, &yield);

        portYIELD_FROM_ISR(yield);

    } else {
        (void)xTaskNotifyIndexed((TaskHandle_t)instance->thread_id,
                                 FURI_EVENT_LOOP_FLAG_NOTIFY_INDEX, flag, eSetBits);
    }
}
...
static void furi_event_loop_object_subscribe(FuriEventLoop *instance, FuriEventLoopObject *object,
                                             const FuriEventLoopContract *contract,
                                          ...
                                          FuriEventLoopEventCallback callback, void *context)
{
    ...
    furi_event_loop_object_subscribe(instance, event_flag, &furi_event_flag_event_loop_contract,
                                         ...
                                         FuriEventLoopEventCallback callback, void *context)
{
    ...
    furi_event_loop_object_subscribe(instance, semaphore, &furi_semaphore_event_loop_contract,
                                     event, callback, context);
}
                                     ...
                                     void *context)
{
    ...
    furi_event_loop_object_subscribe(instance, mutex, &furi_mutex_event_loop_contract, event,
                                     callback, context);
}
...
void furi_event_loop_thread_flag_callback(FuriThreadId thread_id)
{
    ...
    if (FURI_IS_IRQ_MODE()) {
        yield = pdFALSE;
        (void)xTaskNotifyIndexedFromISR(hTask, FURI_EVENT_LOOP_FLAG_NOTIFY_INDEX,
                                        FuriEventLoopFlagThreadFlag, eSetBits, &yield);
        portYIELD_FROM_ISR(yield);
    } else {
        (void)xTaskNotifyIndexed(hTask, FURI_EVENT_LOOP_FLAG_NOTIFY_INDEX,
                                 FuriEventLoopFlagThreadFlag, eSetBits);
    }
}
...
void furi_event_loop_link_notify(FuriEventLoopLink *instance, FuriEventLoopEvent event)
{
    ...
    if (event & FuriEventLoopEventIn) {
        if (instance->item_in)
            furi_event_loop_item_notify(instance->item_in);
    } else if (event & FuriEventLoopEventOut) {
        if (instance->item_out)
            furi_event_loop_item_notify(instance->item_out);
    } else {
        furi_crash();
    }
