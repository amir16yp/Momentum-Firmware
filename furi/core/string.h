 ...
 */
size_t furi_string_replace(FuriString *string, FuriString *needle, FuriString *replace,
                           size_t start);
 ...
 * @return     Return FURI_STRING_FAILURE if 'needle' not found or replace position.
 */
size_t furi_string_replace_str(FuriString *string, const char needle[], const char replace[],
                               size_t start);
 ...
 * @param      replace  The replace
 */
void furi_string_replace_all(FuriString *string, const FuriString *needle,
...
/** Test if the string ends with the given string (case insensitive according to the current
 * locale).
 *
 * @param      string  The FuriString instance
 * @param      end     The end
...
/** Test if the string ends with the given C string (case insensitive according to the current
 * locale).
 *
 * @param      string  The FuriString instance
 * @param      end     The end
...
#define FURI_STRING_SELECT4(func1, func2, a, b, c, d)                                              \
    _Generic((b),                                                                                  \
        ...
        const char *: func2,                                                                       \
        FuriString *: func1,                                                                       \
        const FuriString *: func1)(a, b, c, d)
...
/** Compare string with string (or C string) (case insensitive according to the current locale) and
 * return the sort order.
 *
 * Note: doesn't work with UTF-8 strings.
 * (string, [c]string)
...
#define furi_string_search(...)                                                                    \
    M_APPLY(FURI_STRING_SELECT3, furi_string_search, furi_string_search_str,                       \
...
/** Test if the string ends with the given string (or C string) (case insensitive according to the
 * current locale).
 *
 * (string, [c]string)
 */
...
#define furi_string_replace(...)                                                                   \
    M_APPLY(FURI_STRING_SELECT4, furi_string_replace, furi_string_replace_str,                     \
...
#define FURI_STRING_OPLIST                                                                         \
    (INIT(F_STR_INIT), INIT_SET(F_STR_INIT_SET), SET(furi_string_set), INIT_MOVE(F_STR_INIT_MOVE), \
     MOVE(furi_string_move), SWAP(furi_string_swap), RESET(furi_string_reset),                     \
     EMPTY_P(furi_string_empty), CLEAR(furi_string_free), HASH(furi_string_hash),                  \
     EQUAL(furi_string_equal), CMP(furi_string_cmp), TYPE(FuriString *))
