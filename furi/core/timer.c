...
FuriTimer *furi_timer_alloc(FuriTimerCallback func, FuriTimerType type, void *context)
{
    ...
    const TimerHandle_t hTimer = xTimerCreateStatic(name, portMAX_DELAY, reload, instance,
                                                    furi_timer_callback, &instance->container);
...
void furi_timer_flush(void)
{
    ...
    furi_check(xTimerPendFunctionCall(furi_timer_flush_epilogue, hEvent, 0, portMAX_DELAY) ==
               pdPASS);

    furi_check(xEventGroupWaitBits(hEvent, TIMER_DELETED_EVENT, pdFALSE, pdTRUE, portMAX_DELAY) ==
               TIMER_DELETED_EVENT);
