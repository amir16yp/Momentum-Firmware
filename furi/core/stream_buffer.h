 ...
 * before a task that is blocked on the stream buffer to wait for data is moved out of the blocked
 * state.
 ...
 * @return true if trigger level can be be updated (new trigger level was less than or equal to the
 * stream buffer's length).
 * @return false if trigger level can't be be updated (new trigger level was greater than the stream
 * buffer's length).
 */
 ...
 */
size_t furi_stream_buffer_send(FuriStreamBuffer *stream_buffer, const void *data, size_t length,
                               uint32_t timeout);
 ...
 */
size_t furi_stream_buffer_receive(FuriStreamBuffer *stream_buffer, void *data, size_t length,
                                  uint32_t timeout);
