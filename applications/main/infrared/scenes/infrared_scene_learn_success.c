...
static void infrared_scene_learn_success_dialog_result_callback(DialogExResult result,
                                                                void *context)
{
...
void infrared_scene_learn_success_on_enter(void *context)
{
    ...
    if (infrared_signal_is_raw(signal)) {
        ...
        infrared_text_store_set(infrared, 1, "A: 0x%0*lX\nC: 0x%0*lX\n", addr_digits,
                                message->address, cmd_digits, message->command);

        dialog_ex_set_header(dialog_ex, infrared->text_store[0], 95, 7, AlignCenter, AlignCenter);
        dialog_ex_set_text(dialog_ex, infrared->text_store[1], label_x_offset, 34, AlignLeft,
                           AlignCenter);
    }
