...
const char *const easy_mode_button_names[] = {
    "Power", "Vol_up", "Vol_dn", "Mute", "Ch_up", "Ch_dn", "Ok",    "Up",
    "Down",  "Left",   "Right",  "Menu", "Back",  "Play",  "Pause", "Stop",
    "Next",  "Prev",   "FF",     "Rew",  "Input", "Exit",  "Eject", "Subtitle"};
...
static bool infrared_scene_learn_get_next_name(InfraredApp *infrared, int32_t start_index,
                                               int32_t *next_index)
{
    if (!infrared->remote)
        return false;
...
static void infrared_scene_learn_update_button_name(InfraredApp *infrared, bool increment)
{
    ...
    dialog_ex_set_text(dialog_ex, "Point the remote at IR port\nand press button:", 5, 10,
...
void infrared_scene_learn_on_enter(void *context)
{
    ...
    infrared_worker_rx_set_received_signal_callback(worker, infrared_signal_received_callback,
                                                    context);
    ...
    if (infrared->app_state.is_easy_mode) {
        ...
        dialog_ex_set_text(dialog_ex, "Point the remote at IR port\nand press the button", 5, 10,
                           AlignLeft, AlignCenter);
    }

    dialog_ex_set_left_button_text(dialog_ex, infrared->app_state.is_easy_mode ? "Easy" : "Manual");
    dialog_ex_set_right_button_text(dialog_ex, infrared->app_state.is_decode_forced    ? "Decode"
...
bool infrared_scene_learn_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == InfraredCustomEventTypeSignalReceived) {
            ...
            infrared_worker_rx_enable_signal_decoding(infrared->worker,
                                                      infrared->app_state.is_decode_enabled);
            infrared_worker_rx_force_signal_decoding(infrared->worker,
                                                     infrared->app_state.is_decode_forced);
            dialog_ex_set_right_button_text(infrared->dialog_ex,
