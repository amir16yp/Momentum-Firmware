...
void infrared_scene_universal_bluray_on_enter(void *context)
{
    ...
    // Power button
    button_panel_add_item(button_panel, i, 0, 0, 6, 13, &I_power_19x20, &I_power_hover_19x20,
                          infrared_scene_universal_common_item_callback, context);
    button_panel_add_icon(button_panel, 4, 35, &I_power_text_24x5);
    infrared_brute_force_add_record(brute_force, i++, "Power");

    // Eject button (using mute icon as a placeholder)
    button_panel_add_item(button_panel, i, 1, 0, 39, 13, &I_eject_19x20, &I_eject_hover_19x20,
                          infrared_scene_universal_common_item_callback, context);
    button_panel_add_icon(button_panel, 39, 35, &I_eject_text_19x5);
    infrared_brute_force_add_record(brute_force, i++, "Eject");

    // Play button
    button_panel_add_item(button_panel, i, 0, 1, 6, 42, &I_play_19x20, &I_play_hover_19x20,
                          infrared_scene_universal_common_item_callback, context);
    button_panel_add_icon(button_panel, 6, 64, &I_play_text_19x5);
    infrared_brute_force_add_record(brute_force, i++, "Play");

    // Pause button
    button_panel_add_item(button_panel, i, 1, 1, 39, 42, &I_pause_19x20, &I_pause_hover_19x20,
                          infrared_scene_universal_common_item_callback, context);
    button_panel_add_icon(button_panel, 37, 64, &I_pause_text_23x5);
    infrared_brute_force_add_record(brute_force, i++, "Pause");

    // Fast Backward
    button_panel_add_item(button_panel, i, 0, 2, 6, 71, &I_fast_backward_19x20,
    ...
    // Fast Forward button
    button_panel_add_item(button_panel, i, 1, 2, 39, 71, &I_fast_f_19x20, &I_fast_f_hover_19x20,
                          infrared_scene_universal_common_item_callback, context);
    button_panel_add_icon(button_panel, 39, 93, &I_fast_f_text_19x6);
    infrared_brute_force_add_record(brute_force, i++, "Fast_fo");

    // OK/Select Button
    button_panel_add_item(button_panel, i, 0, 3, 6, 101, &I_ok_19x20, &I_ok_hover_19x20,
                          infrared_scene_universal_common_item_callback, context);
    button_panel_add_icon(button_panel, 6, 123, &I_ok_text_19x5);
    infrared_brute_force_add_record(brute_force, i++, "Ok");

    // Subtitle/CC Button
    button_panel_add_item(button_panel, i, 1, 3, 39, 101, &I_subtitle_19x20,
                          &I_subtitle_hover_19x20, infrared_scene_universal_common_item_callback,
                          context);
...
{
    FURI_LOG_D(TAG, "Universal Blu-ray/DVD scene event: type=%d, event=%ld", event.type,
               event.event);
...
void infrared_scene_universal_bluray_on_exit(void *context)
{
    ...
    InfraredApp *infrared = context;
    FURI_LOG_D(TAG, "Brute force state before exit: started=%d",
