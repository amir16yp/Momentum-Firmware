...
static void infrared_scene_remote_button_menu_callback(void *context, int32_t index, InputType type)
{
    ...
    view_dispatcher_send_custom_event(infrared->view_dispatcher,
                                      infrared_custom_event_pack(custom_type, index));
}
...
void infrared_scene_remote_on_enter(void *context)
{
    ...
    for (size_t i = 0; i < infrared_remote_get_signal_count(remote); ++i) {
        button_menu_add_item(button_menu, infrared_remote_get_signal_name(remote, i), i,
                             infrared_scene_remote_button_menu_callback, ButtonMenuItemTypeCommon,
                             context);
    }

    button_menu_add_item(button_menu, "+", ButtonIndexLearn,
                         infrared_scene_remote_button_menu_callback, ButtonMenuItemTypeControl,
                         context);
...
bool infrared_scene_remote_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeBack) {
        ...
        if (custom_type == InfraredCustomEventTypeTransmitStarted) {
            ...
            furi_assert(button_index < 0);
            if (is_transmitter_idle) {
                scene_manager_set_scene_state(scene_manager, InfraredSceneRemote,
