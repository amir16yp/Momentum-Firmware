...
static void infrared_scene_rpc_show(InfraredApp *infrared)
{
    ...
    scene_manager_set_scene_state(infrared->scene_manager, InfraredSceneRpc,
                                  InfraredRpcStateSending);
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        if (event.event == InfraredCustomEventTypeRpcLoadFile) {
            ...
            if (!INFRARED_ERROR_PRESENT(task_error)) {
                ...
                furi_string_printf(str, "Failed to load\n%s",
        ...
        } else if (event.event == InfraredCustomEventTypeRpcButtonPressName ||
            ...
            if (rpc_state == InfraredRpcStateLoaded) {
                if (event.event == InfraredCustomEventTypeRpcButtonPressName) {
                    ...
                    FURI_LOG_D(TAG, "Sending signal with index \"%ld\"",
                               app_state->current_button_index);
                }
                if (infrared->app_state.current_button_index != InfraredButtonIndexNone) {
                    ...
                    if (!INFRARED_ERROR_PRESENT(error)) {
                        ...
                        rpc_system_app_set_error_code(infrared->rpc_ctx,
                                                      RpcAppSystemErrorCodeInternalParse);
                        rpc_system_app_set_error_text(infrared->rpc_ctx, "Cannot load button data");
                        result = false;
                    }
                }
            }
        ...
        } else if (event.event == InfraredCustomEventTypeRpcButtonPressReleaseName ||
