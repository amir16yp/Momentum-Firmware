...
void infrared_scene_edit_button_select_on_enter(void *context)
{
    ...
    const char *header = infrared->app_state.edit_mode == InfraredEditModeRename ? "Rename Button:"
                                                                                 : "Delete Button:";
    submenu_set_header(submenu, header);

    const size_t button_count = infrared_remote_get_signal_count(remote);
    for (size_t i = 0; i < button_count; ++i) {
        submenu_add_item(submenu, infrared_remote_get_signal_name(remote, i), i,
                         infrared_scene_edit_button_select_submenu_callback, context);
    }
