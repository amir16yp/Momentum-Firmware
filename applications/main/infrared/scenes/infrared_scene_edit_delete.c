...
static void infrared_scene_edit_delete_dialog_result_callback(DialogExResult result, void *context)
{
...
void infrared_scene_edit_delete_on_enter(void *context)
{
    ...
    if (edit_target == InfraredEditTargetButton) {
        ...
        if (infrared_signal_is_raw(infrared->current_signal)) {
            ...
            infrared_text_store_set(infrared, 0, "%s\nRAW\n%zu samples",
            ...
            if (!INFRARED_ERROR_PRESENT(task_error)) {
                ...
                if (INFRARED_ERROR_CHECK(task_error,
                    ...
                    const char *format =
                        "Failed to delete\n\"%s\" is too long.\nTry to edit file from pc";
                    infrared_show_error_message(
                        infrared, format, infrared_remote_get_signal_name(infrared->remote, index));
                } else {
