...
static int32_t infrared_scene_edit_rename_task_callback(void *context)
{
    ...
    if (edit_target == InfraredEditTargetButton) {
        ...
        error = infrared_remote_rename_signal(infrared->remote, app_state->current_button_index,
...
void infrared_scene_edit_rename_on_enter(void *context)
{
    ...
    if (edit_target == InfraredEditTargetButton) {
        ...
        strlcpy(infrared->text_store[0],
                infrared_remote_get_signal_name(remote, current_button_index), enter_name_length);
        ...
        ValidatorIsFile *validator_is_file =
            validator_is_file_alloc_init(furi_string_get_cstr(folder_path), INFRARED_APP_EXTENSION,
    ...
    text_input_set_result_callback(text_input, infrared_text_input_callback, context,
                                   infrared->text_store[0], enter_name_length, false);
        ...
        if (event.event == InfraredCustomEventTypeTextEditDone) {
            ...
            if (!INFRARED_ERROR_PRESENT(task_error)) {
                ...
                if (long_signal) {
                    ...
                    target = infrared_remote_get_signal_name(infrared->remote,
                                                             INFRARED_ERROR_GET_INDEX(task_error));
                }
