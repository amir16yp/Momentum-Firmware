...
static void infrared_rpc_command_callback(const RpcAppSystemEvent *event, void *context)
{
    ...
    if (event->type == RpcAppEventTypeSessionClose) {
        view_dispatcher_send_custom_event(infrared->view_dispatcher,
...
static void infrared_find_vacant_remote_name(FuriString *name, const char *path)
{
    ...
    furi_string_printf(base_path, "%s/%s%s", path, furi_string_get_cstr(name),
    ...
    if (status == FSE_OK) {
        ...
        uint32_t i = 1;
        do {
            furi_string_printf(path_temp, "%s%lu%s", furi_string_get_cstr(base_path), ++i,
                               INFRARED_APP_EXTENSION);
...
static InfraredApp *infrared_alloc(void)
{
    ...
    view_dispatcher_add_view(view_dispatcher, InfraredViewSubmenu,
                             submenu_get_view(infrared->submenu));

    infrared->text_input = text_input_alloc();
    view_dispatcher_add_view(view_dispatcher, InfraredViewTextInput,
                             text_input_get_view(infrared->text_input));

    infrared->dialog_ex = dialog_ex_alloc();
    view_dispatcher_add_view(view_dispatcher, InfraredViewDialogEx,
                             dialog_ex_get_view(infrared->dialog_ex));

    infrared->button_menu = button_menu_alloc();
    view_dispatcher_add_view(view_dispatcher, InfraredViewButtonMenu,
                             button_menu_get_view(infrared->button_menu));
    ...
    view_dispatcher_add_view(view_dispatcher, InfraredViewVariableList,
                             variable_item_list_get_view(infrared->var_item_list));

    infrared->view_stack = view_stack_alloc();
    view_dispatcher_add_view(view_dispatcher, InfraredViewStack,
                             view_stack_get_view(infrared->view_stack));

    infrared->move_view = infrared_move_view_alloc();
    view_dispatcher_add_view(view_dispatcher, InfraredViewMove,
                             infrared_move_view_get_view(infrared->move_view));

    infrared->loading = loading_alloc();
    view_dispatcher_add_view(view_dispatcher, InfraredViewLoading,
                             loading_get_view(infrared->loading));

    if (app_state->is_debug_enabled) {
        infrared->debug_view = infrared_debug_view_alloc();
        view_dispatcher_add_view(view_dispatcher, InfraredViewDebugView,
...
InfraredErrorCode infrared_add_remote_with_button(const InfraredApp *infrared,
                                                  ...
                                                  const InfraredSignal *signal)
{
    ...
    furi_string_cat_printf(new_path, "/%s%s", furi_string_get_cstr(new_name),
                           INFRARED_APP_EXTENSION);
...
InfraredErrorCode infrared_rename_current_remote(const InfraredApp *infrared, const char *new_name)
{
    InfraredRemote *remote = infrared->remote;
...
void infrared_play_notification_message(const InfraredApp *infrared,
...
void infrared_signal_received_callback(void *context, InfraredWorkerSignal *received_signal)
{
    ...
    if (infrared_worker_signal_is_decoded(received_signal)) {
        infrared_signal_set_message(infrared->current_signal,
...
int32_t infrared_app(void *p)
{
    ...
    if (p && strlen(p)) {
        ...
        if (sscanf(p, "RPC %lX", &rpc_ctx) == 1) {
            ...
            rpc_system_app_set_callback(infrared->rpc_ctx, infrared_rpc_command_callback, infrared);
    ...
    if (is_rpc_mode) {
        view_dispatcher_attach_to_gui(infrared->view_dispatcher, infrared->gui,
