...
typedef InfraredErrorCode (*InfraredBatchCallback)(const InfraredBatch *batch,
...
bool infrared_remote_get_signal_index(const InfraredRemote *remote, const char *name, size_t *index)
{
    uint32_t i = 0;
    StringArray_it_t it;
...
static InfraredErrorCode infrared_remote_batch_start(InfraredRemote *remote,
                                                     ...
                                                     const InfraredBatchTarget *target)
{
    ...
    do {
        if (!flipper_format_buffered_file_open_existing(batch_context.ff_in, path_in) ||
            ...
            !flipper_format_write_header_cstr(batch_context.ff_out, INFRARED_FILE_HEADER,
                                              INFRARED_FILE_VERSION)) {
            error = InfraredErrorCodeFileOperationFailed;
            break;
        }
        ...
        for (; batch_context.signal_index < signal_count; ++batch_context.signal_index) {
            error = infrared_signal_read(batch_context.signal, batch_context.ff_in,
                                         batch_context.signal_name);
            if (INFRARED_ERROR_PRESENT(error)) {
    ...
    if (INFRARED_ERROR_PRESENT(error)) {
        ...
        if (status == FSE_OK || status == FSE_EXIST)
            storage_common_remove(storage, path_out);
...
static InfraredErrorCode infrared_remote_insert_signal_callback(const InfraredBatch *batch,
                                                                const InfraredBatchTarget *target)
{
    ...
    if (batch->signal_index == target->signal_index) {
        ...
        StringArray_push_at(batch->remote->signal_names, target->signal_index, target->signal_name);
    }

    // Write the rest normally
    return infrared_signal_save(batch->signal, batch->ff_out,
                                furi_string_get_cstr(batch->signal_name));
}
                                                ...
                                                size_t index)
{
    ...
    return infrared_remote_batch_start(remote, infrared_remote_insert_signal_callback,
                                       &insert_target);
}

static InfraredErrorCode infrared_remote_rename_signal_callback(const InfraredBatch *batch,
                                                                const InfraredBatchTarget *target)
{
    const char *signal_name;
                                                ...
                                                const char *new_name)
{
    ...
    return infrared_remote_batch_start(remote, infrared_remote_rename_signal_callback,
                                       &rename_target);
}

static InfraredErrorCode infrared_remote_delete_signal_callback(const InfraredBatch *batch,
                                                                const InfraredBatchTarget *target)
{
    if (batch->signal_index == target->signal_index) {
        // Do not save the signal to be deleted, remove it from the signal name list instead
        StringArray_remove_v(batch->remote->signal_names, batch->signal_index,
                             batch->signal_index + 1);
    } else {
...
InfraredErrorCode infrared_remote_delete_signal(InfraredRemote *remote, size_t index)
{
    ...
    return infrared_remote_batch_start(remote, infrared_remote_delete_signal_callback,
                                       &delete_target);
}
