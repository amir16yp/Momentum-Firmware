...
static void infrared_cli_print_universal_remotes(void)
{
    ...
    do {
        if (!storage_dir_open(dir, INFRARED_ASSETS_FOLDER))
            break;
...
static bool infrared_cli_save_signal(InfraredSignal *signal, FlipperFormat *file, const char *name)
{
    bool ret = true;
    InfraredErrorCode error = infrared_signal_save(signal, file, name);
    if (INFRARED_ERROR_PRESENT(error)) {
        printf("Failed to save signal: \"%s\" code: 0x%X index: 0x%02X\r\n", name,
               INFRARED_ERROR_GET_CODE(error), INFRARED_ERROR_GET_INDEX(error));
        ret = false;
    }
    return ret;
}

static bool infrared_cli_decode_raw_signal(const InfraredRawSignal *raw_signal,
                                           InfraredDecoderHandler *decoder,
                                           FlipperFormat *output_file, const char *signal_name)
{
    InfraredSignal *signal = infrared_signal_alloc();
    ...
    for (i = 0; i < raw_signal->timings_size; ++i) {
        ...
        if (message) {
            ...
            printf("Protocol: %s address: 0x%lX command: 0x%lX %s\r\n",
                   infrared_get_protocol_name(message->protocol), message->address,
                   message->command, (message->repeat ? "R" : ""));
    ...
    if (i == raw_signal->timings_size) {
        if (!is_decoded && output_file) {
            infrared_signal_set_raw_signal(signal, raw_signal->timings, raw_signal->timings_size,
...
static bool infrared_cli_decode_file(FlipperFormat *input_file, FlipperFormat *output_file)
{
    ...
    while (infrared_signal_read(signal, input_file, tmp) == InfraredErrorCodeNone) {
        ...
        printf("Raw signal: %s, %zu samples\r\n", furi_string_get_cstr(tmp),
               raw_signal->timings_size);
        if (!infrared_cli_decode_raw_signal(raw_signal, decoder, output_file,
                                            furi_string_get_cstr(tmp)))
            break;
        ret = true;
    }
...
static void infrared_cli_process_decode(PipeSide *pipe, FuriString *args)
{
    ...
    do {
        ...
        if (!flipper_format_buffered_file_open_existing(input_file,
    ...
    if (output_file)
        flipper_format_free(output_file);
    furi_record_close(RECORD_STORAGE);
}
...
static void infrared_cli_list_remote_signals(FuriString *remote_name)
{
    ...
    FuriString *remote_path =
        furi_string_alloc_printf("%s/%s%s", INFRARED_ASSETS_FOLDER,
                                 furi_string_get_cstr(remote_name), INFRARED_FILE_EXTENSION);
...
static void infrared_cli_brute_force_signals(PipeSide *pipe, FuriString *remote_name,
                                             FuriString *signal_name)
{
    InfraredBruteForce *brute_force = infrared_brute_force_alloc();
    FuriString *remote_path = furi_string_alloc_printf("%s/%s.ir", INFRARED_ASSETS_FOLDER,
                                                       furi_string_get_cstr(remote_name));

    infrared_brute_force_set_db_filename(brute_force, furi_string_get_cstr(remote_path));
    infrared_brute_force_add_record(brute_force, INFRARED_BRUTE_FORCE_DUMMY_INDEX,
    ...
    do {
        ...
        bool running = infrared_brute_force_start(brute_force, INFRARED_BRUTE_FORCE_DUMMY_INDEX,
                                                  &signal_count);
...
static void infrared_cli_process_universal(PipeSide *pipe, FuriString *args)
{
    ...
    do {
        if (!args_read_string_and_trim(args, arg1))
            break;
        if (!args_read_string_and_trim(args, arg2))
            break;
    } while (false);
