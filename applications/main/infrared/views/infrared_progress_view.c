...
static void infrared_progress_view_draw_callback(Canvas *canvas, void *_model)
{
    ...
    elements_multiline_text_aligned(canvas, x + 32, y + 9, AlignCenter, AlignCenter,
    ...
    if (model->is_paused) {
        snprintf(progress_string, sizeof(progress_string), "%zu/%zu", model->progress,
    ...
    elements_multiline_text_aligned(canvas, x + 33, y + 37, AlignCenter, AlignCenter,
                                    progress_string);
...
bool infrared_progress_view_set_progress(InfraredProgressView *instance, uint16_t progress)
{
    bool result;
    with_view_model(
        instance->view, InfraredProgressViewModel * model,
        {
...
void infrared_progress_view_set_progress_total(InfraredProgressView *progress,
        ...
        instance->view, InfraredProgressViewModel * model,
        {
            if (model->is_paused) {
                if (event->key == InputKeyLeft)
                    instance->input_callback(instance->context,
