...
static void infrared_move_view_draw_callback(Canvas *canvas, void *_model)
{
    ...
    elements_multiline_text_aligned(canvas, canvas_width(canvas) / 2, 0, AlignCenter, AlignTop,
                                    "Select a Button to Move");
    ...
    for (uint32_t i = 0; i < MIN(btn_number, LIST_ITEMS); i++) {
        ...
        canvas_draw_str_aligned(canvas, x_offset + 3, y_offset + 3, AlignLeft, AlignTop,
                                *InfraredMoveViewItemArray_cget(model->labels, idx));
    }

    if (show_scrollbar) {
        elements_scrollbar_pos(canvas, canvas_width(canvas), HEADER_H,
                               canvas_height(canvas) - HEADER_H, model->current_idx, btn_number);
    }
}
...
static bool infrared_move_view_input_callback(InputEvent *event, void *context)
{
    ...
    if ((event->type == InputTypeShort || event->type == InputTypeRepeat) &&
        ...
        with_view_model(
            move_view->view, InfraredMoveViewModel * model,
            {
                const size_t btn_number = InfraredMoveViewItemArray_size(model->labels);
                ...
                if (model->is_moving) {
                    InfraredMoveViewItemArray_swap_at(model->labels, item_idx_prev,
                                                      model->current_idx);
                }
        ...
        with_view_model(
            move_view->view, InfraredMoveViewModel * model,
            {
                if (!model->is_moving) {
                    ...
                    move_view->callback(model->start_idx, model->current_idx,
                                        move_view->callback_context);
                }
...
void infrared_move_view_add_item(InfraredMoveView *move_view, const char *label)
{
    with_view_model(
        move_view->view, InfraredMoveViewModel * model,
        { InfraredMoveViewItemArray_push_back(model->labels, label); }, true);
}
...
InfraredMoveView *infrared_move_view_alloc(void)
{
    ...
    with_view_model(
        move_view->view, InfraredMoveViewModel * model,
        { InfraredMoveViewItemArray_init(model->labels); }, true);
...
void infrared_move_view_free(InfraredMoveView *move_view)
{
    with_view_model(
        move_view->view, InfraredMoveViewModel * model,
        { InfraredMoveViewItemArray_clear(model->labels); }, true);
