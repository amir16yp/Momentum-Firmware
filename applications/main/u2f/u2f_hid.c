...
{
    FURI_LOG_D(WORKER_TAG, "Req cid=%lX cmd=%x len=%u", u2f_hid->packet.cid, u2f_hid->packet.cmd,
               u2f_hid->packet.len);
...
static int32_t u2f_hid_worker(void *context)
{
    ...
    while (1) {
        uint32_t flags = furi_thread_flags_wait(WorkerEvtStop | WorkerEvtConnect |
                                                    WorkerEvtDisconnect | WorkerEvtRequest,
                ...
                if ((packet_buf[4] & U2F_HID_TYPE_MASK) == U2F_HID_TYPE_INIT) {
                    ...
                    if (u2f_hid->req_len_left > 0) {
                        ...
                        if ((cid_temp == u2f_hid->packet.cid) &&
                            ...
                            memcpy(&(u2f_hid->packet.payload[u2f_hid->req_buf_ptr]), &packet_buf[5],
                                   len_cur);
                            u2f_hid->req_buf_ptr += len_cur;
                            u2f_hid->seq_id_last++;
                        }
