...
static void u2f_ecc_sign(mbedtls_ecp_group *grp, const uint8_t *key, uint8_t *hash,
                         uint8_t *signature)
{
    mbedtls_mpi r, s, d;
...
static void u2f_ecc_compute_public_key(mbedtls_ecp_group *grp, const uint8_t *private_key,
                                       U2fPubKey *public_key)
{
    ...
    MCHECK(mbedtls_ecp_point_write_binary(grp, &Q, MBEDTLS_ECP_PF_UNCOMPRESSED, &olen,
                                          (unsigned char *)public_key, sizeof(U2fPubKey)));
...
static uint16_t u2f_register(U2fData *U2F, uint8_t *buf)
{
    ...
    if (u2f_data_check(false) == false) {
        ...
        if (U2F->callback != NULL)
            U2F->callback(U2fNotifyError, U2F->context);
        memcpy(&buf[0], state_not_supported, 2);
        return 2;
    }

    if (U2F->callback != NULL)
        U2F->callback(U2fNotifyRegister, U2F->context);
...
static uint16_t u2f_authenticate(U2fData *U2F, uint8_t *buf)
{
    ...
    if (U2F->callback != NULL)
        U2F->callback(U2fNotifyAuth, U2F->context);
...

    {
        ...
        // Recover private key
        MCHECK(mbedtls_md_hmac_update(&hmac_ctx, req->app_id, sizeof(req->app_id)));
        MCHECK(mbedtls_md_hmac_update(&hmac_ctx, req->key_handle.nonce,
                                      sizeof(req->key_handle.nonce)));
    ...
    if (U2F->callback != NULL)
        U2F->callback(U2fNotifyAuthSuccess, U2F->context);
