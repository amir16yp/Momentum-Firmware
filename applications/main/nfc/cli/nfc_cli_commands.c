...
static const NfcCliCommandDescriptor *nfc_cli_commands[] = {
    &apdu_cmd, &raw_cmd, &emulate_cmd, &mfu_cmd, &scanner_cmd, &dump_cmd, &field_cmd,
};
...
const NfcCliActionDescriptor *nfc_cli_command_get_action_by_name(const NfcCliCommandDescriptor *cmd,
                                                                 const FuriString *name)
{
    ...
    for (size_t i = 0; i < cmd->action_count; i++) {
        ...
        if (furi_string_equal_str(name, action->name))
            return action;
    }
    return NULL;
}
...
static inline const NfcCliKeyDescriptor *
nfc_cli_action_get_key_by_name(const NfcCliActionDescriptor *action, const FuriString *name,
                               bool long_name)
{
    for (size_t i = 0; i < action->key_count; i++) {
        ...
        if ((buf != NULL) && furi_string_equal_str(name, buf))
            return key;
    }
    return NULL;
}

const NfcCliKeyDescriptor *nfc_cli_action_get_key_descriptor(const NfcCliActionDescriptor *action,
                                                             FuriString *argument)
{
    furi_assert(action);
...
void nfc_cli_action_format_info(const NfcCliActionDescriptor *action, FuriString *output)
{
    furi_assert(action);
    furi_assert(output);
    furi_string_printf(output, action->description ? "%s - %s\r\n\n" : "%s\r\n\n", action->name,
                       action->description);
    ...
    furi_string_cat_printf(output, ANSI_FG_BR_GREEN "Keys " ANSI_FLIPPER_BRAND_ORANGE
                                                    "(required) " ANSI_RESET "(optional):\r\n");
    ...
    for (size_t i = 0; i < action->key_count; i++) {
        ...
        if (key->description) {
            const int offset = 20;
            furi_string_cat_printf(output, ANSI_CURSOR_RIGHT_BY("%d") "%s", offset - len,
                                   key->description);
        }
...
void nfc_cli_command_format_info(const NfcCliCommandDescriptor *cmd, FuriString *output)
{
    ...
    if (cmd->action_count > 1) {
        ...
        for (size_t i = 0; i < cmd->action_count; i++) {
            const NfcCliActionDescriptor *action = cmd->actions[i];
            furi_string_cat_printf(output, action->description ? "\t%s\t-\t%s\r\n" : "%s\r\n",
