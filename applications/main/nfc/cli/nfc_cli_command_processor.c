 ...
 */
typedef enum {
    NfcCliProcessorErrorNone, /**< Command was parsed successfully and execute callback will be
                                 invoked*/
    NfcCliProcessorErrorNoneButHelp, /**< There was no error, but help needs to be printed. Command
                                        wil not be executed */
    NfcCliProcessorErrorActionNotFound,  /**< Wrong action was passed as first command parameter */
    NfcCliProcessorErrorKeyNotSupported, /**< Unsupported key was passed in arguments. Details will
                                            be printed in erro_message*/
    NfcCliProcessorErrorKeyParameterInGroup,      /**< Parameter which requires value was passed in
                                                     group. Example: -sckd */
    NfcCliProcessorErrorKeyParameterValueMissing, /**< Value is missing for the parameter which
                                                     requires it */
    ...
    NfcCliProcessorErrorKeyRequiredMissing, /**< Some keys required for command execution is
                                               missing*/
...
static bool nfc_cli_action_can_reuse_context(NfcCliProcessorContext *instance,
                                             const NfcCliActionDescriptor *new_action)
{
    bool result = false;
    do {
        if (instance->action != new_action)
            break;
        if (new_action->can_reuse == NULL)
            break;
...
static NfcCliProcessorError nfc_cli_action_alloc(NfcCliProcessorContext *instance, FuriString *args)
{
    ...
    do {
        ...
        if (!nfc_cli_action_can_reuse_context(instance, action)) {
            ...
            if (action->alloc && action->free) {
                ...
                FURI_LOG_W(TAG,
                           "Free callback not defined for action \"%s\". Skip allocation to avoid "
                           "memory leak.",
                           action->name);
...
static NfcCliArgumentType nfc_cli_get_argument_type(FuriString *argument)
{
    ...
    if (arg_len > 2) {
        ...
        if (ch1 == '-') {
            type =
                (ch2 == '-') ? NfcCliArgumentTypeLongNameKey : NfcCliArgumentTypeShortNameKeyGroup;
        }
    } else if (arg_len == 2) {
        char ch1 = furi_string_get_char(argument, 0);
...
static bool nfc_cli_check_duplicate_keys(NfcCliProcessorContext *instance,
...
static NfcCliProcessorError nfc_cli_parse_single_key(NfcCliProcessorContext *instance,
                                                     FuriString *argument, FuriString *args,
                                                     bool from_group)
{
    ...
    do {
        ...
        if (key == NULL) {
            if (furi_string_equal_str(argument, "h"))
                ...
                furi_string_printf(instance->error_message, "Key \'%s\' is not supported",
        ...
        if (key->features.parameter && from_group) {
            furi_string_printf(instance->error_message, "Parameter key \'%s\' can\'t be grouped",
                               furi_string_get_cstr(argument));
        ...
        if (nfc_cli_check_duplicate_keys(instance, key)) {
            furi_string_printf(instance->error_message, "Duplicated key \'%s\'",
                               furi_string_get_cstr(argument));
            result = NfcCliProcessorErrorKeyDuplication;
            break;
        }

        if (key->features.multivalue && !key->features.parameter)
            break;
        if (key->features.multivalue) {
            ...
            furi_string_printf(instance->error_message, "Missing value for \'%s\'",
                               furi_string_get_cstr(argument));
            break;
        }

        if (key->parse == NULL) {
            furi_string_printf(instance->error_message, "Parse callback for key \'%s\' not defined",
                               furi_string_get_cstr(argument));
        ...
        if (!key->parse(value_str, instance->action_context)) {
            furi_string_printf(instance->error_message,
                               "Unable to parse value \'%s\' for key \'%s\'",
                               furi_string_get_cstr(value_str), furi_string_get_cstr(argument));
            result = NfcCliProcessorErrorKeyParseError;
            break;
        }
        ...
        if (key->features.required)
            instance->required_keys_found++;
    } while (false);
...
static NfcCliProcessorError nfc_cli_parse_group_key(NfcCliProcessorContext *instance,
                                                    FuriString *argument)
{
    NfcCliProcessorError result = NfcCliProcessorErrorNone;
...
static NfcCliProcessorError nfc_cli_parse_argument(NfcCliProcessorContext *instance,
                                                   FuriString *argument, FuriString *args)
{
...
static NfcCliProcessorError nfc_cli_process_arguments(NfcCliProcessorContext *instance,
                                                      FuriString *args)
{
    NfcCliProcessorError result = NfcCliProcessorErrorNone;
...
static inline void nfc_cli_command_process_error(const NfcCliProcessorContext *instance,
                                                 NfcCliProcessorError error)
{
    do {
        if (error == NfcCliProcessorErrorNone)
            break;
...
void nfc_cli_command_processor_run(const NfcCliCommandDescriptor *cmd, PipeSide *pipe,
                                   FuriString *args, void *context)
{
    furi_assert(pipe);
    furi_assert(cmd);
    furi_assert(args);
