...
static void iso14443_3b_format_activation_data(const Iso14443_3bData *data, FuriString *output)
{
    ...
    furi_string_cat_printf(
        output, " BitRate: %d, Protocol: %d, Max Frame Size: %d, Fo: %d, Adc: %d, Fwi: %d",
        info->bit_rate_capability, info->protocol_type, info->max_frame_size, info->fo, info->adc,
        info->fwi);
}

static inline NfcCliRawError nfc_cli_raw_iso14443_3b_activate(NfcGenericInstance *poller,
...
static inline NfcCliRawError nfc_cli_raw_iso14443_3b_txrx(NfcGenericInstance *poller,
                                                          BitBuffer *tx_buffer,
                                                          BitBuffer *rx_buffer, uint32_t timeout)
{
    FURI_LOG_D(TAG, "TxRx");
...
NfcCommand nfc_cli_raw_iso14443_3b_handler(NfcGenericInstance *poller,
                                           ...
                                           NfcCliRawResponse *const response)
{
    ...
    do {
        ...
        if (response->result != NfcCliRawErrorNone)
            break;
        if (BIT_BUFFER_EMPTY(request->tx_buffer))
            break;
