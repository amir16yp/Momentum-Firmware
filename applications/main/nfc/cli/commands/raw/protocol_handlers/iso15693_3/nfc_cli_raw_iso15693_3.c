...
static inline NfcCliRawError nfc_cli_raw_iso15693_3_activate(NfcGenericInstance *poller,
...
static inline NfcCliRawError nfc_cli_raw_iso15693_3_txrx(NfcGenericInstance *poller,
                                                         BitBuffer *tx_buffer, BitBuffer *rx_buffer,
                                                         uint32_t timeout)
{
    FURI_LOG_D(TAG, "TxRx");
...
NfcCommand nfc_cli_raw_iso15693_3_handler(NfcGenericInstance *poller,
                                          const NfcCliRawRequest *request,
                                          NfcCliRawResponse *const response)
{
    do {
        if (request->select) {
            response->result = nfc_cli_raw_iso15693_3_activate(poller, response->activation_string);
        }
