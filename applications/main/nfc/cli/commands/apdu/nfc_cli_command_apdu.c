...
typedef NfcCommand (*NfcCliApduProtocolHandler)(NfcGenericEvent event,
...
ARRAY_DEF(NfcCliApduItemArray, NfcCliApduData,
          (INIT(API_2(ApduItem_init)), SET(API_6(ApduItem_set)), INIT_SET(API_6(ApduItem_init_set)),
...
{
    nfc_cli_printf_array(bit_buffer_get_data(instance->data.tx_buffer),
    ...
    if (rx_size > 0) {
        nfc_cli_printf_array(bit_buffer_get_data(instance->data.rx_buffer),
...
static NfcCommand nfc_cli_apdu_poller_callback(NfcGenericEvent event, void *context)
{
    ...
    if (request_type == NfcCliProtocolRequestTypeAbort) {
        ...
        if (handler)
            command = handler(event, &instance->data);
    }
