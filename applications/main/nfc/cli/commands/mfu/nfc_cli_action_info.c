...
const char *nfc_cli_mfu_capability_container_get_access_description(const uint8_t value, bool read)
{
...
static void nfc_cli_mfu_info_print_ndef(const MfUltralightData *data)
{
    ...
    if (cc->magic == 0xE1) {
        ...
        printf("\r\nMagic number: %02X\r\nVersion %d.%d\r\nSize: [%02X] - %d bytes\r\n", cc->magic,
               cc->version.major, cc->version.minor, cc->size, cc->size * 8);
        printf("Access read: [%02X] - %s", cc->access.read,
               nfc_cli_mfu_capability_container_get_access_description(cc->access.read, true));
        printf("Access write: [%02X] - %s", cc->access.write,
...
static void nfc_cli_mfu_info_print_counter(const MfUltralightData *data)
{
    ...
    if (!mf_ultralight_support_feature(features, MfUltralightFeatureSupportReadCounter))
        return;
...
{
    uint32_t features = mf_ultralight_get_feature_support_set(data->type);
    if (!mf_ultralight_support_feature(features, MfUltralightFeatureSupportReadSignature))
        return;
...
{
    uint32_t features = mf_ultralight_get_feature_support_set(data->type);
    if (!mf_ultralight_support_feature(features, MfUltralightFeatureSupportReadVersion))
        return;
