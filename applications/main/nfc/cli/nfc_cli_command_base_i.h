 ...
 * 2. In nfc_cli_alloc function register command using cli_registry_add_command after
 * nfc_cli_subscribe_commands
 *
 ...
 */
typedef struct {
    bool required : 1;   /**< Command always needs this argument. Missing arguments with this set to
                            true will result execution error.*/
    bool parameter : 1;  /**< Such argument requires value after its name, otherwise it is a simple
                            on/off switch */
    bool multivalue : 1; /**< Such argument can take multiple values after its name, like this "-key
...
#define ADD_NFC_CLI_COMMAND(name, description, actions)                                            \
    static void nfc_cli_command_##name##_callback(PipeSide *pipe, FuriString *args,                \
                                                  void *context);                                  \
                                                                                                   \
    const NfcCliCommandDescriptor name##_cmd = {                                                   \
        #name, #description, COUNT_OF(actions), actions, nfc_cli_command_##name##_callback,        \
    };                                                                                             \
                                                                                                   \
    static void nfc_cli_command_##name##_callback(PipeSide *pipe, FuriString *args, void *context) \
    {                                                                                              \
