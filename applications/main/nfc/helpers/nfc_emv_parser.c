...
static bool nfc_emv_parser_search_data(Storage *storage, const char *file_name, FuriString *key,
                                       FuriString *data)
{
    bool parsed = false;
    ...
    do {
        ...
        if (furi_string_cmp_str(temp_str, nfc_resources_header) ||
            ...
            break;
        if (!flipper_format_read_string(file, furi_string_get_cstr(key), data))
            break;
        parsed = true;
    } while (false);
                                     ...
                                     FuriString *country_name)
{
    ...
    if (nfc_emv_parser_search_data(storage, EXT_PATH("nfc/assets/country_code.nfc"), key,
                                   country_name)) {
        parsed = true;
    }
    furi_string_free(key);
                                      ...
                                      FuriString *currency_name)
{
    ...
    if (nfc_emv_parser_search_data(storage, EXT_PATH("nfc/assets/currency_code.nfc"), key,
                                   currency_name)) {
        parsed = true;
    }
    furi_string_free(key);
