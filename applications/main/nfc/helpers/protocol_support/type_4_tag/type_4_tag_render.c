...
void nfc_render_type_4_tag_info(const Type4TagData *data, NfcProtocolFormatType format_type,
                                FuriString *str)
{
    ...
    if (data->is_tag_specific) {
        ...
        furi_string_cat_printf(str, "Card: %s\n",
                               furi_string_empty(data->platform_name)
                                   ? "unknown"
        ...
        furi_string_cat_printf(str, "Read Lock: %02X%s\n", data->ndef_read_lock,
                               data->ndef_read_lock == 0 ? " (unlocked)" : "");
        furi_string_cat_printf(str, "Write Lock: %02X%s", data->ndef_write_lock,
