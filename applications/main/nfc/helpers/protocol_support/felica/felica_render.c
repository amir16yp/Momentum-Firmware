...
void nfc_render_felica_blocks_count(const FelicaData *data, FuriString *str,
...
void nfc_render_felica_idm(const FelicaData *data, NfcProtocolFormatType format_type,
                           FuriString *str)
{
    furi_string_cat_printf(str, (format_type == NfcProtocolFormatTypeFull) ? "IDm:\n" : "IDm:");

    for (size_t i = 0; i < FELICA_IDM_SIZE; i++) {
        furi_string_cat_printf(str, (format_type == NfcProtocolFormatTypeFull) ? "%02X " : " %02X",
                               data->idm.data[i]);
    }
}

void nfc_render_felica_info(const FelicaData *data, NfcProtocolFormatType format_type,
                            FuriString *str)
{
...
static void nfc_render_felica_block_name(const char *name, FuriString *str,
                                         uint8_t prefix_separator_cnt, uint8_t suffix_separator_cnt)
{
...
static void nfc_render_felica_block(const FelicaBlock *block, FuriString *str, const char *name,
                                    uint8_t prefix_separator_cnt, uint8_t suffix_separator_cnt)
{
...
void nfc_more_info_render_felica_lite_dump(const FelicaData *data, FuriString *str)
{
    ...
    for (size_t i = 0; i < 14; i++) {
        ...
        nfc_render_felica_block(&data->data.fs.spad[i], str, furi_string_get_cstr(name), 20,
                                suf_cnt);
    }
...
void nfc_more_info_render_felica_dir(const FelicaSystem *system, FuriString *str)
{
    ...
    if (area_count == 0 || service_count == 0) {
        ...
        furi_string_cat_printf(str, "%zu areas found.\n%zu services found.\n\n", area_count,
                               service_count);
        furi_string_cat_printf(str, "::: ... are readable services\n||| ... are locked services\n");
    }
    felica_write_directory_tree(system, str);
}

void nfc_more_info_render_felica_blocks(const FelicaData *data, const FelicaSystem *system,
                                        FuriString *str, const uint16_t service_code_key)
{
    furi_string_cat_printf(str, "\n");
    if (data->workflow_type == FelicaLite) {
        furi_string_cat_printf(str, "Blocks: %u\n", data->blocks_total);
