...
static NfcCommand nfc_scene_read_poller_callback_mf_classic(NfcGenericEvent event, void *context)
{
    ...
    if (mfc_event->type == MfClassicPollerEventTypeRequestMode) {
        nfc_device_set_data(instance->nfc_device, NfcProtocolMfClassic,
        ...
        if (mf_classic_key_cache_get_next_key(instance->mfc_key_cache, &sector_num, &key,
                                              &key_type)) {
...
static void nfc_scene_read_menu_on_enter_mf_classic(NfcApp *instance)
{
    ...
    if (!mf_classic_is_card_read(data)) {
        submenu_add_item(submenu, "Extract MFC Keys", SubmenuIndexDetectReader,
        ...
        submenu_add_item(submenu, "Crack nonces in MFKey32", SubmenuIndexCrackNonces,
                         nfc_protocol_support_common_submenu_callback, instance);
    }

    submenu_add_item(submenu, "Show Keys", SubmenuIndexShowKeys,
                     nfc_protocol_support_common_submenu_callback, instance);
}
...
static void nfc_scene_saved_menu_on_enter_mf_classic(NfcApp *instance)
{
    ...
    submenu_add_item(submenu, "Update from Initial Card", SubmenuIndexUpdate,
                     nfc_protocol_support_common_submenu_callback, instance);
...
static bool nfc_scene_read_menu_on_event_mf_classic(NfcApp *instance, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == SubmenuIndexDetectReader) {
            scene_manager_set_scene_state(instance->scene_manager, NfcSceneSaveConfirm,
            ...
            if (!scene_manager_search_and_switch_to_previous_scene(instance->scene_manager,
...
static bool nfc_scene_save_name_on_event_mf_classic(NfcApp *instance, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom && event.event == NfcCustomEventTextInputDone) {
        mf_classic_key_cache_save(instance->mfc_key_cache,
                                  nfc_device_get_data(instance->nfc_device, NfcProtocolMfClassic));
        consumed = true;
    }
...
static NfcCommand nfc_scene_write_poller_callback_mf_classic(NfcGenericEvent event, void *context)
{
    furi_assert(event.protocol == NfcProtocolMfClassic);
