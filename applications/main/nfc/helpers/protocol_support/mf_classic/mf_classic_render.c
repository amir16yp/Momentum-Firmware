...
void nfc_render_mf_classic_info(const MfClassicData *data, NfcProtocolFormatType format_type,
                                FuriString *str)
{
...
static void mf_classic_render_raw_data(const uint8_t *data, size_t size, bool data_read,
                                       FuriString *str)
{
...
static void mf_classic_render_block(const MfClassicData *data, uint8_t block_num, FuriString *str)
{
    if (mf_classic_is_sector_trailer(block_num)) {
        ...
        mf_classic_render_raw_data(sec_tr->access_bits.data, sizeof(MfClassicAccessBits),
                                   access_bits_read, str);
