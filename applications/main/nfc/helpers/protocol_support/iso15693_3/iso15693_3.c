...
static NfcCommand nfc_scene_read_poller_callback_iso15693_3(NfcGenericEvent event, void *context)
{
    ...
    if (iso15693_3_event->type == Iso15693_3PollerEventTypeReady) {
        nfc_device_set_data(instance->nfc_device, NfcProtocolIso15693_3,
...
static NfcCommand nfc_scene_emulate_listener_callback_iso15693_3(NfcGenericEvent event,
                                                                 void *context)
{
    furi_assert(context);
    furi_assert(event.protocol == NfcProtocolIso15693_3);
...
static void nfc_scene_emulate_on_enter_iso15693_3(NfcApp *instance)
{
    ...
    nfc_listener_start(instance->listener, nfc_scene_emulate_listener_callback_iso15693_3,
                       instance);
}

static NfcCommand nfc_scene_write_poller_callback_iso15693_3(NfcGenericEvent event, void *context)
{
    furi_assert(event.protocol == NfcProtocolIso15693_3);
    ...
    if (iso15693_3_event->type == Iso15693_3PollerEventTypeReady) {
        ...
        if (card_data->system_info.block_count != write_data->system_info.block_count ||
            ...
            for (uint16_t i = 0; i < write_data->system_info.block_count; i++) {
                if (iso15693_3_is_block_locked(write_data, i))
                    continue;
                // Skip blocks that already contain the correct data
                if (memcmp(write_block_data + i * block_size, card_block_data + i * block_size,
                           block_size) == 0)
                    continue;
                error = iso15693_3_poller_write_block(poller, write_block_data + i * block_size, i,
                                                      block_size);
