...
void nfc_protocol_support_alloc(NfcProtocol protocol, void *context)
{
    ...
    protocol_support->plugin_manager = plugin_manager_alloc(
        NFC_PROTOCOL_SUPPORT_PLUGIN_APP_ID, NFC_PROTOCOL_SUPPORT_PLUGIN_API_VERSION,
        composite_api_resolver_get(instance->api_resolver));
    do {
        if (plugin_manager_load_single(protocol_support->plugin_manager,
                                       furi_string_get_cstr(plugin_path)) !=
...
static const NfcProtocolSupportBase *nfc_protocol_support_get(NfcProtocol protocol, void *context)
{
    furi_assert(context);
...
static void nfc_protocol_support_scene_info_on_enter(NfcApp *instance)
{
    ...
    if (nfc_protocol_support_has_feature(protocol, instance, NfcProtocolFeatureMoreInfo)) {
        widget_add_button_element(instance->widget, GuiButtonTypeRight, "More",
                                  nfc_protocol_support_common_widget_callback, instance);
    }
...
static bool nfc_protocol_support_scene_info_on_event(NfcApp *instance, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        if (!scene_manager_get_scene_state(instance->scene_manager, NfcSceneSupportedCard)) {
            ...
            scene_manager_search_and_switch_to_previous_scene_one_of(instance->scene_manager,
                                                                     scenes, COUNT_OF(scenes));
            consumed = true;
        }
    }
...
static bool nfc_protocol_support_scene_more_info_on_event(NfcApp *instance, SceneManagerEvent event)
{
    bool consumed = false;
...
static bool nfc_protocol_support_scene_read_on_event(NfcApp *instance, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == NfcCustomEventPollerSuccess) {
            ...
            nfc_poller_stop(instance->poller);
            nfc_poller_free(instance->poller);
            bool card_read = nfc_supported_cards_read(instance->nfc_supported_cards,
                                                      instance->nfc_device, instance->nfc);
            if (card_read) {
                notification_message(instance->notifications, &sequence_success);
                scene_manager_next_scene(instance->scene_manager, NfcSceneReadSuccess);
                dolphin_deed(DolphinDeedNfcReadSuccess);
...
static void nfc_protocol_support_scene_read_menu_on_enter(NfcApp *instance)
{
    ...
    submenu_add_item(submenu, "Save", SubmenuIndexCommonSave,
    ...
    if (scene_manager_has_previous_scene(instance->scene_manager, NfcSceneGenerateInfo)) {
        submenu_add_item(submenu, "Change UID", SubmenuIndexCommonEdit,
                         nfc_protocol_support_common_submenu_callback, instance);
    }

    if (nfc_protocol_support_has_feature(protocol, instance, NfcProtocolFeatureEmulateUid)) {
        submenu_add_item(submenu, "Emulate UID", SubmenuIndexCommonEmulate,
                         nfc_protocol_support_common_submenu_callback, instance);

    } else if (nfc_protocol_support_has_feature(protocol, instance,
                                                NfcProtocolFeatureEmulateFull)) {
        submenu_add_item(submenu, "Emulate", SubmenuIndexCommonEmulate,
                         nfc_protocol_support_common_submenu_callback, instance);
    }

    if (nfc_protocol_support_has_feature(protocol, instance, NfcProtocolFeatureWrite)) {
        submenu_add_item(submenu, "Write", SubmenuIndexCommonWrite,
                         nfc_protocol_support_common_submenu_callback, instance);
    }

    nfc_protocol_support_get(protocol, instance)->scene_read_menu.on_enter(instance);

    submenu_add_item(submenu, "Info", SubmenuIndexCommonInfo,
                     nfc_protocol_support_common_submenu_callback, instance);

    submenu_set_selected_item(instance->submenu, scene_manager_get_scene_state(
                                                     instance->scene_manager, NfcSceneReadMenu));

    view_dispatcher_switch_to_view(instance->view_dispatcher, NfcViewMenu);
}

static bool nfc_protocol_support_scene_read_menu_on_event(NfcApp *instance, SceneManagerEvent event)
{
    bool consumed = false;
...
static void nfc_protocol_support_scene_read_success_on_enter(NfcApp *instance)
{
    ...
    widget_add_button_element(widget, GuiButtonTypeLeft, "Retry",
                              nfc_protocol_support_common_widget_callback, instance);
    widget_add_button_element(widget, GuiButtonTypeRight, "More",
                              nfc_protocol_support_common_widget_callback, instance);
...
static bool nfc_protocol_support_scene_read_success_on_event(NfcApp *instance,
                                                             SceneManagerEvent event)
{
    bool consumed = false;
...
static void nfc_protocol_support_scene_saved_menu_on_enter(NfcApp *instance)
{
    ...
    if (nfc_protocol_support_has_feature(protocol, instance, NfcProtocolFeatureEditUid)) {
        submenu_add_item(submenu, "Edit UID", SubmenuIndexCommonEdit,
                         nfc_protocol_support_common_submenu_callback, instance);
    }
    ...
    if (nfc_has_shadow_file(instance)) {
        submenu_add_item(submenu, "Restore to Original State", SubmenuIndexCommonRestore,
                         nfc_protocol_support_common_submenu_callback, instance);
    }

    submenu_add_item(submenu, "Rename", SubmenuIndexCommonRename,
                     nfc_protocol_support_common_submenu_callback, instance);
    submenu_add_item(submenu, "Delete", SubmenuIndexCommonDelete,
                     nfc_protocol_support_common_submenu_callback, instance);
    submenu_add_item(submenu, "Info", SubmenuIndexCommonInfo,
                     nfc_protocol_support_common_submenu_callback, instance);
...
static bool nfc_protocol_support_scene_saved_menu_on_event(NfcApp *instance,
                                                           SceneManagerEvent event)
{
    bool consumed = false;
...
static void nfc_protocol_support_scene_save_name_on_enter(NfcApp *instance)
{
    ...
    if (name_is_empty) {
        ...
        furi_string_replace_all(prefix, "/", "_");
        name_generator_make_auto(instance->text_store, NFC_TEXT_STORE_SIZE,
    ...
    text_input_set_result_callback(text_input, nfc_protocol_support_common_text_input_done_callback,
                                   instance, instance->text_store, NFC_NAME_SIZE, name_is_empty);

    ValidatorIsFile *validator_is_file =
        validator_is_file_alloc_init(furi_string_get_cstr(folder_path), NFC_APP_EXTENSION,
...
static bool nfc_protocol_support_scene_save_name_on_event(NfcApp *instance, SceneManagerEvent event)
{
    bool consumed = false;
...
static void nfc_protocol_support_scene_emulate_on_enter(NfcApp *instance)
{
    ...
    if (nfc_protocol_support_has_feature(protocol, instance, NfcProtocolFeatureEmulateUid)) {
        widget_add_string_element(widget, 90, 26, AlignCenter, AlignCenter, FontPrimary,
                                  "Emulating UID");
        ...
        if (!furi_string_empty(instance->file_name)) {
            ...
            furi_string_printf(temp_str, "Unsaved\n%s",
                               nfc_device_get_name(instance->nfc_device, NfcDeviceNameTypeFull));
            furi_string_replace_str(temp_str, "Mifare", "MIFARE");
        }
    }

    widget_add_text_box_element(widget, 50, 33, 78, 31, AlignCenter, AlignTop,
                                furi_string_get_cstr(temp_str), false);
    ...
    scene_manager_set_scene_state(instance->scene_manager, NfcSceneEmulate,
...
static bool nfc_protocol_support_scene_emulate_on_event(NfcApp *instance, SceneManagerEvent event)
{
    bool consumed = false;
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == NfcCustomEventListenerUpdate) {
            ...
            if (furi_string_size(instance->text_box_store)) {
                widget_add_button_element(instance->widget, GuiButtonTypeCenter, "Log",
                                          nfc_protocol_support_common_widget_callback, instance);
                scene_manager_set_scene_state(instance->scene_manager, NfcSceneEmulate,
 ...
 */
enum {
    NfcSceneWriteStateSearching, /**< Ask user to touch the card. Event: on_enter, CardLost. Needs
                                    caption. */
...
void nfc_protocol_support_scene_write_widget_callback(GuiButtonType result, InputType type,
                                                      void *context)
{
    NfcApp *instance = context;
...
static void nfc_protocol_support_scene_write_setup_view(NfcApp *instance)
{
    ...
    if (state == NfcSceneWriteStateSearching) {
        ...
        popup_set_text(popup, furi_string_get_cstr(instance->text_box_store), 95, 38, AlignCenter,
                       AlignCenter);
        ...
        widget_add_string_element(widget, 7, 4, AlignLeft, AlignTop, FontPrimary,
                                  "Writing gone wrong!");
        widget_add_string_multiline_element(widget, 7, 17, AlignLeft, AlignTop, FontSecondary,
                                            furi_string_get_cstr(instance->text_box_store));
        widget_add_icon_element(widget, 83, 22, &I_WarningDolphinFlip_45x42);
        widget_add_button_element(widget, GuiButtonTypeLeft, "Retry",
                                  nfc_protocol_support_scene_write_widget_callback, instance);
    } else if (state == NfcSceneWriteStateWrongCard) {
        view = NfcViewWidget;
        widget_add_string_element(widget, 3, 4, AlignLeft, AlignTop, FontPrimary, "Wrong card!");
        widget_add_string_multiline_element(widget, 4, 17, AlignLeft, AlignTop, FontSecondary,
                                            furi_string_get_cstr(instance->text_box_store));
        widget_add_icon_element(widget, 83, 22, &I_WarningDolphinFlip_45x42);
        widget_add_button_element(widget, GuiButtonTypeLeft, "Retry",
                                  nfc_protocol_support_scene_write_widget_callback, instance);
...
{
    scene_manager_set_scene_state(instance->scene_manager, NfcSceneWrite,
        ...
        if (event.event == NfcCustomEventRpcLoadFile) {
            ...
            if (instance->rpc_state == NfcRpcStateIdle) {
                if (nfc_load_file(instance, instance->file_path, false)) {
                    ...
                    rpc_system_app_set_error_code(instance->rpc_ctx,
