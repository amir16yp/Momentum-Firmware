...
static void nfc_scene_more_info_on_enter_mf_ultralight(NfcApp *instance)
{
    ...
    uint32_t scene_state = scene_manager_get_scene_state(instance->scene_manager, NfcSceneMoreInfo);

    if (scene_state == NfcSceneMoreInfoStateASCII) {
        pretty_format_bytes_hex_canonical(instance->text_box_store, MF_ULTRALIGHT_PAGE_SIZE,
                                          PRETTY_FORMAT_FONT_MONOSPACE, (uint8_t *)mfu->page,
                                          mfu->pages_read * MF_ULTRALIGHT_PAGE_SIZE);

        widget_add_text_scroll_element(instance->widget, 0, 0, 128, 48,
                                       furi_string_get_cstr(instance->text_box_store));
        widget_add_button_element(instance->widget, GuiButtonTypeRight, "Raw Data",
                                  nfc_protocol_support_common_widget_callback, instance);

        widget_add_button_element(instance->widget, GuiButtonTypeLeft, "Info",
                                  nfc_protocol_support_common_widget_callback, instance);
        ...
        widget_add_button_element(instance->widget, GuiButtonTypeLeft, "ASCII",
                                  nfc_protocol_support_common_widget_callback, instance);
    }
}
...
static bool nfc_scene_more_info_on_event_mf_ultralight(NfcApp *instance, SceneManagerEvent event)
{
    ...
    if ((event.type == SceneManagerEventTypeCustom && event.event == GuiButtonTypeLeft) ||
        ...
        scene_manager_set_scene_state(instance->scene_manager, NfcSceneMoreInfo,
                                      NfcSceneMoreInfoStateASCII);
...
static NfcCommand nfc_scene_read_poller_callback_mf_ultralight(NfcGenericEvent event, void *context)
{
    ...
    if (mf_ultralight_event->type == MfUltralightPollerEventTypeReadSuccess) {
        nfc_device_set_data(instance->nfc_device, NfcProtocolMfUltralight,
        ...
        if (instance->mf_ul_auth->type == MfUltralightAuthTypeXiaomi) {
            if (mf_ultralight_generate_xiaomi_pass(instance->mf_ul_auth,
            ...
            if (mf_ultralight_generate_amiibo_pass(instance->mf_ul_auth,
                                                   data->iso14443_3a_data->uid,
                                                   data->iso14443_3a_data->uid_len)) {
                mf_ultralight_event->data->auth_context.skip_auth = false;
            }
...
static void nfc_scene_read_and_saved_menu_on_enter_mf_ultralight(NfcApp *instance)
{
    ...
    if (is_locked ||
        ...
        submenu_remove_item(submenu, SubmenuIndexCommonWrite);
        submenu_add_item(submenu, "Write (Keep Key)", SubmenuIndexWriteKeepKey,
                         nfc_protocol_support_common_submenu_callback, instance);
        submenu_add_item(submenu, "Write (Copy Key)", SubmenuIndexWriteCopyKey,
                         nfc_protocol_support_common_submenu_callback, instance);
    }

    if (is_locked) {
        submenu_add_item(submenu, "Unlock", SubmenuIndexUnlock,
                         nfc_protocol_support_common_submenu_callback, instance);
        if (data->type == MfUltralightTypeMfulC) {
            submenu_add_item(submenu, "Unlock with Dictionary", SubmenuIndexDictAttack,
                             nfc_protocol_support_common_submenu_callback, instance);
        }
    }
}
...
static bool nfc_scene_read_and_saved_menu_on_event_mf_ultralight(NfcApp *instance,
...
static NfcCommand nfc_scene_write_poller_callback_mf_ultralight(NfcGenericEvent event,
                                                                void *context)
{
    furi_assert(event.protocol == NfcProtocolMfUltralight);
    ...
    if (mf_ultralight_event->type == MfUltralightPollerEventTypeRequestMode) {
        ...
        if (!instance->mf_ultralight_c_dict_context.dict &&
            ...
            if (keys_dict_check_presence(NFC_APP_MF_ULTRALIGHT_C_DICT_USER_PATH)) {
                instance->mf_ultralight_c_dict_context.dict =
                    keys_dict_alloc(NFC_APP_MF_ULTRALIGHT_C_DICT_USER_PATH,
                                    KeysDictModeOpenExisting, sizeof(MfUltralightC3DesAuthKey));
                instance->mf_ultralight_c_write_context.dict_state = NfcMfUltralightCWriteDictUser;
            }
            if (!instance->mf_ultralight_c_dict_context.dict) {
                instance->mf_ultralight_c_dict_context.dict =
                    keys_dict_alloc(NFC_APP_MF_ULTRALIGHT_C_DICT_SYSTEM_PATH,
                                    KeysDictModeOpenExisting, sizeof(MfUltralightC3DesAuthKey));
        ...
        bool got_key = false;
        if (instance->mf_ultralight_c_dict_context.dict) {
            got_key = keys_dict_get_next_key(instance->mf_ultralight_c_dict_context.dict, key.data,
        ...
        if (!got_key &&
            ...
            instance->mf_ultralight_c_dict_context.dict =
                keys_dict_alloc(NFC_APP_MF_ULTRALIGHT_C_DICT_SYSTEM_PATH, KeysDictModeOpenExisting,
                                sizeof(MfUltralightC3DesAuthKey));
            instance->mf_ultralight_c_write_context.dict_state = NfcMfUltralightCWriteDictSystem;
            if (instance->mf_ultralight_c_dict_context.dict) {
                got_key = keys_dict_get_next_key(instance->mf_ultralight_c_dict_context.dict,
        ...
        if (got_key) {
            ...
            FURI_LOG_D("MfULC",
                       ...
                       key.data[0], key.data[1], key.data[2], key.data[3], key.data[4], key.data[5],
                       key.data[6], key.data[7], key.data[8], key.data[9], key.data[10],
            ...
            instance->mf_ultralight_c_write_context.dict_state = NfcMfUltralightCWriteDictExhausted;
        }
        ...
        if (mf_ultralight_event->data->key_request_data.key_provided) {
            ...
            FURI_LOG_D("MfULC",
                       ...
                       found_key.data[12], found_key.data[13], found_key.data[14],
                       found_key.data[15]);
        }
        FURI_LOG_D("MfULC", "WriteKeyRequest: decision = %s (copy_key=%d)",
