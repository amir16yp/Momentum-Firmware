...
static NfcCommand nfc_scene_read_poller_callback_slix(NfcGenericEvent event, void *context)
{
    ...
    if (slix_event->type == SlixPollerEventTypeReady) {
        nfc_device_set_data(instance->nfc_device, NfcProtocolSlix,
    ...
    if (slix_event->type == SlixPollerEventTypeReady) {
        ...
        if (card_iso_data->system_info.block_count != write_iso_data->system_info.block_count ||
            ...
            for (uint16_t i = 0; i < write_iso_data->system_info.block_count; i++) {
                if (iso15693_3_is_block_locked(write_iso_data, i))
                    continue;
                // Skip blocks that already contain the correct data
                if (memcmp(write_data_ptr + i * block_size, card_data_ptr + i * block_size,
                           block_size) == 0)
                    continue;
                error =
                    slix_poller_write_block(poller, write_data_ptr + i * block_size, i, block_size);
            ...
            if (error == SlixErrorNone) {
                ...
                furi_string_printf(instance->text_box_store, "Block %u: %s", failed_block,
                                   err_name);
                view_dispatcher_send_custom_event(instance->view_dispatcher,
                                                  NfcCustomEventPollerFailure);
            }
