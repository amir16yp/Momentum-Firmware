...
void nfc_scene_mf_desfire_more_info_on_enter(void *context)
{
    ...
    submenu_add_item(submenu, "Card info", SubmenuIndexCardInfo,
                     nfc_protocol_support_common_submenu_callback, nfc);
    ...
    for (uint32_t i = 0; i < simple_array_get_count(data->application_ids); ++i) {
        ...
        furi_string_printf(label, "App %02x%02x%02x", app_id->data[2], app_id->data[1],
                           app_id->data[0]);
        submenu_add_item(submenu, furi_string_get_cstr(label), i + SubmenuIndexDynamic,
                         nfc_protocol_support_common_submenu_callback, nfc);
    }
    ...
    if (state >= MifareDesfireMoreInfoStateItem) {
        submenu_set_selected_item(nfc->submenu,
                                  state - MifareDesfireMoreInfoStateItem + SubmenuIndexDynamic);
        scene_manager_set_scene_state(nfc->scene_manager, NfcSceneMfDesfireMoreInfo,
                                      MifareDesfireMoreInfoStateMenu);
    }
