...
static void nfc_scene_slix_unlock_success_widget_callback(GuiButtonType result, InputType type,
                                                          void *context)
{
...
void nfc_scene_slix_unlock_success_on_enter(void *context)
{
    ...
    widget_add_string_multiline_element(widget, 0, 12, AlignLeft, AlignTop, FontSecondary,
                                        furi_string_get_cstr(temp_str));
    furi_string_free(temp_str);

    widget_add_button_element(widget, GuiButtonTypeLeft, "Retry",
                              nfc_scene_slix_unlock_success_widget_callback, instance);
