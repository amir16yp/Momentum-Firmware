...
// TODO FL-3926: Re-enters backdoor detection between user and system dictionary if no backdoor is
// found
...
NfcCommand nfc_dict_attack_worker_callback(NfcGenericEvent event, void *context)
{
    ...
    if (mfc_event->type == MfClassicPollerEventTypeCardDetected) {
        ...
        mf_classic_get_read_sectors_and_keys(mfc_data, &instance->nfc_dict_context.sectors_read,
        ...
        if (is_cuid_dict) {
            // CUID dictionary: read 7 bytes (1 byte key_idx + 6 bytes key) and filter by exact
            // key_idx
            ...
            if (target_key_idx < 256 &&
                ...
                while (keys_dict_get_next_key(instance->nfc_dict_context.dict, key_with_idx,
            ...
            if (keys_dict_get_next_key(instance->nfc_dict_context.dict, key.data,
                                       sizeof(MfClassicKey))) {
                key_found = true;
                instance->nfc_dict_context.dict_keys_current++;
            }
...
static void nfc_scene_mf_classic_dict_attack_prepare_view(NfcApp *instance)
{
    ...
    if (state == DictAttackStateCUIDDictInProgress) {
        ...
        FuriString *cuid_dict_path =
            furi_string_alloc_printf("%s/mf_classic_dict_%08lx.nfc", EXT_PATH("nfc/assets"),
        ...
        do {
            ...
            if (!buffered_file_stream_open(dict->stream, furi_string_get_cstr(cuid_dict_path),
    ...
    if (state == DictAttackStateUserDictInProgress) {
        do {
            ...
            if (keys_dict_check_presence(NFC_APP_MF_CLASSIC_DICT_SYSTEM_NESTED_PATH)) {
                storage_common_remove(instance->storage,
                                      NFC_APP_MF_CLASSIC_DICT_SYSTEM_NESTED_PATH);
            }
            if (keys_dict_check_presence(NFC_APP_MF_CLASSIC_DICT_SYSTEM_PATH)) {
                storage_common_copy(instance->storage, NFC_APP_MF_CLASSIC_DICT_SYSTEM_PATH,
                                    NFC_APP_MF_CLASSIC_DICT_SYSTEM_NESTED_PATH);
            }
            ...
            storage_common_copy(instance->storage, NFC_APP_MF_CLASSIC_DICT_USER_PATH,
    ...
    dict_attack_set_total_dict_keys(instance->dict_attack,
                                    instance->nfc_dict_context.dict_keys_total);
    instance->nfc_dict_context.dict_keys_current = 0;

    dict_attack_set_callback(instance->dict_attack, nfc_dict_attack_dict_attack_result_callback,
                             instance);
...
void nfc_scene_mf_classic_dict_attack_on_enter(void *context)
{
    ...
    scene_manager_set_scene_state(instance->scene_manager, NfcSceneMfClassicDictAttack,
                                  DictAttackStateCUIDDictInProgress);
        ...
        if (event.event == NfcCustomEventDictAttackComplete) {
            ...
            if (state == DictAttackStateCUIDDictInProgress) {
                if (instance->nfc_dict_context.is_card_present) {
                    ...
                    scene_manager_set_scene_state(instance->scene_manager,
