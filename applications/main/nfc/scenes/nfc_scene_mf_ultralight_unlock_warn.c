...
void nfc_scene_mf_ultralight_unlock_warn_on_enter(void *context)
{
    ...
    if ((type == MfUltralightAuthTypeReader) || (type == MfUltralightAuthTypeManual)) {
        ...
        const char *message =
            (type == MfUltralightAuthTypeReader) ? "Password Captured!" : "Risky Action!";
        ...
        dialog_ex_set_text(dialog_ex, "Wrong password\ncan block your\ncard.", 4, 18, AlignLeft,
                           AlignTop);
    ...
    if ((type == MfUltralightAuthTypeReader) || (type == MfUltralightAuthTypeManual)) {
        if (event.type == SceneManagerEventTypeCustom) {
            if (event.event == DialogExResultRight) {
                ...
                nfc_detected_protocols_set(nfc->detected_protocols, mfu_protocol,
                                           COUNT_OF(mfu_protocol));
