...
void nfc_scene_delete_on_enter(void *context)
{
    ...
    widget_add_text_box_element(nfc->widget, 0, 0, 128, 23, AlignCenter, AlignCenter,
                                furi_string_get_cstr(temp_str), false);
    widget_add_button_element(nfc->widget, GuiButtonTypeLeft, "Cancel",
                              nfc_scene_delete_widget_callback, nfc);
    widget_add_button_element(nfc->widget, GuiButtonTypeRight, "Delete",
                              nfc_scene_delete_widget_callback, nfc);
    ...
    widget_add_string_element(nfc->widget, 64, 24, AlignCenter, AlignTop, FontSecondary,
                              furi_string_get_cstr(temp_str));

    furi_string_set_str(temp_str, nfc_device_get_name(nfc->nfc_device, NfcDeviceNameTypeFull));
    widget_add_string_element(nfc->widget, 64, 34, AlignCenter, AlignTop, FontSecondary,
                              furi_string_get_cstr(temp_str));
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == GuiButtonTypeLeft) {
            ...
            if (nfc_delete(nfc)) {
                ...
                if (!scene_manager_search_and_switch_to_previous_scene(nfc->scene_manager,
                                                                       NfcSceneStart)) {
