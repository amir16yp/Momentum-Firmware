...
void nfc_scene_set_atqa_on_enter(void *context)
{
    ...
    byte_input_set_result_callback(byte_input, nfc_protocol_support_common_byte_input_done_callback,
                                   nfc_scene_set_atqa_byte_input_changed_callback, instance,
                                   instance->byte_input_store, 2);
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == NfcCustomEventByteInputDone) {
            nfc_device_set_data(instance->nfc_device, NfcProtocolIso14443_3a,
                                instance->iso14443_3a_edit_data);
