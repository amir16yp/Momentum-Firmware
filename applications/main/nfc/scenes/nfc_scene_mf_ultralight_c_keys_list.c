...
void nfc_scene_mf_ultralight_c_keys_list_on_enter(void *context)
{
    ...
    if (keys_num > 0) {
        for (size_t i = 0; i < keys_num; i++) {
            ...
            submenu_add_item(instance->submenu, furi_string_get_cstr(temp_str), i,
                             nfc_scene_mf_ultralight_c_keys_list_submenu_callback, instance);
        }
    }
...
bool nfc_scene_mf_ultralight_c_keys_list_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(instance->scene_manager, NfcSceneMfUltralightCKeysDelete,
                                      event.event);
