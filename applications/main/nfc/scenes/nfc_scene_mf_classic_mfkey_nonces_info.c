...
void nfc_scene_mf_classic_mfkey_nonces_info_callback(GuiButtonType result, InputType type,
                                                     void *context)
{
...
void nfc_scene_mf_classic_mfkey_nonces_info_on_enter(void *context)
{
    ...
    widget_add_string_element(instance->widget, 0, 12, AlignLeft, AlignTop, FontSecondary,
                              "Authenticated sectors:");

    mfkey32_logger_get_params_data(instance->mfkey32_logger, temp_str);
    widget_add_text_scroll_element(instance->widget, 0, 22, 128, 42,
                                   furi_string_get_cstr(temp_str));
    widget_add_button_element(instance->widget, GuiButtonTypeCenter, "OK",
                              nfc_scene_mf_classic_mfkey_nonces_info_callback, instance);
...
bool nfc_scene_mf_classic_mfkey_nonces_info_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == GuiButtonTypeCenter) {
            if (mfkey32_logger_save_params(instance->mfkey32_logger,
                                           NFC_APP_MFKEY32_LOGS_FILE_PATH)) {
