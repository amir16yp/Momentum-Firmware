...
void nfc_scene_felica_system_on_enter(void *context)
{
    ...
    submenu_add_item(submenu, "Directory", SubmenuIndexDirectory,
                     nfc_scene_felica_system_submenu_callback, nfc);
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == NfcCustomEventViewExit) {
            ...
            if (event.event == SubmenuIndexDirectory) {
                ...
                widget_add_text_scroll_element(nfc->widget, 0, 0, 128, 64,
                ...
                nfc_more_info_render_felica_blocks(data, system, nfc->text_box_store,
                                                   service->code);
