...
void nfc_scene_mf_ultralight_unlock_menu_on_enter(void *context)
{
    ...
    if (nfc_device_get_protocol(nfc->nfc_device) == NfcProtocolMfUltralight) {
        ...
        if (mfu_data->type != MfUltralightTypeMfulC) {
            submenu_add_item(submenu, "Unlock With Reader", SubmenuIndexMfUlUnlockMenuReader,
                             nfc_scene_mf_ultralight_unlock_menu_submenu_callback, nfc);
        }
    }
    submenu_add_item(submenu, "Auth As Ameebo", SubmenuIndexMfUlUnlockMenuAmeebo,
                     nfc_scene_mf_ultralight_unlock_menu_submenu_callback, nfc);
    submenu_add_item(submenu, "Auth As Xiaomi Air Purifier", SubmenuIndexMfUlUnlockMenuXiaomi,
                     nfc_scene_mf_ultralight_unlock_menu_submenu_callback, nfc);
    submenu_add_item(submenu, "Enter Password Manually", SubmenuIndexMfUlUnlockMenuManual,
                     nfc_scene_mf_ultralight_unlock_menu_submenu_callback, nfc);
...
bool nfc_scene_mf_ultralight_unlock_menu_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        scene_manager_set_scene_state(nfc->scene_manager, NfcSceneMfUltralightUnlockMenu,
                                      event.event);
    }
    return consumed;
}
