...
void nfc_scene_slix_unlock_menu_on_enter(void *context)
{
    ...
    uint32_t state = scene_manager_get_scene_state(instance->scene_manager, NfcSceneSlixUnlockMenu);
    submenu_add_item(submenu, "Enter Password Manually", SubmenuIndexSlixUnlockMenuManual,
                     nfc_scene_slix_unlock_menu_submenu_callback, instance);
    submenu_add_item(submenu, "Auth As TommyBox", SubmenuIndexSlixUnlockMenuTonieBox,
                     nfc_scene_slix_unlock_menu_submenu_callback, instance);
...
bool nfc_scene_slix_unlock_menu_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        scene_manager_set_scene_state(instance->scene_manager, NfcSceneSlixUnlockMenu, event.event);
    }
    return consumed;
}
