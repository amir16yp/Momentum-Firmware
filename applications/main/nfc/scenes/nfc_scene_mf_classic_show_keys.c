...
void nfc_scene_mf_classic_show_keys_on_enter(void *context)
{
    ...
    furi_string_cat_printf(instance->text_box_store,
                           "\nTotal keys found:\n -> %d/%d A keys\n -> %d/%d B keys", found_keys_a,
                           num_sectors, found_keys_b, num_sectors);

    widget_add_text_scroll_element(instance->widget, 2, 2, 124, 60,
                                   furi_string_get_cstr(instance->text_box_store));
    widget_add_button_element(instance->widget, GuiButtonTypeLeft, "Back",
                              nfc_scene_mf_classic_show_keys_callback, instance);
