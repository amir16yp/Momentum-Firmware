...
void nfc_scene_mf_desfire_app_on_enter(void *context)
{
    ...
    submenu_add_item(nfc->submenu, "App info", SubmenuIndexAppInfo,
                     nfc_scene_mf_desfire_app_submenu_callback, nfc);
    ...
    for (uint32_t i = 0; i < simple_array_get_count(app->file_ids); ++i) {
        ...
        submenu_add_item(nfc->submenu, furi_string_get_cstr(label), i + SubmenuIndexDynamic,
                         nfc_scene_mf_desfire_app_submenu_callback, nfc);
    }
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == NfcCustomEventViewExit) {
            ...
            if (event.event == SubmenuIndexAppInfo) {
                ...
                nfc_render_mf_desfire_file_settings_data(file_settings, file_data,
                                                         nfc->text_box_store);
            }
