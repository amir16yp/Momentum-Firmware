...
void nfc_scene_mf_ultralight_c_keys_delete_widget_callback(GuiButtonType result, InputType type,
                                                           void *context)
{
...
void nfc_scene_mf_ultralight_c_keys_delete_on_enter(void *context)
{
    ...
    widget_add_string_element(instance->widget, 64, 0, AlignCenter, AlignTop, FontPrimary,
                              "Delete this key?");
    widget_add_button_element(instance->widget, GuiButtonTypeLeft, "Cancel",
                              nfc_scene_mf_ultralight_c_keys_delete_widget_callback, instance);
    widget_add_button_element(instance->widget, GuiButtonTypeRight, "Delete",
                              nfc_scene_mf_ultralight_c_keys_delete_widget_callback, instance);

    KeysDict *mf_ultralight_c_user_dict =
    ...
    for (size_t i = 0; i < (key_index + 1); i++) {
        bool key_loaded = keys_dict_get_next_key(mf_ultralight_c_user_dict, stack_key.data,
                                                 sizeof(MfUltralightC3DesAuthKey));
    ...
    widget_add_string_element(instance->widget, 64, 32, AlignCenter, AlignCenter, FontSecondary,
...
bool nfc_scene_mf_ultralight_c_keys_delete_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == GuiButtonTypeRight) {
            uint32_t key_index = scene_manager_get_scene_state(instance->scene_manager,
                                                               NfcSceneMfUltralightCKeysDelete);
            KeysDict *mf_ultralight_c_user_dict =
                keys_dict_alloc(NFC_APP_MF_ULTRALIGHT_C_DICT_USER_PATH, KeysDictModeOpenAlways,
                                sizeof(MfUltralightC3DesAuthKey));
            size_t dict_keys_num = keys_dict_get_total_keys(mf_ultralight_c_user_dict);
