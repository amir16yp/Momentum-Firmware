...
void nfc_scene_set_type_on_enter(void *context)
{
    ...
    submenu_add_item(submenu, "NFC-A 7-bytes UID", SubmenuIndexNFCA7,
                     nfc_protocol_support_common_submenu_callback, instance);
    submenu_add_item(submenu, "NFC-A 4-bytes UID", SubmenuIndexNFCA4,
                     nfc_protocol_support_common_submenu_callback, instance);
...
bool nfc_scene_set_type_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == SubmenuIndexNFCA7) {
            ...
            scene_manager_set_scene_state(instance->scene_manager, NfcSceneGenerateInfo,
                                          event.event);
