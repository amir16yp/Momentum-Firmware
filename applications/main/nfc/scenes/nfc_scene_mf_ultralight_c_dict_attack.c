...
NfcCommand nfc_mf_ultralight_c_dict_attack_worker_callback(NfcGenericEvent event, void *context)
{
    ...
    if (poller_event->type == MfUltralightPollerEventTypeRequestMode) {
        ...
        if (keys_dict_get_next_key(instance->mf_ultralight_c_dict_context.dict, key.data,
...
void nfc_scene_mf_ultralight_c_dict_attack_dict_attack_result_callback(DictAttackEvent event,
                                                                       void *context)
{
    furi_assert(context);
    NfcApp *instance = context;
...
void nfc_scene_mf_ultralight_c_dict_attack_prepare_view(NfcApp *instance)
{
    ...
    if (state == DictAttackStateUserDictInProgress) {
        do {
            ...
            instance->mf_ultralight_c_dict_context.dict =
                keys_dict_alloc(NFC_APP_MF_ULTRALIGHT_C_DICT_USER_PATH, KeysDictModeOpenAlways,
                                sizeof(MfUltralightC3DesAuthKey));
    ...
    dict_attack_set_current_dict_key(instance->dict_attack,
                                     instance->mf_ultralight_c_dict_context.dict_keys_current);
    ...
    dict_attack_set_callback(instance->dict_attack,
...
{
    dict_attack_set_card_state(instance->dict_attack,
                               instance->mf_ultralight_c_dict_context.is_card_present);
    dict_attack_set_current_dict_key(instance->dict_attack,
                                     instance->mf_ultralight_c_dict_context.dict_keys_current);
}
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == NfcCustomEventDictAttackComplete) {
            if (state == DictAttackStateUserDictInProgress) {
                if (instance->mf_ultralight_c_dict_context.auth_success) {
                    ...
                    nfc_poller_start(instance->poller,
                                     nfc_mf_ultralight_c_dict_attack_worker_callback, instance);
                    consumed = true;
                }
            } else {
                // Could check if card is fully read here like MFC dict attack, but found key means
                // fully read
                if (instance->mf_ultralight_c_dict_context.auth_success) {
                    notification_message(instance->notifications, &sequence_success);
                } else {
            ...
            if (state == DictAttackStateUserDictInProgress) {
                ...
                nfc_scene_mf_ultralight_c_dict_attack_prepare_view(instance);
                instance->poller = nfc_poller_alloc(instance->nfc, NfcProtocolMfUltralight);
                nfc_poller_start(instance->poller, nfc_mf_ultralight_c_dict_attack_worker_callback,
                                 instance);
            } else {
                notification_message(instance->notifications, &sequence_semi_success);
                scene_manager_next_scene(instance->scene_manager, NfcSceneReadSuccess);
