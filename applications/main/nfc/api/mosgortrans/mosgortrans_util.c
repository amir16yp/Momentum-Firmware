...
void render_section_header(FuriString *str, const char *name, uint8_t prefix_separator_cnt,
...
void parse_transport_type(BlockData *data_block, FuriString *transport)
{
    ...
    case 1:
        uint8_t transport_type = (data_block->transport_type1 || data_block->transport_type2 ||
...
bool mosgortrans_parse_transport_block(const MfClassicBlock *block, FuriString *result)
{
    ...
    case 0x02: {
        ...
        furi_string_cat_printf(result, "Use before: %02d.%02d.%04d\n", card_use_before_date_s.day,
                               card_use_before_date_s.month, card_use_before_date_s.year);
        ...
        furi_string_cat_printf(result, "Valid from: %02d.%02d.%04d\n", card_valid_from_date_s.day,
                               card_valid_from_date_s.month, card_valid_from_date_s.year);
        // valid_to_date
        DateTime card_valid_to_date_s = {0};
        from_days_to_datetime(data_block.valid_to_date, &card_valid_to_date_s, 1992);
        furi_string_cat_printf(result, "Valid to: %02d.%02d.%04d\n", card_valid_to_date_s.day,
                               card_valid_to_date_s.month, card_valid_to_date_s.year);
        // trip_number
        furi_string_cat_printf(result, "Trips: %d\n", data_block.total_trips);
        // trip_from
        DateTime card_start_trip_minutes_s = {0};
        from_seconds_to_datetime(data_block.start_trip_date * 24 * 60 * 60 +
                                     data_block.start_trip_time * 60 +
                                     data_block.start_trip_seconds,
                                 &card_start_trip_minutes_s, 1992);
        furi_string_cat_printf(result, "Trip from: %02d.%02d.%04d %02d:%02d",
                               card_start_trip_minutes_s.day, card_start_trip_minutes_s.month,
                               card_start_trip_minutes_s.year, card_start_trip_minutes_s.hour,
    ...
    case 0x06: {
        ...
        // trip_from
        DateTime card_start_trip_minutes_s = {0};
        from_minutes_to_datetime((data_block.start_trip_date) * 24 * 60 +
                                     data_block.start_trip_time,
                                 &card_start_trip_minutes_s, 1992);
        furi_string_cat_printf(result, "Trip from: %02d.%02d.%04d %02d:%02d\n",
                               card_start_trip_minutes_s.day, card_start_trip_minutes_s.month,
                               card_start_trip_minutes_s.year, card_start_trip_minutes_s.hour,
    ...
    case 0x08: {
        ...
        // valid_to_date
        DateTime card_valid_to_date_s = {0};
        from_days_to_datetime(data_block.valid_from_date + data_block.valid_for_days,
                              &card_valid_to_date_s, 1992);
        furi_string_cat_printf(result, "Valid to: %02d.%02d.%04d", card_valid_to_date_s.day,
                               card_valid_to_date_s.month, card_valid_to_date_s.year);
        break;
    }
    case 0x0A: {
        ...
        // valid_to_date
        DateTime card_valid_to_date_s = {0};
        from_minutes_to_datetime(data_block.valid_from_date * 24 * 60 +
                                     data_block.valid_for_minutes - 1,
                                 &card_valid_to_date_s, 2016);
        furi_string_cat_printf(result, "Valid to: %02d.%02d.%04d", card_valid_to_date_s.day,
                               card_valid_to_date_s.month, card_valid_to_date_s.year);
        // trip_from
        if (data_block.start_trip_minutes) {
            DateTime card_start_trip_minutes_s = {0};
            from_minutes_to_datetime(data_block.valid_from_date * 24 * 60 +
                                         data_block.start_trip_minutes,
                                     &card_start_trip_minutes_s, 2016);
        ...
        if (data_block.minutes_pass) {
            ...
            from_minutes_to_datetime(data_block.valid_from_date * 24 * 60 +
                                     ...
                                     &card_start_switch_trip_minutes_s, 2016);
            furi_string_cat_printf(
                result, "\nTrip switch: %02d.%02d.%04d %02d:%02d",
                card_start_switch_trip_minutes_s.day, card_start_switch_trip_minutes_s.month,
                card_start_switch_trip_minutes_s.year, card_start_switch_trip_minutes_s.hour,
    ...
    case 0x0D: {
        ...
        if (data_block.passage_5_minutes) { // TODO: (-nofl) unused
            DateTime card_start_switch_trip_minutes_s = {0};
            from_minutes_to_datetime(data_block.start_trip_date * 24 * 60 +
                                         data_block.start_trip_time + data_block.passage_5_minutes,
                                     &card_start_switch_trip_minutes_s, 1992);
        }
        // validator
        if (data_block.validator) {
            furi_string_cat_printf(result, "\nValidator: %05d", data_block.validator);
    ...
    case 0x1C2: {
        ...
        DateTime card_valid_from_date_s = {0};
        from_days_to_datetime(data_block.valid_from_date, &card_valid_from_date_s, 1992);
        furi_string_cat_printf(result, "Valid from: %02d.%02d.%04d", card_valid_from_date_s.day,
                               card_valid_from_date_s.month, card_valid_from_date_s.year);
        // valid_to_date
        if (data_block.activate_during) {
            DateTime card_valid_to_date_s = {0};
            from_days_to_datetime(data_block.valid_from_date + data_block.activate_during,
                                  &card_valid_to_date_s, 1992);
            furi_string_cat_printf(result, "\nValid to: %02d.%02d.%04d", card_valid_to_date_s.day,
                                   card_valid_to_date_s.month, card_valid_to_date_s.year);
        } else {
            DateTime card_valid_to_date_s = {0};
            from_minutes_to_datetime(data_block.valid_from_date * 24 * 60 +
        ...
        if (data_block.start_trip_neg_minutes) {
            DateTime card_start_trip_minutes_s = {0};
            from_minutes_to_datetime(data_block.valid_to_date * 24 * 60 +
    ...
    case 0x1C4: {
        ...
        furi_string_cat_printf(result, "Valid from: %02d.%02d.%04d\n", card_use_from_date_s.day,
                               card_use_from_date_s.month, card_use_from_date_s.year);
        // valid_to_date
        DateTime card_use_to_date_s = {0};
        if (data_block.requires_activation) {
            from_days_to_datetime(data_block.valid_from_date + data_block.activate_during,
                                  &card_use_to_date_s, 2016);
        } else {
            from_minutes_to_datetime(data_block.valid_from_date * 24 * 60 +
                                         data_block.valid_for_minutes - 1,
                                     &card_use_to_date_s, 2016);
        ...
        furi_string_cat_printf(result, "Valid to: %02d.%02d.%04d\n", card_use_to_date_s.day,
                               card_use_to_date_s.month, card_use_to_date_s.year);
        // trip_number
        // furi_string_cat_printf(result, "Trips left: %d", data_block.remaining_trips);
        // trip_from
        DateTime card_start_trip_minutes_s = {0};
    ...
    case 0x1C5: {
        ...
        if (data_block.start_trip_minutes) {
            DateTime card_start_trip_minutes_s = {0};
            from_minutes_to_datetime(data_block.start_trip_minutes, &card_start_trip_minutes_s,
                                     2019);
            furi_string_cat_printf(result, "\nTrip from: %02d.%02d.%04d %02d:%02d",
                                   card_start_trip_minutes_s.day, card_start_trip_minutes_s.month,
                                   card_start_trip_minutes_s.year, card_start_trip_minutes_s.hour,
        ...
        if (data_block.metro_ride_with) {
            ...
            from_minutes_to_datetime(data_block.start_trip_minutes + data_block.metro_ride_with,
                                     &card_start_m_trip_minutes_s, 2019);
            furi_string_cat_printf(
                result, "\n(M) from: %02d.%02d.%04d %02d:%02d", card_start_m_trip_minutes_s.day,
                card_start_m_trip_minutes_s.month, card_start_m_trip_minutes_s.year,
                card_start_m_trip_minutes_s.hour, card_start_m_trip_minutes_s.minute);
        }
        if (data_block.minutes_pass) {
            DateTime card_start_change_trip_minutes_s = {0};
            from_minutes_to_datetime(data_block.start_trip_minutes + data_block.minutes_pass,
                                     &card_start_change_trip_minutes_s, 2019);
            furi_string_cat_printf(
                result, "\nTrip edit: %02d.%02d.%04d %02d:%02d",
                card_start_change_trip_minutes_s.day, card_start_change_trip_minutes_s.month,
                card_start_change_trip_minutes_s.year, card_start_change_trip_minutes_s.hour,
    ...
    case 0x1C6: {
        ...
        // valid_to_date
        DateTime card_use_to_date_s = {0};
        from_minutes_to_datetime(data_block.valid_from_date + data_block.valid_for_minutes - 1,
                                 &card_use_to_date_s, 2019);
        furi_string_cat_printf(result, "Valid to: %02d.%02d.%04d", card_use_to_date_s.day,
                               card_use_to_date_s.month, card_use_to_date_s.year);
        // start_trip_minutes
        if (data_block.start_trip_neg_minutes) {
            DateTime card_start_trip_minutes_s = {0};
            from_minutes_to_datetime(data_block.valid_from_date + data_block.valid_for_minutes -
                                         data_block.start_trip_neg_minutes,
                                     &card_start_trip_minutes_s,
                                     2019); //-time
            furi_string_cat_printf(result, "\nTrip from: %02d.%02d.%04d %02d:%02d",
        ...
        if (data_block.metro_ride_with) {
            ...
            from_minutes_to_datetime(data_block.valid_from_date + data_block.valid_for_minutes -
                                         data_block.start_trip_neg_minutes +
                                         data_block.metro_ride_with,
                                     &card_start_trip_m_minutes_s, 2019);
            furi_string_cat_printf(
                result, "\n(M) from: %02d.%02d.%04d %02d:%02d", card_start_trip_m_minutes_s.day,
                card_start_trip_m_minutes_s.month, card_start_trip_m_minutes_s.year,
                card_start_trip_m_minutes_s.hour, card_start_trip_m_minutes_s.minute);
        }
        // transport
        // validator
        if (data_block.validator) {
