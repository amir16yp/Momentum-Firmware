...
static constexpr auto nfc_app_api_table = sort(create_array_t<sym_entry>(
    API_METHOD(gallagher_deobfuscate_and_parse_credential, void,
               (GallagherCredential * credential, const uint8_t *cardholder_data_obfuscated)),
    API_VARIABLE(GALLAGHER_CARDAX_ASCII, const uint8_t *),
    API_METHOD(mosgortrans_parse_transport_block, bool,
               (const MfClassicBlock *block, FuriString *result)),
    API_METHOD(render_section_header, void,
               (FuriString * str, const char *name, uint8_t prefix_separator_cnt,
                uint8_t suffix_separator_cnt)),
    API_METHOD(nfc_append_filename_string_when_present, void,
    ...
    API_METHOD(nfc_protocol_support_common_widget_callback, void,
    ...
    API_METHOD(nfc_protocol_support_common_on_event_empty, bool,
