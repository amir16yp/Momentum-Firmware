...
void hi_generate_key(uint8_t *uid, uint8_t keyA[5][KEY_LENGTH], uint8_t keyB[5][KEY_LENGTH])
{
    ...
    static const uint8_t xor_table_keyB[4][6] = {{0x1F, 0xC4, 0x4D, 0x94, 0x6A, 0x31},
                                                 {0x12, 0xC1, 0x5C, 0x70, 0xDF, 0x31},
                                                 {0x56, 0xF0, 0x13, 0x1B, 0x63, 0xF2},
                                                 {0x4E, 0xFA, 0xC2, 0xF8, 0xC9, 0xCC}};

    static const uint8_t xor_table_keyA[4][6] = {{0xB6, 0xE6, 0xAE, 0x72, 0x91, 0x0D},
...
static bool hi_verify_type(Nfc *nfc, MfClassicType type)
{
    ...
    do {
        HiCardConfig cfg = {};
        if (!hi_get_card_config(&cfg, type))
            break;
        ...
        if (error != MfClassicErrorNone) {
            FURI_LOG_D(TAG, "Failed to read block %u: %d, this is not a HI card", block_num, error);
            break;
        }
...
static bool hi_read(Nfc *nfc, NfcDevice *device)
{
    ...
    do {
        ...
        HiCardConfig cfg = {};
        if (!hi_get_card_config(&cfg, data->type))
            break;
