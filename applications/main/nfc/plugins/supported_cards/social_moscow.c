...
static const MfClassicKeyPair social_moscow_1k_keys[] = {
    ...
    {.a = 0x51119dae5216, .b = 0xd8a274b2e026}, {.a = 0x2aba9519f574, .b = 0xcb9a1f2d7368},
    {.a = 0x84fd7f7a12b6, .b = 0xc7c0adb3284f}, {.a = 0xa0a1a2a3a4a5, .b = 0x7de02a7f6025}};
...
static bool social_moscow_verify_type(Nfc *nfc, MfClassicType type)
{
    ...
    do {
        ...
        if (!social_moscow_get_card_config(&cfg, type))
            break;
...
static bool social_moscow_read(Nfc *nfc, NfcDevice *device)
{
    ...
    do {
        ...
        data->type = type;
        SocialMoscowCardConfig cfg = {};
        if (!social_moscow_get_card_config(&cfg, data->type))
            break;
...
static uint64_t hex_num(uint64_t hex)
{
    ...
    for (uint8_t i = 8; i > 0; --i) {
        result = result * 10 + ((hex >> ((i - 1) * 4)) & 0x0F);
    }
    return result;
}
...
static bool social_moscow_parse(const NfcDevice *device, FuriString *parsed_data)
{
    ...
    do {
        ...
        if ((key_a != cfg.keys[cfg.data_sector].a) || (key_b != cfg.keys[cfg.data_sector].b))
            break;
        ...
        furi_string_cat_printf(
            ...
            card_code, card_region, card_number, card_control, omc_number, month, year,
            data->block[60].data[13], data->block[60].data[14]);
