...
// Credit to https://www.lenrek.net/experiments/compass-tickets/ & MetroDroid project for underlying
// info
//
// This parser can decode the paper single-use and single/multi-day paper passes using Ultralight
// EV1 The plastic cards are DESFire and fully locked down, not much useful info extractable
// TODO:
...
// - Generalize to handle all known Cubic Nextfare Ultralight systems?  Anyone wants to send me
// specimen dumps, hit me up on Discord.
...
static FuriString *ventra_parse_xact(const MfUltralightData *data, uint8_t blk, bool is_pass)
{
    ...
    // If sequence is 0, block isn't used yet (new card with only one active block, typically the
    // first one. Otherwise, the block with higher sequence is the latest transaction, and the other
    // block is prior transaction. Not necessarily in that order on the card.  We need the latest
    // data to compute validity and pretty-print them in reverse chrono.  So this mess sets some
    // globals as to which block is current, computes the validity times, etc.
    if (seq == 0) {
    ...
    if (seq > ventra_high_seq) {
        ...
        // Figure out the soft expiration.  For passes it's easy, the readers update the "exp" field
        // in the transaction record. Tickets, not so much, readers don't update "exp", but each
        // xact record has "minutes since last tap" which is updated and carried forward.  That,
        // plus transaction timestamp, gives the expiration time.
    ...
    char linemap[3] = "PTB";
    char *xact_fmt =
        (line == 2) ? "%c %5d %04d-%02d-%02d %02d:%02d" : "%c %04X %04d-%02d-%02d %02d:%02d";
    // I like a nice concise display showing all the relevant infos without having to scroll...
    // Line   StopID   DateTime
    furi_string_printf(ventra_xact_str, xact_fmt, (line < 3) ? linemap[line] : '?', locus, dt.year,
...
static bool ventra_parse(const NfcDevice *device, FuriString *parsed_data)
{
    ...
    do {
        // This test can probably be improved -- it matches every Ventra I've seen, but will also
        // match others in the same family.  Or maybe we just generalize this parser.
        ...
        // Parse the product record, display interesting data & extract info needed to parse
        // transaction blocks Had this in its own function, ended up just setting a bunch of shitty
        // globals, so inlined it instead.
        ...
        case 0x3F:
            ...
            break;
        case 4: // Last I checked, 3 day passes only available at airport TVMs & social service
                // agencies
            is_pass = true;
            furi_string_cat_printf(ventra_prod_str, "3-Day");
            break;
        default:
            is_pass = true; // There are some card types I don't know what they are, but they parse
                            // like a pass, not a ticket.
        ...
        // Parse the transaction blocks.  This sets a few sloppy globals, but it's too complex and
        // repetitive to inline.
        ...
        if (ventra_high_seq > 1)
            card_state = 2;
        // On "ticket" product, the OTP bits mark off rides used.  Bit 0 seems to be unused, the
        // next 3 are set as rides are used. Some, not all, readers will set the high bits to 0x7
        // when a card is tapped after it's expired or depleted.  Have not seen other combinations,
        // but if we do, we'll make a nice ???.  1-day passes set the OTP bit 1 on first use.  3-day
        // passes do not.  But we don't really care, since they don't matter on passes, unless
        // you're trying to rollback one.
        if (!is_pass) {
            switch (otp) {
            case 0:
                rides_left = 3;
        ...
        furi_string_printf(parsed_data, "\e#Ventra %s (%s)\n",
                           furi_string_get_cstr(ventra_prod_str), card_states[card_state]);

        furi_string_cat_printf(parsed_data, "Exp: %04d-%02d-%02d %02d:%02d\n",
                               ventra_validity_date.year, ventra_validity_date.month,
                               ventra_validity_date.day, ventra_validity_date.hour,
        ...
        furi_string_cat_printf(parsed_data, "Hard Expiry: %04d-%02d-%02d", ventra_exp_date.year,
                               ventra_exp_date.month, ventra_exp_date.day);
