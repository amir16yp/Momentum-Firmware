...
static bool microel_read(Nfc *nfc, NfcDevice *device)
{
    ...
    do {
        ...
        MfClassicKey key = {0};
        bit_lib_num_to_bytes_be(bit_lib_bytes_to_num_be(keyA, KEY_LENGTH), COUNT_OF(key.data),
                                key.data);
        ...
        for (size_t i = 0; i < mf_classic_get_total_sectors_num(data->type); i++) {
            bit_lib_num_to_bytes_be(microel_1k_keys[i].a, sizeof(MfClassicKey), keys.key_a[i].data);
            FURI_BIT_SET(keys.key_a_mask, i);
            bit_lib_num_to_bytes_be(microel_1k_keys[i].b, sizeof(MfClassicKey), keys.key_b[i].data);
...
static bool microel_parse(const NfcDevice *device, FuriString *parsed_data)
{
    ...
    do {
        ...
        if (key != key_for_check_from_array)
            break;
...
static const NfcSupportedCardsPlugin microel_plugin = {
    ...
    .verify = NULL, // the verification I need is based on verifying the keys generated via uid and
                    // try to authenticate not like on mizip that there is default b0 but added
                    // verify in read function
    .read = microel_read,
    .parse = microel_parse,
};
