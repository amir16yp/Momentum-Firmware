...
static inline void extract_ppk_data(Storage *storage, const MfClassicData *data, TicketData *ticket,
                                    bool ticket_number)
{
    if (ticket_number == 0) {
...
static void printf_accompany_card(TicketData *ticket, FuriString *parsed_data)
{
    if (ticket->departure_uic == 0x0000) {
        furi_string_cat_printf(parsed_data, "\e#Unknown SZPPK Card\n   NO TICKET DATA FOUND "
                                            "\n\nTHE TICKET IS NOT ISSUED\nOR LAYOUT IS UNKNOWN\n");
        return;
    } else {
        furi_string_cat_printf(
            parsed_data, "\e#SZPPK Accompany Card\nValid on: %02d-%02d-%04d\nStation: > %s\n",
        ...
        case 0x80:
            furi_string_cat_printf(
                parsed_data, "Status:> ENTERED STATION\nChecked in at:> %02d:%02d\nPPK CNT: %03d\n",
                ticket->tap_time.hour, ticket->tap_time.minute, ticket->ppk_cnt);
            break;
        case 0x1E:
            furi_string_cat_printf(
                parsed_data, "Status:> EXITED STATION\nChecked out at:> %02d:%02d\nPPK CNT: %03d\n",
                ticket->tap_time.hour, ticket->tap_time.minute, ticket->ppk_cnt);
            break;
        default:
...
static void printf_transport_card(FuriString *parsed_data, TicketData *ticket, bool ticket_number)
{
    if (ticket->departure_uic == 0x0000) {
        furi_string_cat_printf(parsed_data, "\e#Unknown SZPPK Card\n   NO TICKET DATA FOUND "
                                            "\n\nTHE TICKET IS NOT ISSUED\nOR LAYOUT IS UNKNOWN\n");
        ...
        furi_string_cat_printf(
            ...
            furi_string_get_cstr(ticket->departure_name),
            furi_string_get_cstr(ticket->destination_name), ticket->valid_from.day,
            ticket->valid_from.month, ticket->valid_from.year, ticket->valid_till.day,
            ticket->valid_till.month, ticket->valid_till.year);
        ...
        case 0x80:
            furi_string_cat_printf(parsed_data,
                                   ...
                                   ticket->tap_time.day, ticket->tap_time.month,
                                   ticket->tap_time.year, ticket->tap_time.hour,
                                   ticket->tap_time.minute, ticket->ppk_cnt);
            break;
        case 0x1E:
            furi_string_cat_printf(parsed_data,
        ...
        default:
            furi_string_cat_printf(parsed_data, "Status:> UNKNOWN (%02X)\nPPK CNT: %03d",
                                   ticket->current_status, ticket->ppk_cnt);
            break;
        }
    }
}
