...
void mizip_generate_key(uint8_t *uid, uint8_t keyA[5][KEY_LENGTH], uint8_t keyB[5][KEY_LENGTH])
{
    ...
    static const uint8_t xor_table_keyA[4][6] = {{0x09, 0x12, 0x5A, 0x25, 0x89, 0xE5},
                                                 {0xAB, 0x75, 0xC9, 0x37, 0x92, 0x2F},
                                                 {0xE2, 0x72, 0x41, 0xAF, 0x2C, 0x09},
                                                 {0x31, 0x7A, 0xB7, 0x2F, 0x44, 0x90}};

    static const uint8_t xor_table_keyB[4][6] = {{0xF1, 0x2C, 0x84, 0x53, 0xD8, 0x21},
...
static bool mizip_verify_type(Nfc *nfc, MfClassicType type)
{
    ...
    do {
        MizipCardConfig cfg = {};
        if (!mizip_get_card_config(&cfg, type))
            break;
        ...
        if (error != MfClassicErrorNone) {
            FURI_LOG_D(TAG, "Failed to read block %u: %d, this is not a MiZIP card", block_num,
                       error);
            break;
        }
...
static bool mizip_read(Nfc *nfc, NfcDevice *device)
{
    ...
    do {
        ...
        // temp fix but fix mf_classic_poller_sync_detect_type because view type mfclassic1k and not
        // verify mfmini
        data->type = MfClassicTypeMini;
        MizipCardConfig cfg = {};
        if (!mizip_get_card_config(&cfg, data->type))
            break;
...
static bool mizip_parse(const NfcDevice *device, FuriString *parsed_data)
{
    ...
    do {
        ...
        if (key != cfg.keys[cfg.verify_sector].b)
            break;
        ...
        uint16_t balance =
            (data->block[credit_pointer].data[2] << 8) | (data->block[credit_pointer].data[1]);
        ...
        furi_string_cat_printf(parsed_data, "\nCurrent Credit: %d.%02d E \n", balance / 100,
                               balance % 100);
        furi_string_cat_printf(parsed_data, "Previous Credit: %d.%02d E \n", previous_balance / 100,
                               previous_balance % 100);
