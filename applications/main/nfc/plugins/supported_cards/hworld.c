...
static bool hworld_verify(Nfc *nfc)
{
    ...
    do {
        ...
        bit_lib_num_to_bytes_be(hworld_standard_keys[ROOM_SECTOR].a, COUNT_OF(standard_key.data),
                                standard_key.data);
        ...
        bit_lib_num_to_bytes_be(hworld_vip_keys[VIP_SECTOR].b, COUNT_OF(vip_key.data),
                                vip_key.data);

        MfClassicError vip_error =
            mf_classic_poller_sync_auth(nfc, block_num, &vip_key, MfClassicKeyTypeB, &auth_context);
...
static bool hworld_read(Nfc *nfc, NfcDevice *device)
{
    ...
    do {
        ...
        for (size_t i = 0; i < mf_classic_get_total_sectors_num(data->type); i++) {
            bit_lib_num_to_bytes_be(hworld_standard_keys[i].a, sizeof(MfClassicKey),
                                    standard_keys.key_a[i].data);
            FURI_BIT_SET(standard_keys.key_a_mask, i);
            bit_lib_num_to_bytes_be(hworld_standard_keys[i].b, sizeof(MfClassicKey),
                                    standard_keys.key_b[i].data);
        ...
        if (standard_error == MfClassicErrorNone) {
            ...
            for (size_t i = 0; i < mf_classic_get_total_sectors_num(data->type); i++) {
                bit_lib_num_to_bytes_be(hworld_vip_keys[i].a, sizeof(MfClassicKey),
                                        vip_keys.key_a[i].data);
                FURI_BIT_SET(vip_keys.key_a_mask, i);
                bit_lib_num_to_bytes_be(hworld_vip_keys[i].b, sizeof(MfClassicKey),
                                        vip_keys.key_b[i].data);
...
bool hworld_parse(const NfcDevice *device, FuriString *parsed_data)
{
    ...
    do {
        ...
        furi_string_cat_printf(parsed_data, "Check-in Date: \n%04u-%02d-%02d\n%02d:%02d:00\n",
