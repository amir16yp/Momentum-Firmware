// Based on parsers written by Leptoptilos and Assasinfil. Also, thanks to WillyJL
// (<me@willyjl.dev>) for help!
...
static inline void sz_uic_to_sta(Storage *storage, const char *file_name, PPKData *ticket)
{
    ...
    if (flipper_format_file_open_existing(file, file_name)) {
        ...
        flipper_format_read_string(file, furi_string_get_cstr(destination_uic),
                                   ticket->destination_name);
        flipper_format_rewind(file);
        flipper_format_read_string(file, furi_string_get_cstr(trip_start_uic),
                                   ticket->trip_start_sta_name);
        flipper_format_rewind(file);
        flipper_format_read_string(file, furi_string_get_cstr(trip_end_uic),
                                   ticket->trip_end_sta_name);
    }
...
static inline void extract_purse_data(const MfClassicData *data, PlantainData *purse,
                                      PPKData *ticket, uint8_t first_ppk_ticket_offset,
                                      uint8_t second_ppk_ticket_offset)
{
    ...
    if (data->type == MfClassicType1k) {
        ...
        for (uint8_t i = 0; i < 3; i++) {
            purse->last_payment_date_data =
                (purse->last_payment_date_data << 8) | data->block[18].data[4 - i];
        }
...
static inline void extract_ppk_data(Storage *storage, const MfClassicData *data, PPKData *ticket,
                                    bool second_ticket)
{
    ...
    const uint32_t tap_timestamp = PPK_CURRENT_EPOCH_START + ticket->tap_data * SECONDS_IN_A_MINUTE;
...
static void printf_plantain_data(FuriString *parsed_data, PlantainData *purse)
{
    ...
    furi_string_cat_printf(parsed_data,
                           ...
                           furi_string_get_cstr(purse->card_number_str), purse->balance,
                           purse->trips_metro, purse->trips_ground, purse->last_trip_time.day,
                           purse->last_trip_time.month, purse->last_trip_time.year,
                           purse->last_trip_time.hour, purse->last_trip_time.minute);

    furi_string_cat_printf(
        parsed_data,
        "\nValidator: %d\nFare: %d RUB\nRefilled on: %02d.%02d.%04d %02d:%02d\nAmount: %d RUB",
        purse->validator, purse->fare, purse->last_payment_date.day, purse->last_payment_date.month,
        purse->last_payment_date.year, purse->last_payment_date.hour,
        purse->last_payment_date.minute, purse->last_payment_amount);
...
static void printf_ppk_data(FuriString *parsed_data, PPKData *ticket, bool ticket_number)
{
    if (ticket_number == 0) {
        ...
        case 0x36:
            ...
            break;
        default:
            furi_string_cat_printf(parsed_data, "Type:> Unknown, 0x%02X",
    ...
    if (ticket->valid_from.day == ticket->valid_till.day) {
        furi_string_cat_printf(parsed_data, "\nValid On: %02d-%02d-%04d", ticket->valid_from.day,
                               ticket->valid_from.month, ticket->valid_from.year);
    } else {
        furi_string_cat_printf(
            parsed_data, "\nValid From: %02d-%02d-%04d\nValid thru:  %02d-%02d-%04d",
            ticket->valid_from.day, ticket->valid_from.month, ticket->valid_from.year,
            ticket->valid_till.day, ticket->valid_till.month, ticket->valid_till.year);
    }
    ...
    if (ticket->current_status == 0) {
        ...
        furi_string_cat_printf(parsed_data,
                               ...
                               ticket->tap_time.day, ticket->tap_time.month, ticket->tap_time.year,
                               ticket->tap_time.hour, ticket->tap_time.minute);
...
static bool plantain_verify_type(Nfc *nfc, MfClassicType type)
{
    ...
    do {
        ...
        if (!plantain_get_card_config(&cfg, type))
            break;
...
{
    return plantain_verify_type(nfc, MfClassicType1k) || plantain_verify_type(nfc, MfClassicType4k);
}
...
static bool plantain_read(Nfc *nfc, NfcDevice *device)
{
    ...
    do {
        ...
        data->type = type;
        PlantainCardConfig cfg = {0};
        if (!plantain_get_card_config(&cfg, data->type))
            break;
        ...
        MfClassicKey key = {0};
        bit_lib_num_to_bytes_be(plantain_4k_keys_legacy[legacy_check_sec_num].a, COUNT_OF(key.data),
                                key.data);

        error =
            mf_classic_poller_sync_auth(nfc, legacy_check_block_num, &key, MfClassicKeyTypeA, NULL);
        if (error == MfClassicErrorNone) {
...
static bool plantain_parse(const NfcDevice *device, FuriString *parsed_data)
{
    ...
    do {
        ...
        if (key != cfg.keys[cfg.data_sector].a)
            break;
