...
static MfClassicKeyPair banapass_keys_if_value_block[] = {
    ...
    {.a = 0xCC5075E42BA1, .b = 0xB9DF35A0814C}, {.a = 0x8AF9C718F23D, .b = 0x58CD5C3673CB},
    {.a = 0xFC80E88EB88C, .b = 0x7A3CDAD7C023}, {.a = 0x30424C029001, .b = 0x024E4E44001F},
    {.a = 0xECBBFA57C6AD, .b = 0x4757698143BD}, {.a = 0x1D30972E6485, .b = 0xF8526D1A8D6D},
    {.a = 0x1300EC8C7E80, .b = 0xF80A65A87FFA}, {.a = 0xDEB06ED4AF8E, .b = 0x4AD96BF28190},
    {.a = 0x000390014D41, .b = 0x0800F9917CB0}, {.a = 0x730050555253, .b = 0x4146D4A956C4},
    {.a = 0x131157FBB126, .b = 0xE69DD9015A43}, {.a = 0x337237F254D5, .b = 0x9A8389F32FBF},
    {.a = 0x7B8FB4A7100B, .b = 0xC8382A233993}, {.a = 0x7B304F2A12A6, .b = 0xFC9418BF788B},
};
...
static bool banapass_verify(Nfc *nfc)
{
    ...
    bit_lib_num_to_bytes_be(banapass_keys_if_value_block[0].a, COUNT_OF(key_a_0.data),
                            key_a_0.data);
...
static bool banapass_read(Nfc *nfc, NfcDevice *device)
{
    ...
    do {
        ...
        for (size_t i = 0; i < mf_classic_get_total_sectors_num(data->type); i++) {
            bit_lib_num_to_bytes_be(banapass_keys_if_access_code[i].a, sizeof(MfClassicKey),
                                    keys.key_a[i].data);
            FURI_BIT_SET(keys.key_a_mask, i);
            bit_lib_num_to_bytes_be(banapass_keys_if_access_code[i].b, sizeof(MfClassicKey),
                                    keys.key_b[i].data);
        ...
        for (size_t i = 0; i < mf_classic_get_total_sectors_num(data->type); i++) {
            bit_lib_num_to_bytes_be(banapass_keys_if_value_block[i].a, sizeof(MfClassicKey),
                                    keys.key_a[i].data);
            FURI_BIT_SET(keys.key_a_mask, i);
            bit_lib_num_to_bytes_be(banapass_keys_if_value_block[i].b, sizeof(MfClassicKey),
                                    keys.key_b[i].data);
            FURI_BIT_SET(keys.key_b_mask, i);
        }
...
static bool banapass_parse(const NfcDevice *device, FuriString *parsed_data)
{
    ...
    do {
        ...
        if (key_a != banapass_keys_if_value_block[0].a)
            break;
        ...
        if (is_block_2_null) {
            ...
            case banapass_key_b_value_block:
                int32_t value = 0;
                uint8_t addr = 0;
                bool value_found = mf_classic_block_to_value(&data->block[2], &value,
                                                             &addr); // block 2 is value block
                if (value_found) {
            ...
            case banapass_key_b_access_code:
                // banapass access code is stored as decimal hex representation in block 2, starts
                // from byte 6, len 10 bytes
                ...
                for (int i = 0; i < 10; i++) {
                    ...
                    if ((access_code[i] >> 4) > 9)
                        access_code_is_bcd = false;
                    if ((access_code[i] & 0x0F) > 9)
                        access_code_is_bcd = false;
                }
                furi_string_cat_printf(parsed_data, "\nBCD valid: %s\n",
