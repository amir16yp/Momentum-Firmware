...
static const uint8_t STANDARD_KEYS[3][6] = {{0x20, 0x31, 0xD1, 0xE5, 0x7A, 0x3B},
...
static const char *const CONCESSION_TYPES[] = {"Pre-issue",  "Standard Fare",
                                               "Student",    NULL,
                                               "Tertiary",   NULL,
                                               "Seniors",    "Health Care",
                                               NULL,         NULL,
...
static bool authenticate_and_read(Nfc *nfc, uint8_t sector, const uint8_t *key,
                                  MfClassicKeyType key_type, MfClassicBlock *block_data)
{
    MfClassicKey mf_key;
...
static bool smartrider_verify(Nfc *nfc)
{
    ...
    for (int i = 0; i < 3; i++) {
        if (!authenticate_and_read(nfc, i * 6, STANDARD_KEYS[i],
...
static inline bool parse_trip_data(const MfClassicBlock *block_data, TripData *trip,
...
static bool smartrider_parse(const NfcDevice *device, FuriString *parsed_data)
{
    ...
    snprintf(sr_data.card_serial_number, sizeof(sr_data.card_serial_number), "%02X%02X%02X%02X%02X",
             data->block[1].data[6], data->block[1].data[7], data->block[1].data[8],
             data->block[1].data[9], data->block[1].data[10]);
    ...
    for (uint8_t block_number = 40; block_number <= 52 && sr_data.trip_count < MAX_TRIPS;
        ...
        if ((block_number != 43 && block_number != 47 && block_number != 51) &&
            ...
            parse_trip_data(&data->block[block_number], &sr_data.trips[sr_data.trip_count],
                            block_number)) {
            sr_data.trip_count++;
        }
    }
    ...
    furi_string_printf(
        ...
        sr_data.balance / 100, sr_data.balance % 100, get_concession_type(sr_data.token),
        ...
        sr_data.purchase_cost / 100, sr_data.purchase_cost % 100, sr_data.auto_load_threshold / 100,
        sr_data.auto_load_threshold % 100, sr_data.auto_load_value / 100,
    ...
    for (uint8_t i = 0; i < sr_data.trip_count; i++) {
        ...
        if (cost > 0) {
            furi_string_cat_printf(parsed_data, "%s %c $%lu.%02lu %s\n", date_str,
                                   sr_data.trips[i].tap_on ? '+' : '-', cost / 100, cost % 100,
                                   sr_data.trips[i].route);
        } else {
            furi_string_cat_printf(parsed_data, "%s %c %s\n", date_str,
                                   sr_data.trips[i].tap_on ? '+' : '-', sr_data.trips[i].route);
        }
    }
