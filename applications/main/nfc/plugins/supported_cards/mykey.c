...
static bool mykey_get_production_date(const St25tbData *data, uint16_t *year_ptr,
                                      uint8_t *month_ptr, uint8_t *day_ptr)
{
    ...
    // dates are coded in a peculiar way, the hexadecimal value should in fact be interpreted as a
    // decimal value so anything in range A-F is invalid.
...
static bool mykey_parse(const NfcDevice *device, FuriString *parsed_data)
{
    ...
    furi_string_cat_printf(parsed_data, "Prod. date: %02X/%02X/%04X\n", mfg_day, mfg_month,
                           mfg_year);
    ...
    if (!is_blank) {
        ...
        if (block3C == 0xFFFFFFFF) {
            ...
            for (int txnOffset = 8; txnOffset > 0; txnOffset--) {
                ...
                if (txnOffset == 8) {
                    furi_string_cat_printf(parsed_data, "Current credit: %d.%02d euros\n",
                                           credit / 100, credit % 100);
                    furi_string_cat(parsed_data, "Op. history (newest first):");
                }

                furi_string_cat_printf(parsed_data, "\n    %02d/%02d/%04d %d.%02d", day, month,
                                       year, credit / 100, credit % 100);
            }
        }
    }
    return true;
