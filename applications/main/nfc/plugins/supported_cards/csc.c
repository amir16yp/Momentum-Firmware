...
bool csc_parse(const NfcDevice *device, FuriString *parsed_data)
{
    ...
    do {
        ...
        // This is zero when you buy the card. but after refilling it, the refilling machine will
        // leave a non-zero signature here
        ...
        if (refill_sign == 0 && refill_times == 1) {
            ...
            furi_string_printf(parsed_data,
                               ...
                               "Card Usages Left: %lu",
                               card_uid, refilled_balance_dollar, refilled_balance_cent,
                               card_lives);
        } else {
            if (xor_result != 0) {
            ...
            furi_string_printf(parsed_data,
                               ...
                               "Top-up Count: %lu\n"
                               "Card Usages Left: %lu",
                               card_uid, current_balance_dollar, current_balance_cent,
                               refilled_balance_dollar, refilled_balance_cent, refill_times,
                               card_lives);
        }
