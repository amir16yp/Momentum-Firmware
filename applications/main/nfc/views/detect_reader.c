...
static void detect_reader_draw_callback(Canvas *canvas, void *model)
{
    ...
    if (m->state == DetectReaderStateStart) {
        ...
        if (furi_string_size(m->uid_str)) {
            elements_multiline_text_aligned(canvas, 64, 64, AlignCenter, AlignBottom,
                                            furi_string_get_cstr(m->uid_str));
        }
...
DetectReader *detect_reader_alloc(void)
{
    ...
    with_view_model(
        ...
        { model->uid_str = furi_string_alloc(); }, false);
...
void detect_reader_free(DetectReader *detect_reader)
{
    ...
    with_view_model(
        detect_reader->view, DetectReaderViewModel * model, { furi_string_free(model->uid_str); },
        false);
...
void detect_reader_set_callback(DetectReader *detect_reader, DetectReaderDoneCallback callback,
                                void *context)
{
    furi_assert(detect_reader);
...
void detect_reader_set_nonces_max(DetectReader *detect_reader, uint16_t nonces_max)
{
    ...
    with_view_model(
        detect_reader->view, DetectReaderViewModel * model, { model->nonces_max = nonces_max; },
        false);
}
