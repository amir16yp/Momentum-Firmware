...
static void dict_attack_draw_mf_classic(Canvas *canvas, DictAttackViewModel *m)
{
    ...
    if (m->nested_phase == MfClassicNestedPhaseCollectNtEnc) {
        ...
        snprintf(draw_str, sizeof(draw_str), "Reuse key check for sector: %d",
    ...
    if (m->nested_phase == MfClassicNestedPhaseAnalyzePRNG ||
    ...
    } else if (m->nested_phase == MfClassicNestedPhaseCalibrate ||
        ...
        if (m->dict_keys_current == 0) {
            ...
            snprintf(draw_str, sizeof(draw_str), "%zu/%zu", m->dict_keys_current,
                     m->dict_keys_total);
        }
    }
    ...
    canvas_set_font(canvas, FontSecondary);
    snprintf(draw_str, sizeof(draw_str), "Keys found: %d/%d", m->keys_found,
...
static void dict_attack_draw_callback(Canvas *canvas, void *model)
{
    ...
    if (!m->card_detected) {
        ...
        canvas_set_font(canvas, FontSecondary);
        elements_multiline_text_aligned(canvas, 64, 23, AlignCenter, AlignTop,
...
DictAttack *dict_attack_alloc(void)
{
    ...
    with_view_model(
        instance->view, DictAttackViewModel * model, { model->header = furi_string_alloc(); },
        false);
...
void dict_attack_reset(DictAttack *instance)
{
    ...
    with_view_model(
        instance->view, DictAttackViewModel * model,
        {
...
void dict_attack_set_header(DictAttack *instance, const char *header)
{
    ...
    with_view_model(
        instance->view, DictAttackViewModel * model, { furi_string_set(model->header, header); },
        true);
}
...
void dict_attack_set_sectors_total(DictAttack *instance, uint8_t sectors_total)
{
    ...
    with_view_model(
        instance->view, DictAttackViewModel * model, { model->sectors_total = sectors_total; },
        true);
}
...
void dict_attack_set_current_sector(DictAttack *instance, uint8_t current_sector)
{
    ...
    with_view_model(
        instance->view, DictAttackViewModel * model, { model->current_sector = current_sector; },
        true);
}
...
void dict_attack_set_total_dict_keys(DictAttack *instance, size_t dict_keys_total)
{
    ...
    with_view_model(
        instance->view, DictAttackViewModel * model, { model->dict_keys_total = dict_keys_total; },
        true);
}
...
void dict_attack_set_current_dict_key(DictAttack *instance, size_t cur_key_num)
{
    ...
    with_view_model(
        instance->view, DictAttackViewModel * model, { model->dict_keys_current = cur_key_num; },
        true);
}
...
void dict_attack_set_nested_target_key(DictAttack *instance, uint16_t nested_target_key)
{
    ...
    with_view_model(
        instance->view, DictAttackViewModel * model,
        { model->nested_target_key = nested_target_key; }, true);
}
