...
void lfrfid_scene_raw_info_on_enter(void *context)
{
    ...
    if (storage_sd_status(app->storage) != FSE_OK) {
        ...
        widget_add_string_element(widget, 64, 0, AlignCenter, AlignTop, FontPrimary, "No SD Card!");
        widget_add_string_multiline_element(widget, 0, 13, AlignLeft, AlignTop, FontSecondary,
                                            "Insert an SD card\n"
                                            "to use this function");

    } else {
        widget_add_text_box_element(widget, 0, 0, 128, 64, AlignLeft, AlignTop,
