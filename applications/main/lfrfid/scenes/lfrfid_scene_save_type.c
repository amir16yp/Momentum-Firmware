...
void lfrfid_scene_save_type_on_enter(void *context)
{
    ...
    for (uint8_t i = 0; i < LFRFIDProtocolMax; i++) {
        if ((strcmp(protocol_dict_get_manufacturer(app->dict, i),
                    protocol_dict_get_name(app->dict, i)) != 0) &&
            (strcmp(protocol_dict_get_manufacturer(app->dict, i), "N/A") != 0)) {
            furi_string_printf(protocol_string, "%s %s",
        ...
        submenu_add_item(submenu, furi_string_get_cstr(protocol_string), i,
                         lfrfid_scene_save_type_submenu_callback, app);
    }
