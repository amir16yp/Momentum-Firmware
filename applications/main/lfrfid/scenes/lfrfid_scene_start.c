...
void lfrfid_scene_start_on_enter(void *context)
{
    ...
    submenu_add_item(submenu, "Read", LfRfidMenuIndexRead, lfrfid_scene_start_submenu_callback,
                     app);
    submenu_add_item(submenu, "Saved", LfRfidMenuIndexSaved, lfrfid_scene_start_submenu_callback,
                     app);
    submenu_add_item(submenu, "Add Manually", LfRfidMenuIndexAddManually,
                     lfrfid_scene_start_submenu_callback, app);
    submenu_add_item(submenu, "Extra Actions", LfRfidMenuIndexExtraActions,
                     lfrfid_scene_start_submenu_callback, app);

    submenu_set_selected_item(submenu,
                              scene_manager_get_scene_state(app->scene_manager, LfRfidSceneStart));
...
bool lfrfid_scene_start_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == LfRfidMenuIndexRead) {
            scene_manager_set_scene_state(app->scene_manager, LfRfidSceneStart,
                                          LfRfidMenuIndexRead);
