...
void lfrfid_scene_enter_password_on_enter(void *context)
{
    ...
    byte_input_set_result_callback(byte_input, lfrfid_text_input_callback, NULL, app, app->password,
                                   4);
...
bool lfrfid_scene_enter_password_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        scene_manager_search_and_switch_to_previous_scene_one_of(scene_manager, prev_scenes,
                                                                 sizeof(prev_scenes[0]));
    }
