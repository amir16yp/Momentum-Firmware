...
static void momentum_app_scene_interface_mainmenu_add_main_submenu_callback(void *context,
                                                                            uint32_t index)
{
...
void momentum_app_scene_interface_mainmenu_add_main_on_enter(void *context)
{
    ...
    for (size_t i = 0; i < FLIPPER_APPS_COUNT; i++) {
        submenu_add_item(submenu, FLIPPER_APPS[i].name, (uint32_t)FLIPPER_APPS[i].name,
                         momentum_app_scene_interface_mainmenu_add_main_submenu_callback, app);
    }
    for (size_t i = 0; i < FLIPPER_EXTERNAL_APPS_COUNT - 1; i++) {
        submenu_add_item(submenu, FLIPPER_EXTERNAL_APPS[i].name,
...
bool momentum_app_scene_interface_mainmenu_add_main_on_event(void *context, SceneManagerEvent event)
{
    UNUSED(context);
    bool consumed = false;
