...
void momentum_app_scene_interface_general_on_enter(void *context)
{
    ...
    item = variable_item_list_add(var_item_list, "Text Scroll", 2,
                                  momentum_app_scene_interface_general_scroll_marquee_changed, app);
    variable_item_set_current_value_index(item, momentum_settings.scroll_marquee);
    variable_item_set_current_value_text(item,
                                         momentum_settings.scroll_marquee ? "Marquee" : "Standard");

    item =
        variable_item_list_add(var_item_list, "Clock Midnight Format", 2,
                               momentum_app_scene_interface_general_midnight_format_changed, app);
    variable_item_set_current_value_index(item, momentum_settings.midnight_format_00);
    variable_item_set_current_value_text(item,
                                         momentum_settings.midnight_format_00 ? "00:XX" : "12:XX");

    item = variable_item_list_add(var_item_list, "Popup Overlay", 2,
                                  momentum_app_scene_interface_general_popup_overlay_changed, app);
...
bool momentum_app_scene_interface_general_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(app->scene_manager, MomentumAppSceneInterfaceGeneral,
                                      event.event);
        consumed = true;
    }
