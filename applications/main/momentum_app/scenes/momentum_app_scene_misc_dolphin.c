...
static void momentum_app_scene_misc_dolphin_dolphin_level_changed(VariableItem *item)
{
    ...
    variable_item_set_current_value_index(item, app->dolphin_xp == 0                ? 0
...
static void momentum_app_scene_misc_dolphin_dolphin_xp_changed(VariableItem *item)
{
    ...
    if (app->dolphin_xp == DOLPHIN_MAX_XP)
        direction = 0;
    if (app->dolphin_xp == 0)
        direction = 2;
    if (direction == 0)
...
void momentum_app_scene_misc_dolphin_on_enter(void *context)
{
    ...
    char level_str[4];
    snprintf(level_str, sizeof(level_str), "%u", level);
    item = variable_item_list_add(var_item_list, "Dolphin Level", DOLPHIN_LEVEL_COUNT + 1,
                                  momentum_app_scene_misc_dolphin_dolphin_level_changed, app);
    ...
    // uin8_t index too small for all levels, use 3 fake items to
    // show buttons and change values in callback
    item = variable_item_list_add(var_item_list, "Dolphin XP", 3,
                                  momentum_app_scene_misc_dolphin_dolphin_xp_changed, app);
    variable_item_set_current_value_index(item, app->dolphin_xp == 0                ? 0
                                                : app->dolphin_xp == DOLPHIN_MAX_XP ? 2
                                                                                    : 1);
    variable_item_set_current_value_text(item, xp_str);
    ...
    char angry_str[4];
    snprintf(angry_str, sizeof(angry_str), "%lu", app->dolphin_angry);
    item = variable_item_list_add(var_item_list, "Dolphin Angry", BUTTHURT_MAX + 1,
                                  momentum_app_scene_misc_dolphin_dolphin_angry_changed, app);
    variable_item_set_current_value_index(item, app->dolphin_angry);
    variable_item_set_current_value_text(item, angry_str);
    variable_item_set_locked(item, settings.happy_mode,
                             "Settings >\n"
                             "Desktop >\n"
                             "Happy Mode\n"
                             "is enabled!");

    item = variable_item_list_add(var_item_list, "Butthurt Timer", COUNT_OF(butthurt_timer_names),
                                  momentum_app_scene_misc_dolphin_butthurt_timer_changed, app);
    value_index = value_index_uint32(momentum_settings.butthurt_timer, butthurt_timer_values,
                                     COUNT_OF(butthurt_timer_values));
...
bool momentum_app_scene_misc_dolphin_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(app->scene_manager, MomentumAppSceneMiscDolphin, event.event);
        consumed = true;
        switch (event.event) {
