...
const char *const menu_style_names[MenuStyleCount] = {
    "List", "Wii", "DSi", "PS4", "Vertical", "C64", "Compact", "Rectum", "CoverFlow",
};
...
void momentum_app_scene_interface_mainmenu_on_enter(void *context)
{
    ...
    item = variable_item_list_add(var_item_list, "Menu Style", MenuStyleCount,
                                  momentum_app_scene_interface_mainmenu_menu_style_changed, app);
    ...
    size_t count = CharList_size(app->mainmenu_app_labels);
    item = variable_item_list_add(var_item_list, "Item", count,
                                  momentum_app_scene_interface_mainmenu_app_changed, app);
    if (count) {
    ...
    item = variable_item_list_add(var_item_list, "Move Item", 3,
                                  momentum_app_scene_interface_mainmenu_move_app_changed, app);
...
bool momentum_app_scene_interface_mainmenu_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(app->scene_manager, MomentumAppSceneInterfaceMainmenu,
                                      event.event);
        consumed = true;
        switch (event.event) {
        ...
        case VarItemListIndexRemoveItem:
            if (!CharList_size(app->mainmenu_app_labels))
                break;
            if (!CharList_size(app->mainmenu_app_exes))
                break;
            ...
            CharList_remove_v(app->mainmenu_app_labels, app->mainmenu_app_index,
                              app->mainmenu_app_index + 1);
            CharList_remove_v(app->mainmenu_app_exes, app->mainmenu_app_index,
                              app->mainmenu_app_index + 1);
            /* fall through */
