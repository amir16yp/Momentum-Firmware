...
static bool momentum_app_scene_misc_spoof_name_validator(const char *text, FuriString *error,
                                                         void *context)
{
    UNUSED(context);
...
void momentum_app_scene_misc_spoof_name_on_enter(void *context)
{
    ...
    text_input_set_result_callback(text_input,
                                   momentum_app_scene_misc_spoof_name_text_input_callback, app,
                                   app->device_name, FURI_HAL_VERSION_ARRAY_NAME_LENGTH, true);
