...
void momentum_app_scene_protocols_freqs_static_var_item_list_callback(void *context, uint32_t index)
{
...
void momentum_app_scene_protocols_freqs_static_on_enter(void *context)
{
    ...
    item = variable_item_list_add(var_item_list, "Static Freq",
                                  FrequencyList_size(app->subghz_static_freqs),
                                  momentum_app_scene_protocols_freqs_static_frequency_changed, app);
