...
const char *const rainbow_interval_names[] = {
    "0.1 S", "0.2 S",  "0.25 S", "0.5 S", "0.75 S", "1 S", "1.25 S",
    "1.5 S", "1.75 S", "2 S",    "2.5 S", "3 S",    "4 S", "5 S",
};
const uint32_t rainbow_interval_values[COUNT_OF(rainbow_interval_names)] = {
    100, 200, 250, 500, 750, 1000, 1250, 1500, 1750, 2000, 2500, 3000, 4000, 5000,
};
...
void momentum_app_scene_misc_screen_on_enter(void *context)
{
    ...
    item = variable_item_list_add(var_item_list, "Dark Mode", 2,
                                  momentum_app_scene_misc_screen_dark_mode_changed, app);
    ...
    item = variable_item_list_add(var_item_list, "Left Handed", 2,
                                  momentum_app_scene_misc_screen_hand_orient_changed, app);
    ...
    item = variable_item_list_add(var_item_list, "RGB Backlight", 2,
                                  momentum_app_scene_misc_screen_rgb_backlight_changed, app);
    ...
    item = variable_item_list_add(var_item_list, "Rainbow LCD", RGBBacklightRainbowModeCount,
                                  momentum_app_scene_misc_screen_rainbow_lcd_changed, app);
    value_index = rgb_backlight_get_rainbow_mode();
    variable_item_set_current_value_index(item, value_index);
    variable_item_set_current_value_text(item, rainbow_lcd_names[value_index]);
    variable_item_set_locked(item, !momentum_settings.rgb_backlight, "Needs RGB\nBacklight!");

    item = variable_item_list_add(var_item_list, "Rainbow Speed", 25,
                                  momentum_app_scene_misc_screen_rainbow_speed_changed, app);
    ...
    item =
        variable_item_list_add(var_item_list, "Rainbow Interval", COUNT_OF(rainbow_interval_values),
                               momentum_app_scene_misc_screen_rainbow_interval_changed, app);
    value_index = value_index_uint32(rgb_backlight_get_rainbow_interval(), rainbow_interval_values,
                                     COUNT_OF(rainbow_interval_values));
    variable_item_set_current_value_index(item, value_index);
    variable_item_set_current_value_text(item, rainbow_interval_names[value_index]);
    variable_item_set_locked(item, !momentum_settings.rgb_backlight, "Needs RGB\nBacklight!");

    item = variable_item_list_add(var_item_list, "Rainbow Saturation", 255,
                                  momentum_app_scene_misc_screen_rainbow_saturation_changed, app);
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        case VarItemListIndexRgbBacklight: {
            ...
            if (change) {
                ...
                variable_item_set_locked(
                    variable_item_list_get(app->var_item_list, VarItemListIndexRainbowLcd), !value,
                    "Needs RGB\nBacklight!");
                variable_item_set_locked(
        ...
        case VarItemListIndexLcdColor2:
            scene_manager_set_scene_state(app->scene_manager, MomentumAppSceneMiscScreenColor,
