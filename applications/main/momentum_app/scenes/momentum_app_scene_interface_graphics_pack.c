...
void momentum_app_scene_interface_graphics_pack_on_enter(void *context)
{
    ...
    submenu_add_item(submenu, "Default", 0,
                     momentum_app_scene_interface_graphics_pack_submenu_callback, app);

    for (size_t i = 0; i < CharList_size(app->asset_pack_names); i++) {
        submenu_add_item(submenu, *CharList_get(app->asset_pack_names, i), i + 1,
                         momentum_app_scene_interface_graphics_pack_submenu_callback, app);
    }
