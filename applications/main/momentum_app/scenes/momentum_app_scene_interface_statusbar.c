...
void momentum_app_scene_interface_statusbar_on_enter(void *context)
{
    ...
    item = variable_item_list_add(var_item_list, "Battery Icon", BatteryIconCount,
                                  momentum_app_scene_interface_statusbar_battery_icon_changed, app);
    ...
    item =
        variable_item_list_add(var_item_list, "Show Clock", 2,
                               momentum_app_scene_interface_statusbar_statusbar_clock_changed, app);
    ...
    item = variable_item_list_add(var_item_list, "Status Icons", 2,
                                  momentum_app_scene_interface_statusbar_status_icons_changed, app);
    ...
    item = variable_item_list_add(var_item_list, "Bar Borders", 2,
                                  momentum_app_scene_interface_statusbar_bar_borders_changed, app);
    ...
    item =
        variable_item_list_add(var_item_list, "Bar Background", 2,
                               momentum_app_scene_interface_statusbar_bar_background_changed, app);
...
bool momentum_app_scene_interface_statusbar_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(app->scene_manager, MomentumAppSceneInterfaceStatusbar,
                                      event.event);
        consumed = true;
        switch (event.event) {
        default:
