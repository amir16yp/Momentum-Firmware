...
void bad_usb_scene_config_ble_name_on_enter(void *context)
{
    ...
    strlcpy(bad_usb->ble_name_buf, bad_usb->script_hid_cfg.ble.name, sizeof(bad_usb->ble_name_buf));
    text_input_set_header_text(text_input, "Set BLE device name");

    text_input_set_result_callback(text_input, bad_usb_scene_config_ble_name_text_input_callback,
                                   bad_usb, bad_usb->ble_name_buf, sizeof(bad_usb->ble_name_buf),
                                   true);
...
bool bad_usb_scene_config_ble_name_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        if (event.event == TextInputResultOk) {
            ...
            strlcpy(bad_usb->script_hid_cfg.ble.name, bad_usb->ble_name_buf,
            ...
            strlcpy(bad_usb->user_hid_cfg.ble.name, bad_usb->ble_name_buf,
