...
void bad_usb_scene_config_connection_callback(VariableItem *item)
{
    ...
    bad_usb_set_interface(bad_usb, bad_usb->interface == BadUsbHidInterfaceBle
                                       ? BadUsbHidInterfaceUsb
                                       : BadUsbHidInterfaceBle);
    variable_item_set_current_value_text(item, bad_usb->interface == BadUsbHidInterfaceBle ? "BLE"
                                                                                           : "USB");
...
static void draw_menu(BadUsbApp *bad_usb)
{
    ...
    item = variable_item_list_add(var_item_list, "Connection", 2,
                                  bad_usb_scene_config_connection_callback, bad_usb);
    ...
    if (bad_usb->interface == BadUsbHidInterfaceBle) {
        ...
        item = variable_item_list_add(var_item_list, "Persist Pairing", 2,
                                      bad_usb_scene_config_ble_persist_pairing_callback, bad_usb);
        ...
        item = variable_item_list_add(var_item_list, "Pairing Mode", GapPairingCount,
                                      bad_usb_scene_config_ble_pairing_mode_callback, bad_usb);
...
void bad_usb_scene_config_on_enter(void *context)
{
    ...
    variable_item_list_set_enter_callback(var_item_list, bad_usb_scene_config_select_callback,
                                          bad_usb);
    draw_menu(bad_usb);
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        if (bad_usb->interface == BadUsbHidInterfaceBle) {
            ...
            case ConfigIndexBleRandomizeMacAddress:
                // Apply to current script config
                furi_hal_random_fill_buf(bad_usb->script_hid_cfg.ble.mac,
                ...
                hid->adjust_config(&bad_usb->script_hid_cfg);
                // Set in user config to save in settings file
                memcpy(bad_usb->user_hid_cfg.ble.mac, bad_usb->script_hid_cfg.ble.mac,
            ...
            case ConfigIndexBleRestoreDefaults:
                ...
                memset(bad_usb->script_hid_cfg.ble.mac, 0, sizeof(bad_usb->script_hid_cfg.ble.mac));
                bad_usb->script_hid_cfg.ble.bonding = true;
                bad_usb->script_hid_cfg.ble.pairing = GapPairingPinCodeVerifyYesNo;
                hid->adjust_config(&bad_usb->script_hid_cfg);
                // Set in user config to save in settings file
                memcpy(&bad_usb->user_hid_cfg.ble, &bad_usb->script_hid_cfg.ble,
            ...
            case ConfigIndexUsbSetManufacturerName:
                scene_manager_set_scene_state(bad_usb->scene_manager, BadUsbSceneConfigUsbName,
                                              true);
            ...
            case ConfigIndexUsbRandomizeVidPid:
                furi_hal_random_fill_buf((void *)bad_usb->usb_vidpid_buf,
            ...
            case ConfigIndexUsbRestoreDefaults:
                ...
                hid->adjust_config(&bad_usb->script_hid_cfg);
                // Set in user config to save in settings file
                memcpy(&bad_usb->user_hid_cfg.usb, &bad_usb->script_hid_cfg.usb,
