...
{
    if (hid_cfg->usb.vid == 0)
        hid_cfg->usb.vid = HID_VID_DEFAULT;
    if (hid_cfg->usb.pid == 0)
        hid_cfg->usb.pid = HID_PID_DEFAULT;
}
...
void hid_ble_adjust_config(BadUsbHidConfig *hid_cfg)
{
    ...
    if (hid_cfg->ble.name[0] == '\0') {
        ...
        snprintf(hid_cfg->ble.name, sizeof(hid_cfg->ble.name), "%s %s", badusb_device_name_prefix,
