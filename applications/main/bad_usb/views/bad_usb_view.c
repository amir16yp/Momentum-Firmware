...
static void bad_usb_draw_callback(Canvas *canvas, void *_model)
{
    ...
    elements_string_fit_width(canvas, disp_str, 128 - 2);
    canvas_draw_str(canvas, 2, 8 + canvas_current_font_height(canvas),
    ...
    if (state == BadUsbStateNotConnected) {
        ...
        canvas_draw_str_aligned(canvas, 127, 46, AlignRight, AlignBottom,
                                furi_string_get_cstr(disp_str));
        ...
        canvas_draw_str_aligned(canvas, 124, 47, AlignRight, AlignBottom,
                                furi_string_get_cstr(disp_str));
        ...
        canvas_draw_str_aligned(canvas, 124, 47, AlignRight, AlignBottom,
                                furi_string_get_cstr(disp_str));
        canvas_set_font(canvas, FontBigNumbers);
        furi_string_printf(disp_str, "%zu",
                           ((model->state.line_cur - 1) * 100) / model->state.line_nb);
        canvas_draw_str_aligned(canvas, 112, 37, AlignRight, AlignBottom,
                                furi_string_get_cstr(disp_str));
        canvas_draw_icon(canvas, 115, 23, &I_Percent_10x14);
        ...
        if (delay) {
            ...
            canvas_draw_str_aligned(canvas, 4, 61, AlignLeft, AlignBottom,
                                    furi_string_get_cstr(disp_str));
        }
        furi_string_printf(disp_str, "%zu/%zu", model->state.line_cur, model->state.line_nb);
        canvas_draw_str_aligned(canvas, 124, 47, AlignRight, AlignBottom,
...
void bad_usb_view_set_button_callback(BadUsb *bad_usb, BadUsbButtonCallback callback, void *context)
{
    furi_assert(bad_usb);
...
void bad_usb_view_set_file_name(BadUsb *bad_usb, const char *name)
{
    furi_assert(name);
    with_view_model(
        bad_usb->view, BadUsbModel * model, { strlcpy(model->file_name, name, MAX_NAME_LEN); },
        true);
}
...
void bad_usb_view_set_layout(BadUsb *bad_usb, const char *layout)
{
    furi_assert(layout);
    with_view_model(
        bad_usb->view, BadUsbModel * model, { strlcpy(model->layout, layout, MAX_NAME_LEN); },
        true);
}
