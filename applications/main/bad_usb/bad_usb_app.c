    ...
    if (flipper_format_file_open_existing(fff, BAD_USB_SETTINGS_PATH)) {
        do {
            ...
            if (flipper_format_read_string(fff, "ble_name", temp_str)) {
                strlcpy(hid_cfg->ble.name, furi_string_get_cstr(temp_str),
            ...
            if (!flipper_format_read_hex(fff, "ble_mac", hid_cfg->ble.mac,
                                         sizeof(hid_cfg->ble.mac))) {
                memset(hid_cfg->ble.mac, 0, sizeof(hid_cfg->ble.mac));
                flipper_format_rewind(fff);
            }

            if (flipper_format_read_string(fff, "usb_manuf", temp_str)) {
                strlcpy(hid_cfg->usb.manuf, furi_string_get_cstr(temp_str),
            ...
            if (flipper_format_read_string(fff, "usb_product", temp_str)) {
                strlcpy(hid_cfg->usb.product, furi_string_get_cstr(temp_str),
...
static void bad_usb_save_settings(BadUsbApp *app)
{
    ...
    if (flipper_format_file_open_always(fff, BAD_USB_SETTINGS_PATH)) {
        do {
            if (!flipper_format_write_header_cstr(fff, BAD_USB_SETTINGS_FILE_TYPE,
                                                  BAD_USB_SETTINGS_VERSION))
                break;
            if (!flipper_format_write_string(fff, "layout", app->keyboard_layout))
                break;
            temp_uint = app->interface;
            if (!flipper_format_write_uint32(fff, "interface", &temp_uint, 1))
                break;
            if (!flipper_format_write_bool(fff, "ble_bonding", &hid_cfg->ble.bonding, 1))
                break;
            temp_uint = hid_cfg->ble.pairing;
            if (!flipper_format_write_uint32(fff, "ble_pairing", &temp_uint, 1))
                break;
            if (!flipper_format_write_string_cstr(fff, "ble_name", hid_cfg->ble.name))
                break;
            if (!flipper_format_write_hex(fff, "ble_mac", (uint8_t *)&hid_cfg->ble.mac,
                                          sizeof(hid_cfg->ble.mac)))
                break;
            if (!flipper_format_write_string_cstr(fff, "usb_manuf", hid_cfg->usb.manuf))
                break;
            if (!flipper_format_write_string_cstr(fff, "usb_product", hid_cfg->usb.product))
                break;
            if (!flipper_format_write_uint32(fff, "usb_vid", &hid_cfg->usb.vid, 1))
                break;
            if (!flipper_format_write_uint32(fff, "usb_pid", &hid_cfg->usb.pid, 1))
                break;
        } while (0);
    }
...
TextInput *bad_usb_app_alloc_text_input(BadUsbApp *app)
{
    ...
    view_dispatcher_add_view(app->view_dispatcher, BadUsbAppViewTextInput,
...
ByteInput *bad_usb_app_alloc_byte_input(BadUsbApp *app)
{
    ...
    view_dispatcher_add_view(app->view_dispatcher, BadUsbAppViewByteInput,
...
BadUsbApp *bad_usb_app_alloc(char *arg)
{
    ...
    view_dispatcher_set_tick_event_callback(app->view_dispatcher, bad_usb_app_tick_event_callback,
                                            250);
    ...
    view_dispatcher_add_view(app->view_dispatcher, BadUsbAppViewWidget,
    ...
    view_dispatcher_add_view(app->view_dispatcher, BadUsbAppViewConfig,
                             variable_item_list_get_view(app->var_item_list));

    app->bad_usb_view = bad_usb_view_alloc();
    view_dispatcher_add_view(app->view_dispatcher, BadUsbAppViewWork,
                             bad_usb_view_get_view(app->bad_usb_view));

    app->text_input = NULL;
    app->byte_input = NULL;

    app->loading = loading_alloc();
    view_dispatcher_add_view(app->view_dispatcher, BadUsbAppViewLoading,
