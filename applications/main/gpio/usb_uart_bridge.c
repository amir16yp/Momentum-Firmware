...
static const CdcCallbacks cdc_cb = {
    vcp_on_cdc_tx_complete,  vcp_on_cdc_rx,      vcp_state_callback,
    vcp_on_cdc_control_line, vcp_on_line_config,
};
...
static void usb_uart_on_irq_rx_dma_cb(FuriHalSerialHandle *handle, FuriHalSerialRxEvent ev,
...
static int32_t usb_uart_worker(void *context)
{
    ...
    if (usb_uart->cfg.flow_pins != 0) {
        furi_assert((size_t)(usb_uart->cfg.flow_pins - 1) < COUNT_OF(flow_pins));
        furi_hal_gpio_init_simple(flow_pins[usb_uart->cfg.flow_pins - 1][0],
                                  GpioModeOutputPushPull);
        furi_hal_gpio_init_simple(flow_pins[usb_uart->cfg.flow_pins - 1][1],
                                  GpioModeOutputPushPull);
    ...
    while (1) {
        ...
        if (events & (WorkerEvtRxDone | WorkerEvtCdcTxComplete)) {
            size_t len = furi_stream_buffer_receive(usb_uart->rx_stream, usb_uart->rx_buf,
                                                    USB_CDC_PKT_LEN, 0);
            if (len > 0) {
                if (furi_semaphore_acquire(usb_uart->tx_sem, 100) == FuriStatusOk) {
                    usb_uart->st.rx_cnt += len;
                    furi_check(furi_mutex_acquire(usb_uart->usb_mutex, FuriWaitForever) ==
                               FuriStatusOk);
        ...
        if (events & WorkerEvtCfgChange) {
            ...
            if (usb_uart->cfg.flow_pins != usb_uart->cfg_new.flow_pins) {
                ...
                if (usb_uart->cfg_new.flow_pins != 0) {
                    ...
                    furi_hal_gpio_init_simple(flow_pins[usb_uart->cfg_new.flow_pins - 1][0],
                                              GpioModeOutputPushPull);
                    furi_hal_gpio_init_simple(flow_pins[usb_uart->cfg_new.flow_pins - 1][1],
                                              GpioModeOutputPushPull);
                }
            ...
            if (usb_uart->cfg.software_de_re != usb_uart->cfg_new.software_de_re) {
                ...
                if (usb_uart->cfg.software_de_re != 0) {
                    ...
                    furi_hal_gpio_init(USB_USART_DE_RE_PIN, GpioModeOutputPushPull, GpioPullNo,
                                       GpioSpeedMedium);
                } else {
                    furi_hal_gpio_init(USB_USART_DE_RE_PIN, GpioModeAnalog, GpioPullNo,
                                       GpioSpeedLow);
                }
            }
