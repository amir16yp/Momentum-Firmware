    ...
    if (model->items > 0) {
        ...
        for (int i = 0; i < model->items; i++) {
            ...
            if (i == 1 || model->items == 1) { // Draw a maximum of two addresses in the first line
                ...
                temp_str[0] = '\0';
            } else if (i == 7 || model->items - 1 ==
                                     i) { // Draw a maximum of three addresses in the third line
...
void gpio_i2c_scanner_set_ok_callback(GpioI2CScanner *gpio_i2c_scanner,
                                      GpioI2CScannerOkCallback callback, void *context)
{
    furi_assert(gpio_i2c_scanner);
