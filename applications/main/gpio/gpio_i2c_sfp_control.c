...
// This is map mapping the connector type to the appropriate name. (see SFF-8024 Rev. 4.9, Table
// 4-3)
const char *sfp_connector_map[256] = {"Unknown or unspecified",
                                      "SC",
...
void gpio_i2c_sfp_run_once(I2CSfpState *i2c_sfp_state)
{
    ...
    if (furi_hal_i2c_is_device_ready(&furi_hal_i2c_handle_external, SFP_I2C_ADDRESS << 1,
                                     response_timeout_ticks)) {
        for (uint8_t i = 0; i <= 254; i++) {
            uint8_t data = 0;
            // Read data from register
            furi_hal_i2c_read_reg_8(&furi_hal_i2c_handle_external, SFP_I2C_ADDRESS << 1, i, &data,
                                    response_timeout_ticks);
            // Save data
            sfp_data[i] = data;
        }
        ...
        printf("Max length: %3u km %4u m %4u m %4u m %4u m\r\n", sfp_data[14], sfp_data[17]*10,
        sfp_data[16]*10, sfp_data[19]*10, sfp_data[18]*10);
        */
