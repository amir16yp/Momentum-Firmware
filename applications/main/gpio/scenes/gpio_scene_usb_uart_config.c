...
static const uint32_t baudrate_list[] = {
    1200, 2400, 4800, 9600, 19200, 28800, 38400, 57600, 115200, 230400, 460800, 921600,
};
...
void gpio_scene_usb_uart_cfg_on_enter(void *context)
{
    ...
    item = variable_item_list_add(var_item_list, "Baudrate",
                                  sizeof(baudrate_list) / sizeof(baudrate_list[0]) + 1,
                                  line_baudrate_cb, app);
    ...
    if (app->usb_uart_cfg->baudrate_mode > 0) {
        ...
        variable_item_set_current_value_text(item, baudrate_mode[app->usb_uart_cfg->baudrate_mode]);
    }
    ...
    item = variable_item_list_add(var_item_list, "RTS/DTR Pins", COUNT_OF(flow_pins), line_flow_cb,
                                  app);
    variable_item_set_current_value_index(item, app->usb_uart_cfg->flow_pins);
    variable_item_set_current_value_text(item, flow_pins[app->usb_uart_cfg->flow_pins]);
    ...
    item = variable_item_list_add(var_item_list, "DE/RE Pin", COUNT_OF(software_de_re),
                                  line_software_de_re_cb, app);
...
{
    GpioApp *app = context;
    scene_manager_set_scene_state(app->scene_manager, GpioAppViewUsbUartCfg,
