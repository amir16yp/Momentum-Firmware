...
SubGhzTxRx *subghz_txrx_alloc(void)
{
    ...
    subghz_environment_set_alutech_at_4n_rainbow_table_file_name(instance->environment,
                                                                 SUBGHZ_ALUTECH_AT_4N_DIR_NAME);
    subghz_environment_set_nice_flor_s_rainbow_table_file_name(instance->environment,
    ...
    subghz_worker_set_overrun_callback(instance->worker,
                                       (SubGhzWorkerOverrunCallback)subghz_receiver_reset);
    subghz_worker_set_pair_callback(instance->worker,
...
void subghz_txrx_set_preset(SubGhzTxRx *instance, const char *preset_name, uint32_t frequency,
                            float latitude, float longitude, uint8_t *preset_data,
...
void subghz_txrx_get_frequency_and_modulation(SubGhzTxRx *instance, FuriString *frequency,
                                              FuriString *modulation, bool long_name)
{
    furi_assert(instance);
    SubGhzRadioPreset *preset = instance->preset;
    if (frequency != NULL) {
        furi_string_printf(frequency, "%03ld.%02ld", preset->frequency / 1000000 % 1000,
...
static uint32_t subghz_txrx_rx(SubGhzTxRx *instance, uint32_t frequency)
{
    ...
    subghz_devices_start_async_rx(instance->radio_device, subghz_worker_rx_callback,
                                  instance->worker);
        ...
        if (instance->transmitter) {
            if (subghz_transmitter_deserialize(instance->transmitter, flipper_format) ==
                ...
                if (strcmp(furi_string_get_cstr(preset->name), "") != 0) {
                    ...
                    FURI_LOG_E(TAG, "Unknown name preset \" %s \"",
                ...
                if (ret == SubGhzTxRxStartTxStateOk) {
                    // Start TX
                    subghz_devices_start_async_tx(instance->radio_device, subghz_transmitter_yield,
                                                  instance->transmitter);
                }
            } else {
                ret = SubGhzTxRxStartTxStateErrorParserOthers;
...
void subghz_txrx_rx_start(SubGhzTxRx *instance)
{
    furi_assert(instance);
    subghz_txrx_stop(instance);
    subghz_txrx_begin(instance, subghz_setting_get_preset_data_by_name(
                                    subghz_txrx_get_setting(instance),
                                    furi_string_get_cstr(instance->preset->name)));
    subghz_txrx_rx(instance, instance->preset->frequency);
}

void subghz_txrx_set_need_save_callback(SubGhzTxRx *instance, SubGhzTxRxNeedSaveCallback callback,
                                        void *context)
{
    furi_assert(instance);
...
void subghz_txrx_receiver_set_ignore_filter(SubGhzTxRx *instance,
...
void subghz_txrx_set_rx_callback(SubGhzTxRx *instance, SubGhzReceiverCallback callback,
                                 void *context)
{
...
void subghz_txrx_set_raw_file_encoder_worker_callback_end(
    SubGhzTxRx *instance, SubGhzProtocolEncoderRAWCallbackEnd callback, void *context)
{
...
SubGhzRadioDeviceType subghz_txrx_radio_device_set(SubGhzTxRx *instance,
...
const char *subghz_txrx_set_preset_internal(SubGhzTxRx *instance, uint32_t frequency, uint8_t index,
                                            uint8_t tx_power)
{
    furi_assert(instance);
    ...
    subghz_txrx_set_preset(instance, preset_name, frequency, NAN, NAN, preset_data,
                           preset_data_size);
