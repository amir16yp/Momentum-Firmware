...
static void subghz_gps_uart_on_irq_cb(FuriHalSerialHandle *handle, FuriHalSerialRxEvent event,
                                      void *context)
{
    UNUSED(handle);
...
static int32_t subghz_gps_uart_worker(void *context)
{
    ...
    while (1) {
        ...
        if (events & WorkerEvtRxDone) {
            size_t len = 0;
            do {
                len = furi_stream_buffer_receive(subghz_gps->rx_stream,
                                                 subghz_gps->rx_buf + rx_offset,
                                                 RX_BUF_SIZE - rx_offset, 0);
...
static void subghz_gps_cat_realtime(SubGhzGPS *subghz_gps, FuriString *descr, float latitude,
                                    float longitude)
{
    float distance =
    ...
    furi_string_cat_printf(
        ...
        distance > 1 ? "km" : "m", angle_str, subghz_gps->fix_hour, subghz_gps->fix_minute,
...
static void subghz_gps_init(SubGhzGPS *subghz_gps, uint32_t baudrate)
{
    ...
    furi_hal_serial_async_rx_start(subghz_gps->serial_handle, subghz_gps_uart_on_irq_cb, subghz_gps,
                                   false);
