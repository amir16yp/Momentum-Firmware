...
bool minmea_parse_gbs(struct minmea_sentence_gbs *frame, const char *sentence)
{
    ...
    char type[6];
    if (!minmea_scan(sentence, "tTfffifff", type, &frame->time, &frame->err_latitude,
                     &frame->err_longitude, &frame->err_altitude, &frame->svid, &frame->prob,
...
bool minmea_parse_rmc(struct minmea_sentence_rmc *frame, const char *sentence)
{
    ...
    if (!minmea_scan(sentence, "tTcfdfdffDfd", type, &frame->time, &validity, &frame->latitude,
                     &latitude_direction, &frame->longitude, &longitude_direction, &frame->speed,
                     &frame->course, &frame->date, &frame->variation, &variation_direction))
        return false;
    if (strcmp(type + 2, "RMC"))
        return false;
...
bool minmea_parse_gga(struct minmea_sentence_gga *frame, const char *sentence)
{
    ...
    if (!minmea_scan(sentence, "tTfdfdiiffcfcf_", type, &frame->time, &frame->latitude,
                     ...
                     &frame->altitude, &frame->altitude_units, &frame->height, &frame->height_units,
                     &frame->dgps_age))
        return false;
    if (strcmp(type + 2, "GGA"))
        return false;
...
bool minmea_parse_gsv(struct minmea_sentence_gsv *frame, const char *sentence)
{
    ...
    if (!minmea_scan(sentence, "tiii;iiiiiiiiiiiiiiii", type, &frame->total_msgs, &frame->msg_nr,
                     &frame->total_sats, &frame->sats[0].nr, &frame->sats[0].elevation,
                     &frame->sats[0].azimuth, &frame->sats[0].snr, &frame->sats[1].nr,
                     &frame->sats[1].elevation, &frame->sats[1].azimuth, &frame->sats[1].snr,
                     &frame->sats[2].nr, &frame->sats[2].elevation, &frame->sats[2].azimuth,
...
bool minmea_parse_vtg(struct minmea_sentence_vtg *frame, const char *sentence)
{
    ...
    if (!minmea_scan(sentence, "t;fcfcfcfcc", type, &frame->true_track_degrees, &c_true,
...
bool minmea_parse_zda(struct minmea_sentence_zda *frame, const char *sentence)
{
    ...
    if (!minmea_scan(sentence, "tTiiiii", type, &frame->time, &frame->date.day, &frame->date.month,
