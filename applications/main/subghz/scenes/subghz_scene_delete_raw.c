...
void subghz_scene_delete_raw_callback(GuiButtonType result, InputType type, void *context)
{
    ...
    if ((result == GuiButtonTypeRight) && (type == InputTypeShort)) {
        view_dispatcher_send_custom_event(subghz->view_dispatcher, SubGhzCustomEventSceneDeleteRAW);
...
void subghz_scene_delete_raw_on_enter(void *context)
{
    ...
    widget_add_string_element(subghz->widget, 38, 25, AlignLeft, AlignTop, FontSecondary,
                              "RAW signal");
    subghz_txrx_get_frequency_and_modulation(subghz->txrx, frequency_str, modulation_str, false);
    widget_add_string_element(subghz->widget, 35, 37, AlignLeft, AlignTop, FontSecondary,
                              furi_string_get_cstr(frequency_str));

    widget_add_string_element(subghz->widget, 72, 37, AlignLeft, AlignTop, FontSecondary,
                              furi_string_get_cstr(modulation_str));

    furi_string_free(frequency_str);
    furi_string_free(modulation_str);

    widget_add_button_element(subghz->widget, GuiButtonTypeRight, "Delete",
                              subghz_scene_delete_raw_callback, subghz);
    widget_add_button_element(subghz->widget, GuiButtonTypeLeft, "Back",
                              subghz_scene_delete_raw_callback, subghz);
