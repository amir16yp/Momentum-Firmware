...
const char *const hopping_mode_text[HOPPING_MODE_COUNT] = {
    "OFF",    "-90dBm", "-85dBm", "-80dBm", "-75dBm", "-70dBm",
    "-65dBm", "-60dBm", "-55dBm", "-50dBm", "-45dBm", "-40dBm",

};
const float hopping_mode_value[HOPPING_MODE_COUNT] = {
    NAN, -90.0f, -85.0f, -80.0f, -75.0f, -70.0f, -65.0f, -60.0f, -55.0f, -50.0f, -45.0f, -40.0f,
};
...
static void subghz_scene_receiver_config_set_ignore_filter(VariableItem *item,
...
uint8_t subghz_scene_receiver_config_hopper_value_index(void *context)
{
    ...
    if (subghz_txrx_hopper_get_state(subghz->txrx) == SubGhzHopperStateOFF) {
        ...
        variable_item_set_current_value_text(
            ...
            " -----");
        return value_index_float(subghz->last_settings->hopping_threshold, hopping_mode_value,
                                 HOPPING_MODE_COUNT);
        ;
    }
}
...
static void subghz_scene_receiver_config_set_frequency(VariableItem *item)
{
    ...
    if (subghz_txrx_hopper_get_state(subghz->txrx) == SubGhzHopperStateOFF) {
        ...
        snprintf(text_buf, sizeof(text_buf), "%lu.%02lu", frequency / 1000000,
        ...
        // Set the preset now.
        subghz_txrx_set_preset(subghz->txrx, furi_string_get_cstr(preset.name), frequency, NAN, NAN,
                               preset.data, preset.data_size);
        ...
        variable_item_set_current_value_index(item,
                                              subghz_setting_get_frequency_default_index(setting));
    }
}
...
static void subghz_scene_receiver_config_set_preset(VariableItem *item)
{
    ...
    subghz_txrx_set_preset(subghz->txrx, preset_name, preset.frequency, NAN, NAN, preset_data,
                           preset_data_size);
...
static void subghz_scene_receiver_config_set_hopping(VariableItem *item)
{
    ...
    if (index == 0) {
        ...
        subghz_txrx_set_preset(subghz->txrx, furi_string_get_cstr(preset.name), frequency, NAN, NAN,
                               preset.data, preset.data_size);
        variable_item_set_current_value_index(frequency_item,
                                              subghz_setting_get_frequency_default_index(setting));
    ...
    subghz_txrx_hopper_set_state(subghz->txrx,
                                 index != 0 ? SubGhzHopperStateRunning : SubGhzHopperStateOFF);
}
...
static void subghz_scene_receiver_config_set_repeater(VariableItem *item)
{
    ...
    if (subghz->repeater == SubGhzRepeaterStateOff &&
        ...
        scene_manager_set_scene_state(subghz->scene_manager, SubGhzSceneReceiverConfig,
                                      SubGhzSettingIndexRepeater);
        view_dispatcher_send_custom_event(subghz->view_dispatcher,
    ...
    // Change BinRAW to ON or OFF as required, and remember whether I changed it! (Put back for the
    // user.)
    if (repeater_value[index] != SubGhzRepeaterStateOff) {
        if ((subghz->filter & SubGhzProtocolFlag_BinRAW) == 0) {
            // Repeater is on, Binraw is Off.
            variable_item_set_current_value_index(bin_raw_menu, 1 /*Index of ON in BIN_Raw menu!*/);
            subghz_scene_receiver_config_set_bin_raw(bin_raw_menu);
            subghz->repeater_bin_raw_was_off = true;
        }

        // Lock the BinRAW menu, Flipper doesnt understand everything so BinRAW makes every key
        // send.
        ...
        if (subghz->repeater_bin_raw_was_off) {
            variable_item_set_current_value_index(bin_raw_menu,
                                                  0 /*Index of OFF in BIN_Raw menu!*/);
            subghz_scene_receiver_config_set_bin_raw(bin_raw_menu);
        }
...
static inline bool subghz_scene_receiver_config_ignore_filter_get_index(SubGhzProtocolFilter filter,
...
static void subghz_scene_receiver_config_var_list_enter_callback(void *context, uint32_t index)
{
    ...
    if (index == SubGhzSettingIndexRemoveDuplicates) {
        ...
        subghz_txrx_set_preset_internal(subghz->txrx, SUBGHZ_LAST_SETTING_DEFAULT_FREQUENCY,
                                        SUBGHZ_LAST_SETTING_DEFAULT_PRESET, subghz->tx_power);
...
void subghz_scene_receiver_config_on_enter(void *context)
{
    ...
    item = variable_item_list_add(subghz->variable_item_list, "Frequency",
                                  subghz_setting_get_frequency_count(setting),
                                  subghz_scene_receiver_config_set_frequency, subghz);
    ...
    item = variable_item_list_add(subghz->variable_item_list, "Modulation",
                                  subghz_setting_get_preset_count(setting),
                                  subghz_scene_receiver_config_set_preset, subghz);
    value_index =
        subghz_scene_receiver_config_next_preset(furi_string_get_cstr(preset.name), subghz);
    variable_item_set_current_value_index(item, value_index);
    variable_item_set_current_value_text(item,
                                         subghz_setting_get_preset_name(setting, value_index));

    if (scene_manager_get_scene_state(subghz->scene_manager, SubGhzSceneReadRAW) !=
        SubGhzCustomEventManagerSet) {
        // Hopping
        value_index = subghz_scene_receiver_config_hopper_value_index(subghz);
        item = variable_item_list_add(subghz->variable_item_list,
                                      value_index ? "Hopping RSSI" : "Hopping", HOPPING_MODE_COUNT,
                                      subghz_scene_receiver_config_set_hopping, subghz);
    ...
    if (scene_manager_get_scene_state(subghz->scene_manager, SubGhzSceneReadRAW) !=
        SubGhzCustomEventManagerSet) {
        item = variable_item_list_add(subghz->variable_item_list, "Bin RAW", COMBO_BOX_COUNT,
                                      subghz_scene_receiver_config_set_bin_raw, subghz);

        value_index = value_index_uint32(subghz->filter, bin_raw_value, COMBO_BOX_COUNT);
        variable_item_set_current_value_index(item, value_index);
        variable_item_set_current_value_text(item, combobox_text[value_index]);
        variable_item_set_locked(item, subghz->repeater != SubGhzRepeaterStateOff,
                                 "Turn off\nRepeater\nto do that!");

        item = variable_item_list_add(subghz->variable_item_list, "Repeater", REPEATER_COUNT,
                                      subghz_scene_receiver_config_set_repeater, subghz);
        ...
        item =
            variable_item_list_add(subghz->variable_item_list, "Remove Duplicates", COMBO_BOX_COUNT,
                                   subghz_scene_receiver_config_set_duplicates, subghz);
        ...
        item = variable_item_list_add(subghz->variable_item_list,
                                      "Delete Old Signals on Full Memory", COMBO_BOX_COUNT,
                                      subghz_scene_receiver_config_set_delete_old_signals, subghz);

        value_index = subghz->last_settings->delete_old_signals;
        variable_item_set_current_value_index(item, value_index);
        variable_item_set_current_value_text(item, combobox_text[value_index]);

        item = variable_item_list_add(subghz->variable_item_list, "Autosave", COMBO_BOX_COUNT,
                                      subghz_scene_receiver_config_set_autosave, subghz);
        ...
        item =
            variable_item_list_add(subghz->variable_item_list, "Ignore ReversRB2", COMBO_BOX_COUNT,
                                   subghz_scene_receiver_config_set_reversrb2, subghz);
        ...
        item = variable_item_list_add(subghz->variable_item_list, "Ignore Alarms", COMBO_BOX_COUNT,
                                      subghz_scene_receiver_config_set_alarms, subghz);
        ...
        item = variable_item_list_add(subghz->variable_item_list, "Ignore Sensors", COMBO_BOX_COUNT,
                                      subghz_scene_receiver_config_set_sensors, subghz);
        ...
        item =
            variable_item_list_add(subghz->variable_item_list, "Ignore Princeton", COMBO_BOX_COUNT,
                                   subghz_scene_receiver_config_set_princeton, subghz);
        ...
        item = variable_item_list_add(subghz->variable_item_list, "Ignore Nice Flor-S / Nice One",
                                      COMBO_BOX_COUNT, subghz_scene_receiver_config_set_niceflors,
                                      subghz);
    ...
    // Enable speaker, will send all incoming noises and signals to speaker so you can listen how
    // your remote sounds like :)
    item = variable_item_list_add(subghz->variable_item_list, "Sound", COMBO_BOX_COUNT,
                                  subghz_scene_receiver_config_set_speaker, subghz);
    value_index = value_index_uint32(subghz_txrx_speaker_get_state(subghz->txrx), speaker_value,
                                     COMBO_BOX_COUNT);
    variable_item_set_current_value_index(item, value_index);
    variable_item_set_current_value_text(item, combobox_text[value_index]);
    ...
    if (scene_manager_get_scene_state(subghz->scene_manager, SubGhzSceneReadRAW) !=
        ...
        variable_item_list_set_enter_callback(subghz->variable_item_list,
    ...
    if (scene_manager_get_scene_state(subghz->scene_manager, SubGhzSceneReadRAW) ==
        SubGhzCustomEventManagerSet) {
        item = variable_item_list_add(subghz->variable_item_list,
                                      "RSSI Threshold:", RAW_THRESHOLD_RSSI_COUNT,
                                      subghz_scene_receiver_config_set_raw_threshold_rssi, subghz);
        value_index = value_index_float(subghz_threshold_rssi_get(subghz->threshold_rssi),
                                        raw_threshold_rssi_value, RAW_THRESHOLD_RSSI_COUNT);
        variable_item_set_current_value_index(item, value_index);
