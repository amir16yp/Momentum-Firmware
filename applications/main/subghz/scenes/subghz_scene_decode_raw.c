                                                 ...
                                                 void *context)
{
    ...
    if (subghz_history_add_to_history(subghz->history, decoder_base, &preset)) {
        ...
        subghz_view_receiver_add_item_to_menu(
            ...
            furi_string_get_cstr(item_time), subghz_history_get_type_protocol(subghz->history, idx),
...
bool subghz_scene_decode_raw_start(SubGhz *subghz)
{
    ...
    do {
        ...
        if (!flipper_format_read_string(subghz_txrx_get_fff_data(subghz->txrx), "File_name",
                                        file_name)) {
    ...
    if (success) {
        ...
        if (subghz_file_encoder_worker_start(subghz->decode_raw_file_worker_encoder,
...
bool subghz_scene_decode_raw_next(SubGhz *subghz)
{
    ...
    for (uint32_t read = SAMPLES_TO_READ_PER_TICK; read > 0; --read) {
        ...
        if (!level_duration_is_reset(level_duration)) {
            ...
            scene_manager_set_scene_state(subghz->scene_manager, SubGhzSceneDecodeRAW,
    ...
    subghz_file_encoder_worker_get_text_progress(subghz->decode_raw_file_worker_encoder,
                                                 progress_str);

    subghz_view_receiver_add_data_progress(subghz->subghz_receiver,
                                           furi_string_get_cstr(progress_str));
...
void subghz_scene_decode_raw_on_enter(void *context)
{
    ...
    subghz_view_receiver_set_callback(subghz->subghz_receiver, subghz_scene_decode_raw_callback,
                                      subghz);
