...
void subghz_scene_save_name_on_enter(void *context)
{
    ...
    name_generator_make_auto_datetime(file_name_buf, SUBGHZ_MAX_LEN_NAME,
                                      SUBGHZ_APP_FILENAME_PREFIX, datetime);
    ...
    if (!subghz_path_is_file(subghz->file_path)) {
        ...
        if (subghz->last_settings->protocol_file_names && decoder_result != NULL &&
            ...
            name_generator_make_auto_datetime(file_name_buf, SUBGHZ_MAX_LEN_NAME,
                                              decoder_result->protocol->name, datetime);
        } else {
            name_generator_make_auto_datetime(file_name_buf, SUBGHZ_MAX_LEN_NAME,
                                              SUBGHZ_APP_FILENAME_PREFIX, datetime);
        }
        ...
        if (scene_manager_get_scene_state(subghz->scene_manager, SubGhzSceneReadRAW) !=
            ...
            if (scene_manager_get_scene_state(subghz->scene_manager, SubGhzSceneReadRAW) ==
                ...
                if (subghz->last_settings->protocol_file_names) {
                    name_generator_make_auto_datetime(file_name_buf, SUBGHZ_MAX_LEN_NAME, "RAW",
                                                      datetime);
                } else {
                    name_generator_make_auto_datetime(file_name_buf, SUBGHZ_MAX_LEN_NAME,
                                                      SUBGHZ_APP_FILENAME_PREFIX, datetime);
    ...
    text_input_set_result_callback(text_input, subghz_scene_save_name_text_input_callback, subghz,
                                   subghz->file_name_tmp, SUBGHZ_MAX_LEN_NAME, dev_name_empty);
...
bool subghz_scene_save_name_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeBack) {
        ...
        if (event.event == SubGhzCustomEventSceneSaveName) {
            if (strcmp(subghz->file_name_tmp, "") != 0) {
                furi_string_cat_printf(subghz->file_path, "/%s%s", subghz->file_name_tmp,
...
#ifdef SUBGHZ_ADD_MANUALLY
                    if (!subghz_save_protocol_to_file(subghz,
                ...
                } else if (scene_manager_has_previous_scene(subghz->scene_manager,
                                                            SubGhzSceneMoreRAW)) {
