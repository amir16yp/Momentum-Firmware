...
void subghz_scene_need_saving_on_enter(void *context)
{
    ...
    widget_add_string_multiline_element(subghz->widget, 64, 13, AlignCenter, AlignCenter,
                                        FontPrimary, "Discard Signals?");
    widget_add_string_multiline_element(subghz->widget, 64, 32, AlignCenter, AlignCenter,
                                        FontSecondary, "All unsaved data\nwill be lost!");

    widget_add_button_element(subghz->widget, GuiButtonTypeRight, "Stay",
                              subghz_scene_need_saving_callback, subghz);
    widget_add_button_element(subghz->widget, GuiButtonTypeLeft, "Continue",
                              subghz_scene_need_saving_callback, subghz);
    ...
    if (event.type == SceneManagerEventTypeBack) {
        ...
        if (event.event == SubGhzCustomEventSceneStay) {
            ...
            if (state == SubGhzRxKeyStateExit) {
                ...
                subghz_txrx_set_preset(subghz->txrx, "AM650", subghz->last_settings->frequency, NAN,
                                       NAN, NULL, 0);
