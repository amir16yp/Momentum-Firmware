...
static bool subghz_scene_receiver_info_update_parser(void *context)
{
    ...
    if (subghz_txrx_load_decoder_by_name_protocol(
        ...
        // we are trying to deserialize without checking for errors, since it is assumed that we
        // just received this chignal
        ...
        subghz_txrx_set_preset(subghz->txrx, furi_string_get_cstr(preset->name), preset->frequency,
                               NAN, NAN, preset->data, preset->data_size);
...
void subghz_scene_receiver_info_draw_widget(SubGhz *subghz)
{
    if (subghz_scene_receiver_info_update_parser(subghz)) {
        ...
        subghz_txrx_get_frequency_and_modulation(subghz->txrx, frequency_str, modulation_str,
                                                 false);
        widget_add_string_element(subghz->widget, 78, 0, AlignLeft, AlignTop, FontSecondary,
                                  furi_string_get_cstr(frequency_str));

        widget_add_string_element(subghz->widget, 113, 0, AlignLeft, AlignTop, FontSecondary,
                                  furi_string_get_cstr(modulation_str));
        subghz_protocol_decoder_base_get_string(subghz_txrx_get_decoder(subghz->txrx), text);
        widget_add_string_multiline_element(subghz->widget, 0, 0, AlignLeft, AlignTop,
                                            FontSecondary, furi_string_get_cstr(text));

        if (subghz->gps) {
            widget_add_button_element(subghz->widget, GuiButtonTypeLeft, "Geo",
                                      subghz_scene_receiver_info_callback, subghz);
        }
        ...
        if (subghz_txrx_protocol_is_serializable(subghz->txrx)) {
            widget_add_button_element(subghz->widget, GuiButtonTypeRight, "Save",
                                      subghz_scene_receiver_info_callback, subghz);
        }
        // Removed static check
        if (subghz_txrx_protocol_is_transmittable(subghz->txrx, false)) {
            widget_add_button_element(subghz->widget, GuiButtonTypeCenter, "Send",
                                      subghz_scene_receiver_info_callback, subghz);
        }
    } else {
        widget_add_icon_element(subghz->widget, 83, 22, &I_WarningDolphinFlip_45x42);
        widget_add_string_element(subghz->widget, 13, 8, AlignLeft, AlignBottom, FontSecondary,
                                  "Error history parse.");
    }
...
bool subghz_scene_receiver_info_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == SubGhzCustomEventSceneReceiverInfoTxStart) {
            ...
            // key concept: we start endless TX until user release OK button, and after this we send
            // last protocols repeats - this guarantee that one press OK will be guarantee send the
            // required minimum protocol data packets for all of this we use
            // subghz_block_generic_global.endless_tx in protocols _yield function.
            subghz->state_notifications = SubGhzNotificationStateTx;
            subghz_block_generic_global.endless_tx = true;
            if (!subghz_tx_start(subghz, subghz_history_get_raw_data(subghz->history,
            ...
            //  send upload "repeat=xx" times, and after will be stoped by the tick event down in
            //  this code
        ...
        case SubGhzNotificationStateTxWait:
            // we wait until hardware TX finished and after stop TX and start RX, else just blink
            // led
            if (!subghz_devices_is_async_complete_tx(subghz->txrx->radio_device)) {
                ...
                if (!scene_manager_has_previous_scene(subghz->scene_manager,
                                                      SubGhzSceneDecodeRAW)) {
                    subghz_txrx_rx_start(subghz->txrx);
                    subghz_txrx_hopper_unpause(subghz->txrx);
                    if (!subghz_history_full(subghz->history)) {
                        subghz->state_notifications = SubGhzNotificationStateRx;
