...
void subghz_scene_delete_on_enter(void *context)
{
    ...
    furi_string_cat_printf(text_out, "%s %s", furi_string_get_cstr(frequency_str),
                           furi_string_get_cstr(modulation_str));

    widget_add_text_box_element(subghz->widget, 0, 0, 128, 54, AlignCenter, AlignTop,
                                furi_string_get_cstr(text_out), false);
    ...
    widget_add_button_element(subghz->widget, GuiButtonTypeRight, "Delete",
                              subghz_scene_delete_callback, subghz);
    widget_add_button_element(subghz->widget, GuiButtonTypeLeft, "Cancel",
                              subghz_scene_delete_callback, subghz);
