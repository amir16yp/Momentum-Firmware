...
void subghz_scene_show_error_on_enter(void *context)
{
    ...
    widget_add_string_multiline_element(subghz->widget, 81, 24, AlignCenter, AlignCenter,
                                        FontSecondary, furi_string_get_cstr(subghz->error_str));
    if (scene_manager_get_scene_state(subghz->scene_manager, SubGhzSceneShowError) ==
        SubGhzCustomEventManagerSet) {
        widget_add_button_element(subghz->widget, GuiButtonTypeRight, "OK",
                                  subghz_scene_show_error_callback, subghz);
    } else {
