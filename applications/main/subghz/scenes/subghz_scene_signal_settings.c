...
static const char *const counter_mode_text[COUNTER_MODE_COUNT] = {
    "System", "Mode 1", "Mode 2", "Mode 3", "Mode 4", "Mode 5", "Mode 6", "Mode 7",
};
...
static const Protocols protocols[] = {
    {"Nice FloR-S", 3}, {"CAME Atomo", 4}, {"Alutech AT-4N", 3}, {"KeeLoq", 8}, {"Phoenix_V2", 3},
};
...
void subghz_scene_signal_settings_counter_mode_changed(VariableItem *item)
{
    ...
    if (flipper_format_file_open_existing(fff_data_file, file_path)) {
        if (flipper_format_insert_or_update_uint32(fff_data_file, "CounterMode", &counter_mode,
                                                   1)) {
...
void subghz_scene_signal_settings_variable_item_list_enter_callback(void *context, uint32_t index)
{
    ...
    if (index == 1) {
        ...
        byte_input_set_result_callback(byte_input, subghz_scene_signal_settings_byte_input_callback,
                                       NULL, subghz, cnt_byte_ptr, cnt_byte_count);
    ...
    if (index == 2) {
        ...
        byte_input_set_result_callback(byte_input, subghz_scene_signal_settings_byte_input_callback,
                                       NULL, subghz, btn_byte_ptr, btn_byte_count);
        view_dispatcher_switch_to_view(subghz->view_dispatcher, SubGhzViewIdByteInput);
    }
}
...
void subghz_scene_signal_settings_on_enter(void *context)
{
    ...
    // Open file and check is it contains allowed protocols and CounterMode variable - if not then
    // CounterMode will stay 0xff if file contain allowed protocol but not contain CounterMode value
    // then setup default CounterMode value = 0 and available CounterMode count for this protocol if
    // file contain CounterMode value then load it
    if (!flipper_format_file_open_existing(fff_data_file, file_path)) {
        FURI_LOG_E(TAG, "Error open file %s", file_path);
    } else {
        flipper_format_read_string(fff_data_file, "Protocol", tmp_text);
        // compare available protocols names, load CounterMode value from file and setup
        // variable_item_list values_count
        for (uint8_t i = 0; i < PROTOCOLS_COUNT i++) {
            if (!strcmp(furi_string_get_cstr(tmp_text), protocols[i].name)) {
                ...
                if (flipper_format_read_uint32(fff_data_file, "CounterMode", &tmp_counter_mode,
                                               1)) {
    ...
    variable_item_list_set_enter_callback(
        variable_item_list, subghz_scene_signal_settings_variable_item_list_enter_callback, subghz);

    item = variable_item_list_add(variable_item_list, "Counter Mode", mode_count,
                                  subghz_scene_signal_settings_counter_mode_changed, subghz);
    ...
    // deserialaze and decode loaded sugbhz file and push data to subghz_block_generic_global
    // variable
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == SubGhzCustomEventByteInputDone) {
            ...
            case 1:
                ...
                case 2:
                    // set new cnt value and override_flag to global variable and call transmit to
                    // generate and save subghz signal
                    counter16 = __bswap16(counter16);
