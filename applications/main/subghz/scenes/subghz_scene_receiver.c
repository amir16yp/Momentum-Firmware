...
static void subghz_scene_receiver_update_statusbar(void *context)
{
    ...
    if (!subghz_history_get_text_space_left(subghz->history, history_stat_str,
                                            subghz->last_settings->delete_old_signals, show_sats,
        ...
        subghz_view_receiver_add_data_statusbar(
            subghz->subghz_receiver, furi_string_get_cstr(frequency_str),
            furi_string_get_cstr(modulation_str), furi_string_get_cstr(history_stat_str),
            subghz_txrx_hopper_get_state(subghz->txrx) != SubGhzHopperStateOFF,
            READ_BIT(subghz->filter, SubGhzProtocolFlag_BinRAW) > 0, show_sats, subghz->repeater);
    ...
    subghz_view_receiver_set_radio_device_type(subghz->subghz_receiver,
                                               subghz_txrx_radio_device_get(subghz->txrx));
}
...
static void subghz_scene_add_to_history_callback(SubGhzReceiver *receiver,
    ...
    if (subghz_history_add_to_history(history, decoder_base, &preset)) {
        ...
        if (subghz->repeater != SubGhzRepeaterStateOff) {
            ...
            subghz_view_receiver_add_item_to_menu(
                subghz->subghz_receiver, furi_string_get_cstr(item_name),
                furi_string_get_cstr(item_time), subghz_history_get_type_protocol(history, idx),
            ...
            if (decoder_base->protocol->flag & SubGhzProtocolFlag_Save &&
                ...
                free(dir);
                // Save
                subghz_save_protocol_to_file(subghz, subghz_history_get_raw_data(history, idx),
...
void subghz_scene_receiver_on_enter(void *context)
{
    ...
    for (uint16_t i = 0; i < subghz_history_get_item(history); i++) {
        ...
        subghz_view_receiver_add_item_to_menu(
            subghz->subghz_receiver, furi_string_get_cstr(item_name),
            furi_string_get_cstr(item_time), subghz_history_get_type_protocol(history, i),
    ...
    subghz_view_receiver_set_callback(subghz->subghz_receiver, subghz_scene_receiver_callback,
                                      subghz);
    ...
    subghz_txrx_speaker_set_state(subghz->txrx, subghz->last_settings->enable_sound
    ...
    if (subghz->repeater != SubGhzRepeaterStateOff) {
        // User had BinRAW on if the last settings had BinRAW on, if not, repeater is on, and BinRAW
        // goes on, but State CHanged is false!
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        case SubGhzCustomEventViewReceiverBack:
            ...
            if (subghz_history_get_last_index(subghz->history)) {
                ...
                scene_manager_search_and_switch_to_previous_scene(subghz->scene_manager,
                                                                  SubGhzSceneStart);
            }
            consumed = true;
            break;
        ...
        case SubGhzCustomEventViewReceiverConfig:
            // Actually signals are received but SubGhzNotificationStateRx is not working inside
            // Config Scene
            scene_manager_set_scene_state(subghz->scene_manager, SubGhzViewIdReceiver,
        ...
        case SubGhzCustomEventViewRepeaterStart:
            ...
            if (subghz_txrx_tx_start(subghz->txrx, key_repeat_data) != SubGhzTxRxStartTxStateOk) {
                ...
                uint32_t repeat_time =
                    ((subghz->repeater & SubGhzRepeaterStateOnLong) != 0) ? 2 * repeatnormal * tmpTe
                    : ((subghz->repeater & SubGhzRepeaterStateOnShort) != 0) ? 1 * tmpTe
                                                                             : repeatnormal * tmpTe;
        ...
        case SubGhzCustomEventViewRepeaterStop:
            subghz_txrx_stop(subghz->txrx);
            subghz_history_delete_item(subghz->history,
                                       subghz_history_get_last_index(subghz->history) - 1);
            subghz_txrx_rx_start(subghz->txrx);
        ...
        case SubGhzCustomEventViewReceiverOKLong:
            ...
            if (subghz_txrx_tx_start(
                    subghz->txrx,
                    subghz_history_get_raw_data(subghz->history, subghz_view_receiver_get_idx_menu(
                                                                     subghz->subghz_receiver))) !=
