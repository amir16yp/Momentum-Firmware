...
static void subghz_load_custom_presets(SubGhzSetting *setting)
{
    ...
    static const char *presets[][2] = {
        // FM95
        {"FM95", "02 0D 0B 06 08 32 07 04 14 00 13 02 12 04 11 83 10 67 15 24 18 18 19 16 1D 91 1C "
                 "00 1B 07 20 FB 22 10 21 56 00 00 C0 00 00 00 00 00 00 00"},

        // #2-FSK 200khz BW / 135kHz Filter/ 15.86Khz Deviation + Ramping
        {"FM15k", "02 0D 03 47 08 32 0B 06 15 32 14 00 13 00 12 00 11 32 10 A7 18 18 19 1D 1D 92 "
                  "1C 00 1B 04 20 FB 22 17 21 B6 00 00 00 12 0E 34 60 C5 C1 C0"},

        // Pagers
        {"Pagers", "02 0D 07 04 08 32 0B 06 10 64 11 93 12 0C 13 02 14 00 15 15 18 18 19 16 1B 07 "
                   "1C 00 1D 91 20 FB 21 56 22 10 00 00 C0 00 00 00 00 00 00 00"},
    };
...
SubGhz *subghz_alloc(bool alloc_for_tx_only)
{
    ...
    view_dispatcher_set_custom_event_callback(subghz->view_dispatcher,
                                              subghz_custom_event_callback);
    view_dispatcher_set_navigation_event_callback(subghz->view_dispatcher,
                                                  subghz_back_event_callback);
    view_dispatcher_set_tick_event_callback(subghz->view_dispatcher, subghz_tick_event_callback,
                                            100);
    ...
    if (!alloc_for_tx_only) {
        ...
        view_dispatcher_add_view(subghz->view_dispatcher, SubGhzViewIdMenu,
                                 submenu_get_view(subghz->submenu));

#ifndef SUBGHZ_ADD_MANUALLY
        // Receiver
        subghz->subghz_receiver = subghz_view_receiver_alloc();
        view_dispatcher_add_view(subghz->view_dispatcher, SubGhzViewIdReceiver,
                                 subghz_view_receiver_get_view(subghz->subghz_receiver));
#endif
    }
    // Popup
    subghz->popup = popup_alloc();
    view_dispatcher_add_view(subghz->view_dispatcher, SubGhzViewIdPopup,
                             popup_get_view(subghz->popup));
    if (!alloc_for_tx_only) {
        // Text Input
        subghz->text_input = text_input_alloc();
        view_dispatcher_add_view(subghz->view_dispatcher, SubGhzViewIdTextInput,
                                 text_input_get_view(subghz->text_input));

        // Byte Input
        subghz->byte_input = byte_input_alloc();
        view_dispatcher_add_view(subghz->view_dispatcher, SubGhzViewIdByteInput,
                                 byte_input_get_view(subghz->byte_input));

        // Custom Widget
        subghz->widget = widget_alloc();
        view_dispatcher_add_view(subghz->view_dispatcher, SubGhzViewIdWidget,
                                 widget_get_view(subghz->widget));
    }
    // Dialog
    ...
    view_dispatcher_add_view(subghz->view_dispatcher, SubGhzViewIdTransmitter,
                             subghz_view_transmitter_get_view(subghz->subghz_transmitter));
#endif
    if (!alloc_for_tx_only) {
        // Variable Item List
        subghz->variable_item_list = variable_item_list_alloc();
        view_dispatcher_add_view(subghz->view_dispatcher, SubGhzViewIdVariableItemList,
        ...
        view_dispatcher_add_view(
            subghz->view_dispatcher, SubGhzViewIdFrequencyAnalyzer,
    ...
    view_dispatcher_add_view(subghz->view_dispatcher, SubGhzViewIdReadRAW,
    ...
    if (!alloc_for_tx_only) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            if (frequency == SUBGHZ_LAST_SETTING_DEFAULT_FREQUENCY)
                found_default = true;
        }
        if (!found_last) {
            if (found_default) {
        ...
        subghz_txrx_set_preset_internal(subghz->txrx, subghz->last_settings->frequency,
                                        subghz->last_settings->preset_index, subghz->tx_power);
...
int32_t subghz_app(char *p)
{
    ...
    if (p && strlen(p)) {
        ...
        if (sscanf(p, "RPC %lX", &rpc_ctx) == 1) {
            ...
            view_dispatcher_attach_to_gui(subghz->view_dispatcher, subghz->gui,
        ...
        if (subghz_txrx_is_database_loaded(subghz->txrx)) {
            ...
            scene_manager_set_scene_state(subghz->scene_manager, SubGhzSceneShowError,
                                          SubGhzCustomEventManagerSet);
            furi_string_set(subghz->error_str,
                            "No SD card or\ndatabase found.\nSome app function\nmay be reduced.");
