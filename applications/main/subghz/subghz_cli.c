...
void subghz_cli_command_tx(PipeSide *pipe, FuriString *args, void *context)
{
    ...
    if (furi_string_size(args)) {
        ...
        if (parse_err) {
            cli_print_usage("subghz tx",
                            "<3 Byte Key: in hex> <Frequency: in Hz> <Te us> <Repeat count> "
                            "<Device: 0 - CC1101_INT, 1 - CC1101_EXT>",
    ...
    printf("Transmitting at %lu, key %lx, te %lu, repeat %lu device %lu. Press CTRL+C to stop\r\n",
           frequency, key, te, repeat, device_ind);

    FuriString *flipper_format_string =
        furi_string_alloc_printf("Protocol: Princeton\n"
                                 "Bit: 24\n"
                                 "Key: 00 00 00 00 00 %02X %02X %02X\n"
                                 "TE: %lu\n"
                                 "Repeat: %lu\n",
                                 (uint8_t)((key >> 16) & 0xFFU), (uint8_t)((key >> 8) & 0xFFU),
                                 (uint8_t)(key & 0xFFU), te, repeat);
...
static void subghz_cli_command_rx_callback(SubGhzReceiver *receiver,
...
void subghz_cli_command_rx(PipeSide *pipe, FuriString *args, void *context)
{
    ...
    printf("Listening at frequency: %lu device: %lu. Press CTRL+C to stop\r\n", frequency,
           device_ind);
    LevelDuration level_duration;
    while (!cli_is_pipe_broken_or_is_etx_next_char(pipe)) {
        int ret = furi_stream_buffer_receive(instance->stream, &level_duration,
                                             sizeof(LevelDuration), 10);
...
void subghz_cli_command_decode_raw(PipeSide *pipe, FuriString *args, void *context)
{
    ...
    do {
        if (furi_string_size(args)) {
            if (!args_read_string_and_trim(args, file_name)) {
                cli_print_usage("subghz decode_raw", "<file_name: path_RAW_file>",
                                furi_string_get_cstr(args));
                break;
            }
        }
        ...
        if (!flipper_format_file_open_existing(fff_data_file, furi_string_get_cstr(file_name))) {
            printf("subghz decode_raw \033[0;31mError open file\033[0m %s\r\n",
    ...
    if (check_file) {
        ...
        printf("Listening at \033[0;33m%s\033[0m.\r\n\r\nPress CTRL+C to stop\r\n\r\n",
               furi_string_get_cstr(file_name));
...
{ // -V524
    ...
    do {
        if (furi_string_size(args)) {
            if (!args_read_string_and_trim(args, file_name)) {
                cli_print_usage("subghz tx_from_file: ",
                                "<file_name: path_file> <Repeat count> <Device: 0 - CC1101_INT, 1 "
                                "- CC1101_EXT>",
                                furi_string_get_cstr(args));
                break;
            }
        ...
        if (!flipper_format_file_open_existing(fff_data_file, furi_string_get_cstr(file_name))) {
            printf("subghz tx_from_file: \033[0;31mError open file\033[0m %s\r\n",
                   furi_string_get_cstr(file_name));
            break;
        }
        ...
        if (!strcmp(furi_string_get_cstr(temp_str), "FuriHalSubGhzPresetCustom")) {
            ...
            if (!flipper_format_read_hex(fff_data_file, "Custom_preset_data", custom_preset_data,
                                         custom_preset_data_size)) {
                printf("subghz tx_from_file: \033[0;31mCustom_preset_data read error\033[0m\r\n");
                break;
            }
            subghz_devices_load_preset(device,
        ...
        if (!strcmp(furi_string_get_cstr(temp_str), "RAW")) { // if RAW protocol
            subghz_protocol_raw_gen_fff_data(fff_data_raw, furi_string_get_cstr(file_name),
    ...
    if (check_file) {
        ...
        printf("Listening at \033[0;33m%s\033[0m. Frequency=%lu, Protocol=%s\r\n\r\nPress CTRL+C "
               "to stop\r\n\r\n",
               furi_string_get_cstr(file_name), frequency, furi_string_get_cstr(temp_str));
        do {
...
static void subghz_cli_command_print_usage(void)
{
    ...
    printf("\tchat <frequency:in Hz> <device: 0 - CC1101_INT, 1 - CC1101_EXT>\t - Chat with other "
           "Flippers\r\n");
    printf("\ttx <3 byte Key: in hex> <frequency: in Hz> <te: us> <repeat: count> <device: 0 - "
           "CC1101_INT, 1 - CC1101_EXT>\t - Transmitting key\r\n");
    printf("\trx <frequency:in Hz> <device: 0 - CC1101_INT, 1 - CC1101_EXT>\t - Receive\r\n");
    printf("\trx_raw <frequency:in Hz>\t - Receive RAW\r\n");
    printf("\tdecode_raw <file_name: path_RAW_file>\t - Testing\r\n");
    printf("\ttx_from_file <file_name: path_file> <repeat: count> <device: 0 - CC1101_INT, 1 - "
           "CC1101_EXT>\t - Transmitting from file\r\n");
    ...
    if (furi_hal_rtc_is_flag_set(FuriHalRtcFlagDebug)) {
        ...
        printf("\tencrypt_keeloq <path_decrypted_file> <path_encrypted_file> <IV:16 bytes in "
               "hex>\t - Encrypt keeloq manufacture keys\r\n");
        printf("\tencrypt_raw <path_decrypted_file> <path_encrypted_file> <IV:16 bytes in hex>\t - "
               "Encrypt RAW data\r\n");
    }
}
...
static void subghz_cli_command_encrypt_raw(PipeSide *pipe, FuriString *args)
{
    ...
    do {
        ...
        if (!subghz_keystore_raw_encrypted_save(furi_string_get_cstr(source),
                                                furi_string_get_cstr(destination), iv)) {
            printf("Failed to save Keystore");
            break;
        }
...
static void subghz_cli_command_chat(PipeSide *pipe, FuriString *args)
{
    ...
    if (!furi_hal_region_is_frequency_allowed(frequency)) {
        printf("In your settings/region, only reception on this frequency (%lu) is allowed,\r\n"
    ...
    while (!exit) {
        ...
        case SubGhzChatEventInputData:
            if (chat_event.c == CliKeyETX) {
                ...
                while (!subghz_chat_worker_write(subghz_chat,
            ...
            case SubGhzChatEventUserEntrance:
                furi_string_printf(sysmsg, "\033[0;34m%s joined chat.\033[0m\r\n",
                                   furi_hal_version_get_name_ptr());
                subghz_chat_worker_write(subghz_chat, (uint8_t *)furi_string_get_cstr(sysmsg),
                                         strlen(furi_string_get_cstr(sysmsg)));
                break;
            case SubGhzChatEventUserExit:
                furi_string_printf(sysmsg, "\033[0;31m%s left chat.\033[0m\r\n",
                                   furi_hal_version_get_name_ptr());
                subghz_chat_worker_write(subghz_chat, (uint8_t *)furi_string_get_cstr(sysmsg),
                                         strlen(furi_string_get_cstr(sysmsg)));
                furi_delay_ms(10);
