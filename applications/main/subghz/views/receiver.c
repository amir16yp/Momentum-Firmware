...
void subghz_receiver_rssi(SubGhzViewReceiver *instance, float rssi)
{
    ...
    with_view_model(
        instance->view, SubGhzViewReceiverModel * model,
        {
...
void subghz_view_receiver_set_lock(SubGhzViewReceiver *subghz_receiver, bool lock)
{
    ...
    if (lock == true) {
        ...
        with_view_model(
            ...
            { model->bar_show = SubGhzViewReceiverBarShowLock; }, true);
        furi_timer_start(subghz_receiver->timer, 1000);
    } else {
        with_view_model(
            subghz_receiver->view, SubGhzViewReceiverModel * model,
            { model->bar_show = SubGhzViewReceiverBarShowDefault; }, true);
    }
}

void subghz_view_receiver_set_callback(SubGhzViewReceiver *subghz_receiver,
                                       SubGhzViewReceiverCallback callback, void *context)
{
    furi_assert(subghz_receiver);
...
static void subghz_view_receiver_flip_string_callback(void *context)
{
    ...
    SubGhzViewReceiver *subghz_receiver = context;
    with_view_model(
        subghz_receiver->view, SubGhzViewReceiverModel * model, { model->show_time = false; },
        true);
}

void subghz_view_receiver_add_item_to_menu(SubGhzViewReceiver *subghz_receiver, const char *name,
                                           const char *time, uint8_t type, uint16_t repeats)
{
    furi_assert(subghz_receiver);
    with_view_model(
        subghz_receiver->view, SubGhzViewReceiverModel * model,
...
void subghz_view_receiver_add_data_statusbar(SubGhzViewReceiver *subghz_receiver,
                                             const char *frequency_str, const char *preset_str,
                                             const char *history_stat_str, bool hopping_enabled,
                                             bool bin_raw_enabled, bool show_sats,
                                             SubGhzRepeaterState repeater_state)
{
    furi_assert(subghz_receiver);
    with_view_model(
...
void subghz_view_receiver_add_data_progress(SubGhzViewReceiver *subghz_receiver,
                                            const char *progress_str)
{
    furi_assert(subghz_receiver);
    with_view_model(
        subghz_receiver->view, SubGhzViewReceiverModel * model,
        { furi_string_set(model->progress_str, progress_str); }, true);
}
                                                ...
                                                SubGhzRadioDeviceType device_type)
{
    ...
    with_view_model(
        subghz_receiver->view, SubGhzViewReceiverModel * model,
        { model->device_type = device_type; }, true);
}
...
void subghz_view_receiver_draw(Canvas *canvas, SubGhzViewReceiverModel *model)
{
    ...
    if (!model->nodraw) {
        ...
        for (size_t i = 0; i < MIN(model->history_item, MENU_ITEMS); ++i) {
            ...
            if ((model->idx == idx) && model->show_time) {
                ...
                furi_string_printf(str_buff, "x%u: %s", item_menu->repeats + 1,
    ...
    case SubGhzViewReceiverBarShowToUnlockPress:
        ...
        if (model->history_item == 0 && model->mode == SubGhzViewReceiverModeLive) {
            ...
            canvas_draw_str(canvas, canvas_width(canvas) - (string_width + horizontal_offset),
                            vertical_offset, str);
        } else {
        ...
        if (!furi_string_empty(model->history_stat_str)) {
            canvas_draw_str_aligned(canvas, 114, 62, AlignRight, AlignBottom,
    ...
    default: {
        ...
        if (model->history_item == 0 && model->mode == SubGhzViewReceiverModeLive) {
            if (*frequency_str) {
                canvas_draw_str(canvas, 44 + canvas_string_width(canvas, frequency_str) + 1, 62,
                                "MHz");
            }
            const char *str = furi_string_get_cstr(model->preset_str);
            const uint8_t vertical_offset = 7;
...
static void subghz_view_receiver_timer_callback(void *context)
{
    ...
    if (subghz_receiver->lock_count < UNLOCK_CNT) {
        subghz_receiver->callback(SubGhzCustomEventViewReceiverOffDisplay,
                                  subghz_receiver->context);
    }
    subghz_receiver->lock_count = 0;
}
...
bool subghz_view_receiver_input(InputEvent *event, void *context)
{
    ...
    if (subghz_receiver->lock == true) {
        with_view_model(
            subghz_receiver->view, SubGhzViewReceiverModel * model,
            { model->bar_show = SubGhzViewReceiverBarShowToUnlockPress; }, true);
        ...
        if (subghz_receiver->lock_count >= UNLOCK_CNT) {
            subghz_receiver->callback(SubGhzCustomEventViewReceiverUnlock,
                                      subghz_receiver->context);
            with_view_model(
                subghz_receiver->view, SubGhzViewReceiverModel * model,
                { model->bar_show = SubGhzViewReceiverBarShowUnlock; }, true);
    ...
    if (event->key == InputKeyBack && event->type == InputTypeShort) {
        ...
        with_view_model(
            subghz_receiver->view, SubGhzViewReceiverModel * model,
            {
                if (model->history_item != 0) {
                    // Callback
                    subghz_receiver->callback(SubGhzCustomEventViewReceiverDeleteItem,
                                              subghz_receiver->context);
                }
            },
            false);
...
SubGhzViewReceiver *subghz_view_receiver_alloc(void)
{
    ...
    subghz_receiver->lock = false;
    subghz_receiver->lock_count = 0;
    view_allocate_model(subghz_receiver->view, ViewModelTypeLocking,
                        sizeof(SubGhzViewReceiverModel));
    ...
    subghz_receiver->flip_time_timer = furi_timer_alloc(subghz_view_receiver_flip_string_callback,
                                                        FuriTimerTypeOnce, subghz_receiver);
