...
void subghz_view_transmitter_set_callback(SubGhzViewTransmitter *subghz_transmitter,
                                          SubGhzViewTransmitterCallback callback, void *context)
{
...
void subghz_view_transmitter_add_data_to_show(SubGhzViewTransmitter *subghz_transmitter,
...
void subghz_view_transmitter_set_radio_device_type(SubGhzViewTransmitter *subghz_transmitter,
                                                   SubGhzRadioDeviceType device_type)
{
    furi_assert(subghz_transmitter);
    with_view_model(
...
static void subghz_view_transmitter_button_right(Canvas *canvas, const char *str)
{
    ...
    canvas_draw_icon(canvas, x + horizontal_offset, y - button_height + vertical_offset - 1,
                     &I_ButtonCenter_7x7);
    canvas_draw_str(canvas, x + horizontal_offset + icon_width_with_offset, y - vertical_offset,
                    str);
    canvas_invert_color(canvas);
}
...
void subghz_view_transmitter_draw(Canvas *canvas, SubGhzViewTransmitterModel *model)
{
    ...
    elements_multiline_text_aligned(canvas, 0, 0, AlignLeft, AlignTop,
                                    furi_string_get_cstr(model->key_str));
    ...
    if (can_be_sent) {
        if (event->key == InputKeyOk && event->type == InputTypePress) {
            ...
            with_view_model(
                ...
                true);
            subghz_transmitter->callback(SubGhzCustomEventViewTransmitterSendStart,
                                         subghz_transmitter->context);
            return true;
        } else if (event->key == InputKeyOk && event->type == InputTypeRelease) {
            subghz_transmitter->callback(SubGhzCustomEventViewTransmitterSendStop,
                                         subghz_transmitter->context);
            return true;
                    ...
                    subghz_transmitter->view, SubGhzViewTransmitterModel * model,
                    {
                        ...
                        if (subghz_custom_btn_get_original() != 0) {
                            if (subghz_custom_btn_set(temp_btn_id)) {
                                furi_string_printf(model->temp_button_id, "%01X",
...
SubGhzViewTransmitter *subghz_view_transmitter_alloc(void)
{
    ...
    view_allocate_model(subghz_transmitter->view, ViewModelTypeLocking,
                        sizeof(SubGhzViewTransmitterModel));
    view_set_context(subghz_transmitter->view, subghz_transmitter);
    view_set_draw_callback(subghz_transmitter->view,
                           (ViewDrawCallback)subghz_view_transmitter_draw);
