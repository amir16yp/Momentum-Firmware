...
void subghz_read_raw_set_callback(SubGhzReadRAW *subghz_read_raw, SubGhzReadRAWCallback callback,
                                  void *context)
{
...
void subghz_read_raw_add_data_statusbar(SubGhzReadRAW *instance, const char *frequency_str,
...
void subghz_read_raw_set_radio_device_type(SubGhzReadRAW *instance,
                                           SubGhzRadioDeviceType device_type)
{
    furi_assert(instance);
    with_view_model(
...
#define SUBGHZ_RAW_SIN_AMPLITUDE 11
    for (int i = 113; i > 0; i--) {
        canvas_draw_line(
            canvas, i,
            32 - subghz_read_raw_tab_sin(i + model->ind_sin * 16) / SUBGHZ_RAW_SIN_AMPLITUDE, i + 1,
            32 + subghz_read_raw_tab_sin((i + model->ind_sin * 16 + 1) * 2) /
                     SUBGHZ_RAW_SIN_AMPLITUDE);
        canvas_draw_line(
            canvas, i + 1,
            32 - subghz_read_raw_tab_sin(i + model->ind_sin * 16) / SUBGHZ_RAW_SIN_AMPLITUDE, i + 2,
            32 + subghz_read_raw_tab_sin((i + model->ind_sin * 16 + 1) * 2) /
                     SUBGHZ_RAW_SIN_AMPLITUDE);
    }
}
...
#define SUBGHZ_RAW_END_SCALE 115
    ...
    if (model->rssi_history_end == false) {
        for (int i = SUBGHZ_RAW_END_SCALE; i > 0; i -= 15) {
            ...
            canvas_draw_line(canvas, i - 10, SUBGHZ_RAW_TOP_SCALE, i - 10,
                             SUBGHZ_RAW_TOP_SCALE + 2);
        }
    } else {
        for (int i = SUBGHZ_RAW_END_SCALE - model->ind_write % 15; i > -15; i -= 15) {
            canvas_draw_line(canvas, i, SUBGHZ_RAW_TOP_SCALE, i, SUBGHZ_RAW_TOP_SCALE + 4);
            if (SUBGHZ_RAW_END_SCALE > i + 5)
                canvas_draw_line(canvas, i + 5, SUBGHZ_RAW_TOP_SCALE, i + 5,
                                 SUBGHZ_RAW_TOP_SCALE + 2);
...
void subghz_read_raw_draw_rssi(Canvas *canvas, SubGhzReadRAWModel *model)
{
    ...
    if (model->rssi_history_end == false) {
        ...
        canvas_draw_line(canvas, model->ind_write + 1, 47, model->ind_write + 1,
        ...
        canvas_draw_line(canvas, SUBGHZ_READ_RAW_RSSI_HISTORY_SIZE - 1, 47,
                         SUBGHZ_READ_RAW_RSSI_HISTORY_SIZE - 1, 47 - model->rssi_current);
        canvas_draw_line(canvas, SUBGHZ_READ_RAW_RSSI_HISTORY_SIZE + 1, 47,
                         SUBGHZ_READ_RAW_RSSI_HISTORY_SIZE + 1, 47 - model->rssi_current);

        for (uint8_t i = 13; i < 47; i += width * 2) {
            canvas_draw_line(canvas, SUBGHZ_READ_RAW_RSSI_HISTORY_SIZE, i,
                             SUBGHZ_READ_RAW_RSSI_HISTORY_SIZE, i + width);
        }
        canvas_draw_line(canvas, SUBGHZ_READ_RAW_RSSI_HISTORY_SIZE - 2, 12,
                         SUBGHZ_READ_RAW_RSSI_HISTORY_SIZE + 2, 12);
        canvas_draw_line(canvas, SUBGHZ_READ_RAW_RSSI_HISTORY_SIZE - 1, 13,
                         SUBGHZ_READ_RAW_RSSI_HISTORY_SIZE + 1, 13);
    }
}
...
void subghz_read_raw_draw(Canvas *canvas, SubGhzReadRAWModel *model)
{
    ...
    if (model->not_showing_samples) {
        ...
        canvas_draw_str(canvas, 77, 7,
                        (model->device_type == SubGhzRadioDeviceTypeInternal) ? "R: Int"
                                                                              : "R: Ext");
    } else {
    ...
    case SubGhzReadRAWStatusLoadKeyIDLE:
        ...
        elements_button_center(canvas, "Send");
        elements_text_box(canvas, 4, 20, 110, 30, AlignCenter, AlignCenter,
                          furi_string_get_cstr(model->file_name), true);
        break;
...
void subghz_read_raw_set_status(SubGhzReadRAW *instance, SubGhzReadRAWStatus status,
                                const char *file_name, float raw_threshold_rssi)
{
    furi_assert(instance);
