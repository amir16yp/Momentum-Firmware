...
bool subghz_tx_start(SubGhz *subghz, FlipperFormat *flipper_format)
{
    ...
    case SubGhzTxRxStartTxStateErrorParserOthers:
        dialog_message_show_storage_error(subghz->dialogs,
                                          "Error in protocol\nparameters\ndescription");
        break;
...
bool subghz_key_load(SubGhz *subghz, const char *file_path, bool show_dialog)
{
    ...
    Stream *fff_data_stream = flipper_format_get_raw_stream(subghz_txrx_get_fff_data(subghz->txrx));
    ...
    do {
        ...
        if (!strcmp(furi_string_get_cstr(temp_str), "CUSTOM")) {
            ...
            if (!subghz_setting_load_custom_preset(setting, furi_string_get_cstr(temp_str),
                                                   fff_data_file)) {
        ...
        subghz_txrx_set_preset(subghz->txrx, furi_string_get_cstr(temp_str), temp_data32, temp_lat,
                               temp_lon, preset_data, preset_data_size);
        ...
        if (!strcmp(furi_string_get_cstr(temp_str), "RAW")) {
            ...
            stream_copy_full(flipper_format_get_raw_stream(fff_data_file),
                             flipper_format_get_raw_stream(fff_data));
        }

        if (subghz_txrx_load_decoder_by_name_protocol(subghz->txrx,
...
bool subghz_get_next_name_file(SubGhz *subghz, uint8_t max_len)
{
    ...
    if (subghz_path_is_file(subghz->file_path)) {
        ...
        storage_get_next_filename(storage, furi_string_get_cstr(file_path),
                                  furi_string_get_cstr(file_name), SUBGHZ_APP_FILENAME_EXTENSION,
                                  file_name, max_len);

        furi_string_printf(temp_str, "%s/%s%s", furi_string_get_cstr(file_path),
                           furi_string_get_cstr(file_name), SUBGHZ_APP_FILENAME_EXTENSION);
...
bool subghz_save_protocol_to_file(SubGhz *subghz, FlipperFormat *flipper_format,
...
void subghz_save_to_file(void *context)
{
    ...
    if (subghz_path_is_file(subghz->file_path)) {
        subghz_save_protocol_to_file(subghz, subghz_txrx_get_fff_data(subghz->txrx),
...
bool subghz_load_protocol_from_file(SubGhz *subghz)
{
    ...
    dialog_file_browser_set_basic_options(&browser_options, SUBGHZ_APP_FILENAME_EXTENSION,
                                          &I_sub1_10px);
    browser_options.base_path = SUBGHZ_APP_FOLDER;

    // Input events and views are managed by file_select
    bool res = dialog_file_browser_show(subghz->dialogs, subghz->file_path, subghz->file_path,
                                        &browser_options);
...
bool subghz_rename_file(SubGhz *subghz)
{
    ...
    if (furi_string_cmp(subghz->file_path_tmp, subghz->file_path)) {
        FS_Error fs_result =
            storage_common_rename(storage, furi_string_get_cstr(subghz->file_path_tmp),
                                  furi_string_get_cstr(subghz->file_path));
