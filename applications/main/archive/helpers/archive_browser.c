...
static void archive_folder_open_cb(void *context, uint32_t item_cnt, int32_t file_idx, bool is_root)
{
    furi_assert(context);
                ...
                if (model->item_cnt <= BROWSER_SORT_THRESHOLD) {
                    ...
                    if (selected != NULL) {
                        for (uint32_t i = 0; i < model->item_cnt; i++) {
                            if (!furi_string_cmp(files_array_get(model->files, i)->path,
                                                 selected)) {
                                model->item_idx = i;
                                break;
                            }
...
void archive_file_browser_set_path(ArchiveBrowserView *browser, FuriString *path,
                                   const char *filter_ext, bool skip_assets, bool hide_dot_files,
...
size_t archive_file_get_array_size(ArchiveBrowserView *browser)
{
    ...
    uint16_t size = 0;
    with_view_model(
        browser->view, ArchiveBrowserViewModel * model, { size = files_array_size(model->files); },
        false);
    return size;
}
...
void archive_file_array_rm_selected(ArchiveBrowserView *browser)
{
    ...
    with_view_model(
        browser->view, ArchiveBrowserViewModel * model,
        {
            files_array_remove_v(model->files, model->item_idx - model->array_offset,
...
void archive_file_array_rm_all(ArchiveBrowserView *browser)
{
    ...
    with_view_model(
        browser->view, ArchiveBrowserViewModel * model, { files_array_reset(model->files); },
        false);
}
...
void archive_add_file_item(ArchiveBrowserView *browser, bool is_folder, const char *name)
{
    ...
    with_view_model(
        browser->view, ArchiveBrowserViewModel * model,
        { files_array_push_back(model->files, item); }, false);
    ArchiveFile_t_clear(&item);
}
            ...
            if (show) {
                ...
                if (archive_is_item_in_array(model, model->item_idx)) {
                    ArchiveFile_t *selected =
                        files_array_get(model->files, model->item_idx - model->array_offset);
                    selected->fav = archive_is_favorite("%s", furi_string_get_cstr(selected->path));
...
void archive_switch_tab(ArchiveBrowserView *browser, InputKey key)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (tab == ArchiveTabInternal && !momentum_settings.show_internal_tab)
            continue;
        break;
    }
    ...
    if (tab == ArchiveTabFavorites) {
        ...
        if (archive_is_dir_exists(browser->path)) {
            ...
            archive_file_browser_set_path(browser, browser->path, archive_get_tab_ext(tab),
                                          skip_assets, hide_dot_files, NULL);
