...
static void archive_loader_callback(const void *message, void *context)
{
    ...
    if (event->type == LoaderEventTypeNoMoreAppsInQueue) {
        view_dispatcher_send_custom_event(archive->view_dispatcher, ArchiveBrowserEventListRefresh);
    }
}
...
static void archive_mount_disk_image(ArchiveBrowserView *browser, ArchiveFile_t *selected)
{
    ...
    do {
        ...
        if (!storage_file_open(disk_image, furi_string_get_cstr(selected->path),
                               FSAM_READ | FSAM_WRITE, FSOM_OPEN_EXISTING))
            break;
    ...
    if (disk_image && !browser->disk_image)
        storage_file_free(disk_image);
    furi_record_close(RECORD_STORAGE);
}
...
static void archive_run_in_app(ArchiveBrowserView *browser, ArchiveFile_t *selected, bool favorites)
{
    ...
    if (selected->type == ArchiveFileTypeSetting) {
        ...
        if (slash != FURI_STRING_FAILURE) {
            ...
            loader_start_with_gui_error(loader, furi_string_get_cstr(app_name),
                                        furi_string_get_cstr(app_args));
        ...
        // Folders are handled by archive, so we should only get here with run_with_default_app()
        // outside archive
...
void run_with_default_app(const char *path)
{
    ...
    FileInfo info;
    Storage *storage = furi_record_open(RECORD_STORAGE);
    bool is_dir = storage_common_stat(storage, path, &info) == FSE_OK && info.flags & FSF_DIRECTORY;
    furi_record_close(RECORD_STORAGE);
...
void archive_scene_browser_on_enter(void *context)
{
    ...
    archive->loader_stop_subscription =
        furi_pubsub_subscribe(loader_get_pubsub(archive->loader), archive_loader_callback, archive);
    ...
    scene_manager_set_scene_state(archive->scene_manager, ArchiveAppSceneBrowser,
                                  SCENE_STATE_DEFAULT);
}
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        case ArchiveBrowserEventFileMenuShow:
            if (selected->type == ArchiveFileTypeDiskImage &&
                ...
                archive_show_file(furi_record_open(RECORD_LOADER),
            ...
            if (!favorites) {
                ...
                with_view_model(
                    browser->view, ArchiveBrowserViewModel * model,
                    {
                        if (model->clipboard != NULL) {
                            ...
                            path_concat(furi_string_get_cstr(browser->path),
                                        furi_string_get_cstr(base), path_dst);
                ...
                if (path_src && path_dst) {
                    ...
                    if (error != FSE_OK) {
                        ...
                        furi_string_cat_printf(dialog_msg, "Cannot %s:\n%s", copy ? "copy" : "move",
                                               storage_error_get_desc(error));
                        dialog_message_show_storage_error(archive->dialogs,
                        ...
                        view_dispatcher_send_custom_event(archive->view_dispatcher,
        ...
        case ArchiveBrowserEventSearch: {
            ...
            if (open)
                scene_manager_next_scene(archive->scene_manager, ArchiveAppSceneSearch);
            consumed = true;
            break;
        }
        ...
        case ArchiveBrowserEventExit:
            if (!archive_is_home(browser)) {
                ...
                if (archive->loader_stop_subscription) {
                    furi_pubsub_unsubscribe(loader_get_pubsub(archive->loader),
                                            archive->loader_stop_subscription);
