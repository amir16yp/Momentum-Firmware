...
void archive_scene_search_on_enter(void *context)
{
    ...
    text_input_set_result_callback(text_input, archive_scene_search_text_input_callback, context,
                                   archive->text_store, MAX_NAME_LEN, false);
...
uint32_t archive_scene_search_dirwalk(void *context)
{
    ...
    if (dir_walk_open(dir_walk, STORAGE_EXT_PATH_PREFIX)) {
        while (scene_manager_get_scene_state(archive->scene_manager, ArchiveAppSceneSearch)) {
            ...
            if (!file_info_is_dir(&fileinfo)) {
                furi_string_set(name, furi_string_get_cstr(path) +
                                          furi_string_search_rchar(path, '/') + 1);
    ...
    furi_string_set(archive_get_file_at(archive->browser, 0)->path, "/app:search/Search for files");
...
bool archive_scene_search_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == SCENE_SEARCH_CUSTOM_EVENT) {
            ...
            archive->search_thread =
                furi_thread_alloc_ex("ArchiveSearchWorker", 1024,
                                     (FuriThreadCallback)archive_scene_search_dirwalk, archive);
