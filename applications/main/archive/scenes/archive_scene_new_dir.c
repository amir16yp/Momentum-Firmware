...
void archive_scene_new_dir_on_enter(void *context)
{
    ...
    text_input_set_result_callback(text_input, archive_scene_new_dir_text_input_callback, context,
...
bool archive_scene_new_dir_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == SCENE_NEW_DIR_CUSTOM_EVENT) {
            ...
            path_concat(furi_string_get_cstr(archive->browser->path), archive->text_store,
                        path_dst);
            ...
            if (error != FSE_OK) {
                ...
                furi_string_cat_printf(dialog_msg, "Cannot mkdir:\n%s",
