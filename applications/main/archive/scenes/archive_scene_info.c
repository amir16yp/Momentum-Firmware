...
const char *const units[] = {"Bytes", "KiB", "MiB", "GiB", "TiB"};
    ...
    if (dir_walk_open(dir_walk, furi_string_get_cstr(current->path))) {
        while (scene_manager_get_scene_state(instance->scene_manager, ArchiveAppSceneInfo)) {
            ...
            if (!file_info_is_dir(&fileinfo) || is_last) {
                ...
                snprintf(buf, sizeof(buf), unit ? "Size: %s\e#%.2f\e# %s" : "Size: %s\e#%.0f\e# %s",
                         is_last ? "" : "... ", show, units[unit]);
                widget_element_text_box_set_text(instance->size_element, buf);

                if (instance->count_element) {
                    snprintf(buf, sizeof(buf), "Items: %s\e#%lu\e#", is_last ? "" : "... ",
                             item_count);
...
static uint32_t archive_scene_info_md5sum(void *context)
{
    ...
    ArchiveFile_t *current = archive_get_current_file(instance->browser);
    bool result = false;
    if (storage_file_open(file, furi_string_get_cstr(current->path), FSAM_READ,
                          FSOM_OPEN_EXISTING)) {
        uint8_t output[16];
...
void archive_scene_info_on_enter(void *context)
{
    ...
    widget_add_button_element(instance->widget, GuiButtonTypeLeft, "Back",
                              archive_scene_info_widget_callback, instance);
    ...
    widget_add_text_box_element(instance->widget, 1, 12, 126, 20, AlignLeft, AlignTop,
                                furi_string_get_cstr(dirname), true);
    ...
    FileInfo fileinfo;
    bool is_dir = false;
    if (storage_common_stat(furi_record_open(RECORD_STORAGE), furi_string_get_cstr(current->path),
                            &fileinfo) != FSE_OK) {
        ...
        snprintf(buf, sizeof(buf), unit ? "Size: \e#%.2f\e# %s" : "Size: \e#%.0f\e# %s", show,
                 units[unit]);
    }

    WidgetElement *size_element = widget_add_text_box_element(instance->widget, 1, 31, 126, 13,
                                                              AlignLeft, AlignTop, buf, true);
    WidgetElement *count_element = NULL;
    if (is_dir) {
        snprintf(buf, sizeof(buf), "Items: ... \e#0\e#");
        count_element = widget_add_text_box_element(instance->widget, 1, 42, 126, 13, AlignLeft,
                                                    AlignTop, buf, true);
    }
    ...
    if (!is_dir) {
        ...
        strlcat(buf, "\e*", sizeof(buf));
        size_element = widget_add_text_box_element(instance->widget, 0, 43, 128, 24, AlignRight,
                                                   AlignTop, buf, false);
    }
