...
void archive_scene_rename_on_enter(void *context)
{
    ...
    text_input_set_result_callback(text_input, archive_scene_rename_text_input_callback, archive,
                                   archive->text_store, MAX_NAME_LEN, false);

    // Init validator to show message to user that name already exist
    ValidatorIsFile *validator_is_file = validator_is_file_alloc_init(
        furi_string_get_cstr(path_folder), furi_string_get_cstr(archive->file_extension),
        archive->text_store);
...
bool archive_scene_rename_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == SCENE_RENAME_CUSTOM_EVENT) {
            ...
            furi_string_cat_printf(path_dst, "/%s%s", archive->text_store,
            ...
            FS_Error error =
                archive_copy_rename_file_or_dir(archive->browser, path_src, path_dst, false, false);
