...
void ibutton_scene_emulate_on_enter(void *context)
{
    ...
    widget_add_text_box_element(widget, 52, 24, 75, 40, AlignCenter, AlignTop,
                                furi_string_get_cstr(tmp), true);

    widget_add_string_multiline_element(widget, 88, 10, AlignCenter, AlignTop, FontPrimary,
                                        "Emulating");
...
bool ibutton_scene_emulate_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeTick) {
        ...
        if (event.event == iButtonCustomEventWorkerEmulated) {
            ...
            scene_manager_set_scene_state(ibutton->scene_manager, iButtonSceneEmulate,
                                          EMULATE_TIMEOUT_TICKS);
        }
    }
