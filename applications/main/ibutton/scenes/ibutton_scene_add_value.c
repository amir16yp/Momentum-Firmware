...
void ibutton_scene_add_value_on_enter(void *context)
{
    ...
    byte_input_set_result_callback(ibutton->byte_input, ibutton_scene_add_type_byte_input_callback,
                                   ibutton_scene_add_type_byte_changed_callback, context,
                                   editable_data.ptr, editable_data.size);
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        if (event.event == iButtonCustomEventByteEditResult) {
            if (scene_manager_has_previous_scene(scene_manager, iButtonSceneAddType)) {
                ...
                furi_string_printf(ibutton->file_path, "%s/%s%s", IBUTTON_APP_FOLDER,
                                   ibutton->key_name, IBUTTON_APP_FILENAME_EXTENSION);
        ...
        if (scene_manager_has_previous_scene(scene_manager, iButtonSceneSavedKeyMenu)) {
            if (!ibutton_load_key(ibutton, true)) {
                consumed = scene_manager_search_and_switch_to_previous_scene(scene_manager,
                                                                             iButtonSceneStart);
            }
        }
    }
