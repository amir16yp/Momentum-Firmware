...
void ibutton_scene_save_name_on_enter(void *context)
{
    ...
    if (is_new_file) {
        name_generator_make_auto(
            ibutton->key_name, IBUTTON_KEY_NAME_SIZE,
            ibutton_protocols_get_name(ibutton->protocols,
                                       ibutton_key_get_protocol_id(ibutton->key)));
    }

    text_input_set_header_text(text_input, "Name the key");
    text_input_set_result_callback(text_input, ibutton_scene_save_name_text_input_callback, ibutton,
                                   ibutton->key_name, IBUTTON_KEY_NAME_SIZE, is_new_file);
...
bool ibutton_scene_save_name_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        if (event.event == iButtonCustomEventTextEditResult) {
            ...
            furi_string_printf(ibutton->file_path, "%s/%s%s", IBUTTON_APP_FOLDER, ibutton->key_name,
                               IBUTTON_APP_FILENAME_EXTENSION);

            if (ibutton_save_key(ibutton)) {
                scene_manager_next_scene(ibutton->scene_manager, iButtonSceneSaveSuccess);

                if (scene_manager_has_previous_scene(ibutton->scene_manager,
                                                     iButtonSceneSavedKeyMenu)) {
                    // Nothing, do not count editing as saving
                } else if (scene_manager_has_previous_scene(ibutton->scene_manager,
                                                            iButtonSceneAddType)) {
                ...
                const uint32_t possible_scenes[] = {iButtonSceneReadKeyMenu,
                                                    iButtonSceneSavedKeyMenu, iButtonSceneAddType};
