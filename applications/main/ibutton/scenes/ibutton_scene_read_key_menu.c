...
void ibutton_scene_read_key_menu_on_enter(void *context)
{
    ...
    submenu_add_item(submenu, "Save", SubmenuIndexSave,
                     ibutton_scene_read_key_menu_submenu_callback, ibutton);
    submenu_add_item(submenu, "Emulate", SubmenuIndexEmulate,
                     ibutton_scene_read_key_menu_submenu_callback, ibutton);

    if (features & iButtonProtocolFeatureWriteId) {
        submenu_add_item(submenu, "Write ID", SubmenuIndexWriteId,
                         ibutton_scene_read_key_menu_submenu_callback, ibutton);
    }

    if (features & iButtonProtocolFeatureWriteCopy) {
        submenu_add_item(submenu, "Full Write on Same Type", SubmenuIndexWriteCopy,
                         ibutton_scene_read_key_menu_submenu_callback, ibutton);
    }

    if (features & iButtonProtocolFeatureExtData) {
        submenu_add_item(submenu, "Data Info", SubmenuIndexViewData,
                         ibutton_scene_read_key_menu_submenu_callback, ibutton);
    }
...
bool ibutton_scene_read_key_menu_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        scene_manager_set_scene_state(ibutton->scene_manager, iButtonSceneReadKeyMenu,
                                      SubmenuIndexSave);
