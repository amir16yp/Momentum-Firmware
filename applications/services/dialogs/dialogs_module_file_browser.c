...
bool dialogs_app_process_module_file_browser(const DialogsAppMessageDataFileBrowser *data)
{
    ...
    view_holder_set_back_callback(view_holder, dialogs_app_file_browser_back_callback,
                                  file_browser_context);

    FileBrowser *file_browser = file_browser_alloc(data->result_path);
    file_browser_set_callback(file_browser, dialogs_app_file_browser_callback,
                              file_browser_context);
    file_browser_configure(file_browser, data->extension, data->base_path, data->skip_assets,
                           data->hide_dot_files, data->file_icon, data->hide_ext);
