...
static void cli_vcp_maybe_receive_data(CliVcp *cli_vcp)
{
    if (!cli_vcp->own_pipe)
        return;
    if (pipe_spaces_available(cli_vcp->own_pipe) < USB_CDC_PKT_LEN)
        return;
...
static void cli_vcp_internal_event_happened(FuriEventLoopObject *object, void *context)
{
    ...
    case CliVcpInternalEventConnected: {
        ...
        pipe_set_data_arrived_callback(cli_vcp->own_pipe, cli_vcp_data_from_shell,
                                       FuriEventLoopEventFlagEdge);
        pipe_set_space_freed_callback(cli_vcp->own_pipe, cli_vcp_shell_ready,
                                      FuriEventLoopEventFlagEdge);
        furi_delay_ms(33); // we are too fast, minicom isn't ready yet
        cli_vcp->shell = cli_shell_alloc(cli_main_motd, NULL, cli_vcp->shell_pipe,
                                         cli_vcp->main_registry, &cli_main_ext_config);
...
static CliVcp *cli_vcp_alloc(void)
{
    ...
    furi_event_loop_subscribe_message_queue(cli_vcp->event_loop, cli_vcp->message_queue,
                                            FuriEventLoopEventIn, cli_vcp_message_received,
                                            cli_vcp);

    cli_vcp->internal_evt_queue =
        furi_message_queue_alloc(VCP_MESSAGE_Q_LEN, sizeof(CliVcpInternalEvent));
    furi_event_loop_subscribe_message_queue(cli_vcp->event_loop, cli_vcp->internal_evt_queue,
                                            FuriEventLoopEventIn, cli_vcp_internal_event_happened,
                                            cli_vcp);
