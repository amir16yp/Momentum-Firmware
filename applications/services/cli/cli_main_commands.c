...
void cli_command_date(PipeSide *pipe, FuriString *args, void *context)
{
    ...
    if (furi_string_size(args) > 0) {
        ...
        int ret = sscanf(furi_string_get_cstr(args), "%hu-%hu-%hu %hu:%hu:%hu %hu", &year, &month,
                         &day, &hours, &minutes, &seconds, &weekday);
        ...
        if (ret != 7) {
            printf("Invalid datetime format, use `%s`. sscanf %d %s", "%Y-%m-%d %H:%M:%S %u", ret,
        ...
        printf("New datetime is: " CLI_DATE_FORMAT, datetime.year, datetime.month, datetime.day,
               datetime.hour, datetime.minute, datetime.second, datetime.weekday);
    } else {
        furi_hal_rtc_get_datetime(&datetime);
        printf(CLI_DATE_FORMAT, datetime.year, datetime.month, datetime.day, datetime.hour,
...
bool cli_command_log_level_set_from_string(FuriString *level)
{
    ...
    if (furi_log_level_from_string(furi_string_get_cstr(level), &log_level)) {
        ...
        printf("<log debug> — debug information including <log info> (may impact system "
               "performance)\r\n");
        printf("<log trace> — system traces including <log debug> (may impact system "
               "performance)\r\n");
    }
    return false;
}
...
void cli_command_log(PipeSide *pipe, FuriString *args, void *context)
{
    ...
    if (restore_log_level) {
        // There will be strange behaviour if log level is set from settings while log command is
        // running
...
void cli_command_sysctl_log_level(PipeSide *pipe, FuriString *args, void *context)
{
    ...
    if (!furi_log_level_from_string(furi_string_get_cstr(args), &log_level)) {
        cli_print_usage("sysctl log_level", "<error|warn|info|default|debug|trace>",
                        furi_string_get_cstr(args));
        return;
    }
...
static void cli_command_top(PipeSide *pipe, FuriString *args, void *context)
{
    ...
    do {
        ...
        printf("Threads: %zu, ISR Time: %0.2f%%, Uptime: %luh%lum%lus" ANSI_ERASE_LINE(
                   ANSI_ERASE_FROM_CURSOR_TO_END) "\r\n",
               furi_thread_list_size(thread_list),
               (double)furi_thread_list_get_isr_time(thread_list), uptime / 60 / 60,
        ...
        printf(
            ...
            memmgr_get_total_heap(), memmgr_get_free_heap(), memmgr_get_minimum_free_heap(),
            memmgr_heap_get_max_free_block());

        printf("%-17s %-20s %-10s %5s %12s %6s %10s %7s %5s" ANSI_ERASE_LINE(
                   ANSI_ERASE_FROM_CURSOR_TO_END) "\r\n",
               "AppID", "Name", "State", "Prio", "Stack start", "Stack", "Stack Min", "Heap",
               "%CPU");

        for (size_t i = 0; i < furi_thread_list_size(thread_list); i++) {
            const FuriThreadListItem *item = furi_thread_list_get_at(thread_list, i);
            printf("%-17s %-20s %-10s %5d   0x%08lx %6lu %10lu %7zu %5.1f" ANSI_ERASE_LINE(
                       ANSI_ERASE_FROM_CURSOR_TO_END) "\r\n",
                   item->app_id, item->name, item->state, item->priority, item->stack_address,
                   item->stack_size, item->stack_min_free, item->heap, (double)item->cpu);
        }
...
void cli_command_i2c(PipeSide *pipe, FuriString *args, void *context)
{
    ...
    for (uint8_t row = 0; row < 0x8; row++) {
        ...
        for (uint8_t column = 0; column <= 0xF; column++) {
            bool ret = furi_hal_i2c_is_device_ready(&furi_hal_i2c_handle_external,
                                                    ((row << 4) + column) << 1, 2);
            printf("%c ", ret ? '#' : '-');
        }
        printf("\r\n");
...
void cli_command_echo(PipeSide *pipe, FuriString *args, void *context)
{
    ...
    while (true) {
        ...
        if (memchr(buffer, CliKeyETX, read))
            break;
...
{
    cli_registry_add_command(registry, "!", CliCommandFlagParallelSafe, cli_command_info,
                             (void *)true);
    cli_registry_add_command(registry, "info", CliCommandFlagParallelSafe, cli_command_info, NULL);
    cli_registry_add_command(registry, "device_info", CliCommandFlagParallelSafe, cli_command_info,
                             (void *)true);
    ...
    cli_registry_add_command(registry, "free_blocks", CliCommandFlagParallelSafe,
                             cli_command_free_blocks, NULL);
    cli_registry_add_command(registry, "echo", CliCommandFlagParallelSafe, cli_command_echo, NULL);
    cli_registry_add_command(registry, "sleep", CliCommandFlagParallelSafe, cli_command_sleep,
                             NULL);
}
