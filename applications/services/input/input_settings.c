...
void input_settings_load(InputSettings *settings)
{
    ...
    do {
        // take version from settings file metadata, if cant then break and fill settings with 0 and
        // save to settings file;
        uint8_t version;
        if (!saved_struct_get_metadata(INPUT_SETTINGS_PATH, NULL, &version, NULL))
            break;
        ...
        if (version == INPUT_SETTINGS_VER) {
            success = saved_struct_load(INPUT_SETTINGS_PATH, settings, sizeof(InputSettings),
                                        INPUT_SETTINGS_MAGIC, INPUT_SETTINGS_VER);
        }
...
void input_settings_save(const InputSettings *settings)
{
    ...
    const bool success = saved_struct_save(INPUT_SETTINGS_PATH, settings, sizeof(InputSettings),
                                           INPUT_SETTINGS_MAGIC, INPUT_SETTINGS_VER);
