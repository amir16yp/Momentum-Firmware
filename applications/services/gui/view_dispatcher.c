...
ViewDispatcher *view_dispatcher_alloc_ex(FuriEventLoop *loop)
{
    ...
    view_port_draw_callback_set(view_dispatcher->view_port, view_dispatcher_draw_callback,
                                view_dispatcher);
    view_port_input_callback_set(view_dispatcher->view_port, view_dispatcher_input_callback,
                                 view_dispatcher);
    view_port_ascii_callback_set(view_dispatcher->view_port, view_dispatcher_ascii_callback,
                                 view_dispatcher);
    ...
    furi_event_loop_subscribe_message_queue(view_dispatcher->event_loop,
                                            view_dispatcher->input_queue, FuriEventLoopEventIn,
                                            view_dispatcher_run_input_callback, view_dispatcher);

    view_dispatcher->ascii_queue =
        furi_message_queue_alloc(VIEW_DISPATCHER_QUEUE_LEN, sizeof(AsciiEvent));
    furi_event_loop_subscribe_message_queue(view_dispatcher->event_loop,
                                            view_dispatcher->ascii_queue, FuriEventLoopEventIn,
                                            view_dispatcher_run_ascii_callback, view_dispatcher);

    view_dispatcher->event_queue =
        furi_message_queue_alloc(VIEW_DISPATCHER_QUEUE_LEN, sizeof(uint32_t));
    furi_event_loop_subscribe_message_queue(view_dispatcher->event_loop,
                                            view_dispatcher->event_queue, FuriEventLoopEventIn,
                                            view_dispatcher_run_event_callback, view_dispatcher);
...
void view_dispatcher_run(ViewDispatcher *view_dispatcher)
{
    ...
    uint32_t tick_period =
        view_dispatcher->tick_period == 0 ? FuriWaitForever : view_dispatcher->tick_period;

    if (view_dispatcher->is_event_loop_owned)
        furi_event_loop_tick_set(view_dispatcher->event_loop, tick_period,
...
void view_dispatcher_attach_to_gui(ViewDispatcher *view_dispatcher, Gui *gui,
...
{
    ViewDispatcher *view_dispatcher = context;
    furi_check(furi_message_queue_put(view_dispatcher->input_queue, event, FuriWaitForever) ==
               FuriStatusOk);
}
...
bool view_dispatcher_ascii_callback(AsciiEvent *event, void *context)
{
    ...
    ViewDispatcher *view_dispatcher = context;
    furi_check(furi_message_queue_put(view_dispatcher->ascii_queue, event, FuriWaitForever) ==
               FuriStatusOk);
    return true;
}
...
void view_dispatcher_handle_input(ViewDispatcher *view_dispatcher, InputEvent *event)
{
    ...
    if (event->type == InputTypePress) {
        ...
        FURI_LOG_D(TAG, "non-complementary input, discarding key: %s, type: %s, sequence: %p",
                   input_get_key_name(event->key), input_get_type_name(event->type),
                   (void *)event->sequence);
        return;
    }
    ...
    if (view_dispatcher->current_view &&
        ...
        FURI_LOG_D(TAG,
                   "View changed while key press %p -> %p. Sending key: %s, type: %s, sequence: %p "
                   "to previous view",
                   view_dispatcher->ongoing_input_view, view_dispatcher->current_view,
                   input_get_key_name(event->key), input_get_type_name(event->type),
                   (void *)event->sequence);
...
void view_dispatcher_send_custom_event(ViewDispatcher *view_dispatcher, uint32_t event)
{
    ...
    furi_check(furi_message_queue_put(view_dispatcher->event_queue, &event, FuriWaitForever) ==
               FuriStatusOk);
}
