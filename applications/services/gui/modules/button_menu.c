...
static void button_menu_draw_text(Canvas *canvas, uint8_t item_x, uint8_t item_y, const char *text,
                                  bool selected, ButtonMenuModel *model)
{
    const size_t text_width = canvas_string_width(canvas, text);
    if (text_width < ITEM_WIDTH - 8 || (!selected && text_width <= ITEM_WIDTH - 6)) {
        canvas_draw_str_aligned(canvas, item_x + (ITEM_WIDTH / 2), item_y + (ITEM_HEIGHT / 2),
    ...
    if (selected) {
        if (text_width >= ITEM_WIDTH - 8) {
            elements_scrollable_text_line(canvas, item_x + 4, item_y + ITEM_HEIGHT - 4,
                                          ITEM_WIDTH - 8, disp_str, model->scroll_counter, false);
            draw_static = false;
        }
    }
    ...
    if (draw_static) {
        ...
        canvas_draw_str_aligned(canvas, item_x + (ITEM_WIDTH / 2), item_y + (ITEM_HEIGHT / 2),
                                AlignCenter, AlignCenter, furi_string_get_cstr(disp_str));
    }

    furi_string_free(disp_str);
}

static void button_menu_draw_control_button(Canvas *canvas, uint8_t item_position, const char *text,
                                            bool selected, ButtonMenuModel *model)
{
    furi_assert(canvas);
    furi_assert(text);
...
static void button_menu_draw_common_button(Canvas *canvas, uint8_t item_position, const char *text,
                                           bool selected, ButtonMenuModel *model)
{
    furi_assert(canvas);
    furi_assert(text);
...
static void button_menu_view_draw_callback(Canvas *canvas, void *_model)
{
    ...
    if (model->header) {
        ...
        if (header_width >= ITEM_WIDTH - 8) {
            elements_scrollable_text_line(canvas, 3, 13, ITEM_WIDTH - 8, disp_str,
                                          model->scroll_counter, false);
        } else {
    ...
    for (size_t item_position = first_item;
        ...
        if (item->type == ButtonMenuItemTypeControl) {
            button_menu_draw_control_button(canvas, item_position % BUTTONS_PER_SCREEN, item->label,
                                            (item_position == model->position), model);
        } else if (item->type == ButtonMenuItemTypeCommon) {
            button_menu_draw_common_button(canvas, item_position % BUTTONS_PER_SCREEN, item->label,
                                           (item_position == model->position), model);
        }
    }
}
...
{
    ButtonMenu *button_menu = context;
    with_view_model(button_menu->view, ButtonMenuModel * model, { model->scroll_counter++; }, true);
}
...
ButtonMenu *button_menu_alloc(void)
{
    ...
    with_view_model(
        button_menu->view, ButtonMenuModel * model,
        {
            ...
            model->header = NULL;
            model->scroll_counter = 0;
            model->scroll_timer = furi_timer_alloc(button_menu_process_timer_callback,
                                                   FuriTimerTypePeriodic, button_menu);
