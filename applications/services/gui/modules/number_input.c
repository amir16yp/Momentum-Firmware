...
static const NumberInputKey keyboard_keys_row_1[] = {
    ...
    {'3', 33, 12}, {'4', 44, 12}, {backspace_symbol, 100, 3},
};

static const NumberInputKey keyboard_keys_row_2[] = {
    {'5', 0, 26},  {'6', 11, 26},         {'7', 22, 26},          {'8', 33, 26},
    {'9', 44, 26}, {sign_symbol, 55, 17}, {enter_symbol, 95, 17},
};
...
static void number_input_backspace_cb(NumberInputModel *model)
{
    ...
    furi_string_set_strn(model->text_buffer, furi_string_get_cstr(model->text_buffer),
                         text_length - 1);
...
static void number_input_add_digit(NumberInputModel *model, char *newChar)
{
    ...
    if ((model->max_value >= 0 && is_number_too_large(model)) ||
        ...
        // you still need to be able to type invalid numbers in some cases to reach valid numbers on
        // later keypress
        furi_string_printf(model->text_buffer, "%ld", model->current_number);
        return;
    }
        ...
        for (size_t column = 0; column < column_count; column++) {
            ...
            if (keys[column].text == enter_symbol) {
                if (is_number_too_small(model) || is_number_too_large(model)) {
                    // in some cases you need to be able to type a number smaller/larger than the
                    // limits (expl. min = 50, clear all and editor must allow to type 9 and later 0
                    // for 90)
                    ...
                    if (model->selected_row == row && model->selected_column == column) {
                        ...
                        canvas_draw_icon(canvas, keyboard_origin_x + keys[column].x,
                                         keyboard_origin_y + keys[column].y, &I_KeySave_22x11);
                    }
                }
                ...
                if (model->selected_row == row && model->selected_column == column) {
                    ...
                    canvas_draw_icon(canvas, keyboard_origin_x + keys[column].x,
                                     keyboard_origin_y + keys[column].y, &I_KeyBackspace_17x11);
                }
            } else if (keys[column].text == sign_symbol) {
                if (model->selected_row == row && model->selected_column == column) {
                    canvas_draw_icon(canvas, keyboard_origin_x + keys[column].x,
                                     keyboard_origin_y + keys[column].y, &I_KeySignSelected_21x11);
                } else {
                    canvas_draw_icon(canvas, keyboard_origin_x + keys[column].x,
                                     keyboard_origin_y + keys[column].y, &I_KeySign_21x11);
                }
            } else {
                if (model->selected_row == row && model->selected_column == column) {
                    elements_slightly_rounded_box(canvas, keyboard_origin_x + keys[column].x - 3,
...
void number_input_set_result_callback(NumberInput *number_input, NumberInputCallback callback,
...
void number_input_set_header_text(NumberInput *number_input, const char *text)
{
    furi_check(number_input);
    with_view_model(
        number_input->view, NumberInputModel * model, { furi_string_set(model->header, text); },
        true);
}
