...
static inline void date_time_input_draw_block(Canvas *canvas, int32_t x, int32_t y, size_t w,
...
static inline void date_time_input_draw_text(Canvas *canvas, int32_t x, int32_t y, size_t w,
                                             size_t h, Font font, EditState state, const char *text)
{
    furi_assert(canvas);
    furi_assert(text);
...
static void date_time_input_draw_hour_24hr_callback(Canvas *canvas, DateTimeInputModel *model)
{
    ...
    date_time_input_draw_block(canvas, 30, ROW_1_Y, 28, ROW_1_H, FontBigNumbers,
                               get_state(model, 1, 0, hour), buffer);
...
static void date_time_input_draw_hour_12hr_callback(Canvas *canvas, DateTimeInputModel *model)
{
    ...
    date_time_input_draw_block(canvas, 8, ROW_1_Y, 50, ROW_1_H, FontBigNumbers,
                               get_state(model, 1, 0, hour), buffer);
    canvas_draw_box(canvas, 60, ROW_1_Y + ROW_1_H - 7, 2, 2);
    canvas_draw_box(canvas, 60, ROW_1_Y + ROW_1_H - 7 - 6, 2, 2);

    snprintf(buffer, sizeof(buffer), "%02u", hour);
    date_time_input_draw_text(canvas, 8, ROW_1_Y, 30, ROW_1_H, FontBigNumbers,
                              get_state(model, 1, 0, hour), buffer);

    // The AM and PM text shift by 1 pixel so compensate to make them line up
    if (model->datetime->hour < 12) {
        date_time_input_draw_text(canvas, 30, ROW_1_Y + 3, 30, ROW_1_H, FontPrimary,
                                  get_state(model, 1, 0, hour), "AM");
    } else {
        date_time_input_draw_text(canvas, 31, ROW_1_Y + 3, 30, ROW_1_H, FontPrimary,
                                  get_state(model, 1, 0, hour), "PM");
    }
}
...
static void date_time_input_draw_time_callback(Canvas *canvas, DateTimeInputModel *model)
{
    ...
    date_time_input_draw_block(canvas, 64, ROW_1_Y, 28, ROW_1_H, FontBigNumbers,
                               get_state(model, 1, 1, minute), buffer);
    ...
    date_time_input_draw_block(canvas, 98, ROW_1_Y, 28, ROW_1_H, FontBigNumbers,
                               get_state(model, 1, 2, second), buffer);
}
...
static void date_time_input_draw_date_callback(Canvas *canvas, DateTimeInputModel *model)
{
    ...
    date_time_input_draw_block(canvas, 2, ROW_0_Y, 56, ROW_0_H, FontBigNumbers,
                               get_state(model, 0, 0, year), buffer);
    snprintf(buffer, sizeof(buffer), "%02u", model->datetime->month);
    date_time_input_draw_block(canvas, 64, ROW_0_Y, 28, ROW_0_H, FontBigNumbers,
                               get_state(model, 0, 1, month), buffer);
    canvas_draw_box(canvas, 64 - 5, ROW_0_Y + (ROW_0_H / 2), 4, 2);
    snprintf(buffer, sizeof(buffer), "%02u", model->datetime->day);
    date_time_input_draw_block(canvas, 98, ROW_0_Y, 28, ROW_0_H, FontBigNumbers,
                               get_state(model, 0, 2, day), buffer);
...
static bool date_time_input_view_input_callback(InputEvent *event, void *context)
{
    ...
    with_view_model(
        instance->view, DateTimeInputModel * model,
        {
...
DateTimeInput *date_time_input_alloc(void)
{
    ...
    with_view_model(
        date_time_input->view, DateTimeInputModel * model,
        {
...
void date_time_input_set_result_callback(DateTimeInput *date_time_input,
                                         DateTimeChangedCallback changed_callback,
                                         DateTimeDoneCallback done_callback, void *callback_context,
...
void date_time_input_set_editable_fields(DateTimeInput *date_time_input, bool year, bool month,
                                         bool day, bool hour, bool minute, bool second)
{
    furi_check(date_time_input);
