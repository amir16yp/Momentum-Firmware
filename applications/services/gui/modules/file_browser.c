...
#define M_OPL_BrowserItem_t()                                                                      \
    (INIT(API_2(BrowserItem_t_init)), SET(API_6(BrowserItem_t_set)),                               \
     INIT_SET(API_6(BrowserItem_t_init_set)), CLEAR(API_2(BrowserItem_t_clear)),                   \
     CMP(API_6(BrowserItem_t_cmp)), SWAP(M_SWAP_DEFAULT), EQUAL(API_6(M_EQUAL_DEFAULT)))
...
static void browser_folder_open_cb(void *context, uint32_t item_cnt, int32_t file_idx,
                                   bool is_root);
static void browser_list_load_cb(void *context, uint32_t list_load_offset);
static void browser_list_item_cb(void *context, FuriString *item_path, bool is_folder,
                                 bool is_last);
...
void file_browser_configure(FileBrowser *browser, const char *extension, const char *base_path,
                            bool skip_assets, bool hide_dot_files, const Icon *file_icon,
                            bool hide_ext)
{
    furi_check(browser);
...
{
    furi_check(browser);
    browser->worker = file_browser_worker_alloc(path, browser->base_path, browser->ext_filter,
                                                browser->skip_assets, browser->hide_dot_files);
...
void file_browser_set_item_callback(FileBrowser *browser, FileBrowserLoadItemCallback callback,
                                    void *context)
{
    furi_check(browser);
    ...
    with_view_model(
        browser->view, FileBrowserModel * model,
        {
            ...
            if ((model->item_cnt > (LIST_ITEMS - 1)) &&
                ...
                model->list_offset = CLAMP(model->item_idx - (int32_t)(LIST_ITEMS - 2),
                                           (int32_t)model->item_cnt - bounds, 0);
            } else if (model->list_offset > model->item_idx - bounds) {
                model->list_offset =
...
static void browser_folder_open_cb(void *context, uint32_t item_cnt, int32_t file_idx, bool is_root)
{
    furi_check(context);
    ...
    with_view_model(
        browser->view, FileBrowserModel * model,
        {
            ...
            if (is_root) {
                ...
                load_offset = CLAMP(model->item_idx - ITEM_LIST_LEN_MAX / 2 - 1,
                                    (int32_t)model->item_cnt - 1, 0);
            }
            model->array_offset = 0;
            model->list_offset = 0;
    ...
    if (!is_last) {
        ...
        if (is_folder) {
            ...
            if (browser->item_callback) {
                ...
                if (!browser->item_callback(item_path, browser->item_context,
                                            &item.custom_icon_data, item.display_name)) {
        ...
        if (furi_string_empty(item.display_name)) {
            path_extract_filename(item_path, item.display_name,
        ...
        with_view_model(
            browser->view, FileBrowserModel * model, { items_array_push_back(model->items, item); },
            instant_update);
                ...
                if (model->item_cnt <= BROWSER_SORT_THRESHOLD) {
                    ...
                    if (selected != NULL) {
                        for (uint32_t i = 0; i < model->item_cnt; i++) {
                            if (!furi_string_cmp(items_array_get(model->items, i)->name,
                                                 selected)) {
                                model->item_idx = i;
                                break;
                            }
...
static void browser_draw_frame(Canvas *canvas, uint16_t idx, bool scrollbar)
{
    ...
    canvas_draw_box(canvas, 0, Y_OFFSET + idx * FRAME_HEIGHT, (scrollbar ? 122 : 127),
                    FRAME_HEIGHT);
    ...
    canvas_draw_dot(canvas, scrollbar ? 121 : 126,
                    (Y_OFFSET + idx * FRAME_HEIGHT) + (FRAME_HEIGHT - 1));
}
...
static void browser_draw_list(Canvas *canvas, FileBrowserModel *model)
{
    ...
    for (uint32_t i = 0; i < MIN(model->item_cnt, LIST_ITEMS); i++) {
        ...
        if (custom_icon_data) { //-V547
            ...
            canvas_draw_bitmap(canvas, 2, Y_OFFSET + 1 + i * FRAME_HEIGHT, 10, 10,
                               custom_icon_data);
            ...
            canvas_draw_icon(canvas, 2, Y_OFFSET + 1 + i * FRAME_HEIGHT,
                             BrowserItemIcons[item_type]);
        }
        elements_scrollable_text_line(canvas, 15, Y_OFFSET + 9 + i * FRAME_HEIGHT,
                                      (show_scrollbar ? MAX_LEN_PX - 6 : MAX_LEN_PX), filename,
                                      scroll_counter, (model->item_idx != idx));
    }

    if (show_scrollbar) {
        elements_scrollbar_pos(canvas, 126, Y_OFFSET, canvas_height(canvas) - Y_OFFSET,
                               model->item_idx, model->item_cnt);
    }

    uint32_t folder_item_cnt = (model->is_root) ? (model->item_cnt) : (model->item_cnt - 1);
    if (folder_item_cnt == 0) {
        canvas_set_color(canvas, ColorBlack);
        canvas_draw_str_aligned(canvas, canvas_width(canvas) / 2, canvas_height(canvas) / 2,
...
static bool file_browser_view_input_callback(InputEvent *event, void *context)
{
    ...
    with_view_model(
        browser->view, FileBrowserModel * model,
        { is_loading = model->folder_loading || model->list_loading; }, false);
    ...
    if (is_loading && event->key != InputKeyBack) {
        ...
        if (event->type == InputTypeShort || event->type == InputTypeRepeat) {
            with_view_model(
                browser->view, FileBrowserModel * model,
                {
                    ...
                    if (event->key == InputKeyUp) {
                        ...
                        if (browser_is_list_load_required(model)) {
                            model->list_loading = true;
                            int32_t load_offset = CLAMP(model->item_idx - ITEM_LIST_LEN_MAX / 4 * 3,
                                                        (int32_t)model->item_cnt, 0);
                            file_browser_worker_load(browser->worker, load_offset,
                                                     ITEM_LIST_LEN_MAX);
                        }
                        ...
                        if (browser_is_list_load_required(model)) {
                            model->list_loading = true;
                            int32_t load_offset = CLAMP(model->item_idx - ITEM_LIST_LEN_MAX / 4 * 1,
                                                        (int32_t)model->item_cnt, 0);
                            file_browser_worker_load(browser->worker, load_offset,
                                                     ITEM_LIST_LEN_MAX);
                        }
            ...
            with_view_model(
                browser->view, FileBrowserModel * model, { model->button_held_for_ticks = 0; },
                true);
        }
        ...
        if (event->type == InputTypeShort) {
            ...
            if (selected_item) {
                if (selected_item->type == BrowserItemTypeBack) {
                    ...
                    path_concat(file_browser_worker_get_path_current(browser->worker),
                                furi_string_get_cstr(selected_item->name), tmp_path);
                    ...
                    path_concat(file_browser_worker_get_path_current(browser->worker),
                                furi_string_get_cstr(selected_item->name), browser->result_path);
                    if (browser->callback) {
