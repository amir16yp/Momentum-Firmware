...
static bool browser_folder_init(BrowserWorker *browser, FuriString *path, FuriString *filename,
                                uint32_t *item_cnt, int32_t *file_idx)
{
    bool state = false;
    FileInfo file_info;
        ...
        while (1) {
            ...
            if ((storage_file_get_error(directory) == FSE_OK) && (name_temp[0] != '\0')) {
                ...
                if (total_files_cnt == LONG_LOAD_THRESHOLD) {
                    // There are too many files in folder and counting them will take some time -
...
// Load files list by chunks, like it was originally, not compatible with sorting, sorting needs to
// be disabled to use this
static bool browser_folder_load_chunked(BrowserWorker *browser, FuriString *path, uint32_t offset,
                                        uint32_t count)
{
    FileInfo file_info;
            ...
            if (storage_file_get_error(directory) == FSE_OK) {
                ...
                if (browser_filter_by_name(browser, name_str, file_info_is_dir(&file_info))) {
                    ...
                    if (browser->list_item_cb) {
                        browser->list_item_cb(browser->cb_ctx, name_str,
                                              file_info_is_dir(&file_info), false);
                    }
                    items_cnt++;
                }
            } else {
    ...
    do {
        ...
        while (storage_dir_read(directory, &file_info, name_temp, FILE_NAME_LEN_MAX) &&
            ...
            if (browser_filter_by_name(browser, name_str, file_info_is_dir(&file_info))) {
                furi_string_printf(name_str, "%s/%s", furi_string_get_cstr(path), name_temp);
                if (browser->list_item_cb) {
                    browser->list_item_cb(browser->cb_ctx, name_str, file_info_is_dir(&file_info),
                                          false);
                }
                items_cnt++;
            }
...
static int32_t browser_worker(void *context)
{
    ...
    while (1) {
        uint32_t flags = furi_thread_flags_wait(WORKER_FLAGS_ALL, FuriFlagWaitAny, FuriWaitForever);
        ...
        if (flags & WorkerEvtConfigChange) {
            if (browser->keep_selection && furi_string_start_with(path, browser->path_next)) {
                ...
                FURI_LOG_D(TAG, "Config to: %s items: %lu idx: %ld", furi_string_get_cstr(path),
                           items_cnt, file_idx);
        ...
        if (flags & WorkerEvtFolderEnter) {
            ...
            browser_folder_init(browser, path, filename, &items_cnt, &file_idx);
            furi_string_set(browser->path_current, path);
            FURI_LOG_D(TAG, "Enter folder: %s items: %lu idx: %ld", furi_string_get_cstr(path),
                       items_cnt, file_idx);
            if (browser->folder_cb) {
                browser->folder_cb(browser->cb_ctx, items_cnt, file_idx, is_root);
            }
        ...
        if (flags & WorkerEvtFolderExit) {
            ...
            browser_folder_init(browser, path, filename, &items_cnt, &file_idx);
            furi_string_set(browser->path_current, path);
            FURI_LOG_D(TAG, "Exit to: %s items: %lu idx: %ld", furi_string_get_cstr(path),
                       items_cnt, file_idx);
            if (browser->folder_cb) {
                browser->folder_cb(browser->cb_ctx, items_cnt, file_idx, is_root);
            }
        ...
        if (flags & WorkerEvtFolderRefresh) {
            ...
            int32_t file_idx = 0;
            browser_folder_init(browser, path, filename, &items_cnt, &file_idx);
            FURI_LOG_D(TAG, "Refresh folder: %s items: %lu idx: %ld", furi_string_get_cstr(path),
                       items_cnt, file_idx);
            if (browser->folder_cb) {
                browser->folder_cb(browser->cb_ctx, items_cnt, file_idx, is_root);
            }
        ...
        if (flags & WorkerEvtLoad) {
            FURI_LOG_D(TAG, "Load offset: %lu cnt: %lu", browser->load_offset, browser->load_count);
            if (items_cnt > BROWSER_SORT_THRESHOLD) {
                browser_folder_load_chunked(browser, path, browser->load_offset,
                                            browser->load_count);
            } else {
...
BrowserWorker *file_browser_worker_alloc(FuriString *path, const char *base_path,
                                         const char *ext_filter, bool skip_assets,
                                         bool hide_dot_files)
{
...
void file_browser_worker_set_folder_callback(BrowserWorker *browser,
...
void file_browser_worker_set_list_callback(BrowserWorker *browser, BrowserWorkerListLoadCallback cb)
{
    furi_check(browser);
    browser->list_load_cb = cb;
}

void file_browser_worker_set_item_callback(BrowserWorker *browser, BrowserWorkerListItemCallback cb)
{
    furi_check(browser);
    browser->list_item_cb = cb;
}

void file_browser_worker_set_long_load_callback(BrowserWorker *browser,
...
void file_browser_worker_set_config(BrowserWorker *browser, FuriString *path,
                                    const char *ext_filter, bool skip_assets, bool hide_dot_files)
{
    furi_check(browser);
...
void file_browser_worker_set_filter_ext(BrowserWorker *browser, FuriString *path,
