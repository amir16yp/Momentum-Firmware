...
bool validator_is_file_callback(const char *text, FuriString *error, void *context)
{
    ...
    FuriString *path = furi_string_alloc_printf("%s/%s%s", instance->app_path_folder, text,
...
ValidatorIsFile *validator_is_file_alloc_init(const char *app_path_folder,
                                              const char *app_extension, const char *current_name)
{
