...
static bool widget_element_text_scroll_process_ctrl_symbols(TextScrollLineArray *line,
                                                            FuriString *text)
{
    bool processed = false;

    do {
        if (furi_string_get_char(text, 0) != '\e')
            break;
...
static void widget_element_text_scroll_draw(Canvas *canvas, WidgetElement *element)
{
    ...
    if (TextScrollLineArray_size(model->line_array)) {
        ...
        for (TextScrollLineArray_it(it, model->line_array); !TextScrollLineArray_end_p(it);
            ...
            if (curr_line < model->scroll_pos_current)
                continue;
            ...
            if (y + params->descender > model->y + model->height)
                break;
            ...
            canvas_draw_str_aligned(canvas, x, y, line->horizontal, AlignTop,
                                    furi_string_get_cstr(line->text));
            y += params->leading_default;
        }
        // Draw scroll bar
        if (model->scroll_pos_total > 1) {
            elements_scrollbar_pos(
                canvas, model->x + model->width + WIDGET_ELEMENT_TEXT_SCROLL_BAR_OFFSET, model->y,
                model->height, model->scroll_pos_current, model->scroll_pos_total);
        }
    }
...
WidgetElement *widget_element_text_scroll_create(uint8_t x, uint8_t y, uint8_t width,
