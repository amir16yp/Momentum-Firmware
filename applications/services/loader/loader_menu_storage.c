...
static void loader_menu_storage_settings_callback(DialogExResult result, void *context)
{
    ...
    furi_thread_flags_set(furi_thread_get_id(thread),
...
int32_t loader_menu_storage_settings(void *context)
{
    ...
    if (result != LoaderStatusOk) {
        ...
        view_holder_set_back_callback(view_holder, loader_menu_storage_settings_back,
                                      furi_thread_get_current());

        dialog_ex_set_header(dialog_ex, "Update needed", 64, 0, AlignCenter, AlignTop);
        dialog_ex_set_text(dialog_ex,
                           "Reinstall firmware\n"
                           "to run this app.\n"
                           "Can format SD\n"
                           "here if needed.",
                           3, 17, AlignLeft, AlignTop);
        ...
        if (flag == FormatFlagContinue) {
            ...
            for (uint8_t counter = 5; counter > 0; counter--) {
                ...
                if (counter == 1) {
                    dialog_ex_set_header(dialog_ex, "Formatting...", 70, 32, AlignCenter,
                                         AlignCenter);
                    ...
                    if (error != FSE_OK) {
                        dialog_ex_set_header(dialog_ex, "Cannot Format SD Card", 64, 10,
                                             AlignCenter, AlignCenter);
                        dialog_ex_set_icon(dialog_ex, 0, 0, NULL);
                        dialog_ex_set_text(dialog_ex, storage_error_get_desc(error), 64, 32,
                                           AlignCenter, AlignCenter);
                    } else {
