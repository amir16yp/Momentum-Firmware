...
LoaderMenu *loader_menu_alloc(void (*closed_cb)(void *), void *context, bool settings_only)
{
    ...
    view_holder_set_view(loader_menu->loader->view_holder,
                         loading_get_view(loader_menu->loader->loading));

    loader_menu->subscription = furi_pubsub_subscribe(loader_get_pubsub(loader_menu->loader),
                                                      loader_pubsub_callback, loader_menu);
...
static void loader_menu_settings_menu_callback(void *context, InputType input_type, uint32_t index)
{
    UNUSED(context);
...
static void loader_menu_switch_to_settings(void *context, uint32_t index)
{
    UNUSED(index);
    LoaderMenuApp *app = context;
    furi_timer_pending_callback(loader_menu_set_view_pending, app,
                                (uint32_t)submenu_get_view(app->settings_menu));
...
static void loader_menu_back(void *context)
{
    ...
    if (app->loader_menu->current_view == LoaderMenuViewSettings &&
        ...
        furi_timer_pending_callback(loader_menu_set_view_pending, app,
                                    (uint32_t)menu_get_view(app->primary_menu));
...
static void loader_menu_add_app_entry(LoaderMenuApp *app, const char *name, const Icon *icon,
                                      const char *path, bool custom_icon)
{
    MenuAppList_push_back(app->apps_list, (MenuApp){name, icon, path, custom_icon});
    menu_add_item(app->primary_menu, name, icon, MenuAppList_size(app->apps_list) - 1,
                  loader_menu_apps_callback, app);
}
...
static const Icon *loader_menu_get_ext_icon(Storage *storage, const char *path)
{
    ...
    if (ext && strcasecmp(ext, ".js") == 0)
        return &I_js_script_10px;

    return &I_file_10px;
}

bool loader_menu_load_fap_meta(Storage *storage, FuriString *path, FuriString *name,
                               const Icon **icon)
{
    *icon = NULL;
...
{
    menu_add_item(app->primary_menu, LOADER_APPLICATIONS_NAME, &A_Plugins_14,
                  LoaderMenuIndexApplications, loader_menu_applications_callback, NULL);
    ...
    if (file_stream_open(stream, MAINMENU_APPS_PATH, FSAM_READ, FSOM_OPEN_EXISTING) &&
        ...
        for (size_t i = 0; i < FLIPPER_APPS_COUNT; i++) {
            loader_menu_add_app_entry(app, FLIPPER_APPS[i].name, FLIPPER_APPS[i].icon, NULL, false);
        }
        // Until count - 1 because last app is hardcoded below
        for (size_t i = 0; i < FLIPPER_EXTERNAL_APPS_COUNT - 1; i++) {
            loader_menu_add_app_entry(app, FLIPPER_EXTERNAL_APPS[i].name,
                                      FLIPPER_EXTERNAL_APPS[i].icon, NULL, false);
        }
    }
    ...
    menu_add_item(app->primary_menu, last->name, last->icon, LoaderMenuIndexLast,
                  loader_menu_last_callback, NULL);
    menu_add_item(app->primary_menu, "Settings", &A_Settings_14, LoaderMenuIndexSettings,
                  loader_menu_switch_to_settings, app);
...
static void loader_menu_build_submenu(LoaderMenuApp *app, LoaderMenu *loader_menu)
{
    for (size_t i = 0; i < FLIPPER_SETTINGS_APPS_COUNT; i++) {
        submenu_add_item_ex(app->settings_menu, FLIPPER_SETTINGS_APPS[i].name, i,
                            loader_menu_settings_menu_callback, NULL);
    }
...
static void loader_menu_app_free(LoaderMenuApp *app)
{
    ...
    view_holder_set_view(app->loader_menu->loader->view_holder,
