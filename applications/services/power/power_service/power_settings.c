...
void power_settings_load(PowerSettings *settings)
{
    ...
    do {
        uint8_t version;
        if (!saved_struct_get_metadata(POWER_SETTINGS_PATH, NULL, &version, NULL))
            break;

        // if config actual version - load it directly
        if (version == POWER_SETTINGS_VER) {
            success = saved_struct_load(POWER_SETTINGS_PATH, settings, sizeof(PowerSettings),
            ...
            success = saved_struct_load(POWER_SETTINGS_PATH, settings_previous,
                                        sizeof(PowerSettingsPrevious), POWER_SETTINGS_MAGIC,
