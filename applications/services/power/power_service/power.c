...
static void power_draw_battery_callback(Canvas *canvas, void *context)
{
    ...
    if (battery_icon == BatteryIconOff)
        return;
    ...
    if (power->info.gauge_is_ok) {
        ...
        if ((battery_icon == BatteryIconPercent) &&
            ...
            canvas_draw_str_aligned(canvas, 11, 4, AlignCenter, AlignCenter, batteryPercentile);
                   ...
                   (power->state != PowerStateCharging)) { // if display inverted percentage, white
                                                           // background black text
            canvas_set_font(canvas, FontBatteryPercent);
            canvas_set_color(canvas, ColorBlack);
            canvas_draw_str_aligned(canvas, 11, 4, AlignCenter, AlignCenter, batteryPercentile);
        } else if ((battery_icon == BatteryIconRetro3) &&
        ...
        } else if ((battery_icon == BatteryIconRetro5) &&
            ...
            if (bar_charge < 38) { // both digits are black
                ...
                snprintf(batteryPercentileFirstDigit, sizeof(batteryPercentileFirstDigit), "%c",
                         batteryPercentile[0]);
                canvas_draw_str_aligned(canvas, 9, 4, AlignCenter, AlignCenter,
                                        batteryPercentileFirstDigit);

                // second
                char batteryPercentileSecondDigit[2];
                snprintf(batteryPercentileSecondDigit, sizeof(batteryPercentileSecondDigit), "%c",
                         batteryPercentile[1]);
                canvas_set_color(canvas, ColorBlack);
                canvas_draw_str_aligned(canvas, 15, 4, AlignCenter, AlignCenter,
        ...
        if (power->state == PowerStateCharging) {
            ...
            if (battery_icon == BatteryIconPercent) {
                ...
                canvas_set_font(canvas, FontBatteryPercent);
                canvas_draw_str_aligned(canvas, 16, 4, AlignCenter, AlignCenter, batteryPercentile);
            } else if (battery_icon == BatteryIconInvertedPercent) {
                canvas_set_color(canvas, ColorWhite);
                canvas_draw_box(canvas, 1, 1, 22, 6);
                canvas_draw_icon(canvas, 2, -1, &I_Charging_lightning_9x10);
                ...
                if (bar_charge < 64) { // both digits are black
                    canvas_set_color(canvas, ColorBlack);
                    canvas_draw_str_aligned(canvas, 16, 4, AlignCenter, AlignCenter,
                                            batteryPercentile);
                    ...
                    snprintf(batteryPercentileFirstDigit, sizeof(batteryPercentileFirstDigit), "%c",
                             batteryPercentile[0]);
                    canvas_draw_str_aligned(canvas, 14, 4, AlignCenter, AlignCenter,
                                            batteryPercentileFirstDigit);

                    // second
                    char batteryPercentileSecondDigit[2];
                    snprintf(batteryPercentileSecondDigit, sizeof(batteryPercentileSecondDigit),
                             "%c", batteryPercentile[1]);
                    canvas_set_color(canvas, ColorBlack);
                    canvas_draw_str_aligned(canvas, 20, 4, AlignCenter, AlignCenter,
                                            batteryPercentileSecondDigit);
...
static bool power_update_info(Power *power)
{
    ...
    const bool need_refresh =
        (power->info.charge != info.charge) || (power->info.is_charging != info.is_charging);
    power->info = info;
    return need_refresh;
}
...
static void power_handle_shutdown(Power *power)
{
    ...
    view_holder_send_to_front(power->view_holder);
    view_holder_set_view(power->view_holder,
                         power_unplug_usb_get_view(power->view_power_unplug_usb));
    furi_delay_ms(100);
...
static void power_start_auto_poweroff_timer(Power *power)
{
    ...
    furi_timer_start(power->auto_poweroff_timer,
                     furi_ms_to_ticks(power->settings.auto_poweroff_delay_ms));
}
...
// start|restart timer and events subscription and callbacks for input events (we restart timer when
// user press keys)
...
// check message queue from Loader - is some app started or not (if started we dont do auto
// poweroff)
    ...
    case PowerMessageTypeSwitchOTG:
        ...
        if (power->is_otg_requested) {
            ...
            if (power->info.voltage_vbus < 4.5f) {
                ...
                FURI_LOG_W(TAG, "Postponing OTG enable: VBUS(%0.1f) >= 4.5v",
...
{
    // if charge_supress_percent selected (not OFF) and current charge level equal or higher than
    // selected level then we start supression if we not supress it before.
...
static void power_tick_callback(void *context)
{
    ...
    view_port_enabled_set(power->battery_view_port,
                          momentum_settings.battery_icon != BatteryIconOff);
    if (need_refresh) {
...
static void power_storage_callback(const void *message, void *context)
{
    ...
    if (event->type == StorageEventTypeCardMount) {
        ...
        furi_check(furi_message_queue_put(power->message_queue, &msg, FuriWaitForever) ==
                   FuriStatusOk);
    }
}
...
static Power *power_alloc(void)
{
    ...
    // auto_poweroff
    //---define subscription to loader events message (info about started apps) and define callback
    //for this
    ...
    furi_event_loop_subscribe_message_queue(power->event_loop, power->message_queue,
                                            FuriEventLoopEventIn, power_message_callback, power);
...
int32_t power_srv(void *p)
{
    ...
    // Can't be done in alloc, other things in startup need power service and it would deadlock by
    // waiting for loader
    Loader *loader = furi_record_open(RECORD_LOADER);
