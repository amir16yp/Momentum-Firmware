    ...
    if (furi_string_cmp_str(path, STORAGE_INT_PATH_PREFIX) == 0) {
        ...
        if (error != FSE_OK) {
            ...
            printf("Label: %s\r\nType: Virtual\r\n%luKiB total\r\n%luKiB free\r\n",
                   ...
                   (uint32_t)(total_space / 1024), (uint32_t)(free_space / 1024));
        }
        ...
        if (error != FSE_OK) {
            ...
            printf("Label: %s\r\nType: %s\r\n%luKiB total\r\n%luKiB free\r\n"
                   "%02x%s %s v%i.%i\r\nSN:%04lx %02i/%i\r\n",
                   sd_info.label, sd_api_get_fs_type_text(sd_info.fs_type), sd_info.kb_total,
                   sd_info.kb_free, sd_info.manufacturer_id, sd_info.oem_id, sd_info.product_name,
                   sd_info.product_revision_major, sd_info.product_revision_minor,
                   sd_info.product_serial_number, sd_info.manufacturing_month,
    ...
    if (strint_to_uint32(furi_string_get_cstr(args), NULL, &need_to_read, 10) !=
        ...
        if (storage_file_open(file, furi_string_get_cstr(path), FSAM_WRITE, FSOM_OPEN_APPEND)) {
            ...
            while (need_to_read) {
                ...
                if (read_this_time != to_read_this_time)
                    break;
...
static void storage_cli_stat(PipeSide *pipe, FuriString *path, FuriString *args)
{
    ...
    if (furi_string_cmp_str(path, "/") == 0) {
        ...
        if (error != FSE_OK) {
            ...
            printf("Storage, %luKiB total, %luKiB free\r\n", (uint32_t)(total_space / 1024),
...
static void storage_cli_copy(PipeSide *pipe, FuriString *old_path, FuriString *args)
{
    ...
    if (!args_read_probably_quoted_string_and_trim(args, new_path)) {
        ...
        FS_Error error = storage_common_copy(api, furi_string_get_cstr(old_path),
...
static void storage_cli_rename(PipeSide *pipe, FuriString *old_path, FuriString *args)
{
    ...
    if (!args_read_probably_quoted_string_and_trim(args, new_path)) {
        ...
        FS_Error error = storage_common_rename_safe(api, furi_string_get_cstr(old_path),
                                                    furi_string_get_cstr(new_path));
...
static void storage_cli_migrate(PipeSide *pipe, FuriString *old_path, FuriString *args)
{
    ...
    if (!args_read_probably_quoted_string_and_trim(args, new_path)) {
        ...
        FS_Error error = storage_common_migrate(api, furi_string_get_cstr(old_path),
                                                furi_string_get_cstr(new_path));
...
static void storage_cli_extract(PipeSide *pipe, FuriString *old_path, FuriString *args)
{
    ...
    do {
        ...
        printf("Decompression %s in %lu ticks \r\n", success ? "success" : "failed",
               end_tick - start_tick);
    } while (false);
...
static const StorageCliCommand storage_cli_commands[] = {
    {
        "write_chunk",
        "read data from cli and append it to file, <args> should contain how many bytes you want "
        "to write",
        &storage_cli_write_chunk,
    },
    {
        "read_chunks",
        "read data from file and print file size and content to cli, <args> should contain how "
        "many bytes you want to read in block",
...
void storage_on_system_start(void)
{
    ...
    cli_registry_add_command(registry, "storage",
                             CliCommandFlagParallelSafe | CliCommandFlagUseShellThread, storage_cli,
                             NULL);
    cli_registry_add_command(registry, "factory_reset", CliCommandFlagParallelSafe,
                             storage_cli_factory_reset, NULL);
