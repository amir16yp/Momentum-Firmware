...
static bool sd_mount_card_internal(StorageData *storage, bool notify)
{
    ...
    while (result == false && counter > 0 && furi_hal_sd_is_present()) {
        ...
        if (!result) {
            furi_delay_ms(1000);
            FURI_LOG_E(TAG, "init cycle %d, error: %s", counter, storage_data_status_text(storage));
            counter--;
        }
    }
...
static bool storage_ext_file_open(void *ctx, File *file, const char *path,
                                  FS_AccessMode access_mode, FS_OpenMode open_mode)
{
    ...
    if (open_mode & FSOM_OPEN_EXISTING)
        _mode |= FA_OPEN_EXISTING;
    if (open_mode & FSOM_OPEN_ALWAYS)
        _mode |= FA_OPEN_ALWAYS;
    if (open_mode & FSOM_OPEN_APPEND)
        _mode |= FA_OPEN_APPEND;
    if (open_mode & FSOM_CREATE_NEW)
        _mode |= FA_CREATE_NEW;
    if (open_mode & FSOM_CREATE_ALWAYS)
        _mode |= FA_CREATE_ALWAYS;
...
static uint16_t storage_ext_file_read(void *ctx, File *file, void *buff,
...
static uint16_t storage_ext_file_write(void *ctx, File *file, const void *buff,
...
static bool storage_ext_file_seek(void *ctx, File *file, const uint32_t offset,
...
static bool storage_ext_dir_read(void *ctx, File *file, FileInfo *fileinfo, char *name,
...
static FS_Error storage_ext_common_fs_info(void *ctx, const char *fs_path, uint64_t *total_space,
                                           uint64_t *free_space)
{
    UNUSED(fs_path);
#ifdef FURI_RAM_EXEC
...
FS_Error storage_process_virtual_init(StorageData *storage, File *image, StorageData *image_storage)
{
    if (storage->status != StorageStatusNotReady)
...
FS_Error storage_process_virtual_format(StorageData *storage)
{
    ...
    if (storage->status == StorageStatusOK)
        return FSE_ALREADY_OPEN;
    if (storage->status != StorageStatusNotMounted)
        return FSE_NOT_READY;
    SDData *sd_data = storage->data;
...
{
    UNUSED(pdrv);
    if (!count || sector > UINT32_MAX / SCSI_BLOCK_SIZE)
        return RES_PARERR;
...
static const Diskio_drvTypeDef mnt_driver = {
    mnt_driver_initialize, mnt_driver_status, mnt_driver_read, mnt_driver_write, mnt_driver_ioctl,
};
