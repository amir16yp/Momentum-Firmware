 ...
 */
typedef struct {
    bool (*const open)(void *context, File *file, const char *path, FS_AccessMode access_mode,
 ...
 *      @return success flag (if next object not exist also returns false and set error_id to
 * FSE_NOT_EXIST)
 *
...
typedef struct {
    ...
    bool (*const close)(void *context, File *file);
    bool (*const read)(void *context, File *file, FileInfo *fileinfo, char *name,
...
typedef struct {
    ...
    FS_Error (*const fs_info)(void *context, const char *fs_path, uint64_t *total_space,
                              uint64_t *free_space);
