...
typedef enum {
    ...
    StorageStatusNotReady,   /**< storage not ready (not initialized or waiting for data storage to
                                appear) */
...
LIST_DEF(StorageFileList, StorageFile,
         (INIT(API_2(storage_file_init)), SET(API_6(storage_file_init_set)),
          INIT_SET(API_6(storage_file_set)), CLEAR(API_2(storage_file_clear))))
