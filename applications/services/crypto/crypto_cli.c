...
void crypto_cli_print_usage(void)
{
    ...
    printf("Cmd list:\r\n");
    printf("\tencrypt <key_slot:int> <iv:hex>\t - Using key from secure enclave and IV encrypt "
           "plain text with AES256CBC and encode to hex\r\n");
    printf("\tdecrypt <key_slot:int> <iv:hex>\t - Using key from secure enclave and IV decrypt hex "
           "encoded encrypted with AES256CBC data to plain text\r\n");
    printf("\thas_key <key_slot:int>\t - Check if secure enclave has key in slot\r\n");
    printf("\tstore_key <key_slot:int> <key_type:str> <key_size:int> <key_data:hex>\t - Store key "
           "in secure enclave. !!! NON-REVERSIBLE OPERATION - READ MANUAL FIRST !!!\r\n");
}
...
void crypto_cli_encrypt(PipeSide *pipe, FuriString *args)
{
    ...
    do {
        ...
        if (size > 0) {
            ...
            if (!furi_hal_crypto_encrypt((const uint8_t *)furi_string_get_cstr(input), output,
                                         size)) {
        ...
        if (key_slot > 0) {
            ...
            if (key_slot > 1) {
                if (!furi_hal_crypto_enclave_load_key(key_slot - 1, iv)) {
                    printf("Slot %d before %d is empty, which is not allowed", key_slot - 1,
                           key_slot);
                    break;
                }
