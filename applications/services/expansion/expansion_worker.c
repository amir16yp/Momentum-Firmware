...
static void expansion_worker_serial_rx_callback(FuriHalSerialHandle *handle,
...
static size_t expansion_worker_receive_callback(uint8_t *data, size_t data_size, void *context)
{
    ...
    while (true) {
        received_size += furi_stream_buffer_receive(instance->rx_buf, data + received_size,
...
static inline bool expansion_worker_receive_frame(ExpansionWorker *instance, ExpansionFrame *frame)
{
    return expansion_protocol_decode(frame, expansion_worker_receive_callback, instance) ==
           ExpansionProtocolStatusOk;
}

static size_t expansion_worker_send_callback(const uint8_t *data, size_t data_size, void *context)
{
    ExpansionWorker *instance = context;
...
static inline bool expansion_worker_send_frame(ExpansionWorker *instance,
...
static bool expansion_worker_send_status_response(ExpansionWorker *instance,
...
static bool expansion_worker_send_data_response(ExpansionWorker *instance, const uint8_t *data,
                                                size_t data_size)
{
...
static void expansion_worker_rpc_send_callback(void *context, uint8_t *data, size_t data_size)
{
    ...
    for (size_t sent_data_size = 0; sent_data_size < data_size;) {
        if (furi_semaphore_acquire(instance->tx_semaphore,
                                   furi_ms_to_ticks(EXPANSION_PROTOCOL_TIMEOUT_MS)) !=
            FuriStatusOk) {
            furi_thread_flags_set(furi_thread_get_id(instance->thread), ExpansionWorkerFlagError);
            break;
        }
        ...
        if (!expansion_worker_send_data_response(instance, data + sent_data_size,
                                                 current_data_size))
            break;
...
static bool expansion_worker_rpc_session_open(ExpansionWorker *instance)
{
    ...
    if (instance->rpc_session) {
        ...
        rpc_session_set_send_bytes_callback(instance->rpc_session,
                                            expansion_worker_rpc_send_callback);
    }
...
static bool expansion_worker_handle_state_handshake(ExpansionWorker *instance,
                                                    const ExpansionFrame *rx_frame)
{
    bool success = false;

    do {
        if (rx_frame->header.type != ExpansionFrameTypeBaudRate)
            break;
        ...
        if (furi_hal_serial_is_baud_rate_supported(instance->serial_handle, baud_rate)) {
            ...
            if (!expansion_worker_send_status_response(instance, ExpansionFrameErrorNone))
                break;
...
static bool expansion_worker_handle_state_connected(ExpansionWorker *instance,
                                                    const ExpansionFrame *rx_frame)
{
    bool success = false;
    ...
    do {
        if (rx_frame->header.type == ExpansionFrameTypeControl) {
            ...
            if (command == ExpansionFrameControlCommandStartRpc) {
                if (!expansion_worker_rpc_session_open(instance))
                    break;
...
static bool expansion_worker_handle_state_rpc_active(ExpansionWorker *instance,
                                                     const ExpansionFrame *rx_frame)
{
    bool success = false;
    ...
    do {
        if (rx_frame->header.type == ExpansionFrameTypeData) {
            ...
            const size_t size_consumed =
                rpc_session_feed(instance->rpc_session, rx_frame->content.data.bytes,
                                 rx_frame->content.data.size, EXPANSION_PROTOCOL_TIMEOUT_MS);
            if (size_consumed != rx_frame->content.data.size)
                break;
            ...
            if (rx_frame->content.status.error != ExpansionFrameErrorNone)
                break;
...
static inline void expansion_worker_state_machine(ExpansionWorker *instance)
{
    ...
    while (true) {
        if (!expansion_worker_receive_frame(instance, &rx_frame))
            break;
        if (!expansion_handlers[instance->state](instance, &rx_frame))
            break;
    }
}
...
ExpansionWorker *expansion_worker_alloc(FuriHalSerialId serial_id)
{
    ...
    instance->thread =
        furi_thread_alloc_ex(TAG "Worker", EXPANSION_WORKER_STACK_SZIE, expansion_worker, instance);
