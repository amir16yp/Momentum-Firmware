...
static bool bt_keys_storage_save(BtKeysStorage *instance)
{
    ...
    bool saved = saved_struct_save(furi_string_get_cstr(instance->file_path), save_data,
                                   sizeof(GapRootSecurityKeys) + instance->current_size,
                                   BT_KEYS_STORAGE_MAGIC, BT_KEYS_STORAGE_VERSION);

    free(save_data);
    return saved;
}

static bool bt_keys_storage_load_keys_and_pairings(BtKeysStorage *instance,
                                                   const uint8_t *file_data, size_t data_size)
{
...
static bool bt_keys_storage_file_exists(const char *file_path)
{
    ...
    FileInfo file_info;
    const bool ret =
        storage_common_stat(storage, file_path, &file_info) == FSE_OK && file_info.size != 0;
                                          ...
                                          uint8_t *file_version)
{
    ...
    if (!saved_struct_get_metadata(file_path, &magic, &version, &size)) {
        ...
        FURI_LOG_E(TAG, "File version %d is newer than supported version %d", version,
...
bool bt_keys_storage_is_changed(BtKeysStorage *instance)
{
    ...
    do {
        ...
        const bool data_loaded = saved_struct_load(file_path, data_buffer, payload_size,
                                                   BT_KEYS_STORAGE_MAGIC, file_version);
...
static bool bt_keys_storage_load_legacy_pairings(BtKeysStorage *instance, const uint8_t *file_data,
                                                 size_t payload_size)
{
...
bool bt_keys_storage_load(BtKeysStorage *instance)
{
    ...
    do {
        if (!saved_struct_load(file_path, file_data, payload_size, BT_KEYS_STORAGE_MAGIC,
                               file_version)) {
...
bool bt_keys_storage_update(BtKeysStorage *instance, uint8_t *start_addr, uint32_t size)
{
    ...
    FURI_LOG_I(TAG, "Base address: %p. Start update address: %p. Size changed: %lu",
               (void *)instance->nvm_sram_buff, start_addr, size);
