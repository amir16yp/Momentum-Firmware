...
static void bt_storage_callback(const void *message, void *context)
{
    ...
    if (event->type == StorageEventTypeCardMount) {
        ...
        furi_check(furi_message_queue_put(bt->message_queue, &msg, FuriWaitForever) ==
                   FuriStatusOk);
    }
}
...
static bool bt_pin_code_verify_event_handler(Bt *bt, uint32_t pin)
{
    ...
    dialog_message_set_text(bt->dialog_message, furi_string_get_cstr(pin_str), 64, 4, AlignCenter,
                            AlignTop);
...
static uint16_t bt_serial_event_callback(SerialServiceEvent event, void *context)
{
    ...
    if (event.event == SerialServiceEventTypeDataReceived) {
        ...
        if (bytes_processed != event.data.size) {
            FURI_LOG_E(TAG, "Only %zu of %u bytes processed by RPC", bytes_processed,
                       event.data.size);
        }
...
static bool bt_on_gap_event_callback(GapEvent event, void *context)
{
    ...
    if (event.type == GapEventTypeConnected) {
        ...
        BtMessage message = {.type = BtMessageTypePinCodeShow,
                             .data.pin_code = event.data.pin_code};
        furi_check(furi_message_queue_put(bt->message_queue, &message, FuriWaitForever) ==
                   FuriStatusOk);
        ret = true;
    ...
    if (!bt->rpc_session && bt->status == BtStatusConnected) {
        ...
        if (furi_hal_bt_check_profile_type(bt->current_profile, ble_profile_serial)) {
            ...
            if (bt->rpc_session) {
                ...
                rpc_session_set_buffer_is_empty_callback(bt->rpc_session,
                                                         bt_serial_buffer_is_empty_callback);
                rpc_session_set_context(bt->rpc_session, bt);
                ble_profile_serial_set_event_callback(bt->current_profile, RPC_BUFFER_SIZE,
                                                      bt_serial_event_callback, bt);
                ble_profile_serial_set_rpc_active(bt->current_profile,
...
static void bt_change_profile(Bt *bt, BtMessage *message)
{
    if (furi_hal_bt_is_gatt_gap_supported()) {
        ...
        bt->current_profile = furi_hal_bt_change_app(
            message->data.profile.template, message->data.profile.params,
            bt_keys_storage_get_root_keys(bt->keys_storage), bt_on_gap_event_callback, bt);
...
static void bt_start_application(Bt *bt)
{
    if (!bt->current_profile) {
        bt->current_profile = furi_hal_bt_change_app(
            ble_profile_serial, NULL, bt_keys_storage_get_root_keys(bt->keys_storage),
            bt_on_gap_event_callback, bt);
...
int32_t bt_srv(void *p)
{
    ...
    while (1) {
        furi_check(furi_message_queue_get(bt->message_queue, &message, FuriWaitForever) ==
                   FuriStatusOk);
        FURI_LOG_D(TAG, "call %d, lock 0x%p, result 0x%p", message.type, (void *)message.lock,
                   (void *)message.result);
        if (message.type == BtMessageTypeUpdateStatus) {
            ...
            bt_keys_storage_update(bt->keys_storage, message.data.key_storage_data.start_address,
