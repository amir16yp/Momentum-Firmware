...
void desktop_scene_hw_mismatch_on_enter(void *context)
{
    ...
    scene_manager_set_scene_state(desktop->scene_manager, DesktopSceneHwMismatch,
                                  (uint32_t)text_buffer);

    snprintf(text_buffer, 256, "HW target: %d\nFW target: %d", furi_hal_version_get_hw_target(),
             version_get_target(NULL));
    popup_set_context(popup, desktop);
    popup_set_header(popup, "!!!! HW Mismatch !!!!", 64, 12 + STATUS_BAR_Y_SHIFT, AlignCenter,
                     AlignBottom);
