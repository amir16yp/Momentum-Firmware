...
static void desktop_scene_pin_input_update_wrong_count(DesktopScenePinInputState *state,
                                                       Desktop *desktop)
{
    uint32_t attempts = furi_hal_rtc_get_pin_fails();
    if (attempts > 0) {
        furi_string_printf(state->enter_pin_string, "Wrong Attempts: %lu", attempts);
        desktop_view_pin_input_set_label_tertiary(desktop->pin_input_view, 64, 60,
                                                  furi_string_get_cstr(state->enter_pin_string));
    } else {
...
void desktop_scene_pin_input_on_enter(void *context)
{
    ...
    desktop_view_pin_input_set_back_callback(desktop->pin_input_view,
                                             desktop_scene_pin_input_back_callback);
    desktop_view_pin_input_set_timeout_callback(desktop->pin_input_view,
                                                desktop_scene_pin_input_back_callback);
    desktop_view_pin_input_set_done_callback(desktop->pin_input_view,
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        case DesktopPinInputEventUnlockFailed:
            ...
            if (pin_timeout > 0) {
                ...
                desktop_view_pin_input_set_label_secondary(desktop->pin_input_view, 25, 25,
                                                           "Wrong PIN try again:");
        ...
        case DesktopPinInputEventResetWrongPinLabel:
            ...
            desktop_view_pin_input_set_label_primary(desktop->pin_input_view, 0, 0, NULL);
            desktop_view_pin_input_set_label_secondary(desktop->pin_input_view, 44, 25,
                                                       "Enter PIN:");
            desktop_scene_pin_input_update_wrong_count(state, desktop);
            consumed = true;
            break;
        ...
        case DesktopPinInputEventBack:
            scene_manager_search_and_switch_to_previous_scene(desktop->scene_manager,
                                                              DesktopSceneLocked);
