...
static uint8_t bubble_animation_get_frame_index(BubbleAnimationViewModel *model)
{
    ...
    if (model->current_frame < animation->passive_frames) {
        ...
        icon_index = (model->current_frame - animation->passive_frames) % animation->active_frames +
...
static const FrameBubble *bubble_animation_pick_bubble(BubbleAnimationViewModel *model, bool active)
{
...
static void bubble_animation_next_frame(BubbleAnimationViewModel *model)
{
    ...
    if (model->current_frame < model->current->passive_frames) {
        ...
        ++model->current_frame;
        model->active_cycle += !((model->current_frame - model->current->passive_frames) %
