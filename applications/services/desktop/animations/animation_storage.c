...
void animation_handler_select_manifest()
{
    ...
    if (use_asset_pack) {
        furi_string_printf(anim_dir, "%s/%s/Anims", ASSET_PACKS_PATH, momentum_settings.asset_pack);
    ...
    strlcpy(ANIMATION_MANIFEST_FILE, furi_string_get_cstr(manifest),
            sizeof(ANIMATION_MANIFEST_FILE));
    furi_string_free(manifest);
    furi_string_free(anim_dir);
}

static bool animation_storage_load_single_manifest_info(StorageAnimationManifestInfo *manifest_info,
                                                        const char *name)
{
...
void animation_storage_fill_animation_list(StorageAnimationList_t *animation_list)
{
    ...
    do {
        ...
        do {
            ...
            if (!flipper_format_read_string(file, "Name", read_string))
                break;
...
static bool animation_storage_load_frames(Storage *storage, const char *name,
                                          BubbleAnimation *animation, uint32_t *frame_order,
                                          uint8_t width, uint8_t height)
{
    ...
    for (int i = 0; i < icon->frame_count; ++i) {
        ...
        if (file_info.size > max_filesize) {
            FURI_LOG_E(TAG, "Filesize %llu, max: %zu (width %u, height %u)", file_info.size,
                       max_filesize, width, height);
            break;
        }
        if (!storage_file_open(file, furi_string_get_cstr(filename), FSAM_READ,
                               FSOM_OPEN_EXISTING)) {
    ...
    if (!frames_ok) {
        FURI_LOG_E(TAG, "Load \'%s\' failed, %ux%u, size: %llu", furi_string_get_cstr(filename),
...
static bool animation_storage_load_bubbles(BubbleAnimation *animation, FlipperFormat *ff)
{
    ...
    do {
        if (!flipper_format_read_uint32(ff, "Bubble slots", &u32value, 1))
            break;
        if (u32value > 20)
            break;
        ...
        for (int i = 0; i < animation->frame_bubble_sequences_count; ++i) {
            FURI_CONST_ASSIGN_PTR(animation->frame_bubble_sequences[i],
                                  malloc(sizeof(FrameBubble)));
        }

        const FrameBubble *bubble = animation->frame_bubble_sequences[0];
        int8_t index = -1;
        for (;;) {
            if (!flipper_format_read_int32(ff, "Slot", &current_slot, 1))
                break;
            if ((current_slot != 0) && (index == -1))
                break;
            ...
            if (index >= animation->frame_bubble_sequences_count)
                break;
            ...
            if (!flipper_format_read_string(ff, "AlignH", str))
                break;
            if (!animation_storage_cast_align(str, (Align *)&bubble->bubble.align_h))
                break;
            if (!flipper_format_read_string(ff, "AlignV", str))
                break;
            if (!animation_storage_cast_align(str, (Align *)&bubble->bubble.align_v))
                break;
...
static BubbleAnimation *animation_storage_load_animation(const char *name)
{
    ...
    do {
        ...
        if (!flipper_format_read_uint32(ff, "Width", &width, 1))
            break;
        if (!flipper_format_read_uint32(ff, "Height", &height, 1))
            break;

        if (!flipper_format_read_uint32(ff, "Passive frames", &u32value, 1))
            break;
        animation->passive_frames = u32value;
        if (!flipper_format_read_uint32(ff, "Active frames", &u32value, 1))
            break;
        animation->active_frames = u32value;

        uint8_t frames = animation->passive_frames + animation->active_frames;
        uint32_t count = 0;
        if (!flipper_format_get_value_count(ff, "Frames order", &count))
            break;
        if (count != frames) {
        ...
        if (!flipper_format_read_uint32(ff, "Frames order", u32array, frames))
            break;
        ...
        if (!flipper_format_read_uint32(ff, "Active cycles", &u32value, 1))
            break; //-V779
        animation->active_cycles = u32value;
        if (!flipper_format_read_uint32(ff, "Frame rate", &u32value, 1))
            break;
        ...
        if (!flipper_format_read_uint32(ff, "Duration", &u32value, 1))
            break;
        animation->duration = u32value;
        if (!flipper_format_read_uint32(ff, "Active cooldown", &u32value, 1))
            break;
        animation->active_cooldown = u32value;

        if (!animation_storage_load_bubbles(animation, ff))
            break;
        success = true;
    } while (0);
