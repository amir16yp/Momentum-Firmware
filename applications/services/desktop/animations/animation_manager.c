...
static void animation_manager_replace_current_animation(AnimationManager *animation_manager,
...
static void animation_manager_start_new_idle(AnimationManager *animation_manager)
{
    ...
    furi_timer_start(animation_manager->idle_animation_timer,
...
void animation_manager_free(AnimationManager *animation_manager)
{
    ...
    Dolphin *dolphin = furi_record_open(RECORD_DOLPHIN);
    furi_pubsub_unsubscribe(dolphin_get_pubsub(dolphin),
                            animation_manager->pubsub_subscription_dolphin);
    furi_record_close(RECORD_DOLPHIN);

    Storage *storage = furi_record_open(RECORD_STORAGE);
    furi_pubsub_unsubscribe(storage_get_pubsub(storage),
                            animation_manager->pubsub_subscription_storage);
    furi_record_close(RECORD_STORAGE);
...
void animation_manager_unload_and_stall_animation(AnimationManager *animation_manager)
{
    ...
    FURI_LOG_I(TAG, "Unload animation \'%s\'",
...
void animation_manager_load_and_continue_animation(AnimationManager *animation_manager)
{
        ...
        if (!blocked) {
            ...
            if (restore_animation) {
                ...
                if (valid && strcmp(manifest_info->name, HARDCODED_ANIMATION_NAME) != 0) {
                    animation_manager_replace_current_animation(animation_manager,
                                                                restore_animation);
                    animation_manager->state = AnimationManagerStateIdle;
                ...
                FURI_LOG_E(TAG, "Failed to restore \'%s\'",
                           furi_string_get_cstr(animation_manager->freezed_animation_name));
            }
        }
    } else {
    ...
    FURI_LOG_I(TAG, "Load animation \'%s\'",
               animation_storage_get_meta(animation_manager->current_animation)->name);
...
static void animation_manager_switch_to_one_shot_view(AnimationManager *animation_manager)
{
    ...
    one_shot_view_set_interact_callback(animation_manager->one_shot_view,
                                        animation_manager_interact_callback, animation_manager);
