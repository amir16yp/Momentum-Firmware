...
void desktop_keybinds_migrate(Desktop *desktop)
{
    if (!storage_common_exists(desktop->storage, DESKTOP_KEYBINDS_PATH)) {
        OldKeybinds old;
        const bool success =
            saved_struct_load(OLD_DESKTOP_KEYBINDS_PATH, &old, sizeof(old),
                              OLD_DESKTOP_KEYBINDS_MAGIC, OLD_DESKTOP_KEYBINDS_VER);
...
static FuriString *desktop_keybinds_load_one(Desktop *desktop, DesktopKeybindType type,
                                             DesktopKeybindKey key)
{
    ...
    if (flipper_format_file_open_existing(file, DESKTOP_KEYBINDS_PATH)) {
        FuriString *keybind_name = furi_string_alloc_printf("%s%s", desktop_keybind_types[type],
                                                            desktop_keybind_keys[key]);
...
void desktop_keybinds_load(Desktop *desktop, DesktopKeybinds *keybinds)
{
    ...
    if (flipper_format_file_open_existing(file, DESKTOP_KEYBINDS_PATH)) {
        for (DesktopKeybindType type = 0; type < DesktopKeybindTypeMAX; type++) {
            for (DesktopKeybindKey key = 0; key < DesktopKeybindKeyMAX; key++) {
                furi_string_printf(keybind_name, "%s%s", desktop_keybind_types[type],
                                   desktop_keybind_keys[key]);
                if (!flipper_format_read_string(file, furi_string_get_cstr(keybind_name),
                                                (*keybinds)[type][key])) {
        ...
        for (DesktopKeybindType type = 0; type < DesktopKeybindTypeMAX; type++) {
            for (DesktopKeybindKey key = 0; key < DesktopKeybindKeyMAX; key++) {
                ...
                if (!flipper_format_write_string_cstr(
                        file, furi_string_get_cstr(keybind_name),
...
{
    if (_type != InputTypeShort && _type != InputTypeLong)
        return;
    ...
    if (furi_string_equal(keybind, "_")) {
        ...
        loader_start_detached_with_gui_error(desktop->loader, EXT_PATH("apps/Tools/nightstand.fap"),
                                             "");
