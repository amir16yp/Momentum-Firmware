...
{
    NotificationAppMessage m = {.type = SaveSettingsMessage, .back_event = furi_event_flag_alloc()};
    furi_check(furi_message_queue_put(app->queue, &m, FuriWaitForever) == FuriStatusOk);
    furi_event_flag_wait(m.back_event, NOTIFICATION_EVENT_COMPLETE, FuriFlagWaitAny,
                         FuriWaitForever);
    furi_event_flag_free(m.back_event);
}

// internal layer
static void notification_apply_internal_led_layer(NotificationLedLayer *layer, uint8_t layer_value)
{
    furi_assert(layer);
...
// notification layer
static void notification_apply_notification_led_layer(NotificationLedLayer *layer,
...
// message processing
static void notification_process_notification_message(NotificationApp *app,
    ...
    while (notification_message != NULL) {
        ...
        case NotificationMessageTypeLedDisplayBacklight:
            ...
            if (notification_message->data.led.value > 0x00) {
                notification_apply_notification_led_layer(&app->display,
        ...
        case NotificationMessageTypeLedDisplayBacklightEnforceOn:
            ...
            if (app->display_led_lock == 1) {
                notification_apply_internal_led_layer(&app->display,
                                                      notification_message->data.led.value *
                                                          display_brightness_setting);
            }
            break;
        ...
        case NotificationMessageTypeLedBlinkStart:
            // store and send on delay or after seq
            led_active = true;
            furi_hal_light_blink_start(notification_message->data.led_blink.color,
        ...
        case NotificationMessageTypeVibro:
            if (notification_message->data.vibro.on) {
                if (vibro_setting)
                    notification_vibro_on(force_vibro);
            } else {
                notification_vibro_off();
            }
        ...
        case NotificationMessageTypeSoundOn:
            notification_sound_on(notification_message->data.sound.frequency,
...
static void notification_process_internal_message(NotificationApp *app,
                                                  NotificationAppMessage *message)
{
    uint32_t notification_message_index = 0;
    const NotificationMessage *notification_message;
    ...
    while (notification_message != NULL) {
        ...
        case NotificationMessageTypeLedBlue:
            ...
            notification_apply_internal_led_layer(&app->led[i],
...
{
    return saved_struct_load(NOTIFICATION_SETTINGS_PATH, &app->settings,
                             sizeof(NotificationSettings), NOTIFICATION_SETTINGS_MAGIC,
                             NOTIFICATION_SETTINGS_VERSION);
}

static bool notification_save_settings(NotificationApp *app)
{
    return saved_struct_save(NOTIFICATION_SETTINGS_PATH, &app->settings,
                             sizeof(NotificationSettings), NOTIFICATION_SETTINGS_MAGIC,
                             NOTIFICATION_SETTINGS_VERSION);
}
