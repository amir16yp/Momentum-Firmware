...
static bool region_istream_decode_band(pb_istream_t *stream, const pb_field_t *field, void **arg)
{
    ...
    *arg = region;
    ...
    FURI_LOG_I(TAG, "Add allowed band: start %luHz, stop %luHz, power_limit %ddBm, duty_cycle %u%%",
               band.start, band.end, band.power_limit, band.duty_cycle);
    return true;
}
...
static int32_t region_load_file(void *context)
{
    ...
    do {
        ...
        } else if (!storage_file_open(file, SUBGHZ_REGION_FILENAME, FSAM_READ,
                                      FSOM_OPEN_EXISTING)) {
...
static void region_loader_release_callback(FuriThread *thread, FuriThreadState state, void *context)
{
    UNUSED(context);
