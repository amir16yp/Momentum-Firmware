 ...
 * @param[in,out] context pointer to the user-defined context object. Will be passed to the
 * callback.
 */
void rpc_system_app_set_callback(RpcAppSystem *rpc_app, RpcAppSystemCallback callback,
                                 void *context);
...
/**
 * @brief Send a confirmation that the application using an RpcAppSystem instance has handled the
 * event.
 *
 ...
 * The error code value has no meaning within the subsystem, i.e. it is only passed through to the
 * client.
 *
