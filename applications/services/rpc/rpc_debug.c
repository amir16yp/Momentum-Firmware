...
static size_t rpc_debug_print_file_msg(FuriString *str, const char *prefix,
                                       const PB_Storage_File *msg_file, size_t msg_files_size)
{
    size_t cnt = 0;

    for (size_t i = 0; i < msg_files_size; ++i, ++msg_file) {
        furi_string_cat_printf(str, "%s[%c] size: %5lu", prefix,
        ...
        if (msg_file->data && msg_file->data->size) {
            furi_string_cat_printf(str, " (%d):\'%.*s%s\'", msg_file->data->size,
                                   MIN(msg_file->data->size, 30), msg_file->data->bytes,
...
void rpc_debug_print_message(const PB_Main *message)
{
    ...
    furi_string_cat_printf(str, "PB_Main: {\r\n\tresult: %d cmd_id: %lu (%s)\r\n",
                           message->command_status, message->command_id,
    ...
    case PB_Main_storage_info_response_tag: {
        ...
        furi_string_cat_printf(str, "\t\tfree_space: %llu\r\n",
    ...
    case PB_Main_storage_rename_request_tag: {
        ...
        furi_string_cat_printf(str, "\t\tnew_path: %s\r\n",
    ...
    case PB_Main_gui_send_input_event_request_tag:
        ...
        furi_string_cat_printf(str, "\t\type: %d\r\n",
