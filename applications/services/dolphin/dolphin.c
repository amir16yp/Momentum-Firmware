...
static Dolphin *dolphin_alloc(void)
{
    ...
    dolphin->butthurt_timer =
        furi_event_loop_timer_alloc(dolphin->event_loop, dolphin_butthurt_timer_callback,
                                    FuriEventLoopTimerTypePeriodic, dolphin);
    ...
    dolphin->clear_limits_timer =
        furi_event_loop_timer_alloc(dolphin->event_loop, dolphin_clear_limits_timer_callback,
                                    FuriEventLoopTimerTypePeriodic, dolphin);
...
static void dolphin_event_send_async(Dolphin *dolphin, DolphinEvent *event)
{
    ...
    furi_assert(event);
    event->flag = NULL;
    furi_check(furi_message_queue_put(dolphin->event_queue, event, FuriWaitForever) ==
               FuriStatusOk);
}
...
static void dolphin_event_send_wait(Dolphin *dolphin, DolphinEvent *event)
{
    ...
    furi_check(furi_message_queue_put(dolphin->event_queue, event, FuriWaitForever) ==
               FuriStatusOk);
    furi_check(furi_event_flag_wait(event->flag, DOLPHIN_LOCK_EVENT_FLAG, FuriFlagWaitAny,
                                    FuriWaitForever) == DOLPHIN_LOCK_EVENT_FLAG);
...
static void dolphin_process_event(FuriEventLoopObject *object, void *context)
{
    ...
    if (event.type == DolphinEventTypeDeed) {
        ...
        event.stats->butthurt =
            (dolphin->state->data.flags & DolphinFlagHappyMode) ? 0 : dolphin->state->data.butthurt;
...
int32_t dolphin_srv(void *p)
{
    ...
    furi_event_loop_subscribe_message_queue(dolphin->event_loop, dolphin->event_queue,
                                            FuriEventLoopEventIn, dolphin_process_event, dolphin);

    dolphin_reset_butthurt_timer(dolphin);
    furi_event_loop_timer_start(dolphin->clear_limits_timer, CLEAR_LIMITS_PERIOD_TICKS);

    furi_event_loop_tick_set(dolphin->event_loop, CLEAR_LIMITS_UPDATE_PERIOD_TICKS,
                             dolphin_update_clear_limits_timer_period, dolphin);

    furi_event_loop_pend_callback(dolphin->event_loop, dolphin_update_clear_limits_timer_period,
                                  dolphin);
