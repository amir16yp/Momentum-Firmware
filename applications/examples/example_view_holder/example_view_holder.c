...
int32_t example_view_holder_app(void *arg)
{
    ...
    text_box_set_text(text_box, "ViewHolder is being used\n"
    ...
    // We need to somehow stall the application thread so that the view stays on the screen
    // (otherwise the app will just exit and won't display anything) and at the same time we need a
    // way to quit out of the application.

    // In this example, a simple FuriApiLock instance is used. A real-world application is likely to
    // have some kind of event handling loop here instead. (see the ViewDispatcher example or one of
    // FuriEventLoop examples for that).
    ...
    // This call will block the application thread from running until the exit lock gets unlocked
    // somehow (the only way it can happen in this example is via the back callback).
    api_lock_wait_unlock_and_free(exit_lock);

    // The back key has been pressed, which unlocked the exit lock. The application is about to
    // exit.
