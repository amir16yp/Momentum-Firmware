 ...
 *
 * This application simulates a use case where some data data stream comes from a separate thread
 * (or hardware) and a stream buffer is used to act as an intermediate buffer. The worker thread
 * produces 10 iterations of 32 bytes of simulated data, and each time when the buffer is
 * half-filled, the data is taken out of it and printed to the debug output. After completing all
 * iterations, the application exits.
 */
...
// This funciton is being run in a separate thread to simulate data coming from a producer thread or
// some device.
static int32_t event_loop_stream_buffer_app_worker_thread(void *context)
{
    ...
    for (uint32_t i = 0; i < WORKER_ITERATION_COUNT; ++i) {
        ...
        for (uint32_t j = 0; j < STREAM_BUFFER_SIZE; ++j) {
            ...
            // Put the byte in the buffer. Depending on the use case, it may or may be not
            // acceptable to wait for free space to become available.
            furi_check(furi_stream_buffer_send(app->stream_buffer, &data, 1, FuriWaitForever) == 1);
...
// This function is being run each time when the number of bytes in the buffer is above its trigger
// level.
static void event_loop_stream_buffer_app_event_callback(FuriEventLoopObject *object, void *context)
{
    furi_assert(context);
    EventLoopStreamBufferApp *app = context;
...
static EventLoopStreamBufferApp *event_loop_stream_buffer_app_alloc(void)
{
    ...
    app->worker_thread = furi_thread_alloc_ex("EventLoopStreamBufferWorker", 1024,
                                              event_loop_stream_buffer_app_worker_thread, app);
    ...
    furi_event_loop_subscribe_stream_buffer(app->event_loop, app->stream_buffer,
                                            ...
                                            event_loop_stream_buffer_app_event_callback, app);
