 ...
 * @brief Example application that demonstrates multiple primitives used with two FuriEventLoop
 * instances.
 *
 * This application simulates a complex use case of having two concurrent event loops (each one
 * executing in its own thread) using a stream buffer for communication and additional timers and
 * message passing to handle the keypad input. Additionally, it shows how to use thread signals to
 * stop an event loop in another thread. The GUI functionality is there only for the purpose of
 * exclusive access to the input events.
 *
...
// This function is executed each time the data is taken out of the stream buffer. It is used to
// restart the worker timer.
static void event_loop_multi_app_stream_buffer_worker_callback(FuriEventLoopObject *object,
                                                               void *context)
{
    furi_assert(context);
...
static void event_loop_multi_app_worker_timer_callback(void *context)
{
    ...
    furi_check(furi_stream_buffer_send(worker->stream_buffer, &data, sizeof(data), 0) ==
               sizeof(data));
}

static EventLoopMultiAppWorker *event_loop_multi_app_worker_alloc(FuriStreamBuffer *stream_buffer)
{
    ...
    worker->timer =
        furi_event_loop_timer_alloc(worker->event_loop, event_loop_multi_app_worker_timer_callback,
                                    FuriEventLoopTimerTypeOnce, worker);
    ...
    furi_event_loop_subscribe_stream_buffer(worker->event_loop, worker->stream_buffer,
...
// This function is executed in the GUI context each time an input event occurs (e.g. the user
// pressed a key)
...
static void event_loop_multi_app_stream_buffer_callback(FuriEventLoopObject *object, void *context)
{
    furi_assert(context);
    EventLoopMultiApp *app = context;
    ...
    uint8_t data[STREAM_BUFFER_SIZE];
    // IMPORTANT: No waiting in the event handlers!
    furi_check(furi_stream_buffer_receive(app->stream_buffer, &data, sizeof(data), 0) ==
               sizeof(data));
...
static EventLoopMultiApp *event_loop_multi_app_alloc(void)
{
    ...
    app->worker_thread =
        furi_thread_alloc_ex("EventLoopMultiWorker", 1024, event_loop_multi_app_worker_thread, app);
    ...
    app->exit_timer =
        furi_event_loop_timer_alloc(app->event_loop, event_loop_multi_app_exit_timer_callback,
                                    FuriEventLoopTimerTypePeriodic, app);
    ...
    furi_event_loop_subscribe_message_queue(app->event_loop, app->input_queue, FuriEventLoopEventIn,
                                            event_loop_multi_app_input_queue_callback, app);
    // Notify the event loop about new data in the stream buffer
    furi_event_loop_subscribe_stream_buffer(app->event_loop, app->stream_buffer,
                                            FuriEventLoopEventIn | FuriEventLoopEventFlagEdge,
                                            event_loop_multi_app_stream_buffer_callback, app);
