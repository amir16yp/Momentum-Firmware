...
static void example_thermo_read_temperature(ExampleThermoContext *context)
{
    ...
    do {
        ...
        const uint8_t resolution_mode =
            (buf.fields.config >> DS18B20_CFG_RESOLUTION_POS) & DS18B20_CFG_RESOLUTION_MASK;
...
/* Periodically requests measurements and reads temperature. This function runs in a separare
 * thread. */
...
static void example_thermo_draw_callback(Canvas *canvas, void *ctx)
{
    ...
    snprintf(text_store, TEXT_STORE_SIZE, "to GPIO pin %ld",
...
int32_t example_thermo_main(void *p)
{
    ...
    /* Start the application's main loop. It won't return until the application was requested to
     * exit. */
