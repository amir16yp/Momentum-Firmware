...
static void ble_beacon_app_scene_add_type_byte_input_callback(void *context)
{
    ...
    view_dispatcher_send_custom_event(ble_beacon->view_dispatcher,
                                      BleBeaconAppCustomEventDataEditResult);
}
...
void ble_beacon_app_scene_input_beacon_data_on_enter(void *context)
{
    ...
    byte_input_set_result_callback(ble_beacon->byte_input,
                                   ble_beacon_app_scene_add_type_byte_input_callback, NULL, context,
                                   ble_beacon->beacon_data, sizeof(ble_beacon->beacon_data));
