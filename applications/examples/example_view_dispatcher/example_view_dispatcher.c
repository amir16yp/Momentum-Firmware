...
static bool example_view_dispatcher_app_custom_event_callback(void *context, uint32_t event)
{
    furi_assert(context);
    ExampleViewDispatcherApp *app = context;
    // The event numerical value can mean different things (the application is responsible to uphold
    // its chosen convention) In this example, the only possible meaning is the view index to switch
    // to.
...
static void example_view_dispatcher_app_button_callback(GuiButtonType button_type,
                                                        InputType input_type, void *context)
{
    furi_assert(context);
    ExampleViewDispatcherApp *app = context;
...
static ExampleViewDispatcherApp *example_view_dispatcher_app_alloc()
{
    ...
    widget_add_string_multiline_element(app->widget, 64, 32, AlignCenter, AlignCenter,
                                        FontSecondary, "Press the Button below");
    widget_add_button_element(app->widget, GuiButtonTypeCenter, "Switch View",
                              example_view_dispatcher_app_button_callback, app);
    // Create and initialize the Submenu view.
    app->submenu = submenu_alloc();
    submenu_add_item(app->submenu, "Do Nothing", SubmenuIndexNothing, NULL, NULL);
    submenu_add_item(app->submenu, "Switch View", SubmenuIndexSwitchView,
                     example_view_dispatcher_app_submenu_callback, app);
    // Create the ViewDispatcher instance.
    app->view_dispatcher = view_dispatcher_alloc();
    // Let the GUI know about this ViewDispatcher instance.
    view_dispatcher_attach_to_gui(app->view_dispatcher, gui, ViewDispatcherTypeFullscreen);
    // Register the views within the ViewDispatcher instance. This alone will not show any of them
    // on the screen. Each view must have its own index to refer to it later (it is best done via an
    // enumeration as shown here).
    view_dispatcher_add_view(app->view_dispatcher, ViewIndexWidget, widget_get_view(app->widget));
    view_dispatcher_add_view(app->view_dispatcher, ViewIndexSubmenu,
    ...
    view_dispatcher_set_custom_event_callback(app->view_dispatcher,
                                              example_view_dispatcher_app_custom_event_callback);
    // Set the navigation, or back button callback. It will be called if the user pressed the Back
    // button and the event was not handled in the currently displayed view.
    view_dispatcher_set_navigation_event_callback(app->view_dispatcher,
                                                  example_view_dispatcher_app_navigation_callback);
    // The context will be passed to the callbacks as a parameter, so we have access to our
    // application object.
