    ...
    for (BtTestParamArray_it(it, model->params); !BtTestParamArray_end_p(it);
        ...
        if (param_position < params_on_screen) {
            ...
            if (position == model->position) {
                ...
                elements_slightly_rounded_box(canvas, 0, param_y + 1, param_width,
                                              param_height - 2);
            ...
            canvas_draw_str(canvas, 61, param_text_y,
                            furi_string_get_cstr(param->current_value_text));
...
BtTestParam *bt_test_param_add(BtTest *bt_test, const char *label, uint8_t values_count,
                               BtTestParamChangeCallback change_callback, void *context)
{
    BtTestParam *param = NULL;
    furi_assert(label);
    furi_assert(bt_test);
