...
MU_TEST(test_varint_rand_u)
{
    ...
    for (size_t i = 0; i < 200000; i++) {
        ...
        mu_assert_int_eq(varint_uint32_pack(rand_value, data),
                         varint_uint32_unpack(&out_value, data, 8));
...
MU_TEST(test_varint_rand_i)
{
    ...
    for (size_t i = 0; i < 200000; i++) {
        ...
        mu_assert_int_eq(varint_int32_pack(rand_value, data),
                         varint_int32_unpack(&out_value, data, 8));
        mu_assert_int_eq(rand_value, out_value);
    }
}
