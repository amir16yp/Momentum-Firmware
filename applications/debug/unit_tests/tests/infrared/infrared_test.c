...
static bool infrared_test_prepare_file(const char *protocol_name)
{
    ...
    furi_string_printf(test->file_path, "%s%s%s%s", IR_TEST_FILES_DIR, IR_TEST_FILE_PREFIX,
                       protocol_name, IR_TEST_FILE_SUFFIX);

    do {
        uint32_t format_version;
        if (!flipper_format_buffered_file_open_existing(test->ff,
                                                        furi_string_get_cstr(test->file_path)))
            break;
        if (!flipper_format_read_header(test->ff, file_type, &format_version))
            break;
        if (furi_string_cmp_str(file_type, "IR tests file") || format_version != 1)
            break;
        success = true;
    } while (false);
...
static bool infrared_test_load_raw_signal(FlipperFormat *ff, const char *signal_name,
                                          uint32_t **timings, uint32_t *timings_count)
{
    FuriString *buf;
    ...
    do {
        ...
        if (!is_name_found)
            break;
        if (!flipper_format_read_string(ff, "type", buf) || furi_string_cmp_str(buf, "raw"))
            break;
        if (!flipper_format_get_value_count(ff, "data", timings_count))
            break;
        if (!*timings_count)
            break;
...
static bool infrared_test_read_message(FlipperFormat *ff, InfraredMessage *message)
{
    ...
    do {
        ...
        if (!flipper_format_read_hex(ff, "command", (uint8_t *)&message->command, sizeof(uint32_t)))
            break;
        if (!flipper_format_read_bool(ff, "repeat", &message->repeat, 1))
            break;
        success = true;
    } while (false);
...
static bool infrared_test_load_messages(FlipperFormat *ff, const char *signal_name,
                                        InfraredMessage **messages, uint32_t *messages_count)
{
    FuriString *buf;
    buf = furi_string_alloc();
    bool success = false;
    ...
    do {
        ...
        if (!flipper_format_read_string(ff, "type", buf) ||
            ...
            break;
        if (!flipper_format_read_uint32(ff, "count", messages_count, 1))
            break;
        if (!*messages_count)
            break;
...
static void infrared_test_compare_message_results(const InfraredMessage *message_decoded,
...
static void infrared_test_run_encoder_fill_array(InfraredEncoderHandler *handler, uint32_t *timings,
                                                 uint32_t *timings_len, bool *start_level)
{
    uint32_t duration = 0;
    bool level = false;
    bool level_read;
...
static void infrared_test_run_encoder(InfraredProtocol protocol, uint32_t test_index)
{
    ...
    mu_assert(infrared_test_load_messages(test->ff, furi_string_get_cstr(buf), &input_messages,
                                          &input_messages_count),
              "Failed to load messages from file");

    furi_string_printf(buf, "encoder_expected%ld", test_index);
    mu_assert(infrared_test_load_raw_signal(test->ff, furi_string_get_cstr(buf), &expected_timings,
                                            &expected_timings_count),
...
static void infrared_test_run_encoder_decoder(InfraredProtocol protocol, uint32_t test_index)
{
    ...
    for (uint32_t message_counter = 0; message_counter < input_messages_count; ++message_counter) {
        ...
        timings_count = 200;
        infrared_test_run_encoder_fill_array(test->encoder_handler, timings, &timings_count,
                                             &level);
        furi_check(timings_count <= 200);
...
static void infrared_test_run_decoder(InfraredProtocol protocol, uint32_t test_index)
{
    ...
    mu_assert(infrared_test_prepare_file(infrared_get_protocol_name(protocol)),
              "Failed to prepare test file");

    furi_string_printf(buf, "decoder_input%ld", test_index);
    mu_assert(infrared_test_load_raw_signal(test->ff, furi_string_get_cstr(buf), &timings,
                                            &timings_count),
              "Failed to load raw signal from file");

    furi_string_printf(buf, "decoder_expected%ld", test_index);
    mu_assert(infrared_test_load_messages(test->ff, furi_string_get_cstr(buf), &messages,
                                          &messages_count),
              "Failed to load messages from file");
    ...
    for (uint32_t i = 0; i < timings_count; ++i) {
        ...
        if (timings[i] > INFRARED_RAW_RX_TIMING_DELAY_US) {
            ...
            if (message_decoded_check) {
                /* infrared_decode() can reset message, but we have to call infrared_decode() to
                 * perform real simulation: infrared_check() by timeout, then infrared_decode() when
                 * meet edge */
...
MU_TEST(infrared_test_ac_database)
{
    ...
    mu_assert(infrared_brute_force_calculate_messages(test->brutedb) == InfraredErrorCodeNone,
