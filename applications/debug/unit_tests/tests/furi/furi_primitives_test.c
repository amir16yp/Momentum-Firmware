...
static void test_furi_message_queue(TestFuriPrimitivesData *data)
{
    ...
    mu_assert_int_eq(MESSAGE_QUEUE_ELEMENT_SIZE,
                     furi_message_queue_get_message_size(message_queue));
...
static void test_furi_stream_buffer(TestFuriPrimitivesData *data)
{
    ...
    for (uint8_t i = 0;; ++i) {
        ...
        mu_assert_int_eq(STREAM_BUFFER_SIZE - i,
                         furi_stream_buffer_spaces_available(stream_buffer));
    ...
    for (uint8_t i = 0;; ++i) {
        mu_assert_int_eq(STREAM_BUFFER_SIZE - i, furi_stream_buffer_bytes_available(stream_buffer));
