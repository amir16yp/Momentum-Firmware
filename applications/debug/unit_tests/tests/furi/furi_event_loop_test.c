...
static void test_furi_event_loop_producer_message_queue_callback(FuriEventLoopObject *object,
                                                                 void *context)
{
    furi_check(context);
    ...
    FURI_LOG_I(TAG, "producer MessageQueue: %lu %lu", data->producer.message_queue_count,
    ...
    if (data->producer.message_queue_count == MESSAGE_COUNT / 2) {
        ...
        furi_event_loop_subscribe_message_queue(
            ...
            test_furi_event_loop_producer_message_queue_callback, data);

    } else if (data->producer.message_queue_count == MESSAGE_COUNT) {
        furi_event_loop_unsubscribe(data->producer.event_loop, data->message_queue);
        furi_event_loop_pend_callback(data->producer.event_loop,
                                      test_furi_event_loop_pending_callback, &data->producer);
        return;
    }

    data->producer.message_queue_count++;

    furi_check(furi_message_queue_put(data->message_queue, &data->producer.message_queue_count,
                                      0) == FuriStatusOk);

    furi_delay_us(furi_hal_random_get() % 100);
}

static void test_furi_event_loop_producer_stream_buffer_callback(FuriEventLoopObject *object,
                                                                 void *context)
{
    furi_check(context);
    ...
    FURI_LOG_I(TAG, "producer StreamBuffer: %lu %lu", producer->stream_buffer_count,
    ...
    if (producer->stream_buffer_count == MESSAGE_COUNT / 2) {
        ...
        furi_event_loop_subscribe_stream_buffer(
            producer->event_loop, data->stream_buffer, FuriEventLoopEventOut,
            test_furi_event_loop_producer_stream_buffer_callback, data);
    ...
    furi_check(furi_stream_buffer_send(data->stream_buffer, &producer->stream_buffer_count,
                                       sizeof(uint32_t), 0) == sizeof(uint32_t));

    furi_delay_us(furi_hal_random_get() % 100);
}

static void test_furi_event_loop_producer_event_flag_callback(FuriEventLoopObject *object,
                                                              void *context)
{
    furi_check(context);
    ...
    if (data->producer.event_flag_count == EVENT_FLAG_COUNT / 2) {
        ...
        furi_event_loop_subscribe_event_flag(
            data->producer.event_loop, data->event_flag, FuriEventLoopEventOut,
            test_furi_event_loop_producer_event_flag_callback, data);
...
static void test_furi_event_loop_producer_semaphore_callback(FuriEventLoopObject *object,
                                                             void *context)
{
    furi_check(context);
    ...
    FURI_LOG_I(TAG, "producer Semaphore: %lu %lu", producer->semaphore_count,
               consumer->semaphore_count);
    ...
    if (producer->semaphore_count == MESSAGE_COUNT / 2) {
        ...
        furi_event_loop_subscribe_semaphore(producer->event_loop, data->semaphore,
                                            FuriEventLoopEventOut,
                                            test_furi_event_loop_producer_semaphore_callback, data);
...
static int32_t test_furi_event_loop_producer(void *p)
{
    ...
    for (uint32_t i = 0; i < RUN_COUNT; ++i) {
        ...
        furi_event_loop_subscribe_message_queue(
            producer->event_loop, data->message_queue, FuriEventLoopEventOut,
            test_furi_event_loop_producer_message_queue_callback, data);
        furi_event_loop_subscribe_stream_buffer(
            producer->event_loop, data->stream_buffer, FuriEventLoopEventOut,
            test_furi_event_loop_producer_stream_buffer_callback, data);
        furi_event_loop_subscribe_event_flag(
            producer->event_loop, data->event_flag, FuriEventLoopEventOut,
            test_furi_event_loop_producer_event_flag_callback, data);
        furi_event_loop_subscribe_semaphore(producer->event_loop, data->semaphore,
                                            FuriEventLoopEventOut,
                                            test_furi_event_loop_producer_semaphore_callback, data);
...
static void test_furi_event_loop_consumer_message_queue_callback(FuriEventLoopObject *object,
                                                                 void *context)
{
    furi_check(context);
    ...
    furi_check(furi_message_queue_get(data->message_queue, &data->consumer.message_queue_count,
                                      0) == FuriStatusOk);

    FURI_LOG_I(TAG, "consumer MessageQueue: %lu %lu", data->producer.message_queue_count,
               data->consumer.message_queue_count);
    ...
    if (data->consumer.message_queue_count == MESSAGE_COUNT / 2) {
        ...
        furi_event_loop_subscribe_message_queue(
            ...
            test_furi_event_loop_consumer_message_queue_callback, data);

    } else if (data->consumer.message_queue_count == MESSAGE_COUNT) {
        furi_event_loop_unsubscribe(data->consumer.event_loop, data->message_queue);
        furi_event_loop_pend_callback(data->consumer.event_loop,
                                      test_furi_event_loop_pending_callback, &data->consumer);
    }
}

static void test_furi_event_loop_consumer_stream_buffer_callback(FuriEventLoopObject *object,
                                                                 void *context)
{
    furi_check(context);
    ...
    furi_check(furi_stream_buffer_receive(data->stream_buffer, &consumer->stream_buffer_count,
                                          sizeof(uint32_t), 0) == sizeof(uint32_t));
    ...
    if (consumer->stream_buffer_count == MESSAGE_COUNT / 2) {
        ...
        furi_event_loop_subscribe_stream_buffer(
            consumer->event_loop, data->stream_buffer, FuriEventLoopEventIn,
            test_furi_event_loop_consumer_stream_buffer_callback, data);
...
static void test_furi_event_loop_consumer_event_flag_callback(FuriEventLoopObject *object,
                                                              void *context)
{
    furi_check(context);
    ...
    furi_check(furi_event_flag_wait(data->event_flag, consumer_flags, FuriFlagWaitAny, 0) &
               consumer_flags);
    ...
    if (data->consumer.event_flag_count == EVENT_FLAG_COUNT / 2) {
        ...
        furi_event_loop_subscribe_event_flag(
            data->consumer.event_loop, data->event_flag, FuriEventLoopEventIn,
            test_furi_event_loop_consumer_event_flag_callback, data);
...
static void test_furi_event_loop_consumer_semaphore_callback(FuriEventLoopObject *object,
                                                             void *context)
{
    furi_check(context);
    ...
    FURI_LOG_I(TAG, "consumer Semaphore: %lu %lu", producer->semaphore_count,
               consumer->semaphore_count);

    if (consumer->semaphore_count == MESSAGE_COUNT / 2) {
        furi_event_loop_unsubscribe(consumer->event_loop, data->semaphore);
        furi_event_loop_subscribe_semaphore(consumer->event_loop, data->semaphore,
                                            FuriEventLoopEventIn,
                                            test_furi_event_loop_consumer_semaphore_callback, data);
...
static int32_t test_furi_event_loop_consumer(void *p)
{
    ...
    for (uint32_t i = 0; i < RUN_COUNT; ++i) {
        ...
        furi_event_loop_subscribe_message_queue(
            consumer->event_loop, data->message_queue, FuriEventLoopEventIn,
            test_furi_event_loop_consumer_message_queue_callback, data);
        furi_event_loop_subscribe_stream_buffer(
            consumer->event_loop, data->stream_buffer, FuriEventLoopEventIn,
            test_furi_event_loop_consumer_stream_buffer_callback, data);
        furi_event_loop_subscribe_event_flag(
            consumer->event_loop, data->event_flag, FuriEventLoopEventIn,
            test_furi_event_loop_consumer_event_flag_callback, data);
        furi_event_loop_subscribe_semaphore(consumer->event_loop, data->semaphore,
                                            FuriEventLoopEventIn,
                                            test_furi_event_loop_consumer_semaphore_callback, data);
...
void test_furi_event_loop_self_unsubscribe(void)
{
    ...
    furi_event_loop_subscribe_semaphore(event_loop, semaphore, FuriEventLoopEventIn,
                                        test_self_unsub_semaphore_callback, event_loop);
