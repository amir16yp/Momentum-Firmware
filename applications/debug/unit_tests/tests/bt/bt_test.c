...
// Identity root key
static const uint8_t gap_legacy_irk[16] = {0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0,
                                           0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0};
// Encryption root key
static const uint8_t gap_legacy_erk[16] = {0xfe, 0xdc, 0xba, 0x09, 0x87, 0x65, 0x43, 0x21,
...
void bt_test_alloc(void)
{
    ...
    bt_keys_storage_set_ram_params(bt_test->bt_keys_storage, bt_test->nvm_ram_buff_dut,
                                   BT_TEST_NVM_RAM_BUFF_SIZE);
}
...
static void bt_test_keys_storage_profile(void)
{
    ...
    mu_assert(bt_keys_storage_update(bt_test->bt_keys_storage, bt_test->nvm_ram_buff_dut,
    ...
    mu_assert(memcmp(bt_test->nvm_ram_buff_ref, bt_test->nvm_ram_buff_dut,
                     nvm_change_size_on_connection) == 0,
              "Wrong buffer loaded");
    ...
    for (size_t i = nvm_disconnect_update_offset;
         i < nvm_disconnect_update_offset + nvm_disconnect_update_size; i++) {
        bt_test->nvm_ram_buff_dut[i] = rand();
        bt_test->nvm_ram_buff_ref[i] = bt_test->nvm_ram_buff_dut[i];
    }
    mu_assert(bt_keys_storage_update(bt_test->bt_keys_storage,
                                     &bt_test->nvm_ram_buff_dut[nvm_disconnect_update_offset],
                                     nvm_disconnect_update_size),
              "Failed to update key storage on initial disconnect");
    memset(bt_test->nvm_ram_buff_dut, 0, BT_TEST_NVM_RAM_BUFF_SIZE);
    mu_assert(bt_keys_storage_load(bt_test->bt_keys_storage), "Failed to load NVM");
    mu_assert(memcmp(bt_test->nvm_ram_buff_ref, bt_test->nvm_ram_buff_dut, nvm_total_size) == 0,
              "Wrong buffer loaded");
}

static void bt_test_keys_remove_test_file(void)
{
    mu_assert(storage_simply_remove(bt_test->storage, BT_TEST_KEY_STORAGE_FILE_PATH),
              "Can't remove test file");
}

// Helper function to create a version 0 file manually
static bool bt_test_create_v0_file(const char *file_path, const uint8_t *nvm_data, size_t nvm_size)
{
    // Version 0 files use saved_struct format with magic 0x18, version 0, containing only BLE
    // pairing data
    return saved_struct_save(file_path, nvm_data, nvm_size, BT_KEYS_STORAGE_MAGIC_TEST,
                             0); // Version 0
}
...
static void bt_test_migration_v0_to_v1(void)
{
    ...
    mu_assert(bt_test_create_v0_file(BT_TEST_MIGRATION_FILE_PATH, test_nvm_data, test_nvm_size),
    ...
    mu_assert(memcmp(test_nvm_data, loaded_buffer, test_nvm_size) == 0,
    ...
    mu_assert(memcmp(migrated_keys->irk, gap_legacy_irk, sizeof(gap_legacy_irk)) == 0,
              "IRK not set to legacy after migration");
    mu_assert(memcmp(migrated_keys->erk, gap_legacy_erk, sizeof(gap_legacy_erk)) == 0,
...
static void bt_test_migration_preserves_pairings_and_keys(void)
{
    ...
    mu_assert(bt_test_create_v0_file(BT_TEST_MIGRATION_FILE_PATH, pairing_data, pairing_data_size),
    ...
    for (int iteration = 0; iteration < 2; iteration++) {
        ...
        mu_assert(memcmp(pairing_data, buffer, pairing_data_size) == 0,
        ...
        if (iteration == 0)
            ...
            mu_assert(memcmp(&keys_after_first_load, keys, sizeof(GapRootSecurityKeys)) == 0,
...
static void bt_test_delete_generates_new_keys_and_not_legacy(void)
{
    ...
    mu_assert(memcmp(original_keys_copy, new_keys, sizeof(original_keys_copy)) != 0,
              "Root keys were not regenerated after delete");
    mu_assert(memcmp(new_keys->irk, gap_legacy_irk, sizeof(gap_legacy_irk)) != 0,
              "IRK after delete should not match legacy");
    mu_assert(memcmp(new_keys->erk, gap_legacy_erk, sizeof(gap_legacy_erk)) != 0,
