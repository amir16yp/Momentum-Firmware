...
{
    mu_assert(sizeof(DialogsFileBrowserOptions) == 28,
              "Changes to `DialogsFileBrowserOptions` should also be reflected in "
              "`dialog_file_browser_set_basic_options`");

    DialogsFileBrowserOptions options;
    dialog_file_browser_set_basic_options(&options, ".fap", NULL);
    // note: this assertions can safely be changed, their primary purpose is to remind the
    // maintainer to update `dialog_file_browser_set_basic_options` by including all structure
    // fields in it
