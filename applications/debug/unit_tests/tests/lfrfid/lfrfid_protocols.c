...
MU_TEST(test_lfrfid_protocol_em_read_simple)
{
    ...
    for (size_t i = 0; i < EM_TEST_EMULATION_TIMINGS_COUNT * 10; i++) {
        bool pulse_pop =
            pulse_glue_push(pulse_glue, em_test_timings[i % EM_TEST_EMULATION_TIMINGS_COUNT] >= 0,
...
{
    ProtocolDict *dict = protocol_dict_alloc(lfrfid_protocols, LFRFIDProtocolMax);
    mu_assert_int_eq(HID10301_TEST_DATA_SIZE,
                     protocol_dict_get_data_size(dict, LFRFIDProtocolH10301));
    ...
    for (size_t i = 0; i < HID10301_TEST_EMULATION_TIMINGS_COUNT * 10; i++) {
        bool pulse_pop = pulse_glue_push(
            pulse_glue, hid10301_test_timings[i % HID10301_TEST_EMULATION_TIMINGS_COUNT] >= 0,
...
MU_TEST(test_lfrfid_protocol_h10301_emulate_simple)
{
    ...
    for (size_t i = 0; i < HID10301_TEST_EMULATION_TIMINGS_COUNT; i++) {
        ...
        if (level_duration_get_level(level_duration)) {
            mu_assert_int_eq(hid10301_test_timings[i], level_duration_get_duration(level_duration));
        } else {
            mu_assert_int_eq(hid10301_test_timings[i],
...
{
    ProtocolDict *dict = protocol_dict_alloc(lfrfid_protocols, LFRFIDProtocolMax);
    mu_assert_int_eq(IOPROX_XSF_TEST_DATA_SIZE,
                     protocol_dict_get_data_size(dict, LFRFIDProtocolIOProxXSF));
    ...
    for (size_t i = 0; i < IOPROX_XSF_TEST_EMULATION_TIMINGS_COUNT * 10; i++) {
        bool pulse_pop = pulse_glue_push(
            pulse_glue, ioprox_xsf_test_timings[i % IOPROX_XSF_TEST_EMULATION_TIMINGS_COUNT] >= 0,
...
MU_TEST(test_lfrfid_protocol_ioprox_xsf_emulate_simple)
{
    ...
    for (size_t i = 0; i < IOPROX_XSF_TEST_EMULATION_TIMINGS_COUNT; i++) {
        ...
        if (level_duration_get_level(level_duration)) {
            mu_assert_int_eq(ioprox_xsf_test_timings[i],
                             level_duration_get_duration(level_duration));
        } else {
            mu_assert_int_eq(ioprox_xsf_test_timings[i],
                             -level_duration_get_duration(level_duration));
...
{
    ProtocolDict *dict = protocol_dict_alloc(lfrfid_protocols, LFRFIDProtocolMax);
    mu_assert_int_eq(INDALA26_TEST_DATA_SIZE,
                     protocol_dict_get_data_size(dict, LFRFIDProtocolIndala26));
    ...
    for (size_t i = 0; i < INDALA26_EMULATION_TIMINGS_COUNT; i++) {
        ...
        if (level_duration_get_level(level_duration)) {
            mu_assert_int_eq(indala26_test_timings[i], level_duration_get_duration(level_duration));
        } else {
            mu_assert_int_eq(indala26_test_timings[i],
                             -level_duration_get_duration(level_duration));
        }
    }
...
MU_TEST(test_lfrfid_protocol_fdxb_read_simple)
{
    ...
    for (size_t i = 0; i < FDXB_TEST_EMULATION_TIMINGS_COUNT * 10; i++) {
        bool pulse_pop = pulse_glue_push(
            pulse_glue, fdxb_test_timings[i % FDXB_TEST_EMULATION_TIMINGS_COUNT] >= 0,
...
#define INDALA224_TEST_DATA                                                                        \
    {                                                                                              \
        0x80, 0x00, 0x00, 0x01, 0xB2, 0x35, 0x23, 0xA6, 0xC2, 0xE3, 0x1E, 0xBA, 0x3C, 0xBE, 0xE4,  \
            0xAF, 0xB3, 0xC6, 0xAD, 0x1F, 0xCF, 0x64, 0x93, 0x93, 0x92, 0x8C, 0x14, 0xE5           \
    }
...
{
    ProtocolDict *dict = protocol_dict_alloc(lfrfid_protocols, LFRFIDProtocolMax);
    mu_assert_int_eq(INDALA224_TEST_DATA_SIZE,
                     protocol_dict_get_data_size(dict, LFRFIDProtocolIndala224));
