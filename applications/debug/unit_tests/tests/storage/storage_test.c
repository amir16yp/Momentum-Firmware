...
static bool storage_file_read_write_test(File *file, uint8_t *data, size_t test_size)
{
    ...
    do {
        if (!storage_file_open(file, filename, FSAM_WRITE, FSOM_CREATE_ALWAYS))
            break;
        if (test_size != storage_file_write(file, data, test_size))
            break;
        storage_file_close(file);
        ...
        if (!storage_file_open(file, filename, FSAM_READ, FSOM_OPEN_EXISTING))
            break;
        if (test_size != storage_file_read(file, data, test_size))
            break;
        storage_file_close(file);
...
static const char *const storage_copy_test_paths[] = {
    "1", "11", "111", "1/2", "1/22", "1/222", "11/1", "111/2", "111/22", "111/22/33",
};
...
MU_TEST(storage_file_rename)
{
    ...
    mu_assert_int_eq(FSE_OK, storage_common_rename(storage, UNIT_TESTS_PATH("file.old"),
                                                   UNIT_TESTS_PATH("file.new")));
    mu_assert_int_eq(FSE_NOT_EXIST,
                     storage_common_stat(storage, UNIT_TESTS_PATH("file.old"), NULL));
...
MU_TEST(storage_equiv_and_subdir)
{
    ...
    mu_assert_int_eq(true, storage_common_equivalent_path(storage, UNIT_TESTS_PATH("blah"),
                                                          UNIT_TESTS_PATH("blah")));
    mu_assert_int_eq(true, storage_common_equivalent_path(storage, UNIT_TESTS_PATH("blah/"),
                                                          UNIT_TESTS_PATH("blah/")));
    mu_assert_int_eq(false, storage_common_equivalent_path(storage, UNIT_TESTS_PATH("blah"),
                                                           UNIT_TESTS_PATH("blah-blah")));
    mu_assert_int_eq(false, storage_common_equivalent_path(storage, UNIT_TESTS_PATH("blah/"),
                                                           UNIT_TESTS_PATH("blah-blah/")));

    mu_assert_int_eq(
        true, storage_common_is_subdir(storage, UNIT_TESTS_PATH("blah"), UNIT_TESTS_PATH("blah")));
    mu_assert_int_eq(true, storage_common_is_subdir(storage, UNIT_TESTS_PATH("blah"),
                                                    UNIT_TESTS_PATH("blah/blah")));
    mu_assert_int_eq(false, storage_common_is_subdir(storage, UNIT_TESTS_PATH("blah/blah"),
                                                     UNIT_TESTS_PATH("blah")));
    mu_assert_int_eq(false, storage_common_is_subdir(storage, UNIT_TESTS_PATH("blah"),
                                                     UNIT_TESTS_PATH("blah-blah")));
...
static const char *storage_test_apps[] = {
    "-_twilight_-", "-_rainbow_-", "-_pinkie_-", "-_apple_-", "-_flutter_-", "-_rare_-",
};
...
MU_TEST(test_storage_common_migrate)
{
    ...
    mu_assert_int_eq(FSE_OK, storage_common_migrate(storage, UNIT_TESTS_PATH("migrate_old"),
                                                    UNIT_TESTS_PATH("migrate_new")));
    ...
    mu_assert_int_eq(FSE_OK, storage_common_migrate(storage, UNIT_TESTS_PATH("migrate_old.file"),
                                                    UNIT_TESTS_PATH("migrate_new.file")));
...
MU_TEST(test_md5_calc)
{
    ...
    const uint8_t md5[MD5_HASH_SIZE] = {
        0x2a, 0x45, 0x6f, 0xa4, 0x3e, 0x75, 0x08, 0x8f,
        0xdd, 0xe4, 0x1c, 0x93, 0x15, 0x9d, 0x62, 0xa2,
    };
