        ...
        while (true) {
            ...
            if (!result) {
                ...
                if (!instance->cli) {
                    ...
                    dialog_message_set_header(message, "Failed Test:", 64, 30, AlignCenter,
                                              AlignBottom);
                    dialog_message_set_text(message, file_basename_cstr, 64, 34, AlignCenter,
                                            AlignTop);
        ...
        if (instance->minunit_run != 0) {
            ...
            if (!instance->cli) {
                ...
                snprintf(text, sizeof(text),
                         ...
                         "Consumed: %lu ms\n"
                         "Leaked: %ld",
                         instance->total_failed, cycle_counter, heap_before - heap_after);
                DialogsApp *dialogs = furi_record_open(RECORD_DIALOGS);
                DialogMessage *message = dialog_message_alloc();
                dialog_message_set_header(message,
                                          instance->total_failed == 0 ? "PASSED" : "FAILED", 64, 8,
