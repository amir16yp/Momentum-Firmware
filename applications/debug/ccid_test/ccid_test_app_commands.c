...
void handle_instruction_02(uint8_t p1, uint8_t p2, uint16_t lc, uint16_t le,
...
// response SW1=0x90, SW2=0x00
void handle_instruction_03(uint8_t p1, uint8_t p2, uint16_t lc,
                           ISO7816_Response_APDU *response_apdu)
{
    if (p1 == 0 && p2 == 0 && lc == 2) {
        response_apdu->DataLen = 0;
...
void handle_instruction_04(uint8_t p1, uint8_t p2, uint16_t lc, uint16_t le,
...
void iso7816_process_command(const ISO7816_Command_APDU *command_apdu,
                             ISO7816_Response_APDU *response_apdu)
{
    ...
    if (command_apdu->CLA == 0x01) {
        ...
        case 0x01:
            ...
            break;
        case 0x02:
            handle_instruction_02(command_apdu->P1, command_apdu->P2, command_apdu->Lc,
                                  command_apdu->Le, response_apdu);
            break;
        case 0x03:
            handle_instruction_03(command_apdu->P1, command_apdu->P2, command_apdu->Lc,
                                  response_apdu);
            break;
        case 0x04:
            handle_instruction_04(command_apdu->P1, command_apdu->P2, command_apdu->Lc,
                                  command_apdu->Le, command_apdu->Data, response_apdu);
            break;
        default:
