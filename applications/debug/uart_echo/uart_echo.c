...
static void uart_echo_view_draw_callback(Canvas *canvas, void *_model)
{
    ...
    for (size_t i = 0; i < LINES_ON_SCREEN; i++) {
        canvas_draw_str(canvas, 0, (i + 1) * (canvas_current_font_height(canvas) - 1),
                        furi_string_get_cstr(model->list[i]->text));

        if (i == model->line) {
            uint8_t width = canvas_string_width(canvas, furi_string_get_cstr(model->list[i]->text));

            canvas_draw_box(canvas, width, (i) * (canvas_current_font_height(canvas) - 1) + 2, 2,
...
static void uart_echo_on_irq_cb(FuriHalSerialHandle *handle, FuriHalSerialRxEvent event,
                                void *context)
{
    furi_assert(context);
    UNUSED(handle);
...
static UartEchoApp *uart_echo_app_alloc(uint32_t baudrate, FuriHalSerialDataBits data_bits,
...
int32_t uart_echo_app(void *p)
{
    ...
    if (p) {
        ...
        do {
            if (strint_to_uint32(parse_ptr, &parse_ptr, &baudrate, 10) != StrintParseNoError)
                break;
        ...
        if (!parse_success) {
            FURI_LOG_I(TAG, "Couldn't parse baud rate and framing (%s). Applying defaults (%d_8N1)",
                       (const char *)p, DEFAULT_BAUD_RATE);
        }
    }
