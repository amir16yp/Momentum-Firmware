...
static void file_browser_scene_start_ok_callback(GuiButtonType result, InputType type,
                                                 void *context)
{
    UNUSED(result);
    furi_assert(context);
...
void file_browser_scene_start_on_enter(void *context)
{
    ...
    widget_add_string_multiline_element(app->widget, 64, 20, AlignCenter, AlignTop, FontSecondary,
                                        "Press OK to start");
