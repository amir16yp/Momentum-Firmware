...
void subghz_test_carrier_set_callback(SubGhzTestCarrier *subghz_test_carrier,
                                      SubGhzTestCarrierCallback callback, void *context)
{
...
void subghz_test_carrier_draw(Canvas *canvas, SubGhzTestCarrierModel *model)
{
    ...
    // Frequency
    snprintf(buffer, sizeof(buffer), "Freq: %03ld.%03ld.%03ld Hz",
             model->real_frequency / 1000000 % 1000, model->real_frequency / 1000 % 1000,
    ...
    if (model->status == SubGhzTestCarrierModelStatusRx) {
        snprintf(buffer, sizeof(buffer), "RSSI: %ld.%ld dBm", (int32_t)(model->rssi),
    ...
    with_view_model(
        subghz_test_carrier->view, SubGhzTestCarrierModel * model,
        {
            ...
            if (event->key == InputKeyLeft) {
                ...
                if (model->frequency < subghz_frequencies_count_testing - 1)
                    model->frequency++;
            } else if (event->key == InputKeyDown) {
                if (model->path > 0)
                    model->path--;
            } else if (event->key == InputKeyUp) {
                if (model->path < FuriHalSubGhzPath868)
                    model->path++;
            ...
            if (model->status == SubGhzTestCarrierModelStatusRx) {
                ...
                furi_hal_gpio_init(&gpio_cc1101_g0, GpioModeOutputPushPull, GpioPullNo,
                                   GpioSpeedLow);
                furi_hal_gpio_write(&gpio_cc1101_g0, true);
                if (!furi_hal_subghz_tx()) {
                    furi_hal_gpio_init(&gpio_cc1101_g0, GpioModeInput, GpioPullNo, GpioSpeedLow);
                    subghz_test_carrier->callback(SubGhzTestCarrierEventOnlyRx,
                                                  subghz_test_carrier->context);
                }
...
SubGhzTestCarrier *subghz_test_carrier_alloc(void)
{
    ...
    view_allocate_model(subghz_test_carrier->view, ViewModelTypeLocking,
                        sizeof(SubGhzTestCarrierModel));
    ...
    subghz_test_carrier->timer = furi_timer_alloc(subghz_test_carrier_rssi_timer_callback,
                                                  FuriTimerTypePeriodic, subghz_test_carrier);
