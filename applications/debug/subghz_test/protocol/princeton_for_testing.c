...
void subghz_encoder_princeton_for_testing_set(SubGhzEncoderPrinceton *instance, uint32_t key,
                                              size_t repeat, uint32_t frequency)
{
    furi_assert(instance);
...
void subghz_encoder_princeton_for_testing_print_log(void *context)
{
    ...
    FURI_LOG_I(TAG "Encoder", "Radio tx_time=%luus  ON=%luus, OFF=%luus, DutyCycle=%lu,%lu%%",
               instance->time_high + instance->time_low, instance->time_high, instance->time_low,
               (uint32_t)duty_cycle, (uint32_t)((duty_cycle - (uint32_t)duty_cycle) * 100UL));
}
...
void subghz_decoder_princeton_for_testing_set_callback(SubGhzDecoderPrinceton *instance,
                                                       SubGhzDecoderPrincetonCallback callback,
                                                       void *context)
{
...
static void subghz_decoder_princeton_for_testing_add_bit(SubGhzDecoderPrinceton *instance,
                                                         uint8_t bit)
{
...
void subghz_decoder_princeton_for_testing_parse(SubGhzDecoderPrinceton *instance, bool level,
                                                uint32_t duration)
{
    ...
    case PrincetonDecoderStepCheckDuration:
        if (!level) {
            if (duration >= ((uint32_t)instance->te_short * 10 + instance->te_delta)) {
                ...
                if (instance->code_count_bit == instance->code_min_count_bit_for_found) {
                    ...
                    if (instance->callback)
                        instance->callback(instance, instance->context);
                }
                instance->code_found = 0;
                instance->code_count_bit = 0;
                instance->te = 0;
            ...
            if ((DURATION_DIFF(instance->te_last, instance->te_short) < instance->te_delta) &&
                ...
                instance->parser_step = PrincetonDecoderStepSaveDuration;
            } else if ((DURATION_DIFF(instance->te_last, instance->te_long) <
                        instance->te_delta * 3) &&
