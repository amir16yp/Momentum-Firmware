...
BatteryTestApp *battery_test_alloc(void)
{
    ...
    view_dispatcher_set_tick_event_callback(app->view_dispatcher,
                                            battery_test_battery_info_update_model, 500);
    view_dispatcher_attach_to_gui(app->view_dispatcher, app->gui, ViewDispatcherTypeFullscreen);

    // Views
    app->battery_info = battery_info_alloc();
    view_set_previous_callback(battery_info_get_view(app->battery_info),
                               battery_test_exit_confirm_view);
    view_dispatcher_add_view(app->view_dispatcher, BatteryTestAppViewBatteryInfo,
    ...
    view_dispatcher_add_view(app->view_dispatcher, BatteryTestAppViewExitDialog,
