...
static void expansion_test_app_serial_rx_callback(FuriHalSerialHandle *handle,
...
static void expansion_test_app_start(ExpansionTestApp *instance)
{
    ...
    furi_hal_serial_async_rx_start(instance->handle, expansion_test_app_serial_rx_callback,
                                   instance, false);
}
...
static size_t expansion_test_app_receive_callback(uint8_t *data, size_t data_size, void *context)
{
    ...
    while (true) {
        received_size += furi_stream_buffer_receive(instance->buf, data + received_size,
                                                    data_size - received_size, 0);
        if (received_size == data_size)
            break;

        const uint32_t flags = furi_thread_flags_wait(EXPANSION_TEST_APP_ALL_FLAGS, FuriFlagWaitAny,
...
static size_t expansion_test_app_send_callback(const uint8_t *data, size_t data_size, void *context)
{
    ExpansionTestApp *instance = context;
...
static bool expansion_test_app_send_status_response(ExpansionTestApp *instance,
...
static bool expansion_test_app_send_baud_rate_request(ExpansionTestApp *instance,
                                                      uint32_t baud_rate)
{
    ExpansionFrame frame = {
...
static bool expansion_test_app_send_control_request(ExpansionTestApp *instance,
...
static bool expansion_test_app_send_data_request(ExpansionTestApp *instance, const uint8_t *data,
                                                 size_t data_size)
{
...
static bool expansion_test_app_rpc_encode_callback(pb_ostream_t *stream, const pb_byte_t *data,
                                                   size_t data_size)
{
    ...
    while (size_sent < data_size) {
        ...
        if (!expansion_test_app_send_data_request(instance, data + size_sent, current_size))
            break;
        if (!expansion_test_app_receive_frame(instance, &instance->frame))
            break;
        if (!expansion_test_app_is_success_response(&instance->frame))
            break;
...
static bool expansion_test_app_receive_rpc_request(ExpansionTestApp *instance, PB_Main *message)
{
    ...
    do {
        if (!expansion_test_app_receive_frame(instance, &instance->frame))
            break;
        if (!expansion_test_app_send_status_response(instance, ExpansionFrameErrorNone))
            break;
        if (instance->frame.header.type != ExpansionFrameTypeData)
            break;
        pb_istream_t stream = pb_istream_from_buffer(instance->frame.content.data.bytes,
                                                     instance->frame.content.data.size);
        if (!pb_decode_ex(&stream, &PB_Main_msg, message, PB_DECODE_DELIMITED))
            break;
        success = true;
    } while (false);
...
static bool expansion_test_app_wait_ready(ExpansionTestApp *instance)
{
    ...
    do {
        if (!expansion_test_app_receive_frame(instance, &instance->frame))
            break;
        if (instance->frame.header.type != ExpansionFrameTypeHeartbeat)
            break;
        success = true;
    } while (false);
...
static bool expansion_test_app_handshake(ExpansionTestApp *instance)
{
    ...
    do {
        if (!expansion_test_app_send_baud_rate_request(instance, 230400))
            break;
        if (!expansion_test_app_receive_frame(instance, &instance->frame))
            break;
        if (!expansion_test_app_is_success_response(&instance->frame))
...
static bool expansion_test_app_enable_otg(ExpansionTestApp *instance, bool enable)
{
    ...
    do {
        const ExpansionFrameControlCommand command =
            enable ? ExpansionFrameControlCommandEnableOtg : ExpansionFrameControlCommandDisableOtg;
        if (!expansion_test_app_send_control_request(instance, command))
            break;
        if (!expansion_test_app_receive_frame(instance, &instance->frame))
            break;
        if (!expansion_test_app_is_success_response(&instance->frame))
...
static bool expansion_test_app_start_rpc(ExpansionTestApp *instance)
{
    ...
    do {
        if (!expansion_test_app_send_control_request(instance,
...
static bool expansion_test_app_rpc_mkdir(ExpansionTestApp *instance)
{
    ...
    do {
        if (!expansion_test_app_send_rpc_request(instance, &instance->msg))
            break;
        if (!expansion_test_app_receive_rpc_request(instance, &instance->msg))
            break;
        if (!expansion_test_app_is_success_rpc_message(&instance->msg))
            break;
        success = true;
    } while (false);
...
static bool expansion_test_app_rpc_write(ExpansionTestApp *instance)
{
    ...
    do {
        if (!storage_file_open(file, APP_ASSETS_PATH(TEST_FILE_NAME), FSAM_READ,
                               FSOM_OPEN_EXISTING))
            break;
...
static bool expansion_test_app_rpc_alert(ExpansionTestApp *instance)
{
    ...
    do {
        ...
        if (!expansion_test_app_receive_rpc_request(instance, &instance->msg))
            break;
        if (instance->msg.which_content != PB_Main_empty_tag)
            break;
...
static bool expansion_test_app_rpc_enable_otg(ExpansionTestApp *instance, bool enable)
{
    ...
    instance->msg.content.gpio_set_otg_mode.mode =
        enable ? PB_Gpio_GpioOtgMode_ON : PB_Gpio_GpioOtgMode_OFF;
    instance->msg.has_next = false;
...
static bool expansion_test_app_idle(ExpansionTestApp *instance, uint32_t num_cycles)
{
    ...
    for (num_cycles_done = 0; num_cycles_done < num_cycles; ++num_cycles_done) {
        if (!expansion_test_app_send_heartbeat(instance))
            break;
        if (!expansion_test_app_receive_frame(instance, &instance->frame))
            break;
        if (instance->frame.header.type != ExpansionFrameTypeHeartbeat)
...
int32_t expansion_test_app(void *p)
{
    ...
    do {
        if (!expansion_test_app_send_presence(instance))
            break;
        if (!expansion_test_app_wait_ready(instance))
            break;
        if (!expansion_test_app_handshake(instance))
            break;
        if (!expansion_test_app_enable_otg(instance, true))
            break;
        if (!expansion_test_app_idle(instance, 5))
            break;
        if (!expansion_test_app_start_rpc(instance))
            break;
        if (!expansion_test_app_rpc_enable_otg(instance, false))
            break;
        if (!expansion_test_app_idle(instance, 5))
            break;
        if (!expansion_test_app_rpc_mkdir(instance))
            break;
        if (!expansion_test_app_rpc_write(instance))
            break;
        if (!expansion_test_app_rpc_alert(instance))
            break;
        if (!expansion_test_app_rpc_enable_otg(instance, true))
            break;
        if (!expansion_test_app_idle(instance, 5))
            break;
        if (!expansion_test_app_stop_rpc(instance))
            break;
        if (!expansion_test_app_enable_otg(instance, false))
            break;
        success = true;
    } while (false);
