...
static void desktop_settings_scene_keybinds_action_submenu_callback(void *context, uint32_t index)
{
    ...
    if (action_type == DesktopSettingsAppKeybindActionTypeMainApp) {
        for (size_t i = 0; i < FLIPPER_APPS_COUNT; i++) {
            submenu_add_item(submenu, FLIPPER_APPS[i].name, (uint32_t)FLIPPER_APPS[i].name,
                             desktop_settings_scene_keybinds_action_submenu_callback, app);
        ...
        for (size_t i = 0; i < EXTRA_KEYBINDS_COUNT; i++) {
            submenu_add_item(submenu, EXTRA_KEYBINDS[i], (uint32_t)EXTRA_KEYBINDS[i],
                             desktop_settings_scene_keybinds_action_submenu_callback, app);
