...
void desktop_settings_scene_pin_menu_on_enter(void *context)
{
    ...
    if (!desktop_pin_code_is_set()) {
        submenu_add_item(submenu, "Set PIN", DesktopSettingsCustomEventSetPin,
                         desktop_settings_scene_pin_menu_submenu_callback, app);

    } else {
        submenu_add_item(submenu, "Change PIN", DesktopSettingsCustomEventChangePin,
                         desktop_settings_scene_pin_menu_submenu_callback, app);

        submenu_add_item(submenu, "Remove PIN", DesktopSettingsCustomEventDisablePin,
                         desktop_settings_scene_pin_menu_submenu_callback, app);
    }
...
bool desktop_settings_scene_pin_menu_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        case DesktopSettingsCustomEventChangePin:
            scene_manager_set_scene_state(app->scene_manager, DesktopSettingsAppScenePinAuth,
