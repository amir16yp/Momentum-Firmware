...
static DialogMessageButton about_screen_product(DialogsApp *dialogs, DialogMessage *message)
{
    ...
    FuriString *screen_header = furi_string_alloc_printf("Product: %s\n"
                                                         "Model: %s",
                                                         furi_hal_version_get_model_name(),
                                                         furi_hal_version_get_model_code());

    FuriString *screen_text =
        furi_string_alloc_printf("FCC ID: %s\n"
                                 "IC: %s",
                                 furi_hal_version_get_fcc_id(), furi_hal_version_get_ic_id());

    dialog_message_set_header(message, furi_string_get_cstr(screen_header), 0, 0, AlignLeft,
                              AlignTop);
    dialog_message_set_text(message, furi_string_get_cstr(screen_text), 0, 26, AlignLeft, AlignTop);
...
static DialogMessageButton about_screen_icon2(DialogsApp *dialogs, DialogMessage *message)
{
    ...
    dialog_message_set_text(message, furi_hal_version_get_mic_id(), 63, 27, AlignLeft, AlignCenter);
    result = dialog_message_show(dialogs, message);
...
static DialogMessageButton about_screen_cert_china_1(DialogsApp *dialogs, DialogMessage *message)
{
    ...
    dialog_message_set_text(message, furi_hal_version_get_srrc_id(), 55, 11, AlignLeft,
                            AlignBottom);
    result = dialog_message_show(dialogs, message);
...
static DialogMessageButton about_screen_cert_taiwan(DialogsApp *dialogs, DialogMessage *message)
{
    ...
    dialog_message_set_text(message, furi_hal_version_get_ncc_id(), 39, 30, AlignLeft, AlignBottom);
    result = dialog_message_show(dialogs, message);
...
static DialogMessageButton about_screen_hw_version(DialogsApp *dialogs, DialogMessage *message)
{
    ...
    furi_string_cat_printf(buffer, "%d.F%dB%dC%d %s:%s %s\n", furi_hal_version_get_hw_version(),
                           furi_hal_version_get_hw_target(), furi_hal_version_get_hw_body(),
                           furi_hal_version_get_hw_connect(), furi_hal_version_get_hw_region_name(),
                           furi_hal_region_get_name(), my_name ? my_name : "Unknown");
...
static DialogMessageButton about_screen_fw_version(DialogsApp *dialogs, DialogMessage *message)
{
    ...
    if (!ver) { //-V1051
        ...
        furi_string_cat_printf(buffer, "%s [%s]\n%s%s [%d.%d] %s\n[%d] ", version_get_version(ver),
                               version_get_builddate(ver),
                               version_get_dirty_flag(ver) ? "[!] " : "", version_get_githash(ver),
                               api_major, api_minor, c2_ver ? c2_ver->StackTypeString : "<none>",
                               version_get_target(ver));
        if (!strcmp(version_get_version(ver), "rectum-dev") &&
...
const AboutDialogScreen about_screens[] = {
    about_screen_product,     about_screen_hw_version,   about_screen_fw_version,
    about_screen_compliance,  about_screen_address,      about_screen_icon1,
    about_screen_icon2,       about_screen_cert_china_0, about_screen_cert_china_1,
    about_screen_cert_taiwan, about_screen_cert_mexico,
};
