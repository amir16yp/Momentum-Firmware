...
void power_settings_scene_reboot_on_enter(void *context)
{
    ...
    submenu_add_item(submenu, "Firmware Upgrade", PowerSettingsRebootSubmenuIndexDfu,
                     power_settings_scene_reboot_submenu_callback, app);
    submenu_add_item(submenu, "Reboot Flipper", PowerSettingsRebootSubmenuIndexOs,
                     power_settings_scene_reboot_submenu_callback, app);
...
bool power_settings_scene_reboot_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == PowerSettingsRebootSubmenuIndexDfu) {
            scene_manager_set_scene_state(app->scene_manager, PowerSettingsAppSceneRebootConfirm,
                                          RebootTypeDFU);
