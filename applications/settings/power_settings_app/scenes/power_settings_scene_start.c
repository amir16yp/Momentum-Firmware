...
const char *const auto_poweroff_delay_text[AUTO_POWEROFF_DELAY_COUNT] = {
    ...
    "90min", "2h",   "6h",    "12h",   "24h",   "48h"};

const uint32_t auto_poweroff_delay_value[AUTO_POWEROFF_DELAY_COUNT] = {
    0,       300000,  600000,   900000,   1800000,  2700000,  3600000,
    5400000, 7200000, 21600000, 43200000, 86400000, 172800000};

#define CHARGE_SUPRESS_STEP 5

// change variable_item_list visible text and charge_supress_percent_settings when user change item
// in variable_item_list
...
// change variable_item_list visible text and app_poweroff_delay_time_settings when user change item
// in variable_item_list
...
void power_settings_scene_start_on_enter(void *context)
{
    ...
    item = variable_item_list_add(variable_item_list, "Auto PowerOff", AUTO_POWEROFF_DELAY_COUNT,
                                  power_settings_scene_start_auto_poweroff_delay_changed, app);

    value_index = value_index_uint32(app->settings.auto_poweroff_delay_ms,
                                     auto_poweroff_delay_value, AUTO_POWEROFF_DELAY_COUNT);
    variable_item_set_current_value_index(item, value_index);
    variable_item_set_current_value_text(item, auto_poweroff_delay_text[value_index]);

    item = variable_item_list_add(variable_item_list, "Limit Charge", 100 / CHARGE_SUPRESS_STEP,
                                  power_settings_scene_start_charge_supress_percent_changed, app);
    ...
    variable_item_list_set_enter_callback(variable_item_list,
                                          power_settings_scene_start_submenu_callback, app);
