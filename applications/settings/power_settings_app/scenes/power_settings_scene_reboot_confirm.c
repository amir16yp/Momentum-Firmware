...
void power_settings_scene_reboot_confirm_on_enter(void *context)
{
    ...
    if (reboot_type == RebootTypeDFU) {
        ...
        dialog_ex_set_text(dialog, "Needed for device maintenance\nor firmware upgrades", 64, 14,
        ...
        dialog_ex_set_text(dialog, "May help with some firmware\n issues", 64, 14, AlignCenter,
                           AlignTop);
    } else {
