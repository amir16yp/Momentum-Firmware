...
{
    uint8_t day_per_month = datetime_get_days_per_month(datetime_is_leap_year(dt->year), dt->month);
    if (dt->day > day_per_month) {
        dt->day = day_per_month;
    }
}

static inline void clock_settings_module_draw_block(Canvas *canvas, int32_t x, int32_t y, size_t w,
                                                    size_t h, Font font, EditState state,
                                                    const char *text)
{
...
static void clock_settings_module_draw_time_callback(Canvas *canvas,
                                                     ClockSettingsModuleViewModel *model)
{
    ...
    clock_settings_module_draw_block(canvas, 32, ROW_0_Y, 28, ROW_0_H, FontBigNumbers,
                                     get_state(model, 0, 0), buffer);
    ...
    clock_settings_module_draw_block(canvas, 66, ROW_0_Y, 28, ROW_0_H, FontBigNumbers,
                                     get_state(model, 0, 1), buffer);
    ...
    clock_settings_module_draw_block(canvas, 100, ROW_0_Y, 28, ROW_0_H, FontBigNumbers,
                                     get_state(model, 0, 2), buffer);
}

static void clock_settings_module_draw_date_callback(Canvas *canvas,
                                                     ClockSettingsModuleViewModel *model)
{
    char buffer[64];
    ...
    clock_settings_module_draw_block(canvas, 44, ROW_1_Y, 17, ROW_1_H, FontPrimary,
                                     get_state(model, 1, 0), buffer);
    canvas_draw_box(canvas, 71 - 6, ROW_1_Y + ROW_1_H - 4, 2, 2);
    // Month
    snprintf(buffer, sizeof(buffer), "%02u", model->current.month);
    clock_settings_module_draw_block(canvas, 71, ROW_1_Y, 17, ROW_1_H, FontPrimary,
                                     get_state(model, 1, 1), buffer);
    canvas_draw_box(canvas, 98 - 6, ROW_1_Y + ROW_1_H - 4, 2, 2);
    // Year
    snprintf(buffer, sizeof(buffer), "%04u", model->current.year);
    clock_settings_module_draw_block(canvas, 98, ROW_1_Y, 30, ROW_1_H, FontPrimary,
                                     get_state(model, 1, 2), buffer);
}

static void clock_settings_module_draw_alarm_callback(Canvas *canvas,
                                                      ClockSettingsModuleViewModel *model)
{
    char buffer[64];
    ...
    clock_settings_module_draw_block(canvas, 58, ROW_2_Y, 17, ROW_2_H, FontPrimary,
                                     get_state(model, 2, 0), buffer);
    ...
    clock_settings_module_draw_block(canvas, 81, ROW_2_Y, 17, ROW_2_H, FontPrimary,
                                     get_state(model, 2, 1), buffer);

    clock_settings_module_draw_block(canvas, 106, ROW_2_Y, 22, ROW_2_H, FontPrimary,
                                     get_state(model, 2, 2), model->alarm_enabled ? "On" : "Off");
}
...
static bool clock_settings_module_input_navigation_callback(InputEvent *event,
                                                            ClockSettingsModuleViewModel *model)
{
    if (event->key == InputKeyUp) {
        ...
        if (model->row < ROW_COUNT - 1)
            model->row++;
    } else if (event->key == InputKeyOk) {
        model->editing = !model->editing;
    } else if (event->key == InputKeyRight) {
        if (model->column < COLUMN_COUNT - 1)
            model->column++;
...
static bool clock_settings_module_input_time_callback(InputEvent *event,
                                                      ClockSettingsModuleViewModel *model)
{
    if (event->key == InputKeyUp) {
        if (model->column == 0) {
...
static bool clock_settings_module_input_date_callback(InputEvent *event,
                                                      ClockSettingsModuleViewModel *model)
{
    if (event->key == InputKeyUp) {
        if (model->column == 0) {
            if (model->current.day < 31)
                model->current.day++;
        } else if (model->column == 1) {
...
static bool clock_settings_module_input_alarm_callback(InputEvent *event,
                                                       ClockSettingsModuleViewModel *model)
{
    if (event->key == InputKeyUp) {
        if (model->column == 0) {
...
static bool clock_settings_module_input_callback(InputEvent *event, void *context)
{
    ...
    with_view_model(
        instance->view, ClockSettingsModuleViewModel * model,
        {
...
ClockSettingsModule *clock_settings_module_alloc(FuriEventLoop *event_loop)
{
    ...
    instance->timer = furi_event_loop_timer_alloc(event_loop, clock_settings_module_timer_callback,
                                                  FuriEventLoopTimerTypePeriodic, instance);
