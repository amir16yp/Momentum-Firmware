...
static void storage_settings_scene_unmounted_dialog_callback(DialogExResult result, void *context)
{
...
void storage_settings_scene_unmounted_on_enter(void *context)
{
    ...
    if (sd_status == FSE_NOT_READY) {
        ...
        if (error == FSE_OK) {
            ...
            dialog_ex_set_text(dialog_ex, "Flipper can use\nSD card now.", 3, 22, AlignLeft,
                               AlignTop);
            notification_message(app->notification, &sequence_blink_green_100);
        } else {
            dialog_ex_set_header(dialog_ex, "Cannot Mount SD Card", 64, 3, AlignCenter, AlignTop);
            dialog_ex_set_text(dialog_ex, storage_error_get_desc(error), 3, 22, AlignLeft,
                               AlignTop);
