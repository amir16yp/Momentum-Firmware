...
static void storage_settings_scene_internal_info_dialog_callback(DialogExResult result,
                                                                 void *context)
{
...
void storage_settings_scene_internal_info_on_enter(void *context)
{
    ...
    if (error != FSE_OK) {
        dialog_ex_set_header(dialog_ex, "Internal Storage Error", 64, 10, AlignCenter, AlignCenter);
        dialog_ex_set_text(dialog_ex, storage_error_get_desc(error), 64, 32, AlignCenter,
                           AlignCenter);
    } else {
        furi_string_printf(app->text_string,
        ...
        if (free_flash < 1024) {
            ...
            furi_string_cat_printf(app->text_string, "Flash: %.2f KiB free",
                                   (double)free_flash / 1024);
        }

        dialog_ex_set_text(dialog_ex, furi_string_get_cstr(app->text_string), 4, 4, AlignLeft,
                           AlignTop);
    }
