...
static void storage_settings_scene_wipe_device_dialog_callback(DialogExResult result, void *context)
{
...
void storage_settings_scene_wipe_device_on_enter(void *context)
{
    ...
    dialog_ex_set_text(dialog_ex, "Everything will be erased\r\nData and settings will be lost!",
...
bool storage_settings_scene_wipe_device_on_event(void *context, SceneManagerEvent event)
{
    ...
    uint32_t counter = scene_manager_get_scene_state(app->scene_manager, StorageSettingsWipeDevice);
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        case DialogExResultRight:
            ...
            if (counter < STORAGE_SETTINGS_SCENE_WIPE_DEVICE_CONFIRM_COUNT) {
                furi_string_printf(app->text_string, "%ld presses left",
                                   STORAGE_SETTINGS_SCENE_WIPE_DEVICE_CONFIRM_COUNT - counter);
                dialog_ex_set_text(app->dialog_ex, furi_string_get_cstr(app->text_string), 64, 32,
                                   AlignCenter, AlignCenter);
                scene_manager_set_scene_state(app->scene_manager, StorageSettingsWipeDevice,
                                              counter);
            } else {
