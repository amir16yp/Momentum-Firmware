...
void storage_settings_scene_sd_info_on_enter(void *context)
{
    ...
    if (sd_status != FSE_OK) {
        ...
        dialog_ex_set_text(dialog_ex, "Try to reinsert\nor format SD\ncard.", 3, 19, AlignLeft,
                           AlignTop);
        dialog_ex_set_center_button_text(dialog_ex, "Ok");
    } else {
        furi_string_printf(app->text_string, "Label: %s\nType: %s\n", sd_info.label,
                           sd_api_get_fs_type_text(sd_info.fs_type));

        if (sd_info.kb_total < 1024) {
            furi_string_cat_printf(app->text_string, "Total: %lu KiB\n", sd_info.kb_total);
        } else if (sd_info.kb_total < 1024 * 1024) {
            furi_string_cat_printf(app->text_string, "Total: %.2f MiB\n",
                                   (double)sd_info.kb_total / 1024);
        } else {
            furi_string_cat_printf(app->text_string, "Total: %.2f GiB\n",
        ...
        if (sd_info.kb_free < 1024) {
            ...
            furi_string_cat_printf(app->text_string, "Free: %.2f MiB",
                                   (double)sd_info.kb_free / 1024);
        } else {
            furi_string_cat_printf(app->text_string, "Free: %.2f GiB",
                                   (double)sd_info.kb_free / (1024 * 1024));
        }
        furi_string_cat_printf(app->text_string, "  (%.2f%%)\n",
                               (double)sd_info.kb_free * 100 / sd_info.kb_total);

        furi_string_cat_printf(app->text_string, "%02X%s %s v%i.%i\nSN:%04lX %02i/%i",
                               sd_info.manufacturer_id, sd_info.oem_id, sd_info.product_name,
        ...
        dialog_ex_set_text(dialog_ex, furi_string_get_cstr(app->text_string), 4, 1, AlignLeft,
                           AlignTop);
    }
