...
static void storage_settings_scene_benchmark_dialog_callback(DialogExResult result, void *context)
{
...
static bool storage_settings_scene_bench_write(Storage *api, uint16_t size, const uint8_t *data,
                                               uint32_t *speed)
{
...
static bool storage_settings_scene_bench_read(Storage *api, size_t size, uint8_t *data,
                                              uint32_t *speed)
{
    File *file = storage_file_alloc(api);
    bool result = true;
...
static void storage_settings_scene_benchmark(StorageSettings *app)
{
    ...
    for (size_t i = 0; i < BENCH_COUNT; i++) {
        if (!storage_settings_scene_bench_write(app->fs_api, bench_size[i], bench_data,
                                                &bench_w_speed[i]))
            break;
        ...
        dialog_ex_set_text(dialog_ex, furi_string_get_cstr(app->text_string), 0, 32, AlignLeft,
                           AlignCenter);

        if (!storage_settings_scene_bench_read(app->fs_api, bench_size[i], bench_data,
                                               &bench_r_speed[i]))
            break;
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        case DialogExResultCenter:
            if (app->from_favorites) {
                ...
                consumed = scene_manager_search_and_switch_to_previous_scene(app->scene_manager,
