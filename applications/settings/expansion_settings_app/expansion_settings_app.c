...
static ExpansionSettingsApp *expansion_settings_app_alloc(void)
{
    ...
    item = variable_item_list_add(app->var_item_list, "Listen UART", COUNT_OF(expansion_uart_text),
                                  expansion_settings_app_uart_changed, app);
    ...
    view_set_previous_callback(variable_item_list_get_view(app->var_item_list),
                               expansion_settings_app_exit);
    view_dispatcher_add_view(app->view_dispatcher, ExpansionSettingsViewVarItemList,
