...
void findmy_scene_config_on_enter(void *context)
{
    ...
    item = variable_item_list_add(var_item_list, "Broadcast Interval", 10,
                                  findmy_scene_config_broadcast_interval_changed, app);
    ...
    item = variable_item_list_add(var_item_list, "Transmit Power", 7,
                                  findmy_scene_config_transmit_power_changed, app);
    variable_item_set_current_value_index(item, app->state.transmit_power);
    char power_str[7];
    ...
    item = variable_item_list_add(
        var_item_list, "Matthew KuKanich, Thanks to Chapoly1305, WillyJL, OpenHaystack, Testers", 1,
        NULL, NULL);
