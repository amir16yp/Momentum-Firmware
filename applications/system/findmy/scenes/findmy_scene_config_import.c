...
static const char *parse_nrf_connect(FindMy *app, const char *path)
{
    ...
    do {
        ...
        if (furi_string_size(line) < sizeof(mac) * 3 - 1)
            break;
        error = NULL;
...
static const char *parse_open_haystack(FindMy *app, const char *path)
{
    ...
    do {
        ...
        error = "Base64 failed";
        size_t decoded_len;
        uint8_t *public_key = base64_decode((uint8_t *)furi_string_get_cstr(line),
                                            furi_string_size(line), &decoded_len);
        if (decoded_len != 28) {
            free(public_key);
            break;
        }
...
bool findmy_scene_config_import_on_event(void *context, SceneManagerEvent event)
{
    ...
    if (event.type == SceneManagerEventTypeCustom) {
        ...
        if (dialog_file_browser_show(app->dialogs, path, path, &browser_options)) {
            ...
            scene_manager_set_scene_state(app->scene_manager, FindMySceneConfigImportResult,
                                          (uint32_t)error);
