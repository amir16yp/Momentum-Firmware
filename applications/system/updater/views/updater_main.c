    ...
    with_view_model(
        main_view->view, UpdaterProgressModel * model,
        {
            ...
            if (furi_string_cmp_str(model->status, message)) {
                ...
                update = true;
            } else if ((model->rendered_progress > progress) ||
...
bool updater_main_input(InputEvent *event, void *context)
{
    ...
    if ((event->type == InputTypeShort) && (event->key == InputKeyOk)) {
        view_dispatcher_send_custom_event(main_view->view_dispatcher,
...
static void updater_main_draw_callback(Canvas *canvas, void *_model)
{
    ...
    if (model->failed) {
        ...
        elements_multiline_text_aligned(canvas, 75, 26, AlignCenter, AlignTop,
                                        furi_string_get_cstr(model->status));

        canvas_draw_str_aligned(canvas, 18, 55, AlignLeft, AlignTop,
                                "to retry, hold       to abort");
        ...
        canvas_draw_str_aligned(canvas, 64, 51, AlignCenter, AlignTop,
                                furi_string_get_cstr(model->status));
...
UpdaterMainView *updater_main_alloc(void)
{
    ...
    with_view_model(
        main_view->view, UpdaterProgressModel * model,
        { model->status = furi_string_alloc_set("Waiting for SD card"); }, true);
