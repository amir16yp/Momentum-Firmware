...
static bool update_task_pre_update(UpdateTask *update_task)
{
    ...
    path_concat(furi_string_get_cstr(update_task->update_path), INT_BACKUP_DEFAULT_FILENAME,
                backup_file_path);
    ...
    do {
        ...
        while ((entry_ptr = resource_manifest_reader_next(manifest_reader))) {
            ...
            if (entry_ptr->type == ResourceManifestEntryTypeFile) {
                ...
                path_concat(STORAGE_EXT_PATH_PREFIX, furi_string_get_cstr(entry_ptr->name),
                            file_path);
                ...
                if (result != FSE_OK && result != FSE_EXIST) {
                    FURI_LOG_E(TAG, "%s remove failed, cause %s", furi_string_get_cstr(file_path),
        ...
        while ((entry_ptr = resource_manifest_reader_previous(manifest_reader))) {
            ...
            if (entry_ptr->type == ResourceManifestEntryTypeDirectory) {
                ...
                do {
                    ...
                    FS_Error result = storage_common_remove(update_task->storage,
                                                            furi_string_get_cstr(folder_path));
                    if (result != FSE_OK && result != FSE_EXIST) {
                        FURI_LOG_E(TAG, "%s remove failed, cause %s",
                                   furi_string_get_cstr(folder_path),
                                   storage_error_get_desc(result));
                    }
                } while (false);
...
static bool update_task_post_update(UpdateTask *update_task)
{
    ...
    do {
        ...
        if (update_task->state.groups & UpdateTaskStageGroupResources) {
            path_concat(furi_string_get_cstr(update_task->update_path),
                        furi_string_get_cstr(update_task->manifest->resource_bundle), file_path);

            CHECK_RESULT(
                tar_archive_open(archive, furi_string_get_cstr(file_path),
        ...
        if (update_task->state.groups & UpdateTaskStageGroupSplashscreen) {
            ...
            if (storage_common_stat(update_task->storage, FIRSTBOOT_FLAG_PATH, NULL) ==
                ...
                if (storage_common_stat(update_task->storage, furi_string_get_cstr(tmp_path),
                                        NULL) != FSE_OK) {
            ...
            storage_common_copy(update_task->storage, furi_string_get_cstr(tmp_path),
                                SLIDESHOW_FS_PATH);
