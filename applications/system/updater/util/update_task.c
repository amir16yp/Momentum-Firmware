...
static void update_task_calc_completed_stages(UpdateTask *update_task)
{
    ...
    for (UpdateTaskStage past_stage = UpdateTaskStageProgress;
         past_stage < update_task->state.stage; ++past_stage) {
...
void update_task_set_progress(UpdateTask *update_task, UpdateTaskStage stage, uint8_t progress)
{
    if (stage != UpdateTaskStageProgress) {
        ...
        if (stage >= UpdateTaskStageError) {
            furi_string_printf(update_task->state.status, "%s\n#[%d-%d]",
                               update_task_get_error_message(update_task->state.stage,
    ...
    if (update_task->status_change_cb) {
        if (update_stage_is_error(update_task->state.stage)) {
            (update_task->status_change_cb)(furi_string_get_cstr(update_task->state.status),
                                            adapted_progress,
            ...
            (update_task->status_change_cb)(s, adapted_progress,
                                            update_stage_is_error(update_task->state.stage),
                                            update_task->status_change_cb_state);
            free(s);
        }
...
bool update_task_open_file(UpdateTask *update_task, FuriString *filename)
{
    ...
    tmp_path = furi_string_alloc_set(update_task->update_path);
    path_append(tmp_path, furi_string_get_cstr(filename));
    bool open_success = storage_file_open(update_task->file, furi_string_get_cstr(tmp_path),
                                          FSAM_READ, FSOM_OPEN_EXISTING);
    furi_string_free(tmp_path);
    return open_success;
}

static void update_task_worker_thread_cb(FuriThread *thread, FuriThreadState state, void *context)
{
    UNUSED(context);
...
bool update_task_parse_manifest(UpdateTask *update_task)
{
    ...
    do {
        ...
        if (!update_operation_get_current_package_manifest_path(update_task->storage,
                                                                manifest_path)) {
            break;
        }
