...
// Header
    ...
    elements_multiline_text_aligned(canvas, 0, 16, AlignLeft, AlignTop,
                                    furi_string_get_cstr(min_interval_str));
    ...
    elements_multiline_text_aligned(canvas, 0, 28, AlignLeft, AlignTop,
                                    furi_string_get_cstr(max_interval_str));
...
static void hid_mouse_jiggler_stealth_timer_callback(void *context)
{
    ...
    with_view_model(
        hid_mouse_jiggler->view, HidMouseJigglerStealthModel * model,
        {
            if (model->running) {
...
HidMouseJigglerStealth *hid_mouse_jiggler_stealth_alloc(Hid *hid)
{
    ...
    view_allocate_model(hid_mouse_jiggler->view, ViewModelTypeLocking,
                        sizeof(HidMouseJigglerStealthModel));
    ...
    hid_mouse_jiggler->timer = furi_timer_alloc(hid_mouse_jiggler_stealth_timer_callback,
                                                FuriTimerTypePeriodic, hid_mouse_jiggler);
...
void hid_mouse_jiggler_stealth_set_connected_status(HidMouseJigglerStealth *hid_mouse_jiggler,
                                                    bool connected)
{
    furi_assert(hid_mouse_jiggler);
    with_view_model(
        hid_mouse_jiggler->view, HidMouseJigglerStealthModel * model,
        { model->connected = connected; }, true);
}
