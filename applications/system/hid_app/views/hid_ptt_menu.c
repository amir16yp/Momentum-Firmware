...
ARRAY_DEF(PushToTalkMenuItemArray, PushToTalkMenuItem,
          (INIT(API_2(PushToTalkMenuItem_init)), SET(API_6(PushToTalkMenuItem_set)),
           INIT_SET(API_6(PushToTalkMenuItem_init_set)), CLEAR(API_2(PushToTalkMenuItem_clear))))
...
static void hid_ptt_menu_draw_list(Canvas *canvas, void *context,
                                   const PushToTalkMenuItemArray_t items)
{
    furi_assert(context);
    ...
    for (PushToTalkMenuItemArray_it(it, items); !PushToTalkMenuItemArray_end_p(it);
        ...
        if (item_position < items_on_screen) {
            if (position == model->position) {
                ...
                elements_slightly_rounded_box(canvas, 0,
    ...
    elements_scrollbar_pos(canvas, 128, 17, 46, model->position,
                           PushToTalkMenuItemArray_size(items));
}
...
void ptt_menu_add_item_to_list(HidPushToTalkMenu *hid_ptt_menu, uint32_t list_index,
                               const char *label, uint32_t index,
                               PushToTalkMenuItemCallback callback, void *callback_context)
{
    ...
    with_view_model(
        hid_ptt_menu->view, HidPushToTalkMenuModel * model,
        {
            ...
            for (PushToTalkMenuItemArray_it(it, new_list->items);
                 !PushToTalkMenuItemArray_end_p(it); PushToTalkMenuItemArray_next(it)) {
...
void ptt_menu_process_long_ok(HidPushToTalkMenu *hid_ptt_menu)
{
    ...
    if (item && list && hid_ptt_menu->long_ok_callback) {
        hid_ptt_menu->long_ok_callback(hid_ptt_menu->long_ok_callback_context, list->index,
                                       list->label, item->index, item->label);
    }
}

void ptt_menu_set_long_ok_callback(HidPushToTalkMenu *hid_ptt_menu,
                                   PushToTalkMenuLongOkCallback callback, void *callback_context)
{
    furi_assert(hid_ptt_menu);
