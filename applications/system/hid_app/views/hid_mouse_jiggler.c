...
static void hid_mouse_jiggler_draw_callback(Canvas *canvas, void *context)
{
    ...
    if (model->interval_idx != 0)
        canvas_draw_icon(canvas, 74, 19, &I_ButtonLeft_4x7);
...
static void hid_mouse_jiggler_timer_callback(void *context)
{
    ...
    with_view_model(
        hid_mouse_jiggler->view, HidMouseJigglerModel * model,
        {
            if (model->running) {
                model->counter++;
                hid_hal_mouse_move(hid_mouse_jiggler->hid,
...
HidMouseJiggler *hid_mouse_jiggler_alloc(Hid *hid)
{
    ...
    hid_mouse_jiggler->timer = furi_timer_alloc(hid_mouse_jiggler_timer_callback,
...
void hid_mouse_jiggler_set_connected_status(HidMouseJiggler *hid_mouse_jiggler, bool connected)
{
    furi_assert(hid_mouse_jiggler);
    with_view_model(
        hid_mouse_jiggler->view, HidMouseJigglerModel * model, { model->connected = connected; },
        true);
}
