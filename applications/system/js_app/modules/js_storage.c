...
static void js_storage_open_file(struct mjs *mjs)
{
    ...
    static const JsValueEnumVariant js_storage_fsom_variants[] = {
        {"open_existing", FSOM_OPEN_EXISTING}, {"open_always", FSOM_OPEN_ALWAYS},
        {"open_append", FSOM_OPEN_APPEND},     {"create_new", FSOM_CREATE_NEW},
...
static void js_storage_read_directory(struct mjs *mjs)
{
    ...
    while (storage_dir_read(dir, &file_info, name, sizeof(name))) {
        ...
        furi_check(storage_common_timestamp(storage, furi_string_get_cstr(file_path), &timestamp) ==
                   FSE_OK);
...
static void js_storage_next_available_filename(struct mjs *mjs)
{
    ...
    int32_t max_len;
    JS_VALUE_PARSE_ARGS_OR_RETURN(mjs, &js_storage_naf_args, &dir_path, &file_name, &file_ext,
                                  &max_len);
