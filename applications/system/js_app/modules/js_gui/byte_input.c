...
static mjs_val_t input_transformer(struct mjs *mjs, FuriSemaphore *semaphore,
...
static bool header_assign(struct mjs *mjs, ByteInput *input, JsViewPropValue value,
                          JsByteKbContext *context)
{
    UNUSED(mjs);
...
static bool len_assign(struct mjs *mjs, ByteInput *input, JsViewPropValue value,
                       JsByteKbContext *context)
{
    UNUSED(mjs);
    UNUSED(input);
    ...
    if (new_buffer_size < context->default_data_size) {
        ...
        mjs_prepend_errorf(mjs, MJS_BAD_ARGS_ERROR,
                           "length must be larger than defaultData length");
        return false;
    }
    context->buffer_size = new_buffer_size;
    context->buffer = realloc(context->buffer, context->buffer_size); //-V701
    byte_input_set_result_callback(input, (ByteInputCallback)input_callback, NULL, context,
                                   context->buffer, context->buffer_size);
    return true;
}

static bool default_data_assign(struct mjs *mjs, ByteInput *input, JsViewPropValue value,
                                JsByteKbContext *context)
{
    UNUSED(mjs);
    ...
    if (context->buffer_size > context->default_data_size) {
        ...
        memset(context->buffer + context->default_data_size, 0x00,
               context->buffer_size - context->default_data_size);
    }
...
static const JsViewDescriptor view_descriptor = {
    ...
    .props = {
        ...
        (JsViewPropDescriptor){
            .name = "length", .type = JsViewPropTypeNumber, .assign = (JsViewPropAssign)len_assign},
