...
static bool element_get_position(struct mjs *mjs, mjs_val_t element, int32_t *x, int32_t *y)
{
    ...
    if (!mjs_is_number(x_in) || !mjs_is_number(y_in))
        return false;
...
static bool element_get_size(struct mjs *mjs, mjs_val_t element, int32_t *w, int32_t *h)
{
    ...
    if (!mjs_is_number(w_in) || !mjs_is_number(h_in))
        return false;
 ...
 */
static bool element_get_alignment(struct mjs *mjs, mjs_val_t element, Align *align_v,
                                  Align *align_h)
{
...
static bool js_widget_add_child(struct mjs *mjs, Widget *widget, JsWidgetCtx *context,
                                mjs_val_t child_obj)
{
    UNUSED(context);
    ...
    if (!element_type)
        JS_ERROR_AND_RETURN_VAL(mjs, MJS_BAD_ARGS_ERROR, false,
                                "child object must have `element` property");
    ...
    if ((strcmp(element_type, "string") == 0) || (strcmp(element_type, "string_multiline") == 0)) {
        ...
        if (strcmp(element_type, "string") == 0) {
            widget_add_string_element(widget, x, y, align_h, align_v, font,
                                      mjs_get_string(mjs, &text, NULL));
        } else {
            widget_add_string_multiline_element(widget, x, y, align_h, align_v, font,
                                                mjs_get_string(mjs, &text, NULL));
        }
        ...
        if (!mjs_is_boolean(strip_to_dots_in))
            JS_ERROR_AND_RETURN_VAL(mjs, MJS_BAD_ARGS_ERROR, false,
                                    "failed to fetch element stripToDots");
        bool strip_to_dots = mjs_get_bool(mjs, strip_to_dots_in);
        widget_add_text_box_element(widget, x, y, w, h, align_h, align_v,
                                    mjs_get_string(mjs, &text, NULL), strip_to_dots);
        ...
        if (!btn_name)
            JS_ERROR_AND_RETURN_VAL(mjs, MJS_BAD_ARGS_ERROR, false,
                                    "failed to fetch element button");
        ...
        mjs_val_t text;
        DESTRUCTURE_OR_RETURN(mjs, child_obj, text, &text);
        widget_add_button_element(widget, btn_type, mjs_get_string(mjs, &text, NULL),
                                  (ButtonCallback)js_widget_button_callback, context);
        ...
        if (!mjs_is_foreign(icon_data_in))
            JS_ERROR_AND_RETURN_VAL(mjs, MJS_BAD_ARGS_ERROR, false,
                                    "failed to fetch element iconData");
        ...
        if (!mjs_is_number(x1_in) || !mjs_is_number(y1_in) || !mjs_is_number(x2_in) ||
            !mjs_is_number(y2_in))
            JS_ERROR_AND_RETURN_VAL(mjs, MJS_BAD_ARGS_ERROR, false,
                                    "failed to fetch element positions");
...
static mjs_val_t js_widget_button_event_transformer(struct mjs *mjs, FuriMessageQueue *queue,
                                                    JsWidgetCtx *context)
{
    UNUSED(context);
    JsWidgetButtonEvent event;
