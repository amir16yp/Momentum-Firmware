...
typedef struct {
    ...
    FuriSemaphore *navigation; // FIXME: (-nofl) convert into callback once FuriEventLoop starts
                               // supporting this
} JsGui;
 ...
 */
static mjs_val_t js_gui_vd_custom_transformer(struct mjs *mjs, FuriEventLoopObject *object,
                                              void *context)
{
    UNUSED(context);
    furi_check(object);
 ...
 */
static bool js_gui_view_assign(struct mjs *mjs, const char *name, mjs_val_t value,
                               JsGuiViewData *data)
{
    ...
    for (size_t i = 0; i < descriptor->prop_cnt; i++) {
        ...
        if (expected_type) {
            mjs_prepend_errorf(mjs, MJS_BAD_ARGS_ERROR, "view prop \"%s\" requires %s value", name,
                               expected_type);
            return false;
        } else {
 ...
 */
static bool js_gui_view_internal_set_children(struct mjs *mjs, mjs_val_t children,
                                              JsGuiViewData *data, bool do_reset)
{
    if (do_reset)
...
static void js_gui_vf_make_with(struct mjs *mjs)
{
    ...
    if (mjs_is_array(children)) {
        ...
        if (!js_gui_view_internal_set_children(mjs, children, data, false))
            return;
    }
