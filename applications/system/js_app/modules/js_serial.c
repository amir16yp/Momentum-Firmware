...
static void js_serial_on_async_rx(FuriHalSerialHandle *handle, FuriHalSerialRxEvent event,
                                  void *context)
{
...
static void js_serial_setup(struct mjs *mjs)
{
    ...
    JS_VALUE_PARSE_ARGS_OR_RETURN(mjs, &js_serial_setup_args, &serial_id, &baudrate, &data_bits,
                                  &parity, &stop_bits);
    ...
    if (serial->serial_handle) {
        ...
        furi_hal_serial_async_rx_start(serial->serial_handle, js_serial_on_async_rx, serial, false);
...
static size_t js_serial_receive(JsSerialInst *serial, char *buf, size_t len, uint32_t timeout)
{
    ...
    while (1) {
        ...
        if (flags == 0) { // Timeout
            ...
            size_t rx_len = furi_stream_buffer_receive(serial->rx_stream, &buf[bytes_read],
                                                       len - bytes_read, 0);
            bytes_read += rx_len;
            if (bytes_read == len) {
                break;
...
static bool js_serial_expect_parse_string(struct mjs *mjs, mjs_val_t arg, PatternArray_t patterns)
{
    size_t str_len = 0;
    const char *arg_str = mjs_get_string(mjs, &arg, &str_len);
    if ((str_len == 0) || (arg_str == NULL)) {
...
static bool js_serial_expect_parse_args(struct mjs *mjs, PatternArray_t patterns, uint32_t *timeout)
{
    size_t num_args = mjs_nargs(mjs);
    if (num_args == 2) {
...
static int32_t js_serial_expect_check_pattern_start(PatternArray_t patterns, char value,
