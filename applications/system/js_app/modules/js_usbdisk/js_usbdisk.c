...
static bool file_read(void *ctx, uint32_t lba, uint16_t count, uint8_t *out, uint32_t *out_len,
                      uint32_t out_cap)
{
...
static void js_usbdisk_start(struct mjs *mjs)
{
    ...
    do {
        ...
        if (!storage_file_open(usbdisk->file, usbdisk->path, FSAM_READ | FSAM_WRITE,
                               FSOM_OPEN_EXISTING)) {
