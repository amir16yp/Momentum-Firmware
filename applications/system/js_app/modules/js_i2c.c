...
{
    if (!check_arg_count_range(mjs, 1, 2))
        return;
...
{
    if (!check_arg_count_range(mjs, 2, 3))
        return;
...
{
    if (!check_arg_count_range(mjs, 3, 4))
        return;
    ...
    furi_hal_i2c_acquire(&furi_hal_i2c_handle_external);
    bool result = furi_hal_i2c_trx(&furi_hal_i2c_handle_external, addr, tx_buf, tx_len, rx_buf,
                                   rx_len, timeout);
    furi_hal_i2c_release(&furi_hal_i2c_handle_external);
