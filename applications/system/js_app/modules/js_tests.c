...
static void js_tests_assert_eq(struct mjs *mjs)
{
    ...
    if (mjs_is_number(expected_arg) && mjs_is_number(result_arg)) {
        ...
        if (strcmp(expected, result) == 0) {
            ...
            mjs_prepend_errorf(mjs, MJS_INTERNAL_ERROR, "expected \"%s\", found \"%s\"", expected,
                               result);
        }
        ...
        if (expected == result) {
            FURI_LOG_T(TAG, "eq passed (exp=%s res=%s)", expected ? "true" : "false",
                       result ? "true" : "false");
        } else {
            mjs_prepend_errorf(mjs, MJS_INTERNAL_ERROR, "expected %s, found %s",
                               expected ? "true" : "false", result ? "true" : "false");
        }
    } else {
        JS_ERROR_AND_RETURN(mjs, MJS_INTERNAL_ERROR, "type mismatch (expected %s, result %s)",
                            mjs_typeof(expected_arg), mjs_typeof(result_arg));
    }
...
static void js_tests_assert_float_close(struct mjs *mjs)
{
    ...
    if (ABS(expected - result) > epsilon) {
        mjs_prepend_errorf(mjs, MJS_INTERNAL_ERROR, "expected %f found %f (tolerance=%f)", expected,
                           result, epsilon);
    }
