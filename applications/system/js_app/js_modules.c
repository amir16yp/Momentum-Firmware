...
JsModules *js_modules_create(struct mjs *mjs, CompositeApiResolver *resolver)
{
    ...
    modules->plugin_manager = plugin_manager_alloc(PLUGIN_APP_ID, PLUGIN_API_VERSION,
...
mjs_val_t js_module_require(JsModules *modules, const char *name, size_t name_len)
{
    ...
    if (module_inst) { //-V547
        mjs_prepend_errorf(modules->mjs, MJS_BAD_ARGS_ERROR, "\"%s\" module is already installed",
                           name);
    ...
    if (!module_found) {
        ...
        furi_string_printf(module_path, "%s/js_%s.fal", MODULES_PATH,
                           furi_string_get_cstr(deslashed_name));
        FURI_LOG_I(TAG, "Loading external module %s from %s", name,
                   furi_string_get_cstr(module_path));
        do {
 ...
 */
static JsSdkCompatStatus js_internal_sdk_compatibility_status(int32_t exp_major, int32_t exp_minor)
{
...
static bool js_internal_compat_ask_user(const char *message)
{
    ...
    dialog_message_set_text(dialog, "This script may not\nwork as expected", 79, 32, AlignCenter,
                            AlignCenter);
...
void js_check_sdk_compatibility(struct mjs *mjs)
{
    ...
    if (status != JsSdkCompatStatusCompatible) {
        FURI_LOG_E(TAG, "Script requests JS SDK %ld.%ld, firmware provides JS SDK %d.%d", major,
                   minor, JS_SDK_MAJOR, JS_SDK_MINOR);

        const char *message =
            (status == JsSdkCompatStatusFirmwareTooOld) ? "Outdated Firmware" : "Outdated Script";
