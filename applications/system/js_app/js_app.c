...
static void js_app_compact_trace(FuriString *trace_str)
{
    ...
    if (filename_start > 0) {
        filename_start++;
        furi_string_set_n(file_name, trace_str, filename_start,
...
static JsApp *js_app_alloc(void)
{
    ...
    view_dispatcher_add_view(app->view_dispatcher, JsAppViewLoading,
                             loading_get_view(app->loading));

    app->console_view = console_view_alloc();
    view_dispatcher_add_view(app->view_dispatcher, JsAppViewConsole,
                             console_view_get_view(app->console_view));
